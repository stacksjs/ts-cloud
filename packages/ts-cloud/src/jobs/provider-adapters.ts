...
function contains(expected: JsonValue, observed: JsonValue): boolean {
  if (Array.isArray(expected)) return Array.isArray(observed) && expected.length === observed.length && expected.every((item, index) => contains(item, observed[index]))
  if (expected && typeof expected === 'object') {
    if (!observed || typeof observed !== 'object' || Array.isArray(observed)) return false
    return Object.entries(expected).every(([key, value]) => contains(value, (observed as Record<string, JsonValue>)[key]))
  }
...
export class JobProviderReconciler {
  ...
  async reconcile(job: ScheduledJob, adapter: JobProviderAdapter, options: { apply?: boolean } = {}): Promise<ScheduledJob> {
    if (adapter.provider !== job.provider) return this.store.reconcile(job.id, 'unsupported', { provider: adapter.provider, message: `Adapter ${adapter.provider} cannot reconcile ${job.provider}.` })
    try {
      const desired = desiredState(job)
      const observed = await adapter.observe(job)
      if (observed && adapter.matches(desired, observed)) return this.store.reconcile(job.id, 'in_sync', { provider: adapter.provider, observed })
      if (options.apply === false) return this.store.reconcile(job.id, observed ? 'drifted' : 'unavailable', { provider: adapter.provider, observed: observed ?? null, applyRequired: true })
      await adapter.apply(job, desired)
      if (!job.enabled) return this.store.reconcile(job.id, 'in_sync', { provider: adapter.provider, observed: null, disabled: true })
      const after = await adapter.observe(job)
      return this.store.reconcile(job.id, after && adapter.matches(desired, after) ? 'in_sync' : 'drifted', { provider: adapter.provider, observed: after ?? null, applied: true })
    }
    catch (error) {
      return this.store.reconcile(job.id, 'unavailable', { provider: adapter.provider, error: error instanceof Error ? error.message : String(error) })
    }
  }

  async remove(job: ScheduledJob, adapter: JobProviderAdapter): Promise<void> {
    if (job.origin === 'config') throw new Error('Config-defined jobs must be removed from configuration, not destructively deleted.')
