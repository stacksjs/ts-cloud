...
describe('AutomationIdentityStore', () => {
  it('creates a least-privilege machine actor and stores only a token hash', () => {
    ...
    expect(automation.verifyToken(issued.secret, 'network-hash')).toMatchObject({
      token: { lastNetworkHint: 'network-hash' },
      serviceAccount: { slug: 'production-ci' },
    })
    expect(() =>
      automation.createToken({
        serviceAccountId: created.serviceAccount.id,
        name: 'Too broad',
        capabilities: ['secrets:read'],
        scope: { type: 'environment', id: production.id },
      }),
    ).toThrow('does not grant secrets:read')
    controlPlane.close()
  })
  ...
  it('enforces resource boundaries, expiry, rotation overlap, and immediate revocation', () => {
    ...
    const account = automation.createServiceAccount({
      organizationId: organization.id,
      slug: 'ci-runner',
      name: 'CI Runner',
      roleTemplate: 'deployer',
    ...
    expect(() =>
      automation.createToken({
        ...
        name: 'Staging escape',
        capabilities: ['deployments:create'],
        scope: { type: 'environment', id: staging.id },
      }),
    ).toThrow('does not grant')
    const original = automation.createToken({
      serviceAccountId: account.id,
  ...
  it('replays identical idempotent responses and rejects key reuse with another body', () => {
    ...
    const saved = automation.saveIdempotency({
      tokenId: issued.token.id,
      key: 'deploy-request-123',
      requestHash: 'body-a',
