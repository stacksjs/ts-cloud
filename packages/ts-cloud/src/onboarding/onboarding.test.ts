...
function storedZip(path: string, content = 'hello'): Uint8Array {
  ...
  const bytes = new Uint8Array(30 + name.length + body.length)
...
function tar(path: string, kind = '0'): Uint8Array {
  ...
  bytes.set(encoder.encode(path), 0)
  bytes.set(encoder.encode('00000000000\0'), 124)
  bytes[156] = kind.charCodeAt(0)
  return bytes
}
...
describe('application detection', () => {
  it('detects Dockerfile, Bun/Node, Laravel, PHP, and static projects from explicit evidence', () => {
    expect(
      detectApplication([
        { path: 'Dockerfile', content: 'FROM oven/bun' },
        { path: 'package.json', content: '{"scripts":{"start":"bun index.ts"}}' },
        { path: 'bun.lock' },
      ]).map((item) => item.framework),
    ).toEqual(['dockerfile', 'bun'])
    expect(
      detectApplication([
        { path: 'package.json', content: '{"scripts":{"start":"node server.js"}}' },
        { path: 'package-lock.json' },
      ])[0],
    ).toMatchObject({ framework: 'node', strategy: 'server' })
    expect(
      detectApplication([
        { path: 'artisan' },
        { path: 'composer.json', content: '{"require":{"laravel/framework":"^12"}}' },
      ])[0],
    ).toMatchObject({ framework: 'laravel', confidence: 0.98 })
    expect(detectApplication([{ path: 'composer.json', content: '{"require":{}}' }])[0]).toMatchObject({
      framework: 'php',
    })
  ...
  it('keeps ambiguous evidence ranked and unsupported projects manually configurable', () => {
    const ambiguous = detectApplication([
      { path: 'Dockerfile' },
      { path: 'package.json', content: '{}' },
      { path: 'bun.lock' },
    ])
    expect(ambiguous).toHaveLength(2)
    expect(ambiguous[0]).toMatchObject({ framework: 'dockerfile', confidence: 0.99 })
    expect(detectApplication([{ path: 'README.md' }])[0]).toMatchObject({
      framework: 'unknown',
      strategy: 'buildpack',
      confidence: 0.2,
    })
...
describe('application planning', () => {
  it('round-trips deterministic manifests for four build strategies', () => {
    const drafts = [
      base(
        { kind: 'server', runtime: 'bun', startCommand: 'bun run start' },
        { target: 'server', architecture: 'arm64', port: 3000, healthCheck: { protocol: 'http', path: '/health' } },
      ),
      base(
        { kind: 'static', publishDirectory: 'dist', buildCommand: 'bun run build' },
        { target: 'serverless', architecture: 'x86_64' },
      ),
      base(
        { kind: 'dockerfile', context: '.', dockerfile: 'Dockerfile', target: 'release' },
        { target: 'container', architecture: 'x86_64', port: 8080, healthCheck: { protocol: 'tcp' } },
      ),
      {
        ...base(
          { kind: 'prebuilt_image', image: 'registry.example/acme/web@sha256:abc' },
          { target: 'container', architecture: 'arm64', port: 8080, healthCheck: { protocol: 'http', path: '/ready' } },
        ),
        source: { kind: 'image', image: 'registry.example/acme/web@sha256:abc' } as const,
      },
    ]
  ...
  it('fails invalid combinations before mutation and redacts misplaced secrets', () => {
    const draft = base(
      {
        ...
        buildArgs: { API_TOKEN: 'do-not-leak' },
        secretNames: ['REGISTRY_PASSWORD'],
      },
      { target: 'serverless', architecture: 'x86_64', port: 70000, healthCheck: { protocol: 'http', path: 'health' } },
    )
    ...
    expect(plan.issues.map((item) => item.code)).toEqual(
      expect.arrayContaining([
        'write_only_secret_required',
        'write_only_build_secret_required',
        'unsafe_docker_path',
        'unsupported_serverless_dockerfile',
        'invalid_port',
        'invalid_health_path',
        'invalid_domain',
      ]),
...
describe('application archive inspection', () => {
  it('accepts bounded safe ZIP and TAR entries without extracting them', () => {
    expect(inspectApplicationArchive(storedZip('dist/index.html'), 'site.zip')).toMatchObject({
      format: 'zip',
      entries: 1,
      expandedBytes: 5,
      paths: ['dist/index.html'],
    })
    expect(inspectApplicationArchive(tar('public/index.html'), 'site.tar')).toMatchObject({
      format: 'tar',
      entries: 1,
  ...
  it('stores inspected artifacts with restrictive permissions and deduplicates content', () => {
    ...
    try {
      ...
      const organization = controlPlane.createOrganization({ slug: 'artifacts', name: 'Artifacts' })
      const project = controlPlane.createProject({ organizationId: organization.id, slug: 'app', name: 'App' })
      const artifacts = new ApplicationArtifactStore(controlPlane, { cwd: root, id: () => 'artifact-1' })
      const created = artifacts.create({
        organizationId: organization.id,
        projectId: project.id,
        filename: 'site.zip',
        bytes: storedZip('dist/index.html'),
      })
      ...
      const raw = controlPlane.database
        .query<Record<string, string>, [string]>('SELECT storage_path FROM application_artifacts WHERE id=?')
        .get(created.id)!
  ...
  it('rejects traversal, links, malformed formats, and expansion limits', () => {
    ...
    expect(() => inspectApplicationArchive(storedZip('large', '123456'), 'large.zip', { maxExpandedBytes: 5 })).toThrow(
      'expansion limit',
    )
...
describe('resumable application drafts', () => {
  it('persists only non-secret inputs, resumes optimistically, and marks a confirmed plan applied', () => {
    ...
    input.environment = { APP_ENV: 'production', DATABASE_PASSWORD: { secretRef: 'DATABASE_PASSWORD' } }
    input.requiredSecretNames = ['DATABASE_PASSWORD']
    ...
    const ready = drafts.update(created.id, 1, {
      draft: input,
      step: 'review',
      suppliedSecretNames: ['DATABASE_PASSWORD'],
    })
    ...
    expect(controlPlane.listEvents({ projectId: project.id }).map((event) => event.type)).toEqual([
      'application.draft.created',
      'application.draft.updated',
      'application.draft.applied',
    ])
    controlPlane.close()
  })
  ...
  it('requires explicit target confirmation before creating desired state', () => {
    const controlPlane = new ControlPlaneStore({ path: ':memory:' })
    const organization = controlPlane.createOrganization({ slug: 'apply', name: 'Apply' })
    const project = controlPlane.createProject({ organizationId: organization.id, slug: 'app', name: 'App' })
    const environment = controlPlane.createEnvironment({
      projectId: project.id,
      slug: 'production',
    ...
    const input = base(
      { kind: 'static', publishDirectory: 'dist', buildCommand: 'bun run build' },
      { target: 'serverless', architecture: 'arm64' },
    )
    input.projectId = project.id
    input.environmentId = environment.id
    ...
    expect(applied).toMatchObject({
      resource: { slug: 'web', kind: 'application' },
      operation: { state: 'queued', kind: 'application.create' },
      plan: { valid: true },
    })
    expect(() =>
      applyApplicationDraft({
  ...
  it('queues reproducible create flows for Bun, Laravel, static, Dockerfile, and prebuilt images', () => {
    ...
    const strategies: Array<{
      slug: string
      build: ApplicationDraftInput['build']
      runtime: ApplicationDraftInput['runtime']
    ...
    }> = [
      {
        ...
        runtime: { target: 'server', architecture: 'arm64', port: 3000 },
      },
      {
        slug: 'laravel-api',
        build: {
        ...
        runtime: { target: 'server', architecture: 'x86_64', port: 8080 },
      },
      {
        slug: 'static-site',
        build: { kind: 'static', publishDirectory: 'dist', buildCommand: 'bun run build' },
        runtime: { target: 'serverless', architecture: 'arm64' },
      },
      {
        slug: 'docker-app',
        build: { kind: 'dockerfile', context: '.', dockerfile: 'Dockerfile', target: 'release' },
        runtime: { target: 'container', architecture: 'x86_64', port: 8080 },
      },
      {
        slug: 'oci-app',
        build: { kind: 'prebuilt_image', image: 'registry.example/acme/web@sha256:abc' },
        runtime: { target: 'container', architecture: 'arm64', port: 8080 },
        source: { kind: 'image', image: 'registry.example/acme/web@sha256:abc' },
      },
    ]
    for (const strategy of strategies) {
      ...
      expect(applied.resource.desiredState).toMatchObject({
        manifest: { spec: { build: { kind: strategy.build.kind } } },
      })
      ...
      expect(
        controlPlane.database
          .query<Record<string, number>, [string]>('SELECT build_slot FROM operation_jobs WHERE operation_id=?')
          .get(applied.operation.id)?.build_slot,
      ).toBe(strategy.build.kind === 'prebuilt_image' ? 0 : 1)
    }
    expect(controlPlane.listOperations({ projectId: project.id }).map((operation) => operation.kind)).toEqual(
      Array(5).fill('application.create'),
    )
    expect(
      controlPlane
        .listResources(project.id, environment.id)
        .map((resource) => resource.slug)
        .sort(),
    ).toEqual(strategies.map((strategy) => strategy.slug).sort())
    controlPlane.close()
  })

  it('migrates prototype drafts and refuses embedded secret values', () => {
    const migrated = migrateApplicationDraft({
      schemaVersion: 0,
      appName: 'Legacy',
      applicationSlug: 'legacy',
      projectId: 'project',
      environmentId: 'prod',
      strategy: 'static',
    ...
    expect(migrated).toMatchObject({
      ...
      name: 'Legacy',
      slug: 'legacy',
      build: { kind: 'static', publishDirectory: 'public' },
      requiredSecretNames: ['API_TOKEN'],
    })
    const controlPlane = new ControlPlaneStore({ path: ':memory:' })
    const organization = controlPlane.createOrganization({ slug: 'secrets', name: 'Secrets' })
    const project = controlPlane.createProject({ organizationId: organization.id, slug: 'app', name: 'App' })
    const draft = base(
      { kind: 'dockerfile', context: '.', dockerfile: 'Dockerfile', buildArgs: { API_TOKEN: 'runtime-secret' } },
      { target: 'container', architecture: 'x86_64' },
    )
    draft.projectId = project.id
    expect(() =>
      new ApplicationDraftStore(controlPlane).create({
        organizationId: organization.id,
        projectId: project.id,
        name: 'Unsafe',
        draft,
      }),
    ).toThrow('build secret name')
    controlPlane.close()
  })
})
...
describe('private registry connections', () => {
  it('encrypts, tests, rotates, expires, and disconnects image-pull credentials', async () => {
    ...
    let sequence = 0
    const controlPlane = new ControlPlaneStore({ path: ':memory:' })
    const organization = controlPlane.createOrganization({ slug: 'registry', name: 'Registry' })
    const registries = new RegistryConnectionStore(controlPlane, {
      encryptionKey: 'registry-fixture',
      now: () => now,
      id: () => `registry-${++sequence}`,
    })
    const created = registries.create({
      organizationId: organization.id,
      provider: 'generic',
      name: 'Production OCI',
      host: 'https://registry.example',
      credential: { username: 'robot', password: 'runtime-password' },
      credentialExpiresAt: '2026-02-01T00:00:00.000Z',
    })
    expect(created).toMatchObject({ credentialConfigured: true, status: 'pending' })
    const raw = controlPlane.database
      .query<Record<string, string>, [string]>('SELECT credential_ciphertext FROM registry_connections WHERE id=?')
      .get(created.id)!
    ...
    const fetch = (async (value: string | URL | Request, init?: RequestInit) => {
      calls.push({ url: String(value), authorization: new Headers(init?.headers).get('authorization') ?? undefined })
      return new Response(null, { status: 200 })
    }) as typeof globalThis.fetch
    expect(await registries.test(created.id, { image: 'registry.example/acme/web:release', fetch })).toMatchObject({
      status: 'healthy',
      healthMessage: 'Registry image is readable.',
    })
    expect(calls.map((call) => call.url)).toEqual([
      'https://registry.example/v2/',
      'https://registry.example/v2/acme/web/manifests/release',
    ])
    expect(calls.every((call) => call.authorization?.startsWith('Basic '))).toBe(true)
    expect(
      registries.rotate(created.id, { token: 'replacement' }, { expiresAt: '2026-03-01T00:00:00.000Z' }),
  ...
  it('follows bounded HTTPS bearer challenges without returning credentials', async () => {
    const controlPlane = new ControlPlaneStore({ path: ':memory:' })
    const organization = controlPlane.createOrganization({ slug: 'bearer', name: 'Bearer' })
    const registries = new RegistryConnectionStore(controlPlane, { encryptionKey: 'registry-fixture' })
    const connection = registries.create({
      organizationId: organization.id,
      provider: 'docker_hub',
      name: 'Docker Hub',
      host: 'https://registry-1.docker.io',
      credential: { username: 'robot', password: 'password' },
    })
    ...
    expect(
      await registries.test(connection.id, { image: 'registry-1.docker.io/library/bun:latest', fetch }),
