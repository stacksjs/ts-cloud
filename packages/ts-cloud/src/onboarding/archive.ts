...
export function inspectApplicationArchive(
  bytes: Uint8Array,
  filename: string,
  ...
  const maxExpanded = options.maxExpandedBytes ?? 500 * 1024 * 1024
  ...
  if (filename.toLowerCase().endsWith('.zip') || (bytes[0] === 0x50 && bytes[1] === 0x4b)) {
    ...
    while (offset + 30 <= bytes.length) {
      ...
      const expanded = view.getUint32(offset + 22, true)
  ...
  if (!filename.toLowerCase().match(/\.(?:tar|tar\.gz|tgz)$/))
    throw new Error('Only ZIP and uncompressed TAR artifacts are supported')
  let offset = 0
  while (offset + 512 <= bytes.length) {
    const header = bytes.subarray(offset, offset + 512)
    if (header.every((value) => value === 0)) break
    const text = (start: number, length: number) =>
      new TextDecoder().decode(header.subarray(start, start + length)).replace(/\0.*$/, '')
