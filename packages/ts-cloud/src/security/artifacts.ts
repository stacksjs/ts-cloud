...
export function generateCycloneDxSbom(root: string, options: { now?: Date; serial?: string } = {}): CycloneDxDocument {
  ...
  if (!existsSync(manifestPath)) throw new Error(`Cannot generate an SBOM: ${manifestPath} was not found`)
  ...
  return {
    bomFormat: 'CycloneDX',
    specVersion: '1.5',
    serialNumber: options.serial ?? `urn:uuid:${crypto.randomUUID()}`,
    version: 1,
    metadata: {
...
export async function generateImageSbom(
  ...
  if (exitCode !== 0) throw new Error(stderr.slice(0, 2_000) || `Syft exited with ${exitCode}`)
  return JSON.parse(stdout) as CycloneDxDocument
}

export function attachSbomToRelease(
  posture: SecurityPostureStore,
  scope: { organizationId: string; projectId: string; environmentId?: string; releaseId: string },
  sbom: CycloneDxDocument,
): ReleaseSecurityArtifact {
  const content = JSON.stringify(sbom)
  return posture.addReleaseArtifact({
    ...scope,
    kind: 'sbom',
    format: `cyclonedx-${sbom.specVersion}+json`,
    digest: `sha256:${sha256(content)}`,
    summary: {
      component: sbom.metadata.component.name,
...
export function attachVulnerabilitySummary(
  posture: SecurityPostureStore,
  scope: { organizationId: string; projectId: string; environmentId?: string; releaseId: string },
  ...
  for (const finding of findings) {
    ...
    if (typeof evidence.fixedVersion === 'string' && evidence.fixedVersion) summary.fixable++
  }
  ...
  return posture.addReleaseArtifact({
    ...scope,
    kind: 'vulnerability_summary',
    format: 'ts-cloud-vulnerability-summary+json',
    digest: `sha256:${sha256(content)}`,
    summary,
    content,
    sensitive: true,
    ...
    subject: [
      {
        ...
        digest: { sha256: input.artifactSha256?.replace(/^sha256:/, '') ?? sha256(input.artifact!) },
      },
    ],
    predicateType: 'https://slsa.dev/provenance/v1',
    predicate: {
      buildDefinition: {
        buildType: 'https://ts-cloud.dev/build/v1',
        externalParameters: input.externalParameters ?? {},
        internalParameters: {},
        resolvedDependencies: input.resolvedDependencies ?? [],
      },
      runDetails: {
        builder: { id: 'https://ts-cloud.dev/builders/local/v1' },
        metadata: { invocationId: input.invocationId, startedOn: input.startedAt, finishedOn: input.completedAt },
      },
    },
  }
}

export function attachProvenanceToRelease(
  posture: SecurityPostureStore,
  scope: { organizationId: string; projectId: string; environmentId?: string; releaseId: string },
  ...
  return posture.addReleaseArtifact({
    ...scope,
    kind: 'provenance',
    format: 'in-toto+slsa-v1+json',
    digest: `sha256:${sha256(content)}`,
    summary: {
      subjects: provenance.subject.length,
...
export function verifyArtifactSignature(input: {
  ...
  signature?: string
  publicKey?: string
}): { digestValid: boolean; signature: 'verified' | 'invalid' | 'not_configured' } {
  const expected = input.expectedSha256.replace(/^sha256:/, '').toLowerCase()
  const digestValid = sha256(input.artifact) === expected
  if (!input.signature || !input.publicKey) return { digestValid, signature: 'not_configured' }
  try {
    ...
    return {
      digestValid,
      signature: verifier.verify(input.publicKey, Buffer.from(input.signature, 'base64')) ? 'verified' : 'invalid',
    }
  } catch {
