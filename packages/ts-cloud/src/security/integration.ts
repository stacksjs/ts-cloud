...
export function securityScope(
  context: SecurityControlPlaneContext,
  environment: string,
): SecurityScope & { projectId: string; environmentId: string } {
  const record = context.environments.get(environment)
  if (!record) throw new Error(`Security scope for environment '${environment}' was not found`)
  return { organizationId: context.organization.id, projectId: context.project.id, environmentId: record.id }
}

export function ensureDefaultSecurityPolicies(
  context: SecurityControlPlaneContext,
  actorId?: string,
): SecurityPolicy[] {
  ...
  for (const [slug, environment] of context.environments) {
    if (existing.some((policy) => policy.environmentId === environment.id)) continue
    const production = slug === 'production' || environment.kind === 'production'
    existing.push(
      posture.createPolicy({
...
export function recordPreDeploySecretScan(
  posture: SecurityPostureStore,
  scope: SecurityScope,
  result: ScanResult,
  options: { scannerVersion?: string; skipped?: boolean } = {},
): { run: SecurityScanRun; findings: SecurityPostureFinding[] } {
  const findings: SecurityFindingInput[] = result.findings.map((finding) => ({
    ...
    remediation:
      'Remove and rotate the credential, then reference it through a managed secret or environment variable.',
  }))
...
export function recordSkippedSecretScan(
  posture: SecurityPostureStore,
  scope: SecurityScope,
  reason: string = 'The operator explicitly skipped source-secret scanning.',
): { run: SecurityScanRun; findings: SecurityPostureFinding[] } {
  return posture.recordScan({
    ...scope,
    scannerId: 'source-secrets',
...
function hostFindings(scope: SecurityScope, data: Record<string, any>): SecurityFindingInput[] {
  ...
  if (security?.firewall && !['active', 'configured', 'ok'].includes(String(security.firewall.status))) {
    findings.push({
      ...scope,
      ruleId: 'host-firewall-disabled',
      severity: 'high',
      title: 'Host firewall is not active',
      description: String(security.firewall.summary ?? 'The host firewall did not report an active configuration.'),
      subject: 'host:firewall',
      evidence: {
        status: String(security.firewall.status ?? 'unknown'),
        rules: Array.isArray(security.firewall.rules) ? security.firewall.rules.length : 0,
      },
  ...
  for (const port of security?.ports ?? []) {
    ...
    if (port.exposure === 'public' && Number.isFinite(number) && ![22, 80, 443].includes(number)) {
      findings.push({
        ...scope,
        ruleId: 'unexpected-public-listener',
        severity: 'medium',
        title: `Unexpected public listener on port ${number}`,
        description: `${String(port.processName ?? 'A process')} is listening publicly outside the standard SSH/HTTP/HTTPS ports.`,
        subject: `host:port:${number}`,
        evidence: { protocol: String(port.proto ?? ''), listen: value, process: String(port.processName ?? '') },
        remediation:
          'Bind the service to loopback/private networking or explicitly document and firewall the required public port.',
      })
    }
  }
  for (const certificate of security?.tlsCertificates ?? []) {
    ...
    if (Number.isFinite(days) && days < 30) {
      findings.push({
        ...scope,
        ruleId: 'tls-certificate-expiry',
        severity: days < 14 ? 'high' : 'medium',
        title: `TLS certificate expires in ${days} days`,
        description: `${String(certificate.domain ?? 'A configured domain')} needs renewal.`,
        subject: `tls:${String(certificate.domain ?? 'unknown')}`,
        evidence: {
          domain: String(certificate.domain ?? ''),
          daysRemaining: days,
          expires: String(certificate.expires ?? ''),
        },
  ...
  for (const check of data.diagnostics ?? []) {
    if (check.status === 'pass' || check.status === 'ok') continue
    findings.push({
      ...scope,
      ruleId: `diagnostic-${String(check.name ?? 'unknown')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')}`,
      severity: check.status === 'fail' ? 'high' : 'medium',
      title: String(check.name ?? 'Host security check'),
      description: String(check.detail ?? 'The host security diagnostic did not pass.'),
      subject: `diagnostic:${String(check.name ?? 'unknown')}`,
      evidence: { status: String(check.status ?? 'unknown') },
      remediation: 'Follow the diagnostic guidance, rerun the check, and confirm the finding resolves.',
    })
  }
  return findings
}

export function recordDashboardHostPosture(
  posture: SecurityPostureStore,
  scope: SecurityScope,
  ...
  return posture.recordScan({
    ...scope,
    scannerId: 'host-posture',
    scannerVersion: '1.0.0',
    status: !available ? 'unavailable' : findings.length ? 'failed' : 'passed',
    findings,
...
export function productionChangeReview(
  posture: SecurityPostureStore,
  input: {
  ...
  return {
    ...
    summary: {
      ...
      securityImpact: {
        ...
        assigned: findings.filter((item) => !!item.ownerActorId).length,
      },
