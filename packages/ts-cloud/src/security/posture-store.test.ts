...
describe('SecurityPostureStore finding lifecycle', () => {
  it('deduplicates findings, redacts evidence, resolves remediation, and tracks recurrence', () => {
    ...
    const first = f.posture.recordScan({
      ...scope,
      scannerId: 'fixture',
      ...
      findings: [
        {
          ...
          subject: 'image:openssl@1',
          evidence: { token: 'must-never-persist', installedVersion: '1' },
    ...
    const repeated = f.posture.recordScan({
      ...scope,
      scannerId: 'fixture',
    ...
    const recurrence = f.posture.recordScan({
      ...scope,
      scannerId: 'fixture',
      ...
      findings: [
        {
          ...
          description: 'The issue recurred.',
          subject: 'image:openssl@1',
        },
      ],
    })
    expect(recurrence.findings[0]).toMatchObject({
      id: first.findings[0].id,
      status: 'open',
      recurrenceCount: 1,
      severity: 'high',
    })
    expect(f.controlPlane.listEvents({ organizationId: f.organization.id }).map((event) => event.type)).toContain(
  ...
  it('blocks production, allows a valid waiver, re-blocks on expiry, then allows remediation', () => {
    const f = fixture()
    const scope = { organizationId: f.organization.id, projectId: f.project.id, environmentId: f.environment.id }
    const policy = f.posture.createPolicy({
      ...scope,
      name: 'Production baseline',
      scannerFailMode: 'closed',
      actorId: f.actor.id,
      rules: [
    ...
    const scan = f.posture.recordScan({
      ...scope,
      scannerId: 'container',
      ...
      findings: [
        {
          ...
          severity: 'critical',
          title: 'Known exploit',
          description: 'Exploit is public.',
          subject: 'release-1:libc',
          remediation: 'Rebuild on the patched base image.',
        },
      ],
    })
    ...
    const waiver = f.posture.createWaiver({
      findingId: scan.findings[0].id,
      policyId: policy.id,
      actorId: f.actor.id,
      reason: 'Emergency customer recovery while the patched image builds.',
      referenceUrl: 'https://tickets.example/SEC-1',
      expiresAt: '2026-07-21T13:00:00.000Z',
    })
    expect(f.posture.evaluateGate({ ...scope, policyId: policy.id })).toMatchObject({
      outcome: 'allow',
      waiverIds: [waiver.id],
    })
  ...
  it('supports ownership, comments, sensitive exports, and degraded scanner states', () => {
    ...
    const policy = f.posture.createPolicy({
      ...
      name: 'Fail closed',
      scannerFailMode: 'closed',
      requiredScanners: ['tls'],
      rules: [{ minimumSeverity: 'high', action: 'block' }],
    })
    const scan = f.posture.recordScan({
      ...scope,
      scannerId: 'tls',
      scannerVersion: '1',
      status: 'unavailable',
      error: 'token=not-for-logs',
    })
    expect(scan.run.error).not.toContain('not-for-logs')
    expect(f.posture.evaluateGate({ ...scope, policyId: policy.id }).outcome).toBe('block')

    const finding = f.posture.recordScan({
      ...scope,
      scannerId: 'host',
      scannerVersion: '1',
      status: 'failed',
      findings: [
        {
          ...
          severity: 'medium',
          title: 'Password SSH',
          description: 'Password auth is enabled.',
          subject: 'sshd',
        },
      ],
    }).findings[0]
    ...
    expect(
      f.posture.addComment({
        findingId: finding.id,
        actorId: f.actor.id,
        body: 'Tracked in the host hardening sprint.',
        referenceUrl: 'https://tickets.example/SEC-2',
      }).body,
    ).toContain('hardening')
    expect(f.posture.exportPosture(f.organization.id)).toMatchObject({
  ...
  it('makes a missing required scanner an explicit fail-closed decision', () => {
    ...
    const policy = f.posture.createPolicy({
      ...
      rules: [{ minimumSeverity: 'critical', action: 'block' }],
    })
