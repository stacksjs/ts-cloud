...
describe('SecurityScannerRunner', () => {
  it('bounds scanner runtime and persists an explicit unavailable result', async () => {
    ...
    const result = await runner.run(
      { id: 'hung-scanner', version: '7', timeoutMs: 100, scan: () => new Promise(() => {}) },
      {
  ...
  it('turns real IAM simulation decisions into normalized least-privilege findings', async () => {
    const f = fixture()
    const scanner = new AwsIamCapabilityScanner(['cloudformation:CreateStack', 's3:GetObject'], {
      client: {
        getCallerIdentity: async () => ({ Account: '123456789012', Arn: 'arn:aws:iam::123456789012:role/deployer' }),
        simulatePrincipalPolicy: async () => ({
          EvaluationResults: [
    ...
    expect(result.findings[0].evidence).toMatchObject({
      ...
      principalArn: 'arn:aws:iam::123456789012:role/deployer',
    })
  })
})
