...
import type { EvaluateSecurityGateInput, RecordSecurityScanInput, ReleaseSecurityArtifact, SecurityCheckStatus, SecurityDeployDecision, SecurityFindingComment, SecurityFindingInput, SecurityPolicy, SecurityPolicyAction, SecurityPolicyRule, SecurityPostureFinding, SecurityPostureSummary, SecurityScanRun, SecuritySeverity, SecurityWaiver } from './types'
...
function mapScan(row: Row): SecurityScanRun {
  return {
    ...
    releaseId: optionalString(row.release_id),
    scannerId: String(row.scanner_id),
    scannerVersion: String(row.scanner_version),
    status: String(row.status) as SecurityCheckStatus,
...
function mapFinding(row: Row): SecurityPostureFinding {
  return {
    ...
    severity: String(row.severity) as SecuritySeverity,
    title: String(row.title),
    description: String(row.description),
    evidence: parseJson(row.evidence, {}),
    remediation: optionalString(row.remediation),
    subject: String(row.subject),
    status: String(row.status) as SecurityPostureFinding['status'],
    ownerActorId: optionalString(row.owner_actor_id),
...
function mapPolicy(row: Row): SecurityPolicy {
  return {
    ...
    requiredScanners: parseJson(row.required_scanners, []),
    scannerFailMode: String(row.scanner_fail_mode) as 'open' | 'closed',
...
function mapWaiver(row: Row, now: string): SecurityWaiver {
  ...
  return {
    ...
    referenceUrl: optionalString(row.reference_url),
...
function mapArtifact(row: Row): ReleaseSecurityArtifact {
  return {
    ...
    kind: String(row.kind) as ReleaseSecurityArtifact['kind'],
...
function mapDecision(row: Row): SecurityDeployDecision {
  return {
    ...
    outcome: String(row.outcome) as SecurityDeployDecision['outcome'],
    scannerVersions: parseJson(row.scanner_versions, {}),
    findingIds: parseJson(row.finding_ids, []),
    waiverIds: parseJson(row.waiver_ids, []),
...
function validateRules(rules: SecurityPolicyRule[]): SecurityPolicyRule[] {
  if (!rules.length) throw new Error('A security policy needs at least one rule')
  return rules.map((rule) => {
    if (!(rule.minimumSeverity in SEVERITY_RANK)) throw new Error(`Unknown security severity: ${rule.minimumSeverity}`)
    if (!(rule.action in ACTION_RANK)) throw new Error(`Unknown security policy action: ${rule.action}`)
    return {
      minimumSeverity: rule.minimumSeverity,
      action: rule.action,
      ...(rule.scannerId ? { scannerId: rule.scannerId.trim() } : {}),
    }
  })
}
...
export class SecurityPostureStore {
    ...
    this.controlPlane.transaction(() => {
      this.run(
        ...
        [
          ...
          input.scannerId.trim(),
          input.scannerVersion.trim(),
          input.status,
          safeError(input.error) ?? null,
          json(input.metadata ?? {}),
      ...
      for (const raw of sourceFindings) {
        ...
        const existingRow = this.controlPlane.database
          .query<Row, [string]>('SELECT * FROM security_findings WHERE fingerprint = ?')
          .get(fingerprint)
        const activeWaiver = existingRow
          ? this.controlPlane.database
              .query<Row, [string, string]>(
                `SELECT * FROM security_waivers WHERE finding_id = ? AND revoked_at IS NULL AND expires_at > ? ORDER BY expires_at DESC LIMIT 1`,
              )
              .get(String(existingRow.id), completedAt)
          : undefined
        if (existingRow) {
          const recurrence =
            String(existingRow.status) === 'resolved'
              ? Number(existingRow.recurrence_count) + 1
              : Number(existingRow.recurrence_count)
          const nextStatus = activeWaiver
            ? 'waived'
            : String(existingRow.status) === 'acknowledged'
              ? 'acknowledged'
              : 'open'
          this.run(
          ...
          this.run(
            ...
            [
              ...
              finding.environmentId ?? null,
              finding.resourceId ?? null,
              finding.releaseId ?? null,
              runId,
              input.scannerId,
              input.scannerVersion,
              finding.ruleId.trim(),
      ...
      if (input.status === 'passed' || input.status === 'failed') {
        const candidates = this.controlPlane.database
          .query<Row, [string, string, string | null, string | null, string | null, string | null]>(
        ...
        for (const candidate of candidates) {
          if (observed.has(String(candidate.fingerprint))) continue
          this.run(`UPDATE security_findings SET status = 'resolved', resolved_at = ?, updated_at = ? WHERE id = ?`, [
            completedAt,
            completedAt,
            String(candidate.id),
          ])
          this.controlPlane.appendEvent({
            ...
            payload: { findingId: String(candidate.id), scannerId: input.scannerId, reason: 'not-observed' },
          })
        }
      }
      ...
      this.controlPlane.appendEvent({
        ...
        level: input.status === 'failed' || DEGRADED_CHECKS.has(input.status) ? 'warning' : 'info',
        payload: {
          scanRunId: runId,
          scannerId: input.scannerId,
          scannerVersion: input.scannerVersion,
          status: input.status,
          findingsCount: sourceFindings.length,
          durationMs,
        },
      })
    })
  ...
  listScanRuns(scope: {
    ...
    return this.controlPlane.database
      ...
      .all(
        ...
        Math.min(500, Math.max(1, scope.limit ?? 100)),
      )
      .map(mapScan)
  }
  ...
  listFindings(options: {
    ...
    if (options.environmentId) {
      ...
      bindings.push(options.environmentId)
    }
    if (options.status) {
      where.push('status = ?')
  ...
  assignFinding(findingId: string, ownerActorId: string | undefined, actorId?: string): SecurityPostureFinding {
    ...
    if (!finding) throw new Error('Security finding was not found')
    if (ownerActorId && !this.controlPlane.getActor(ownerActorId)) throw new Error('Finding owner was not found')
    this.run('UPDATE security_findings SET owner_actor_id = ?, updated_at = ? WHERE id = ?', [
      ownerActorId ?? null,
      this.now(),
      findingId,
    ])
    this.controlPlane.appendEvent({
      ...
      actorId,
      type: 'security.finding.assigned',
      payload: { findingId, ownerActorId: ownerActorId ?? null },
    })
  ...
  acknowledgeFinding(findingId: string, actorId?: string): SecurityPostureFinding {
    const finding = this.getFinding(findingId)
    if (!finding) throw new Error('Security finding was not found')
    if (finding.status === 'resolved') throw new Error('A resolved finding cannot be acknowledged')
    this.run(`UPDATE security_findings SET status = 'acknowledged', updated_at = ? WHERE id = ?`, [
      this.now(),
      findingId,
    ])
    this.controlPlane.appendEvent({
  ...
  addComment(input: {
    ...
    if (body.length < 2 || body.length > 4_000) throw new Error('Finding comments must contain 2-4,000 characters')
    const referenceUrl = validateUrl(input.referenceUrl)
    const id = this.idFn()
    const createdAt = this.now()
    this.run(
      'INSERT INTO security_finding_comments (id, finding_id, actor_id, body, reference_url, created_at) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    this.controlPlane.appendEvent({
      ...
      type: 'security.finding.commented',
      payload: { findingId: input.findingId, commentId: id, referenceUrl: referenceUrl ?? null },
    })
  ...
  listComments(findingId: string): SecurityFindingComment[] {
    return this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM security_finding_comments WHERE finding_id = ? ORDER BY created_at, id')
      .all(findingId)
      .map((row) => ({
        id: String(row.id),
        findingId: String(row.finding_id),
  ...
  createWaiver(input: {
    ...
    if (reason.length < 8 || reason.length > 1_000) throw new Error('A waiver reason must contain 8-1,000 characters')
    const now = this.now()
    ...
    this.controlPlane.transaction(() => {
      this.run(
        `INSERT INTO security_waivers (id, finding_id, policy_id, reason, reference_url, created_by_actor_id, expires_at, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          finding.id,
  ...
  revokeWaiver(id: string, actorId: string): SecurityWaiver {
    ...
    if (!waiver) throw new Error('Security waiver was not found')
    if (waiver.revokedAt) return waiver
    ...
    this.controlPlane.transaction(() => {
      ...
      this.run(`UPDATE security_findings SET status = 'open', updated_at = ? WHERE id = ? AND status = 'waived'`, [
        now,
        waiver.findingId,
      ])
      this.controlPlane.appendEvent({
        ...
        resourceId: finding.resourceId,
        actorId,
        type: 'security.waiver.revoked',
        level: 'warning',
        payload: { waiverId: id, findingId: waiver.findingId },
      })
    })
    return this.getWaiver(id)!
  }
  ...
  listWaivers(findingId?: string): SecurityWaiver[] {
    const rows = findingId
      ? this.controlPlane.database
          .query<Row, [string]>('SELECT * FROM security_waivers WHERE finding_id = ? ORDER BY created_at DESC')
          .all(findingId)
  ...
  expireWaivers(): number {
    ...
    for (const row of rows) {
      ...
      this.controlPlane.appendEvent({
        ...
        projectId: optionalString(row.project_id),
        resourceId: optionalString(row.resource_id),
        type: 'security.waiver.expired',
        level: 'warning',
        payload: { waiverId: String(row.waiver_id), findingId: String(row.id) },
      })
    }
    return rows.length
  }
  ...
  createPolicy(input: {
    ...
    if (!name) throw new Error('Security policy name is required')
    const rules = validateRules(input.rules)
    const id = this.idFn()
    const now = this.now()
    const requiredScanners = [
      ...new Set((input.requiredScanners ?? []).map((value) => value.trim()).filter(Boolean)),
    ].sort()
    this.run(
      `INSERT INTO security_policies (id, organization_id, environment_id, name, rules, required_scanners, scanner_fail_mode, created_by_actor_id, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        input.organizationId,
        ...
        name,
        json(rules),
        json(requiredScanners),
        input.scannerFailMode,
        input.actorId ?? null,
        now,
        now,
      ],
    ...
    this.controlPlane.appendEvent({
      ...
      payload: {
        ...
        scannerFailMode: input.scannerFailMode,
        requiredScanners,
        rules: sanitizeControlPlaneValue(rules),
      },
    })
  ...
  updatePolicy(
    ...
    const requiredScanners = input.requiredScanners
      ? [...new Set(input.requiredScanners.map((value) => value.trim()).filter(Boolean))].sort()
    ...
    const result = this.controlPlane.database.run(
      ...
      [
        input.name?.trim() || policy.name,
        json(rules),
        json(requiredScanners),
        input.scannerFailMode ?? policy.scannerFailMode,
        (input.enabled ?? policy.enabled) ? 1 : 0,
        now,
        id,
        expectedVersion,
      ],
    )
    if (result.changes !== 1) throw new Error(`Security policy ${id} changed since version ${expectedVersion}`)
    this.controlPlane.appendEvent({
  ...
  listPolicies(organizationId: string): SecurityPolicy[] {
    return this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM security_policies WHERE organization_id = ? ORDER BY environment_id, name')
      .all(organizationId)
      .map(mapPolicy)
  }
  ...
  evaluateGate(input: EvaluateSecurityGateInput): SecurityDeployDecision {
    ...
    const resolvedPolicy =
      policy && 'rules' in policy && Array.isArray(policy.rules)
        ? (policy as SecurityPolicy)
        : policy
          ? mapPolicy(policy as Row)
          : undefined
    ...
    for (const finding of findings) {
      const waiver = this.listWaivers(finding.id).find(
        (item) => item.state === 'active' && (!item.policyId || item.policyId === resolvedPolicy.id),
      )
      if (waiver) activeWaivers.set(finding.id, waiver)
    }

    const actionFor = (finding: SecurityPostureFinding): SecurityPolicyAction =>
      resolvedPolicy.rules
        .filter(
          (rule) =>
            (!rule.scannerId || rule.scannerId === finding.scannerId) &&
            SEVERITY_RANK[finding.severity] >= SEVERITY_RANK[rule.minimumSeverity],
        )
        .reduce<SecurityPolicyAction>(
          (action, rule) => (ACTION_RANK[rule.action] > ACTION_RANK[action] ? rule.action : action),
          'record',
        )
    ...
    const latestRows = this.controlPlane.database
      .query<Row, [string, string, string]>(
    ...
    const latest = latestRows
      .map(mapScan)
      .map((run) =>
        nowMs - new Date(run.completedAt).getTime() > staleAfterMs ? { ...run, status: 'stale' as const } : run,
      )
    const observedScanners = new Set(latest.map((run) => run.scannerId))
    const missing = resolvedPolicy.requiredScanners
      .filter((scannerId) => !observedScanners.has(scannerId))
      .map((scannerId) => ({
        id: `missing:${scannerId}`,
        organizationId: input.organizationId,
        projectId: input.projectId,
        environmentId: input.environmentId,
        scannerId,
    ...
    const degraded = evaluatedScans.filter(
      (run) => resolvedPolicy.requiredScanners.includes(run.scannerId) && DEGRADED_CHECKS.has(run.status),
    )
    ...
    const outcome: SecurityDeployDecision['outcome'] =
      blockers.length || unavailableBlocks ? 'block' : warnings.length || degraded.length ? 'warn' : 'allow'
    const details: string[] = []
    if (blockers.length) details.push(`${blockers.length} finding${blockers.length === 1 ? '' : 's'} blocked by policy`)
    if (warnings.length) details.push(`${warnings.length} warning${warnings.length === 1 ? '' : 's'} require review`)
    if (degraded.length)
      details.push(
        `${degraded.length} scanner${degraded.length === 1 ? '' : 's'} ${resolvedPolicy.scannerFailMode === 'closed' ? 'failed closed' : 'failed open'}`,
      )
    if (activeWaivers.size)
      details.push(`${activeWaivers.size} active time-limited waiver${activeWaivers.size === 1 ? '' : 's'}`)
    if (!details.length) details.push('All recorded checks satisfy policy')

    const id = this.idFn()
    const createdAt = this.now()
    this.run(
      `INSERT INTO security_deploy_decisions
      ...
      [
        ...
        outcome,
        json(scannerVersions),
        json([...blockers, ...warnings].map((item) => item.id)),
        json([...activeWaivers.values()].map((item) => item.id)),
        details.join('; '),
        createdAt,
      ],
    )
    this.controlPlane.appendEvent({
      ...
      type: 'security.deploy.decision',
      level: outcome === 'block' ? 'error' : outcome === 'warn' ? 'warning' : 'info',
      payload: {
        decisionId: id,
        outcome,
        ...
        waiverIds: [...activeWaivers.values()].map((item) => item.id),
  ...
  getDecision(id: string): SecurityDeployDecision | undefined {
    const row = this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM security_deploy_decisions WHERE id = ?')
      .get(id)
  ...
  listDecisions(environmentId: string, limit: number = 100): SecurityDeployDecision[] {
    return this.controlPlane.database
      .query<Row, [string, number]>(
        'SELECT * FROM security_deploy_decisions WHERE environment_id = ? ORDER BY created_at DESC LIMIT ?',
      )
      .all(environmentId, Math.min(500, Math.max(1, limit)))
      .map(mapDecision)
  }
  ...
  addReleaseArtifact(input: Omit<ReleaseSecurityArtifact, 'id' | 'createdAt'>): ReleaseSecurityArtifact {
    const existing = this.controlPlane.database
      .query<Row, [string, string, string]>(
        'SELECT * FROM release_security_artifacts WHERE release_id = ? AND kind = ? AND digest = ?',
      )
    ...
    const createdAt = this.now()
    this.run(
      `INSERT INTO release_security_artifacts
    ...
    this.controlPlane.appendEvent({
      organizationId: input.organizationId,
      projectId: input.projectId,
      type: 'security.release_artifact.created',
      payload: {
        artifactId: id,
    ...
    return mapArtifact(
      this.controlPlane.database.query<Row, [string]>('SELECT * FROM release_security_artifacts WHERE id = ?').get(id)!,
    )
  }

  listReleaseArtifacts(releaseId: string, includeSensitiveContent: boolean = false): ReleaseSecurityArtifact[] {
    return this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM release_security_artifacts WHERE release_id = ? ORDER BY created_at, kind')
      .all(releaseId)
      .map(mapArtifact)
      .map((artifact) =>
        artifact.sensitive && !includeSensitiveContent ? { ...artifact, content: undefined } : artifact,
      )
  }
  ...
  summary(organizationId: string, projectId?: string, environmentId?: string): SecurityPostureSummary {
    ...
    for (const finding of findings) {
      if (finding.status === 'open' || finding.status === 'acknowledged') open[finding.severity]++
    }
    const checks: Record<SecurityCheckStatus, number> = {
      passed: 0,
      failed: 0,
      skipped: 0,
      unavailable: 0,
    ...
    for (const scan of scans) checks[scan.status]++
    return {
      open,
      waived: findings.filter((item) => item.status === 'waived').length,
      resolved: findings.filter((item) => item.status === 'resolved').length,
      checks,
      lastScannedAt: scans[0]?.completedAt,
    }
  }
  ...
  exportPosture(organizationId: string): JsonValue {
    ...
    return sanitizeControlPlaneValue({
      format: 'ts-cloud-security-posture',
      version: 1,
      exportedAt: this.now(),
      organizationId,
      policies: this.listPolicies(organizationId),
      findings: this.listFindings({ organizationId, limit: 1_000 }),
      waivers: this.listWaivers().filter(
        (waiver) => this.getFinding(waiver.findingId)?.organizationId === organizationId,
      ),
      scans: projects.flatMap((project) => this.listScanRuns({ organizationId, projectId: project.id, limit: 1_000 })),
      decisions: projects.flatMap((project) =>
        this.controlPlane
          .listEnvironments(project.id)
          .flatMap((environment) => this.listDecisions(environment.id, 1_000)),
      ),
    })
  }
}
