...
describe('release security artifacts', () => {
  it('generates a component inventory and links the sensitive SBOM to a release', () => {
    ...
    try {
      ...
      const sbom = generateCycloneDxSbom(root, {
        now: new Date('2026-07-21T12:00:00.000Z'),
        serial: 'urn:uuid:fixture',
      })
      expect(sbom.components.map((component) => component.name)).toEqual(['hono', 'sharp'])
      const artifact = attachSbomToRelease(
        f.posture,
        {
  ...
  it('creates digest-bound provenance and optionally verifies a release signature', () => {
    ...
    ).toEqual({ digestValid: true, signature: 'verified' })
    expect(
      verifyArtifactSignature({
    ...
    ).toEqual({ digestValid: false, signature: 'invalid' })
    ...
    const f = fixture()
    const digestProvenance = createReleaseProvenance({
      artifactName: 'image',
    ...
    ).toMatchObject({ kind: 'provenance', sensitive: true })
  })
})
