...
function page<T extends Record<string, unknown>>(
  ...
  if (decoded) {
    ...
    if (found < 0) throw new ApiServiceError('invalid_cursor', 'The pagination cursor is no longer available.', 400)
    start = found + 1
  }
  ...
  return {
    ...
    page: {
      hasMore,
      nextCursor: hasMore && last ? cursor(Object.fromEntries(keys.map((key) => [key, last[key]]))) : undefined,
    },
    requestId,
  }
}

export function createApiV1Handler(
  options: ApiV1HandlerOptions,
): (request: Request, networkHint?: string) => Promise<Response | undefined> {
  ...
  const response = (body: unknown, status: number, requestId: string, extra: Record<string, string> = {}) =>
  ...
  ) => response({ error: { code, message, requestId, details } } satisfies ApiErrorEnvelope, status, requestId, headers)
  ...
  return async (request: Request, networkHint?: string): Promise<Response | undefined> => {
    ...
    if (!principal)
      return failure('unauthorized', 'A valid bearer token is required.', 401, requestId, undefined, {
        'www-authenticate': 'Bearer realm="ts-cloud", error="invalid_token"',
      })
    ...
    const rateHeaders = {
      ...
      'x-ratelimit-reset': String(Math.ceil((current.start + 60_000) / 1000)),
    }
    if (current.count > limit)
      return failure('rate_limited', 'API rate limit exceeded.', 429, requestId, undefined, {
        ...rateHeaders,
        'retry-after': String(Math.max(1, Math.ceil((current.start + 60_000 - timestamp) / 1000))),
      })
    ...
    try {
      ...
      const composeAction =
        /^\/api\/v1\/compose-applications\/([^/]+)\/(deploy|redeploy|start|stop|scale|delete)$/.exec(url.pathname)
      const composeServices = /^\/api\/v1\/compose-applications\/([^/]+)\/services$/.exec(url.pathname)
      const releaseAction = /^\/api\/v1\/releases\/([^/]+)\/(promote|approve|activate|rollback|health|pin)$/.exec(
        url.pathname,
      )
      if (request.method === 'GET' && url.pathname === '/api/v1/projects')
        ...
        body = page(
          service.listServices(principal, projectId, url.searchParams.get('environmentId') ?? undefined),
          url,
          requestId,
        )
        ...
        const values = previews.previews
          .listInstances({ projectId })
          .filter((preview) => {
            try {
              service.authorize(principal, 'deployments:read', { type: 'resource', id: preview.resourceId })
              return true
            } catch {
              return false
            }
          })
          .map((preview) => ({ ...preview }) as Record<string, unknown>)
        body = page(values, url, requestId)
      } else if (request.method === 'GET' && url.pathname === '/api/v1/compose-templates')
        body = page(
          listComposeTemplates().map((value) => ({ ...value }) as Record<string, unknown>),
          url,
          requestId,
          ['id', 'version'],
        )
      else if (request.method === 'GET' && url.pathname === '/api/v1/releases/capabilities') {
        const kind = String(url.searchParams.get('kind') ?? '') as any
        body = {
          capabilities: releaseStrategyCapabilities({
            kind,
            hasHealthGate: url.searchParams.get('health') === 'true',
            replicas: Number(url.searchParams.get('replicas')) || 1,
          }),
          requestId,
        }
        ...
        const values = releases.releases
          .list({
            projectId,
          ...
          .filter((release) => {
            try {
              service.authorize(principal, 'deployments:read', { type: 'resource', id: release.resourceId })
              return true
            } catch {
              return false
            }
          ...
          .map(
            (release) =>
              ({
                ...release,
                artifact: releases.releases.getArtifact(release.artifactId),
                transitions: releases.releases.transitions(release.id),
                approvals: releases.releases.approvals(release.id),
              }) as Record<string, unknown>,
          )
        body = page(values, url, requestId)
      } else if (request.method === 'POST' && url.pathname === '/api/v1/release-artifacts') {
        const input = await readBody()
        service.authorize(principal, 'applications:manage', { type: 'organization' })
        body = {
          artifact: releases.releases.registerArtifact({
            organizationId: principal.serviceAccount.organizationId,
            digest: String(input.digest ?? ''),
            kind: String(input.kind ?? '') as any,
            uri: String(input.uri ?? ''),
            size: Number(input.size),
            mediaType: typeof input.mediaType === 'string' ? input.mediaType : undefined,
            provenance: input.provenance as any,
            attestation: input.attestation as any,
          }),
          requestId,
        }
        ...
        service.authorize(principal, 'applications:manage', { type: 'resource', id: resourceId })
        body = {
          release: releases.releases.create({
            organizationId: principal.serviceAccount.organizationId,
            ...
            kind: String(input.kind ?? '') as any,
            sourceSha: typeof input.sourceSha === 'string' ? input.sourceSha : undefined,
            config: (input.config as any) ?? {},
            manifest: (input.manifest as any) ?? {},
            provenance: input.provenance as any,
            strategy: String(input.strategy ?? 'atomic') as any,
            healthGate: input.healthGate as any,
            hooks: input.hooks as any,
            drainSeconds: Number(input.drainSeconds) || undefined,
            graceSeconds: Number(input.graceSeconds) || undefined,
            automaticRollback: input.automaticRollback !== false,
            actorId: principal.actor.id,
            trigger: typeof input.trigger === 'string' ? input.trigger : 'api',
            approvalRequired: input.approvalRequired === true,
            replicas: Number(input.replicas) || undefined,
          }),
          requestId,
        }
      } else if (request.method === 'GET' && /^\/api\/v1\/releases\/[^/]+$/.test(url.pathname)) {
        const id = decodeURIComponent(url.pathname.split('/').at(-1)!)
        const release = releases.releases.get(id)
        if (!release) throw new ApiServiceError('not_found', 'Release was not found.', 404)
        service.authorize(principal, 'deployments:read', { type: 'resource', id: release.resourceId })
        body = {
          release,
          artifact: releases.releases.getArtifact(release.artifactId),
          transitions: releases.releases.transitions(id),
          approvals: releases.releases.approvals(id),
          requestId,
        }
      } else if (request.method === 'POST' && releaseAction) {
        const release = releases.releases.get(decodeURIComponent(releaseAction[1]))
        if (!release) throw new ApiServiceError('not_found', 'Release was not found.', 404)
        const input = await readBody()
        const action = releaseAction[2]
        if (action === 'promote') {
          const targetResourceId = String(input.targetResourceId ?? '')
          service.authorize(principal, 'deployments:create', { type: 'resource', id: targetResourceId })
          body = {
            release: releases.releases.promote(release.id, {
              targetEnvironmentId: String(input.targetEnvironmentId ?? ''),
              targetResourceId,
              config: (input.config as any) ?? {},
          ...
          service.authorize(principal, 'deployments:create', { type: 'resource', id: release.resourceId })
          body = {
            release: releases.releases.approve(release.id, {
              actorId: principal.actor.id,
          ...
          service.authorize(principal, 'deployments:create', { type: 'resource', id: release.resourceId })
          body = {
            operation: service.operation(releases.enqueueActivation(release, { actorId: principal.actor.id })),
            requestId,
          }
          ...
          body = {
            operation: service.operation(
              releases.enqueueRollback(release, {
                actorId: principal.actor.id,
                targetReleaseId: typeof input.targetReleaseId === 'string' ? input.targetReleaseId : undefined,
              }),
            ),
            requestId,
          }
          ...
          service.authorize(principal, 'deployments:create', { type: 'resource', id: release.resourceId })
          const result = releases.completeHealthGate(release.id, {
          ...
          body = {
            ...
            rollbackOperation: result.rollbackOperation ? service.operation(result.rollbackOperation) : undefined,
            requestId,
          }
        } else {
          service.authorize(principal, 'deployments:create', { type: 'resource', id: release.resourceId })
          body = {
            release: releases.releases.pin(
              release.id,
              input.pinned !== false,
        ...
        if (!projectId) throw new ApiServiceError('validation_error', 'projectId is required.', 422)
        service.authorize(principal, 'project:read', { type: 'project', id: projectId })
        const values = compose.applications
          .list({ projectId, environmentId: url.searchParams.get('environmentId') ?? undefined })
          .filter((application) => {
            try {
              service.authorize(principal, 'deployments:read', { type: 'resource', id: application.resourceId })
              return true
            } catch {
              return false
            }
          ...
          .map(
            (application) =>
              ({ ...application, services: compose.applications.services(application.id) }) as Record<string, unknown>,
          )
        body = page(values, url, requestId)
        ...
        service.authorize(principal, 'project:read', { type: 'project', id: projectId })
        body = {
          result: compose.applications.preview(String(input.source ?? ''), {
        ...
        const input = await readBody()
        const projectId = String(input.projectId ?? '')
        service.authorize(principal, 'applications:manage', {
          type: 'environment',
          id: String(input.environmentId ?? ''),
        })
        const target = {
          name: String(input.name ?? ''),
          slug: typeof input.slug === 'string' ? input.slug : undefined,
        ...
        const result =
          typeof input.templateId === 'string'
            ? compose.applications.fromTemplate(
                input.templateId,
                input.inputs && typeof input.inputs === 'object' && !Array.isArray(input.inputs)
                  ? (input.inputs as Record<string, string>)
                  : {},
                { ...target, version: typeof input.templateVersion === 'string' ? input.templateVersion : undefined },
              )
            : compose.applications.import(String(input.source ?? ''), target)
        ...
        const application = compose.applications.get(decodeURIComponent(composeServices[1]))
        if (!application) throw new ApiServiceError('not_found', 'Compose application was not found.', 404)
        service.authorize(principal, 'deployments:read', { type: 'resource', id: application.resourceId })
        body = { application, services: compose.applications.services(application.id), requestId }
      } else if (request.method === 'POST' && composeAction) {
        const application = compose.applications.get(decodeURIComponent(composeAction[1]))
        if (!application) throw new ApiServiceError('not_found', 'Compose application was not found.', 404)
        const action = composeAction[2] as 'deploy' | 'redeploy' | 'start' | 'stop' | 'scale' | 'delete'
        const input = await readBody()
        service.authorize(
          principal,
          action === 'delete' || action === 'stop' ? 'deployments:cancel' : 'deployments:create',
          { type: 'resource', id: application.resourceId },
        )
        body = {
          operation: service.operation(
            compose.enqueue(application, action, {
              actorId: principal.actor.id,
              service: typeof input.service === 'string' ? input.service : undefined,
              replicas: Number.isFinite(Number(input.replicas)) ? Number(input.replicas) : undefined,
              removeVolumes: input.removeVolumes === true,
              confirmation: typeof input.confirm === 'string' ? input.confirm : undefined,
            }),
          ),
          requestId,
        }
        ...
        service.authorize(principal, 'project:read', { type: 'project', id: projectId })
        body = page(
          previews.previews.listDefinitions(projectId).map((value) => ({ ...value }) as Record<string, unknown>),
          url,
          requestId,
        )
        ...
        body = {
          definition: previews.previews.createDefinition({
            ...input,
            projectId: String(input.projectId ?? ''),
            resourceId: String(input.resourceId ?? ''),
            baseEnvironmentId: String(input.baseEnvironmentId ?? ''),
            domainPattern: String(input.domainPattern ?? ''),
            createdByActorId: principal.actor.id,
          }),
          requestId,
        }
        ...
        const input = await readBody()
        const policy = previews.previews.getDefinition(String(input.definitionId ?? ''))
        ...
        const persisted = previews.previews.upsert({
          definitionId: policy.id,
          sourceProvider: 'api',
          repository: typeof input.repository === 'string' ? input.repository : 'api',
          branch: String(input.branch ?? ''),
          pullRequestNumber: Number(input.pullRequestNumber) || undefined,
        ...
        body = {
          ...
          operation: service.operation(
            previews.enqueueDeploy(persisted.preview, { created: persisted.created, actorId: principal.actor.id }),
          ),
          requestId,
        }
      } else if (request.method === 'POST' && previewAction) {
        const preview = previews.previews.getInstance(decodeURIComponent(previewAction[1]))
        if (!preview) throw new ApiServiceError('not_found', 'Preview was not found.', 404)
        const input = await readBody()
        const action = previewAction[2]
        if (action === 'destroy') {
          service.authorize(principal, 'deployments:cancel', { type: 'resource', id: preview.resourceId })
          if (input.confirm !== preview.name)
            throw new ApiServiceError('confirmation_required', `Type ${preview.name} to confirm preview teardown.`, 409)
          body = {
            operation: service.operation(previews.enqueueDestroy(preview, 'api', principal.actor.id)),
            requestId,
          }
          ...
          body = {
            operation: service.operation(
              previews.enqueueDeploy(preview, { actorId: principal.actor.id, reason: 'manual_rebuild' }),
            ),
            requestId,
          }
        }
        ...
        const input = await readBody()
        if (!input.dryRun && input.confirm !== 'cleanup previews')
          throw new ApiServiceError('confirmation_required', 'Type cleanup previews to confirm preview cleanup.', 409)
        body = {
          ...previews.cleanup({
            dryRun: input.dryRun === true,
            maxAgeHours: Number(input.maxAgeHours) || undefined,
            keepCount: Number(input.keepCount) || undefined,
            actorId: principal.actor.id,
          }),
          requestId,
        }
        ...
        const values = queue
          ...
          .filter((item) => {
            const requested = item.operation.resourceId
              ? { type: 'resource' as const, id: item.operation.resourceId }
              : item.operation.environmentId
                ? { type: 'environment' as const, id: item.operation.environmentId }
                : item.operation.projectId
                  ? { type: 'project' as const, id: item.operation.projectId }
          ...
          .map((item) => ({
            ...service.operation(item.operation),
            queue: item.job,
            approximatePosition: item.approximatePosition,
          }))
        body = page(values, url, requestId)
        ...
        service.authorize(principal, 'automation:manage', { type: 'organization' })
        const input = await readBody()
        if (input.confirm !== 'update queue limits')
          throw new ApiServiceError(
            'confirmation_required',
            'Type update queue limits to confirm production concurrency changes.',
            409,
          )
        body = {
          concurrency: queue.configureConcurrency(input.concurrency ?? {}, {
            organizationId: principal.serviceAccount.organizationId,
            actorId: principal.actor.id,
          }),
          requestId,
        ...
        service.authorize(principal, 'automation:manage', { type: 'organization' })
        const input = await readBody()
        if (input.confirm !== 'clear completed')
          throw new ApiServiceError(
            'confirmation_required',
            'Type clear completed to confirm queue retention cleanup.',
            409,
          )
        body = {
          deleted: queue.clearCompleted({
        ...
        const requested = operation.resourceId
          ? { type: 'resource' as const, id: operation.resourceId }
          : operation.environmentId
            ? { type: 'environment' as const, id: operation.environmentId }
            : operation.projectId
              ? { type: 'project' as const, id: operation.projectId }
              : { type: 'organization' as const }
        service.authorize(principal, 'deployments:read', requested)
        ...
        if (operationLogs[2] === 'stream') {
          ...
          const stream = new ReadableStream<Uint8Array>({
            start(controller) {
              const flush = () => {
                if (closed) return
                if (!options.identities.verifyToken(bearerToken, networkHint)) {
                  closed = true
                ...
                if (
                  current &&
                  ['succeeded', 'failed', 'cancelled', 'timed_out'].includes(current.state) &&
                  !queue.logs(operation.id, { after: cursor, limit: 1 }).length
                ) {
                  closed = true
                  if (timer) clearInterval(timer)
                  controller.enqueue(
        ...
        body = {
          data: entries,
          cursor: entries.at(-1)?.sequence ?? after,
          hasMore: entries.length >= Math.min(1000, Math.max(1, Number(url.searchParams.get('limit')) || 200)),
          requestId,
        }
        ...
        if (operationAction[2] === 'cancel') {
          ...
          body = {
            operation: service.operation(queue.requestCancellation(operation.id, principal.actor.id)),
            requestId,
          }
        } else {
          ...
          body = {
            operation: service.operation(
              queue.retry(operation.id, String(input.errorClass ?? 'manual'), {
        ...
        body = page(
          options.sources.listConnections(principal.serviceAccount.organizationId) as unknown as Array<
        ...
        if (!connection || connection.organizationId !== principal.serviceAccount.organizationId)
          throw new ApiServiceError('not_found', 'Source connection was not found.', 404)
        if (url.searchParams.get('sync') === 'true') {
          service.authorize(principal, 'sources:manage', { type: 'organization' })
          await syncSourceRepositories(options.sources, connection.id, {
            search: url.searchParams.get('search') ?? undefined,
          })
        }
        body = page(
          options.sources.listRepositories(
            connection.id,
        ...
        if (!connection || connection.organizationId !== principal.serviceAccount.organizationId)
          throw new ApiServiceError('not_found', 'Source connection was not found.', 404)
        body = await listSourceReferences(options.sources, {
          connectionId,
        ...
        if (!Array.isArray(input.files) || input.files.length > 10_000)
          throw new ApiServiceError('validation_error', 'files must contain at most 10,000 entries.', 422)
        body = {
          candidates: detectApplication(
            input.files.map((item: any) => ({
        ...
        body = {
          plan: planApplication(
            input.draft,
            Array.isArray(input.suppliedSecretNames) ? input.suppliedSecretNames.map(String) : [],
          ),
          requestId,
        }
        ...
        const projectId = url.searchParams.get('projectId') ?? ''
        service.authorize(principal, 'applications:read', { type: 'project', id: projectId })
        body = page(
          options.applications.drafts.list(projectId) as unknown as Array<Record<string, unknown>>,
          url,
          requestId,
          ['updatedAt', 'id'],
        )
        ...
        if (!options.applications) throw new ApiServiceError('not_found', 'Application onboarding is unavailable.', 404)
        const input = await readBody()
        const projectId = String(input.projectId ?? input.draft?.projectId ?? '')
        service.authorize(principal, 'applications:manage', { type: 'project', id: projectId })
        const project = options.controlPlane.getProject(projectId)
        ...
        const draft = options.applications.drafts.create({
          organizationId: project.organizationId,
          projectId,
          name: String(input.name ?? input.draft?.name ?? 'Application draft'),
          draft: { ...input.draft, projectId },
          step: input.step,
          suppliedSecretNames: Array.isArray(input.suppliedSecretNames) ? input.suppliedSecretNames.map(String) : [],
          actorId: principal.actor.id,
        })
        ...
        if (!options.applications) throw new ApiServiceError('not_found', 'Application onboarding is unavailable.', 404)
        const input = await readBody()
        const current = options.applications.drafts.get(String(input.id ?? ''))
        ...
        body = {
          draft: options.applications.drafts.update(current.id, Number(input.version), {
            draft: { ...input.draft, projectId: current.projectId },
            step: input.step,
            suppliedSecretNames: Array.isArray(input.suppliedSecretNames)
              ? input.suppliedSecretNames.map(String)
              : undefined,
            actorId: principal.actor.id,
          }),
          requestId,
        ...
        if (!/^[A-Za-z0-9._:-]{8,128}$/.test(idempotencyKey))
          throw new ApiServiceError('validation_error', 'Idempotency-Key must contain 8-128 safe characters.', 422)
        const input = await readBody()
        ...
        if (existing) {
          ...
          if (!operation || !resource)
            throw new ApiServiceError(
              ...
              'The original application operation is no longer available.',
              409,
            )
          return response(
            {
              ...
              plan: planApplication(current.input, current.suppliedSecretNames),
              idempotentReplay: true,
              requestId,
            },
            202,
            requestId,
            { ...rateHeaders, 'idempotent-replayed': 'true' },
          )
        }
        const result = applyApplicationDraft({
          controlPlane: options.controlPlane,
          drafts: options.applications.drafts,
          draftId: current.id,
          expectedVersion: Number(input.version),
          confirmEnvironment: String(input.confirmEnvironment ?? ''),
          actorId: principal.actor.id,
        })
        options.identities.saveIdempotency({
          tokenId: principal.token.id,
          key: idempotencyKey,
          requestHash,
          operationId: result.operation.id,
          responseStatus: 202,
          responseBody: { draftId: current.id },
        })
        return response(
          {
            resource: result.resource,
            operation: service.operation(result.operation),
            plan: result.plan,
        ...
        if (!options.applications) throw new ApiServiceError('not_found', 'Application onboarding is unavailable.', 404)
        service.authorize(principal, 'applications:read', principal.token.scope)
        body = page(
          options.applications.registries.list(principal.serviceAccount.organizationId) as unknown as Array<
            Record<string, unknown>
          >,
          url,
          requestId,
        ...
        service.authorize(principal, 'applications:manage', { type: 'organization' })
        const input = await readBody()
        const registry = options.applications.registries.create({
          organizationId: principal.serviceAccount.organizationId,
          provider: input.provider,
          name: String(input.name ?? 'Container registry'),
          ...
          credential:
            input.token || input.password
              ...
              : undefined,
          credentialExpiresAt: typeof input.credentialExpiresAt === 'string' ? input.credentialExpiresAt : undefined,
          actorId: principal.actor.id,
        })
        ...
        service.authorize(principal, 'applications:manage', { type: 'organization' })
        const input = await readBody()
        const registry = options.applications.registries.get(String(input.id ?? ''))
        if (!registry || registry.organizationId !== principal.serviceAccount.organizationId)
          throw new ApiServiceError('not_found', 'Registry connection was not found.', 404)
        if (input.action === 'test')
          body = {
            registry: await options.applications.registries.test(registry.id, {
          ...
          body = {
            registry: options.applications.registries.rotate(
              registry.id,
              { username: input.username, password: input.password, token: input.token },
              { expiresAt: input.expiresAt, actorId: principal.actor.id },
            ),
            requestId,
          }
        ...
        if (!options.applications?.artifacts)
          throw new ApiServiceError('not_found', 'Application artifact storage is unavailable.', 404)
        ...
        if (declared > 100 * 1024 * 1024)
          throw new ApiServiceError('payload_too_large', 'Artifact exceeds 100 MB.', 413)
        const artifact = options.applications.artifacts.create({
          organizationId: project.organizationId,
          projectId,
          filename: request.headers.get('x-artifact-filename') ?? 'application.zip',
        ...
        if (!['github', 'gitlab', 'bitbucket', 'gitea', 'generic_https', 'generic_ssh'].includes(provider))
          throw new ApiServiceError('validation_error', 'A supported provider is required.', 422)
        const credential =
          provider === 'github' && (input.privateKey || input.appId)
            ? {
                appId: typeof input.appId === 'string' ? input.appId : undefined,
                installationId: typeof input.installationId === 'string' ? input.installationId : undefined,
                privateKey: typeof input.privateKey === 'string' ? input.privateKey : undefined,
              }
            : provider !== 'generic_ssh' && input.token
              ? {
        ...
        options.controlPlane.database.transaction(() => {
          connection = options.sources!.createConnection({
            organizationId: principal.serviceAccount.organizationId,
          ...
          if (typeof input.repositoryUrl === 'string' && typeof input.repositoryFullName === 'string')
            repository = options.sources!.upsertRepository({
              connectionId: connection.id,
              providerRepositoryId: `manual:${input.repositoryFullName}`,
          ...
          if (provider === 'generic_ssh')
            deployKey = options.sources!.createDeployKey({
              connectionId: connection.id,
              name: String(input.deployKeyName ?? 'Repository deploy key'),
              publicKey: String(input.publicKey ?? ''),
              privateKey: String(input.deployPrivateKey ?? ''),
              host: String(input.sshHost ?? ''),
              hostKey: String(input.hostKey ?? ''),
              actorId: principal.actor.id,
            })
        })()
        ...
        if (!connection || connection.organizationId !== principal.serviceAccount.organizationId)
          throw new ApiServiceError('not_found', 'Source connection was not found.', 404)
        const binding = options.sources.createBinding({
          projectId,
          environmentId: typeof input.environmentId === 'string' ? input.environmentId : undefined,
          ...
          defaultBranch: String(input.defaultBranch ?? 'main'),
          branchRule: typeof input.branchRule === 'string' ? input.branchRule : undefined,
          tagRule: typeof input.tagRule === 'string' ? input.tagRule : undefined,
          monorepoRoot: String(input.monorepoRoot ?? '.'),
          includePaths: Array.isArray(input.includePaths) ? input.includePaths.map(String) : [],
          excludePaths: Array.isArray(input.excludePaths) ? input.excludePaths.map(String) : [],
          submodules: input.submodules === true,
          cloneDepth: typeof input.cloneDepth === 'number' ? input.cloneDepth : undefined,
          deployKeyId: typeof input.deployKeyId === 'string' && input.deployKeyId ? input.deployKeyId : undefined,
          autoDeploy: input.autoDeploy !== false,
          pullRequestPreviews: input.pullRequestPreviews !== false,
          actorId: principal.actor.id,
        })
        ...
        const created = options.sources.createWebhook({
          connectionId: connection.id,
          repositoryId: typeof input.repositoryId === 'string' ? input.repositoryId : undefined,
          repositoryFullName: String(input.repositoryFullName ?? ''),
          events: Array.isArray(input.events) ? input.events.map(String) : undefined,
        })
        const endpoint = webhookEndpoint(baseUrl, created.webhook)
        const webhook =
          input.reconcile === false
            ? created.webhook
            : await reconcileSourceWebhook(options.sources, created.webhook.id, baseUrl)
        ...
        if (input.preview === true)
          return response({ preview: true, affectedBindings, requestId }, 200, requestId, rateHeaders)
        return response(
          { ...options.sources.disconnectConnection(connection.id, principal.actor.id), requestId },
          200,
          requestId,
          rateHeaders,
        )
        ...
        request.method === 'GET' &&
        (url.pathname === '/api/v1/events' || url.pathname === '/api/v1/events/stream')
      ) {
        const events = service.listEvents(
          principal,
          url.searchParams.get('projectId') ?? undefined,
          Number(url.searchParams.get('after')) || undefined,
        )
        if (url.pathname.endsWith('/stream')) {
          const encoder = new TextEncoder()
          let after = events.reduce(
            (last, event) => Math.max(last, Number(event.sequence)),
            ...
            start(controller) {
              ...
              timer = setInterval(() => {
                ...
                try {
                  ...
                  if (!next.length) controller.enqueue(encoder.encode(`: heartbeat ${Date.now()}\n\n`))
                } catch {
                  closed = true
                  if (timer) clearInterval(timer)
                  controller.close()
        ...
        try {
          deployment = JSON.parse(text) as ApiDeploymentRequest
        } catch {
          throw new ApiServiceError('invalid_json', 'Request body must be valid JSON.', 400)
        }
        if (
          ...
          (deployment.action !== undefined && !['deploy', 'rollback'].includes(deployment.action))
        )
          throw new ApiServiceError(
            'validation_error',
            'projectId, environmentId, and a valid action are required.',
            422,
          )
        const created = service.createDeployment(principal, deployment, idempotencyKey)
        return response({ operation: created.operation, idempotentReplay: created.replay, requestId }, 202, requestId, {
          ...rateHeaders,
      ...
      if (error instanceof ApiServiceError)
        return failure(
          ...
          error.status,
          requestId,
          error.details as ApiErrorEnvelope['error']['details'],
          rateHeaders,
        )
      if (
        url.pathname.startsWith('/api/v1/source/') ||
        url.pathname.startsWith('/api/v1/application') ||
        url.pathname.startsWith('/api/v1/compose-') ||
        url.pathname === '/api/v1/registry-connections'
      )
        return failure(
          'validation_error',
          error instanceof Error ? error.message : 'Integration request could not be completed.',
          422,
          requestId,
          undefined,
          rateHeaders,
