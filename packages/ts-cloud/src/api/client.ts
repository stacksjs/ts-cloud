...
export class TsCloudClient {
  ...
  releaseAction(releaseId: string, action: 'promote' | 'approve' | 'activate' | 'health' | 'pin', input: Record<string, unknown> = {}): Promise<Record<string, unknown>> { return this.request(`/api/v1/releases/${encodeURIComponent(releaseId)}/${action}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) }) }
