...
export class TsCloudClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    const response = await this.fetchFn(`${this.baseUrl}${path}`, {
      ...init,
      headers: { accept: 'application/json', authorization: `Bearer ${this.options.token}`, ...init.headers },
    })
    const body = (await response.json().catch(() => ({}))) as any
    if (!response.ok)
      throw new TsCloudApiError(
        response.status,
  ...
  listEnvironments(
    ...
    return this.request(
      `/api/v1/projects/${encodeURIComponent(projectId)}/environments?${new URLSearchParams(
        Object.fromEntries(
          Object.entries(options)
            .filter(([, value]) => value !== undefined)
            .map(([key, value]) => [key, String(value)]),
  ...
  listServices(projectId: string, environmentId?: string): Promise<ApiPage<Record<string, unknown>>> {
    return this.request(
      `/api/v1/services?${new URLSearchParams({ projectId, ...(environmentId ? { environmentId } : {}) })}`,
    )
  }
  ...
  listComposeTemplates(): Promise<ApiPage<ComposeTemplate>> {
    return this.request('/api/v1/compose-templates')
  }
  listComposeApplications(
    projectId: string,
    environmentId?: string,
  ): Promise<ApiPage<ComposeApplicationRecord & { services: ComposeServiceState[] }>> {
    return this.request(
      `/api/v1/compose-applications?${new URLSearchParams({ projectId, ...(environmentId ? { environmentId } : {}) })}`,
    )
  }
  previewCompose(input: {
    ...
    projectId: string
    environmentId: string
  }): Promise<{ result: ComposeParseResult; requestId: string }> {
    return this.request('/api/v1/compose-applications/preview', {
      method: 'POST',
  ...
  createComposeApplication(input: {
    ...
    return this.request('/api/v1/compose-applications', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  ): Promise<{ application: ComposeApplicationRecord; services: ComposeServiceState[]; requestId: string }> {
    return this.request(`/api/v1/compose-applications/${encodeURIComponent(applicationId)}/services`)
  }
  composeAction(
    applicationId: string,
    action: 'deploy' | 'redeploy' | 'start' | 'stop' | 'scale' | 'delete',
    input: { service?: string; replicas?: number; removeVolumes?: boolean; confirm?: string } = {},
  ): Promise<ApiOperationResponse> {
    return this.request(`/api/v1/compose-applications/${encodeURIComponent(applicationId)}/${action}`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  releaseCapabilities(
    ...
    input: { health?: boolean; replicas?: number } = {},
  ): Promise<{ capabilities: ReleaseStrategyCapability[]; requestId: string }> {
    return this.request(
      `/api/v1/releases/capabilities?${new URLSearchParams({ kind, health: String(!!input.health), replicas: String(input.replicas ?? 1) })}`,
    )
  }
  listReleases(
    projectId: string,
    input: { environmentId?: string; resourceId?: string } = {},
  ): Promise<ApiPage<ReleaseRecord & { artifact: ReleaseArtifact; transitions: ReleaseTransition[] }>> {
    return this.request(`/api/v1/releases?${new URLSearchParams({ projectId, ...input })}`)
  }
  createReleaseArtifact(input: {
    ...
    provenance?: unknown
    attestation?: unknown
  }): Promise<{ artifact: ReleaseArtifact; requestId: string }> {
    return this.request('/api/v1/release-artifacts', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  }): Promise<{ release: ReleaseRecord; requestId: string }> {
  ...
  releaseAction(
    releaseId: string,
    action: 'promote' | 'approve' | 'activate' | 'rollback' | 'health' | 'pin',
    input: Record<string, unknown> = {},
  ): Promise<Record<string, unknown>> {
    return this.request(`/api/v1/releases/${encodeURIComponent(releaseId)}/${action}`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  detectApplication(input: ApiApplicationDetectionRequest): Promise<ApiApplicationDetectionResponse> {
    return this.request('/api/v1/application-detections', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  planApplication(input: ApiApplicationPlanRequest): Promise<ApiApplicationPlanResponse> {
    return this.request('/api/v1/application-plans', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  createApplicationDraft(
    input: ApiApplicationDraftCreateRequest,
  ): Promise<{ draft: ApplicationDraftRecord; requestId: string }> {
    return this.request('/api/v1/application-drafts', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  updateApplicationDraft(
    input: ApiApplicationDraftUpdateRequest,
  ): Promise<{ draft: ApplicationDraftRecord; requestId: string }> {
    return this.request('/api/v1/application-drafts', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  ): Promise<{ artifact: ApplicationArtifactRecord; requestId: string }> {
    return this.request('/api/v1/application-artifacts', {
      method: 'POST',
      headers: {
  ...
  createRegistryConnection(
    input: ApiRegistryConnectionCreateRequest,
  ): Promise<{ registry: RegistryConnection; requestId: string }> {
    return this.request('/api/v1/registry-connections', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
  ...
  updateRegistryConnection(
    input: ApiRegistryConnectionUpdateRequest,
  ): Promise<{ registry: RegistryConnection; requestId: string }> {
    return this.request('/api/v1/registry-connections', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  listQueue(
    input: { projectId?: string; state?: string; limit?: number; cursor?: string } = {},
  ...
  streamOperationLogs(operationId: string, after = 0): Promise<Response> {
    return this.fetchFn(`${this.baseUrl}/api/v1/operations/${encodeURIComponent(operationId)}/logs/stream`, {
      headers: {
  ...
  cancelOperation(operationId: string): Promise<{ operation: ApiOperationResponse['operation']; requestId: string }> {
    return this.request(`/api/v1/operations/${encodeURIComponent(operationId)}/cancel`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: '{}',
    })
  ...
  retryOperation(
    ...
    errorClass: string,
    delayMs = 0,
  ): Promise<{ operation: ApiOperationResponse['operation']; requestId: string }> {
    return this.request(`/api/v1/operations/${encodeURIComponent(operationId)}/retry`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ errorClass, delayMs }),
    })
  }

  queueSettings(): Promise<ApiQueueSettingsResponse> {
    return this.request('/api/v1/queue/settings')
  }
  ...
  updateQueueSettings(concurrency: Partial<QueueConcurrencyLimits>): Promise<ApiQueueSettingsResponse> {
    return this.request('/api/v1/queue/settings', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ confirm: 'update queue limits', concurrency }),
    })
  }

  listPreviews(projectId?: string): Promise<ApiPage<PreviewInstance>> {
    return this.request(`/api/v1/previews${projectId ? `?projectId=${encodeURIComponent(projectId)}` : ''}`)
  }
  listPreviewDefinitions(projectId: string): Promise<ApiPage<PreviewDefinition>> {
    return this.request(`/api/v1/preview-definitions?projectId=${encodeURIComponent(projectId)}`)
  }
  createPreviewDefinition(
    input: CreatePreviewDefinitionInput,
  ): Promise<{ definition: PreviewDefinition; requestId: string }> {
    return this.request('/api/v1/preview-definitions', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  createPreview(input: {
    ...
    fork?: boolean
    commitSha: string
  }): Promise<{ preview: PreviewInstance; operation: ApiOperationResponse['operation']; requestId: string }> {
    return this.request('/api/v1/previews', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    })
  ...
  destroyPreview(
    ...
    return this.request(`/api/v1/previews/${encodeURIComponent(previewId)}/destroy`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ confirm }),
    })
  }
  extendPreview(previewId: string, hours: number): Promise<{ preview: PreviewInstance; requestId: string }> {
    return this.request(`/api/v1/previews/${encodeURIComponent(previewId)}/extend`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ hours }),
    })
  }
  rebuildPreview(previewId: string): Promise<{ operation: ApiOperationResponse['operation']; requestId: string }> {
    return this.request(`/api/v1/previews/${encodeURIComponent(previewId)}/rebuild`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: '{}',
    })
