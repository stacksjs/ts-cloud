...
function fixture(provider: 'github' | 'generic_https' = 'github') {
  ...
  const created = sources.createWebhook({
    connectionId: connection.id,
    repositoryFullName: 'acme/web',
    endpointToken: 'endpoint-fixture',
    secret: 'webhook-fixture',
    events: ['push', 'pull_request'],
  })
  sources.updateWebhookState(created.webhook.id, {
    providerWebhookId: 'provider-1',
    status: 'healthy',
    reconciled: true,
  })
  return {
    ...
    environment,
    resource,
    connection: sources.getConnection(connection.id)!,
    binding,
...
describe('source webhook ingestion', () => {
  it('verifies the raw GitHub body and enqueues each matching binding exactly once', async () => {
    ...
    const body = raw({
      ...
      commits: [{ added: ['apps/web/src/index.ts'], modified: [], removed: [] }],
    })
    const headers = {
      'x-github-event': 'push',
      'x-github-delivery': 'delivery-1',
      'x-hub-signature-256': signature(f.secret, body),
    }
    const first = await processSourceWebhook({
      sources: f.sources,
    ...
    expect(first.operations[0]).toMatchObject({
      kind: 'deploy.source',
      state: 'queued',
      input: { source: { repository: 'acme/web', commitSha: 'a'.repeat(40), monorepoRoot: '.' } },
    })

    const replay = await processSourceWebhook({
      sources: f.sources,
      controlPlane: f.controlPlane,
      endpointToken: 'endpoint-fixture',
      headers,
    ...
    expect(f.sources.listDeliveries(f.webhook.id)).toMatchObject([
      {
        providerDeliveryId: 'delivery-1',
        signatureStatus: 'verified',
        status: 'enqueued',
        operationId: first.operations[0]?.id,
  ...
  it('rejects invalid signatures without parsing payload details or leaking connection state', async () => {
    const f = fixture()
    const body = raw({
      repository: { full_name: 'unauthorized/private' },
      after: 'b'.repeat(40),
    ...
    const unknown = await processSourceWebhook({
      sources: f.sources,
      controlPlane: f.controlPlane,
      endpointToken: 'not-a-real-endpoint',
      headers: {},
      rawBody: body,
    })
  ...
  it('honors branch/path rules and safely disables ingestion after disconnect', async () => {
    ...
    const docsOnly = raw({
      ...
      after: 'c'.repeat(40),
      repository: { full_name: 'acme/web' },
      commits: [{ modified: ['apps/web/readme.md'] }],
    })
    const ignored = await processSourceWebhook({
      sources: f.sources,
      controlPlane: f.controlPlane,
      endpointToken: 'endpoint-fixture',
      headers: {
        'x-github-event': 'push',
        'x-github-delivery': 'delivery-docs',
        'x-hub-signature-256': signature(f.secret, docsOnly),
      },
      rawBody: docsOnly,
    })
    ...
    const afterDisconnect = await processSourceWebhook({
      sources: f.sources,
      controlPlane: f.controlPlane,
      endpointToken: 'endpoint-fixture',
      headers: {
        'x-github-event': 'push',
        'x-github-delivery': 'delivery-after',
        'x-hub-signature-256': signature(f.secret, docsOnly),
      },
      rawBody: docsOnly,
    })
  ...
  it('requires a fresh timestamp for generic events and enqueues pull-request previews', async () => {
    ...
    const baseHeaders = {
      'x-ts-cloud-event': 'pull_request',
      'x-ts-cloud-delivery': 'generic-1',
      'x-ts-cloud-signature': signature(f.secret, body),
    }
    const stale = await processSourceWebhook({
      sources: f.sources,
      controlPlane: f.controlPlane,
      endpointToken: 'endpoint-fixture',
      headers: { ...baseHeaders, 'x-ts-cloud-timestamp': String(now.getTime() / 1000 - 301) },
      rawBody: body,
      now,
    })
    expect(stale.accepted).toBe(false)

    const accepted = await processSourceWebhook({
      sources: f.sources,
      controlPlane: f.controlPlane,
      endpointToken: 'endpoint-fixture',
      headers: {
        ...baseHeaders,
        'x-ts-cloud-delivery': 'generic-2',
        'x-ts-cloud-timestamp': String(now.getTime() / 1000),
      },
      rawBody: body,
      now,
    })
    expect(accepted).toMatchObject({
      accepted: true,
      status: 'enqueued',
      operations: [{ kind: 'deploy.preview', input: { preview: { number: 17, action: 'opened' } } }],
    })
  })
  ...
  it('creates, updates, and destroys one persistent preview across the PR lifecycle', async () => {
    ...
    const policy = previews.createDefinition({
      ...
      baseEnvironmentId: f.environment.id,
      domainPattern: 'https://{name}.preview.example.com',
      inheritedSecrets: ['PREVIEW_KEY'],
    })
    const now = new Date('2026-07-21T12:00:00.000Z')
    const deliver = async (delivery: string, action: string, sha: string, fork = false) => {
      ...
      return processSourceWebhook({
        sources: f.sources,
        controlPlane: f.controlPlane,
        endpointToken: 'endpoint-fixture',
        headers: {
          'x-ts-cloud-event': 'pull_request',
          'x-ts-cloud-delivery': delivery,
          'x-ts-cloud-signature': signature(f.secret, body),
          'x-ts-cloud-timestamp': String(now.getTime() / 1000),
        },
        rawBody: body,
    ...
    expect(created).toMatchObject({
      ...
      latestOperationId: opened.operations[0]?.id,
    })

    const synchronized = await deliver('preview-sync', 'synchronize', 'b'.repeat(40))
    expect(synchronized).toMatchObject({
      status: 'enqueued',
      operations: [{ kind: 'preview.update', input: { previewId: created.id } }],
    })
    ...
    expect(closed).toMatchObject({
      status: 'enqueued',
      operations: [{ kind: 'preview.destroy', input: { previewId: created.id, reason: 'pull_request_closed' } }],
    })
  ...
  it('rejects untrusted fork previews before credentials enter a durable job', async () => {
    ...
    expect(
      f.controlPlane
        .listEvents({ projectId: f.project.id })
        .some((event) => event.type === 'preview.rejected' && JSON.stringify(event.payload).includes('fork')),
    ).toBe(true)
  })
  ...
  it('creates branch previews outside the production branch rule and tears them down on deletion', async () => {
    ...
    const now = new Date('2026-07-21T12:00:00.000Z')
    const deliver = async (delivery: string, sha: string, deleted = false) => {
      const body = raw({
        event: 'push',
        repository: 'acme/web',
        branch: 'preview/search',
        commitSha: sha,
        changedPaths: ['apps/web/src.ts'],
        deleted,
      })
      return processSourceWebhook({
        sources: f.sources,
    ...
    expect(await deliver('branch-delete', '0'.repeat(40), true)).toMatchObject({
      operations: [{ kind: 'preview.destroy', input: { reason: 'branch_deleted' } }],
    })
  })
  ...
  it('normalizes hosted provider push and pull-request fixtures', () => {
    ...
    ).toMatchObject({ event: 'push', repository: 'acme/web', branch: 'main' })
    expect(
      normalizeSourceEvent(
        'bitbucket',
        ...
        {
          repository: { full_name: 'acme/web' },
          pullrequest: { id: 9, source: { branch: { name: 'feature' }, commit: { hash: 'b'.repeat(40) } } },
        },
      ),
    ).toMatchObject({ event: 'pull_request', action: 'created', pullRequestNumber: 9 })
    expect(
      normalizeSourceEvent(
        'gitea',
        { 'x-gitea-event': 'push' },
        { ref: 'refs/tags/v2', after: 'c'.repeat(40), repository: { full_name: 'acme/web' }, commits: [] },
      ),
    ).toMatchObject({ event: 'push', tag: 'v2' })
    expect(
      normalizeSourceEvent(
        'gitlab',
        { 'x-gitlab-event': 'Push Hook' },
        {
          object_kind: 'push',
          ref: 'refs/heads/preview/old',
          after: '0'.repeat(40),
          project: { path_with_namespace: 'acme/web' },
          commits: [],
        },
      ),
    ).toMatchObject({ branch: 'preview/old', deleted: true })
    expect(
      normalizeSourceEvent(
        'bitbucket',
            ...
            changes: [
              {
                closed: true,
                new: null,
                old: { type: 'branch', name: 'preview/old', target: { hash: 'd'.repeat(40) } },
              },
            ],
          },
        },
    ...
    expect(
      normalizeSourceEvent(
        'gitea',
        { 'x-gitea-event': 'push' },
        { ref: 'refs/heads/preview/old', after: '0'.repeat(40), repository: { full_name: 'acme/web' }, commits: [] },
      ),
    ).toMatchObject({ branch: 'preview/old', deleted: true })
  })
  ...
  it('only reveals endpoint tokens at creation time and requires secure public endpoints', () => {
    const f = fixture()
    expect(webhookEndpoint('https://cloud.example', f.webhook)).toBe(
      'https://cloud.example/api/source/webhooks/endpoint-fixture',
    )
