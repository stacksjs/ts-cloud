...
function connection(provider: SourceProvider, host: string, owner = 'acme'): SourceConnection {
  return {
    ...
    host,
    owner,
    authKind: provider === 'github' ? 'app' : 'access_token',
...
function response(data: unknown, options: { status?: number; headers?: Record<string, string> } = {}): Response {
  return new Response(options.status === 204 ? null : JSON.stringify(data), {
    status: options.status ?? 200,
    headers: { 'Content-Type': 'application/json', ...options.headers },
  })
}

function hostedFixture(provider: 'github' | 'gitlab' | 'bitbucket' | 'gitea'): {
  adapter: SourceProviderAdapter
  requests: Array<{ url: URL; method: string; headers: Headers; body?: any }>
} {
  ...
  const fetcher: SourceFetch = async (input, init = {}) => {
    ...
    if (path.endsWith('/user'))
      return response(
        provider === 'gitlab' ? { username: 'chris' } : { login: 'chris', username: 'chris' },
        provider === 'github' ? { headers: { 'x-oauth-scopes': 'repo:read, metadata:read' } } : {},
      )
    ...
    if (provider === 'github') {
      ...
      if (path.endsWith('/branches'))
        return response([{ name: 'main', commit: { sha: 'a'.repeat(40) }, protected: true }])
      if (path.endsWith('/tags')) return response([{ name: 'v1.0.0', commit: { sha: 'b'.repeat(40) } }])
      if (path.includes('/hooks'))
        return response(
          method === 'GET'
            ? [{ id: 8, active: true, events: ['push'], config: { url: 'https://cloud.example/hooks/source' } }]
            : { id: 8, active: true, events: body.events, config: { url: body.config.url } },
        )
    }
    if (provider === 'gitlab') {
          ...
          [
            {
              ...
              http_url_to_repo: 'https://gitlab.example/acme/web.git',
              default_branch: 'main',
              visibility: 'private',
            },
          ],
      ...
      if (path.endsWith('/repository/branches'))
        return response([{ name: 'main', commit: { id: 'a'.repeat(40) }, protected: true }])
      if (path.endsWith('/repository/tags')) return response([{ name: 'v1.0.0', commit: { id: 'b'.repeat(40) } }])
      if (path.includes('/hooks'))
        return response(
          method === 'GET'
            ? [{ id: 8, url: 'https://cloud.example/hooks/source', push_events: true }]
            : {
                id: 8,
                url: body.url,
                push_events: body.push_events,
                merge_requests_events: body.merge_requests_events,
              },
        )
    }
    if (provider === 'bitbucket') {
      if (/\/repositories\/acme$/.test(path))
        return response({
          values: [
            {
              uuid: '{repo-3}',
              full_name: 'acme/web',
              links: { clone: [{ name: 'https', href: 'https://bitbucket.org/acme/web.git' }] },
      ...
      if (path.endsWith('/refs/branches'))
        return response({ values: [{ name: 'main', target: { hash: 'a'.repeat(40) } }] })
      if (path.endsWith('/refs/tags'))
        return response({ values: [{ name: 'v1.0.0', target: { hash: 'b'.repeat(40) } }] })
      if (path.includes('/hooks'))
        return response(
          method === 'GET'
            ? {
                values: [
                  { uuid: '{hook-8}', active: true, events: ['repo:push'], url: 'https://cloud.example/hooks/source' },
                ],
              }
            : { uuid: '{hook-8}', active: true, events: body.events, url: body.url },
        )
    }
    if (provider === 'gitea') {
      if (path.endsWith('/user/repos'))
          ...
          [
            {
              id: 4,
              full_name: 'acme/web',
              clone_url: 'https://gitea.example/acme/web.git',
              default_branch: 'main',
              private: true,
            },
          ],
      ...
      if (path.includes('/hooks'))
        return response(
          method === 'GET'
            ? [{ id: 8, active: true, events: ['push'], config: { url: 'https://cloud.example/hooks/source' } }]
            : { id: 8, active: true, events: body.events, config: body.config },
        )
    }
  ...
  return {
    adapter: createSourceAdapter(
      connection(provider, hosts[provider]),
  ...
  describe(`${provider} source adapter contract`, () => {
    it('tests access, paginates repositories/refs, and reconciles webhook lifecycle', async () => {
      ...
      expect(repositories.repositories).toMatchObject([
        { fullName: 'acme/web', defaultBranch: 'main', visibility: 'private' },
      ])
      ...
      if (provider === 'github' || provider === 'gitlab') expect(branches.refs[0]?.protected).toBe(true)
      expect(await adapter.listTags('acme/web', { limit: 10 })).toMatchObject({ refs: [{ name: 'v1.0.0' }] })

      const created = await adapter.createWebhook('acme/web', {
        url: 'https://cloud.example/hooks/source',
      ...
      expect(
        await adapter.updateWebhook('acme/web', created.providerWebhookId, {
          url: 'https://cloud.example/hooks/source',
          secret: 'rotated-webhook-value',
          events: ['push'],
        }),
      ).toMatchObject({ active: true })
      await adapter.deleteWebhook('acme/web', created.providerWebhookId)
      expect(requests.some((request) => request.method === 'DELETE')).toBe(true)
      await adapter.setCommitStatus('acme/web', 'c'.repeat(40), {
        state: 'success',
        url: 'https://pr-4.preview.example.com',
        description: 'Preview deployed at exact commit',
      })
      const status = requests.at(-1)!
      expect(status.body).toMatchObject(
        provider === 'bitbucket'
          ? { state: 'SUCCESSFUL', key: 'ts-cloud-preview' }
          : { description: expect.stringContaining('exact commit') },
      )
...
describe('GitHub App authentication and provider failures', () => {
  it('mints a bounded installation token instead of storing an application bearer token', async () => {
    ...
    const adapter = createSourceAdapter(
      connection('github', 'https://github.com'),
      { appId: '42', installationId: '99', privateKey: privateKey.export({ type: 'pkcs8', format: 'pem' }).toString() },
      {
        ...
        fetch: async (input, init) => {
          ...
          if (url.pathname.endsWith('/access_tokens'))
            return response({ token: 'installation-value', expires_at: '2026-07-21T13:00:00.000Z' })
  ...
  it('surfaces rate-limit metadata without returning provider response bodies', async () => {
    const adapter = createSourceAdapter(
      connection('gitea', 'https://gitea.example'),
      { token: 'runtime-value' },
      {
        fetch: async () =>
          response(
            { detail: 'secret provider internals' },
            { status: 429, headers: { 'retry-after': '12', 'x-request-id': 'request-7' } },
          ),
      },
    )
    try {
