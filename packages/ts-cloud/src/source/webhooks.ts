...
function verifySignature(provider: SourceProvider, secret: string, headers: Headers | Record<string, string | undefined>, body: Uint8Array, now: Date): boolean {
  if (provider === 'gitlab') return equal(header(headers, 'x-gitlab-token') ?? '', secret)
  const supplied = provider === 'github' ? header(headers, 'x-hub-signature-256') : provider === 'gitea' ? header(headers, 'x-gitea-signature') : header(headers, 'x-hub-signature') ?? header(headers, 'x-ts-cloud-signature')
...
function deliveryId(provider: SourceProvider, headers: Headers | Record<string, string | undefined>): string | undefined {
  const names: Record<SourceProvider, string[]> = {
    github: ['x-github-delivery'], gitlab: ['x-gitlab-event-uuid', 'x-request-id'], bitbucket: ['x-request-uuid'], gitea: ['x-gitea-delivery'],
    generic_https: ['x-ts-cloud-delivery'], generic_ssh: ['x-ts-cloud-delivery'],
  }
...
function pathsMatch(binding: SourceBinding, changedPaths: string[]): boolean {
  ...
  return relevant.some(path => !binding.includePaths.length || binding.includePaths.some(pattern => glob(path, pattern)))
}

function eventName(provider: SourceProvider, headers: Headers | Record<string, string | undefined>): string {
  return header(headers, provider === 'github' ? 'x-github-event' : provider === 'gitlab' ? 'x-gitlab-event' : provider === 'bitbucket' ? 'x-event-key' : provider === 'gitea' ? 'x-gitea-event' : 'x-ts-cloud-event') ?? 'unknown'
}
...
export async function processSourceWebhook(input: { sources: SourceConnectionStore, controlPlane: ControlPlaneStore, endpointToken: string, headers: Headers | Record<string, string | undefined>, rawBody: Uint8Array, now?: Date }): Promise<ProcessSourceWebhookResult> {
  ...
  const signatureValid = verifySignature(connection.provider, input.sources.getWebhookSecret(webhook.id), input.headers, input.rawBody, input.now ?? new Date())
  if (!signatureValid) {
    input.sources.recordDelivery({ connectionId: connection.id, webhookId: webhook.id, providerDeliveryId, event: eventName(connection.provider, input.headers), signatureStatus: header(input.headers, 'x-hub-signature-256') || header(input.headers, 'x-gitlab-token') || header(input.headers, 'x-hub-signature') || header(input.headers, 'x-gitea-signature') ? 'invalid' : 'missing', status: 'rejected', error: 'Webhook signature verification failed' })
    return unavailable()
  }
  let body: Record<string, any>
  try { body = JSON.parse(new TextDecoder().decode(input.rawBody)) as Record<string, any> }
  ...
  if (!event || !event.repository || !/^[a-f0-9]{7,64}$/i.test(event.commitSha)) {
    input.sources.recordDelivery({ connectionId: connection.id, webhookId: webhook.id, providerDeliveryId, event: eventName(connection.provider, input.headers), signatureStatus: 'verified', status: 'rejected', error: 'Unsupported or malformed source event' })
    return unavailable()
  }
  const recorded = input.sources.recordDelivery({ connectionId: connection.id, webhookId: webhook.id, providerDeliveryId, event: event.event, action: event.action, commitSha: event.commitSha, signatureStatus: 'verified', status: 'accepted',
...
export function webhookEndpoint(baseUrl: string, webhook: SourceWebhook): string {
  ...
  if (base.protocol !== 'https:' && !['localhost', '127.0.0.1', '::1'].includes(base.hostname)) throw new Error('Webhook base URL must use HTTPS')
