...
describe('SourceConnectionStore', () => {
  ...
  it('requires encrypted deploy keys and pinned host keys from the same connection', () => {
    ...
    const binding = f.sources.createBinding({ projectId: f.project.id, connectionId: connection.id, repositoryFullName: 'acme/web', deployKeyId: key.id })
    ...
    expect(f.sources.getBinding(binding.id)).toMatchObject({ deployKeyId: undefined, status: 'disabled', autoDeploy: false })
  })
})
