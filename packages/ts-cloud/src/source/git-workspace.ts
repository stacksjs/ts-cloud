...
function validateRemote(value: string, deployKey?: GitTransportOptions['deployKey']): string {
  ...
  if (remote.startsWith('https://')) {
    ...
    if (url.username || url.password || url.search || url.hash)
      throw new Error('HTTPS Git remotes cannot contain credentials, queries, or fragments')
    return url.href
  }
  ...
  if (!deployKey || deployKey.host.toLowerCase() !== host.toLowerCase())
    throw new Error('SSH Git remote requires a deploy key with a pinned key for the same host')
  return remote
}
...
async function runGit(args: string[], remote: string, options: GitTransportOptions): Promise<string> {
  ...
  const child = Bun.spawn([options.executable ?? 'git', ...args], {
    stdout: 'pipe',
    stderr: 'pipe',
    env: transport.env,
  })
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      child.kill()
      reject(new Error(`Git operation exceeded ${timeoutMs}ms`))
    }, timeoutMs)
  })
  try {
    ...
    if (exitCode !== 0)
      throw new Error(sanitizedError(stderr || `Git exited with ${exitCode}`, remote, options.credential))
    return stdout
  } finally {
...
): Promise<{ branches: SourceRef[]; tags: SourceRef[] }> {
...
export async function cloneSourceBinding(
  input: {
    remote: string
  ...
  if (!/^[A-Za-z0-9._/-]{1,255}$/.test(reference) || reference.startsWith('-') || reference.includes('..'))
  ...
  if (
    sparsePaths.some((path) => !path || path.startsWith('/') || path.split('/').includes('..') || /[\0\r\n]/.test(path))
  )
    throw new Error('Sparse checkout paths must stay inside the repository')
  ...
  if (input.commitSha) {
    if (!/^[a-f0-9]{40,64}$/i.test(input.commitSha))
      throw new Error('Expected source commit must be an immutable 40-64 character SHA')
  ...
  if (input.commitSha && commitSha.toLowerCase() !== input.commitSha.toLowerCase())
    throw new Error(`Git checkout resolved ${commitSha} instead of the requested immutable commit`)
  return {
    directory: input.binding.monorepoRoot === '.' ? destination : join(destination, input.binding.monorepoRoot),
    commitSha,
  }
}
