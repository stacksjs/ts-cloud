...
export interface ConfigurationWarning { key?: string; code: 'reserved' | 'limit' | 'missing_required' | 'stale_reference'; message: string }
...
function metadata(entry: ConfigurationEntry, options: { inherited?: boolean; overridden?: boolean; canReadSecretMetadata?: boolean } = {}): ConfigurationMetadata {
  const secret = entry.kind === 'secret'
  return {
    id: entry.id, key: entry.key, kind: entry.kind, scope: entry.scope, inherited: !!options.inherited, overridden: !!options.overridden, required: entry.required,
    ...
    lastUsedAt: entry.lastUsedAt, rotatedAt: entry.rotatedAt, version: entry.version, updatedAt: entry.updatedAt,
  }
}
...
export class ConfigurationService {
  ...
  constructor(store: ConfigurationStore, options: { encryptionKey: string; fingerprintKey?: string; backends?: ConfigurationSecretBackend[]; now?: () => Date }) {
    this.store = store
    ...
    const configured = options.backends ?? [new LocalEncryptedConfigurationBackend(store.controlPlane, options.encryptionKey, this.now)]
    for (const backend of configured) this.backends.set(backend.kind, backend)
  }

  list(input: { projectId: string; scope?: ConfigurationScope; kind?: ConfigurationKind; search?: string; canReadSecretMetadata?: boolean }): ConfigurationMetadata[] {
    const search = input.search?.toLowerCase()
    return this.store.list(input).filter(entry => !search || entry.key.toLowerCase().includes(search)).map(entry => metadata(entry, { canReadSecretMetadata: input.canReadSecretMetadata }))
  }

  plan(input: { projectId: string; scope: ConfigurationScope; values: Record<string, string>; removeMissing?: boolean }): ConfigurationPlan {
    const existing = this.store.list({ projectId: input.projectId, scope: input.scope }), byKey = new Map(existing.map(entry => [entry.key, entry])), warnings: ConfigurationWarning[] = []
    ...
    const removed = input.removeMissing ? existing.filter(entry => !Object.hasOwn(input.values, entry.key)).map(entry => entry.key) : []
    return { added: sorted(added), changed: sorted(changed), removed: sorted(removed), unchanged: sorted(unchanged), affectedResourceIds: this.affectedResources(input.projectId, input.scope, [...added, ...changed, ...removed]), warnings }
  }

  async set(input: SetConfigurationInput): Promise<{ entry: ConfigurationEntry; mutation: ConfigurationMutationResult; warnings: ConfigurationWarning[] }> {
    const requestHash = this.requestHash(input), idempotencyKey = input.idempotencyKey
    if (idempotencyKey) {
      const prior = this.store.mutation(idempotencyKey)
      if (prior) {
        if (prior.requestHash !== requestHash) throw new Error('Idempotency key was already used for a different configuration mutation.')
        ...
        return { entry: existing, mutation: prior.result.mutation as unknown as ConfigurationMutationResult, warnings: [] }
      }
    }
    if (input.value != null && SENSITIVE_REFERENCE.test(input.value)) throw new Error('Secret references must use the reference field, not a plaintext value field.')
    const current = this.store.find(input.projectId, input.scope, input.key), fingerprint = this.fingerprint(input.value ?? input.reference ?? '')
    ...
    if (input.kind === 'secret') {
      ...
      if (input.reference && input.value == null) {
        if (!await backend.validate(input.reference)) throw new Error('Secret backend reference is unavailable or invalid.')
        ...
        const written = await backend.put({ reference: current?.backend === backendKind ? current.secretRef : undefined, name: this.backendName(input), value: input.value, idempotencyKey: idempotencyKey ?? crypto.randomUUID() })
        secretRef = written.reference; backendVersion = written.version
      }
    }
    ...
    this.audit(input.projectId, input.actorId, current && changed ? 'configuration.updated' : current ? 'configuration.unchanged' : 'configuration.created', entry, affectedResourceIds)
    return { entry, mutation, warnings: this.warnings(input.key, input.value ?? '') }
  }

  async importDotenv(input: Omit<SetConfigurationInput, 'key' | 'kind' | 'value'> & { source: string; secretKeys?: string[] }): Promise<{ document: ReturnType<typeof parseDotenv>; mutation?: ConfigurationMutationResult; warnings: ConfigurationWarning[] }> {
    const document = parseDotenv(input.source)
    if (!document.valid) return { document, warnings: [] }
    const secretKeys = new Set(input.secretKeys ?? []), aggregate: ConfigurationMutationResult = { added: [], changed: [], removed: [], unchanged: [], affectedResourceIds: [], versions: {} }, warnings: ConfigurationWarning[] = []
    for (const [key, value] of Object.entries(document.values)) {
      const result = await this.set({ ...input, source: undefined, key, value, kind: secretKeys.has(key) ? 'secret' : 'variable', idempotencyKey: input.idempotencyKey ? `${input.idempotencyKey}:${key}` : undefined } as SetConfigurationInput)
      aggregate.added.push(...result.mutation.added); aggregate.changed.push(...result.mutation.changed); aggregate.unchanged.push(...result.mutation.unchanged); aggregate.affectedResourceIds.push(...result.mutation.affectedResourceIds); Object.assign(aggregate.versions, result.mutation.versions); warnings.push(...result.warnings)
    }
    aggregate.added = sorted(aggregate.added); aggregate.changed = sorted(aggregate.changed); aggregate.unchanged = sorted(aggregate.unchanged); aggregate.affectedResourceIds = sorted(aggregate.affectedResourceIds)
  ...
  async resolve(input: { projectId: string; environmentId?: string; resourceId?: string; previewId?: string; trustedPreview?: boolean; allowedPreviewSecrets?: string[]; nativeReferences?: boolean; canReadSecretMetadata?: boolean }): Promise<ResolvedConfiguration> {
    ...
    const values: Record<string, string> = {}, entries: Record<string, ConfigurationMetadata> = {}, warnings: ConfigurationWarning[] = [], allowed = new Set(input.allowedPreviewSecrets ?? [])
    for (const [key, entry] of selected) {
      if (input.previewId && entry.kind === 'secret' && entry.scope.type !== 'preview' && (!input.trustedPreview || !allowed.has(key))) continue
  ...
  async reveal(input: { entryId: string; canRevealSecrets: boolean; recentlyAuthenticated: boolean; actorId?: string }): Promise<string> {
  ...
  async remove(input: { entryId: string; expectedVersion: number; confirmed?: boolean; actorId?: string }): Promise<ConfigurationMutationResult> {
    const entry = this.store.get(input.entryId)
    if (!entry) throw new Error('Configuration entry was not found.')
    const dependencies = this.store.dependencies(entry.id), production = entry.scope.environmentId ? this.store.controlPlane.listEnvironments(entry.projectId).some(environment => environment.id === entry.scope.environmentId && environment.kind === 'production') : entry.scope.type === 'project'
    if ((production || dependencies.length) && !input.confirmed) throw new Error('Confirmation is required because this configuration affects production or dependent services.')
    if (entry.kind === 'secret' && entry.backend !== 'external') await this.backend(entry.backend).remove(entry.secretRef!)
    this.store.remove(entry.id, input.expectedVersion)
    const affectedResourceIds = sorted(dependencies.map(item => item.resourceId))
    const result: ConfigurationMutationResult = { added: [], changed: [], removed: [entry.key], unchanged: [], affectedResourceIds, versions: {} }
  ...
  private audit(projectId: string, actorId: string | undefined, type: string, entry: ConfigurationEntry, affectedResourceIds: string[]): void {
    this.store.controlPlane.appendEvent({ projectId, actorId, type, payload: { entryId: entry.id, key: entry.key, kind: entry.kind, scopeType: entry.scope.type, scopeId: entry.scope.id, backend: entry.backend, version: entry.version, affectedResourceIds } })
  }
}
