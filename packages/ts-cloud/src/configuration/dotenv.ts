...
export function parseDotenv(source: string): DotenvDocument {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (!match) {
      diagnostics.push({ line: lineNumber, severity: 'error', code: 'invalid_line', message: 'Expected KEY=VALUE.' })
      continue
    }
    const key = match[1]!,
      raw = match[2]!
    ...
    if (raw.startsWith('"') || raw.startsWith("'")) {
      ...
      while (true) {
        ...
        for (let i = 0; i < current.length; i++) {
          if (quote === '"' && current[i] === '\\' && !escaped) {
            escaped = true
            continue
          }
          if (current[i] === quote && !escaped) {
            closing = i
            break
          }
          escaped = false
        ...
        if (closing >= 0) {
          chunks[chunks.length - 1] = current.slice(0, closing)
          closed = true
          break
        }
      ...
      if (!closed) {
        diagnostics.push({
          ...
          message: `Unterminated quoted value for ${key}.`,
        })
        continue
      }
      value = chunks.join('\n')
    ...
    if (Object.hasOwn(values, key)) {
      ...
      diagnostics.push({
        ...
        code: conflict ? 'conflict' : 'duplicate',
        message: conflict ? `${key} has conflicting values.` : `${key} is duplicated with the same value.`,
      })
