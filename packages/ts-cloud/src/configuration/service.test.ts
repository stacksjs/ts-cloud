...
describe('ConfigurationService', () => {
  ...
  test('makes writes idempotent and tracks rotations and dependent redeploys', async () => {
    ...
    await expect(service.set({ organizationId: organization.id, projectId: project.id, scope, key: 'TOKEN', kind: 'secret', value: 'second', expectedVersion: first.entry.version, idempotencyKey: 'rotate-unconfirmed' })).rejects.toThrow('Confirmation')
    const rotated = await service.set({ organizationId: organization.id, projectId: project.id, scope, key: 'TOKEN', kind: 'secret', value: 'second', expectedVersion: first.entry.version, idempotencyKey: 'rotate-2', confirmed: true })
  ...
  test('copies and moves entries between scopes without reusing managed secret storage', async () => {
    const { organization, project, environment, resource, service } = setup(), source = await service.set({ organizationId: organization.id, projectId: project.id, scope: { type: 'project', id: project.id }, key: 'TOKEN', kind: 'secret', value: 'secret' })
    const copied = await service.transfer({ entryId: source.entry.id, targetScope: { type: 'service', id: resource.id, environmentId: environment.id, resourceId: resource.id }, mode: 'copy', confirmed: true })
    expect(copied.entry.secretRef).not.toBe(source.entry.secretRef)
    expect((await service.resolve({ projectId: project.id, environmentId: environment.id, resourceId: resource.id })).values.TOKEN).toBe('secret')
    const moved = await service.transfer({ entryId: copied.entry.id, targetScope: { type: 'environment', id: environment.id, environmentId: environment.id }, mode: 'move', confirmed: true })
