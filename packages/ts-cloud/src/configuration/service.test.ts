...
function setup() {
  const control = new ControlPlaneStore({ path: ':memory:', now: () => new Date('2026-07-21T12:00:00.000Z') })
  stores.push(control)
  ...
  const store = new ConfigurationStore(control),
    service = new ConfigurationService(store, {
      encryptionKey: 'fixture-encryption',
      fingerprintKey: 'fixture-fingerprint',
...
describe('configuration secret backends', () => {
  test('keeps local values out of the database and authenticates ciphertext', async () => {
    const { control } = setup(),
      backend = new LocalEncryptedConfigurationBackend(control, 'encryption-key'),
      result = await backend.put({ name: 'project/token', value: 'never-store-plaintext', idempotencyKey: 'one' })
    ...
    await expect(
      new LocalEncryptedConfigurationBackend(control, 'wrong-key').resolve(result.reference),
...
describe('ConfigurationService', () => {
  test('resolves deterministic precedence and exposes masked metadata', async () => {
    ...
    await service.set({
      organizationId: organization.id,
      projectId: project.id,
      scope: { type: 'environment', id: environment.id, environmentId: environment.id },
      key: 'ORIGIN',
      kind: 'variable',
      value: 'environment',
    })
    ...
    expect(service.list({ projectId: project.id }).find((entry) => entry.key === 'TOKEN')).toMatchObject({
      key: 'TOKEN',
      kind: 'secret',
      backend: undefined,
      reference: undefined,
  ...
  test('applies function overrides after service values and before preview values', async () => {
    const { control, organization, project, environment, resource, service } = setup(),
      fn = control.createResource({
        projectId: project.id,
        environmentId: environment.id,
        kind: 'function',
        slug: 'http',
    ...
    await service.set({
      organizationId: organization.id,
      projectId: project.id,
      scope: { type: 'function', id: fn.id, environmentId: environment.id, resourceId: fn.id },
      key: 'TIMEOUT',
      kind: 'variable',
      value: 'function',
    })
  ...
  test('previews key-only drift and imports valid dotenv atom names', async () => {
    ...
    const imported = await service.importDotenv({
      organizationId: organization.id,
      projectId: project.id,
      scope,
      source: 'PUBLIC_URL=https://example.test\nTOKEN=hidden',
      secretKeys: ['TOKEN'],
      idempotencyKey: 'dotenv-1',
    })
    const plan = service.plan({
      projectId: project.id,
      scope,
      values: { PUBLIC_URL: 'https://new.test', TOKEN: 'hidden', NEW_KEY: 'value' },
    })
  ...
  test('makes writes idempotent and tracks rotations and dependent redeploys', async () => {
    const { organization, project, resource, store, service } = setup(),
      scope = { type: 'project' as const, id: project.id }
    ...
    const duplicate = await service.set({
      organizationId: organization.id,
      projectId: project.id,
      scope,
      key: 'TOKEN',
    ...
    await expect(
      service.set({
        ...
        idempotencyKey: 'rotate-unconfirmed',
      }),
    ...
    ).rejects.toThrow('different configuration mutation')
  })

  test('does not inherit production secrets into previews unless explicitly trusted and allowed', async () => {
    const { control, organization, project, environment, resource, service } = setup(),
      previews = new PreviewEnvironmentStore(control, {
        id: (() => {
          let id = 0
          return () => `preview-${++id}`
        })(),
      })
    const definition = previews.createDefinition({
      projectId: project.id,
      resourceId: resource.id,
      baseEnvironmentId: environment.id,
      domainPattern: 'https://{name}.example.test',
      inheritedSecrets: ['TOKEN'],
    })
    const preview = previews.upsert({
    ...
    const isolated = await service.resolve({
      projectId: project.id,
      environmentId: environment.id,
      resourceId: resource.id,
      previewId: preview.id,
    })
    const trusted = await service.resolve({
      projectId: project.id,
      environmentId: environment.id,
      resourceId: resource.id,
      previewId: preview.id,
      trustedPreview: true,
      allowedPreviewSecrets: ['TOKEN'],
    })
  ...
  test('requires recent authentication to reveal and confirmation to delete production values', async () => {
    const { organization, project, service } = setup(),
    ...
    await expect(
      service.reveal({ entryId: created.entry.id, canRevealSecrets: true, recentlyAuthenticated: false }),
    ).rejects.toThrow('Recent authentication')
    expect(
      await service.reveal({ entryId: created.entry.id, canRevealSecrets: true, recentlyAuthenticated: true }),
    ).toBe('secret')
    await expect(service.remove({ entryId: created.entry.id, expectedVersion: created.entry.version })).rejects.toThrow(
      'Confirmation',
    )
    expect(
      await service.remove({ entryId: created.entry.id, expectedVersion: created.entry.version, confirmed: true }),
    ).toMatchObject({ removed: ['TOKEN'] })
  })

  test('copies and moves entries between scopes without reusing managed secret storage', async () => {
    const { organization, project, environment, resource, service } = setup(),
      source = await service.set({
        organizationId: organization.id,
        projectId: project.id,
        scope: { type: 'project', id: project.id },
        key: 'TOKEN',
    ...
    const copied = await service.transfer({
      ...
      targetScope: { type: 'service', id: resource.id, environmentId: environment.id, resourceId: resource.id },
      mode: 'copy',
      confirmed: true,
    })
    expect(copied.entry.secretRef).not.toBe(source.entry.secretRef)
    expect(
    ...
    const moved = await service.transfer({
      entryId: copied.entry.id,
      targetScope: { type: 'environment', id: environment.id, environmentId: environment.id },
      mode: 'move',
      confirmed: true,
    })
    expect(moved.mutation.removed).toEqual(['TOKEN'])
    expect(service.store.get(copied.entry.id)).toBeUndefined()
  })
})
