...
export async function synchronizeConfiguredConfiguration(
  ...
  const synchronize = async (scope: ConfigurationScope, desired: Record<string, string>) => {
    const existing = service.store.list({ projectId: controlPlane.project.id, scope, kind: 'variable' }),
      keys = new Set(Object.keys(desired))
    for (const item of existing.filter((item) => item.origin === 'config' && !keys.has(item.key))) {
      await service.remove({ entryId: item.id, expectedVersion: item.version, confirmed: true, actorId })
      result.removed++
    }
    for (const [key, value] of Object.entries(desired)) {
      const current = service.store.find(controlPlane.project.id, scope, key)
      if (current?.origin !== 'config') {
        if (current) result.overridden++
        else {
          await service.set({
  ...
  for (const [slug, configured] of Object.entries(config.environments ?? {})) {
    ...
    const environmentConfig = configured as Record<string, any>,
      variables = { ...(environmentConfig.variables ?? {}), ...(environmentConfig.app?.env ?? {}) }
    ...
    for (const [siteSlug, site] of Object.entries(config.sites ?? {})) {
      const resource = controlPlane.store
        ...
        .find((item) => item.kind === 'application' && item.slug === siteSlug)
      if (resource)
        await synchronize(
          { type: 'service', id: resource.id, environmentId: environment.id, resourceId: resource.id },
          { ...((site as Record<string, any>).env ?? {}) },
        )
    }
    for (const [functionSlug, definition] of Object.entries(config.infrastructure?.functions ?? {})) {
      const resource = controlPlane.store
        .listResources(controlPlane.project.id, environment.id)
        .find((item) => item.kind === 'function' && item.slug === functionSlug)
      if (resource)
        await synchronize(
          { type: 'function', id: resource.id, environmentId: environment.id, resourceId: resource.id },
          { ...((definition as Record<string, any>).environment ?? {}) },
        )
    }
  }
  return result
