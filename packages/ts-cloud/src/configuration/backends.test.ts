...
describe('provider configuration backends', () => {
  test('creates and versions AWS Secrets Manager values with stable references', async () => {
    const calls: any[] = [],
      client = {
        createSecret: async (input: any) => {
          calls.push(['create', input])
          return { ARN: 'arn:aws:secretsmanager:us-east-1:123:secret:app', VersionId: 'v1' }
        },
        putSecretValue: async (input: any) => {
          calls.push(['put', input])
          return { VersionId: 'v2' }
        },
        getSecretValue: async (input: any) => {
          calls.push(['get', input])
          return { SecretString: 'resolved' }
        },
        describeSecret: async (input: any) => {
          calls.push(['describe', input])
          return {}
        },
        deleteSecret: async (input: any) => {
          calls.push(['delete', input])
          return {}
        },
      },
      backend = new AwsSecretsManagerConfigurationBackend(client as any, 'us-east-1')
    const created = await backend.put({ name: 'ts-cloud/project/TOKEN', value: 'first', idempotencyKey: 'request-1' }),
      rotated = await backend.put({
    ...
    expect(rotated).toEqual({ reference: created.reference, version: 'v2' })
    expect(await backend.resolve(created.reference)).toBe('resolved')
    expect(await backend.validate(created.reference)).toBe(true)
    await backend.remove(created.reference)
  ...
  test('uses SecureString and decryption only at the SSM deployment boundary', async () => {
    const calls: any[] = [],
      client = {
        putParameter: async (input: any) => {
          calls.push(['put', input])
          return { Version: 3 }
        },
        getParameter: async (input: any) => {
          calls.push(['get', input])
          return { Parameter: { Value: input.WithDecryption ? 'resolved' : undefined } }
        },
        deleteParameter: async (input: any) => {
          calls.push(['delete', input])
        },
      },
      backend = new AwsSsmConfigurationBackend(client as any, 'us-west-2')
    ...
    expect(await backend.validate(written.reference)).toBe(true)
    expect(await backend.resolve(written.reference)).toBe('resolved')
    await backend.remove(written.reference)
    expect(calls[1][1].WithDecryption).toBe(false)
    expect(calls[2][1].WithDecryption).toBe(true)
  })
