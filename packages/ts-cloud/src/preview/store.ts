...
export class PreviewEnvironmentStore {
  ...
  createDefinition(input: CreatePreviewDefinitionInput): PreviewDefinition {
    ...
    if (publicAccess && !authenticationRequired) throw new Error('Public preview environments require an authentication policy')
    ...
    if (existing) {
      this.controlPlane.database.run(`UPDATE preview_definitions SET base_environment_id=?, enabled=1, branch_rule=?, domain_pattern=?, ttl_hours=?, keep_count=?, public_access=?, authentication_required=?, allow_forks=?, inherited_secrets=?, resource_overrides=?, database_strategy=?, max_monthly_cost=?, max_cpu=?, max_memory_mb=?, cleanup_on_close=?, version=version+1, updated_at=? WHERE id=?`, [input.baseEnvironmentId, input.branchRule ?? null, input.domainPattern, integer(input.ttlHours, existing.ttlHours, 1, 720), integer(input.keepCount, existing.keepCount, 1, 100), publicAccess ? 1 : 0, authenticationRequired ? 1 : 0, input.allowForks ?? existing.allowForks ? 1 : 0, JSON.stringify(input.inheritedSecrets === undefined ? existing.inheritedSecrets : inheritedSecrets), JSON.stringify(sanitizeControlPlaneValue(input.resourceOverrides ?? existing.resourceOverrides)), input.databaseStrategy ?? existing.databaseStrategy, decimal(input.maxMonthlyCost, existing.maxMonthlyCost, 0, 10000), decimal(input.maxCpu, existing.maxCpu, 0.1, 64), integer(input.maxMemoryMb, existing.maxMemoryMb, 128, 131072), input.cleanupOnClose ?? existing.cleanupOnClose ? 1 : 0, now, existing.id])
      this.controlPlane.appendEvent({ projectId: input.projectId, resourceId: input.resourceId, actorId: input.createdByActorId, type: 'preview.definition.updated', payload: { definitionId: existing.id, baseEnvironmentId: input.baseEnvironmentId, version: existing.version + 1 } })
  ...
  extend(id: string, hours: number): PreviewInstance { const current = this.getInstance(id); const policy = current ? this.getDefinition(current.definitionId) : undefined; if (!current || !policy) throw new Error(`Preview ${id} was not found`); const now = this.now(); const bounded = integer(hours, policy.ttlHours, 1, 720); const expires = new Date(Math.max(now.getTime(), new Date(current.expiresAt).getTime()) + bounded * 3600000).toISOString(); this.controlPlane.database.run('UPDATE preview_instances SET expires_at=?, version=version+1, updated_at=? WHERE id=?', [expires, now.toISOString(), id]); return this.getInstance(id)! }
