...
export interface PreviewCleanupCandidate { preview: PreviewInstance, reasons: string[] }
export interface PreviewCleanupResult { candidates: PreviewCleanupCandidate[], operations: ControlPlaneOperation[], dryRun: boolean }
export interface DiscoveredPreviewResource { provider: string, providerResourceId: string, kind: string, tags: Record<string, string>, observedState?: JsonValue, estimatedMonthlyCost?: number }
...
export class PreviewEnvironmentService {
  ...
  cleanup(input: { now?: Date, maxAgeHours?: number, keepCount?: number, dryRun?: boolean, actorId?: string } = {}): PreviewCleanupResult {
    ...
    const add = (preview: PreviewInstance, reason: string) => { const found = candidates.get(preview.id) ?? { preview, reasons: [] }; if (!found.reasons.includes(reason)) found.reasons.push(reason); candidates.set(preview.id, found) }
    for (const preview of active) {
      if (new Date(preview.expiresAt) <= now) add(preview, 'ttl_expired')
      if (input.maxAgeHours && now.getTime() - new Date(preview.createdAt).getTime() >= input.maxAgeHours * 3600000) add(preview, 'max_age')
    }
    const byDefinition = new Map<string, PreviewInstance[]>()
    for (const preview of active) byDefinition.set(preview.definitionId, [...(byDefinition.get(preview.definitionId) ?? []), preview])
    for (const [definitionId, values] of byDefinition) {
      const policy = this.previews.getDefinition(definitionId); const keep = input.keepCount ?? policy?.keepCount
      if (!keep) continue
      values.sort((left, right) => right.createdAt.localeCompare(left.createdAt)).slice(keep).forEach(preview => add(preview, 'keep_count'))
    }
  ...
  reconcile(discovered: DiscoveredPreviewResource[]): { matched: string[], unknownPreviewResources: string[], ignoredUntagged: string[] } {
    const matched: string[] = []; const unknownPreviewResources: string[] = []; const ignoredUntagged: string[] = []
    for (const found of discovered) {
      const previewId = found.tags['ts-cloud:preview']; const projectId = found.tags['ts-cloud:project']; const expiresAt = found.tags['ts-cloud:expires-at']
      if (!previewId || !projectId || !expiresAt) { ignoredUntagged.push(found.providerResourceId); continue }
      const preview = this.previews.getInstance(previewId)
      if (!preview || preview.projectId !== projectId || preview.expiresAt !== expiresAt) { unknownPreviewResources.push(found.providerResourceId); continue }
      this.previews.recordResource({ ...found, previewId: preview.id })
      if (found.estimatedMonthlyCost !== undefined) this.previews.transition(preview.id, preview.status, { costEstimate: found.estimatedMonthlyCost, observedState: { ...preview.observedState as Record<string, JsonValue>, lastReconciledAt: new Date().toISOString() } })
      matched.push(found.providerResourceId)
    }
    if (unknownPreviewResources.length) this.controlPlane.appendEvent({ type: 'preview.reconciliation.leaks', level: 'error', payload: { providerResourceIds: unknownPreviewResources } })
