...
function setup(path = ':memory:', now = new Date('2026-07-01T00:00:00.000Z')) {
  const controlPlane = new ControlPlaneStore({ path, now: () => now })
  ...
  return {
    ...
    environment,
    resource,
    previews: new PreviewEnvironmentStore(controlPlane, { now: () => now }),
  }
}
...
describe('preview environment persistence and policy', () => {
  it('persists definitions and deterministic preview identity across restart', () => {
    ...
    target.controlPlane.close()
    stores.splice(stores.indexOf(target.controlPlane), 1)

    const restarted = new ControlPlaneStore({ path })
    stores.push(restarted)
    const previews = new PreviewEnvironmentStore(restarted)
    expect(previews.getInstance(created.preview.id)).toMatchObject({
      name: expect.stringMatching(/^pr-42-[a-f0-9]{8}$/),
      url: expect.stringMatching(/^https:\/\/pr-42-/),
      commitSha: 'a'.repeat(40),
      status: 'queued',
    })
    const update = previews.upsert({
      definitionId: policy.id,
      sourceProvider: 'github',
      repository: 'acme/web',
      branch: 'feature/search',
    ...
    expect(update).toMatchObject({
      created: false,
      changed: true,
      preview: { id: created.preview.id, commitSha: 'b'.repeat(40) },
    })
  })
  ...
  it('reconfigures an existing resource policy and extends from the injected clock', () => {
    ...
    const original = target.previews.createDefinition({
      projectId: target.project.id,
      resourceId: target.resource.id,
      baseEnvironmentId: target.environment.id,
      domainPattern: 'https://{name}.preview.example.com',
    ...
    const updated = target.previews.createDefinition({
      projectId: target.project.id,
      resourceId: target.resource.id,
      baseEnvironmentId: target.environment.id,
      domainPattern: 'https://{name}.staging.example.com',
      ttlHours: 12,
      allowForks: true,
    })
    ...
    const preview = target.previews.upsert({
  ...
  it('uses collision-safe branch names and never inherits secrets into fork previews', () => {
    ...
    const policy = target.previews.createDefinition({
      ...
      allowForks: true,
      inheritedSecrets: ['PREVIEW_TOKEN'],
    })
    ...
    const fork = target.previews.upsert({
      definitionId: policy.id,
      repository: 'fork/web',
      branch: 'patch',
      pullRequestNumber: 9,
  ...
  it('enforces domain, authentication, immutable SHA, scope, and tagged resource safety', () => {
    const target = setup()
    expect(() =>
      target.previews.createDefinition({
        projectId: target.project.id,
        resourceId: target.resource.id,
        baseEnvironmentId: target.environment.id,
        domainPattern: 'http://{name}.test',
        publicAccess: true,
    ...
    expect(() => target.previews.upsert({ definitionId: policy.id, branch: 'main', commitSha: 'moving-ref' })).toThrow(
      'immutable',
    )
    const preview = target.previews.upsert({
      definitionId: policy.id,
    ...
    expect(() =>
      target.previews.recordResource({
        previewId: preview.id,
        provider: 'aws',
        providerResourceId: 'stack-1',
        kind: 'stack',
        tags: {},
      }),
    ...
    ).toMatchObject({ providerResourceId: 'stack-1' })
  })
  ...
  it('finds expired previews and keeps failed cleanup visible with reconciliation data', () => {
    ...
    const failed = target.previews.transition(preview.id, 'cleanup_failed', {
      teardownError: 'stack delete failed',
      observedState: { leakedResourceIds: ['stack-1'] },
    })
