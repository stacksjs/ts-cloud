...
export function resolveQueuedDeploymentCommand(store: ControlPlaneStore, operation: ControlPlaneOperation): QueuedDeploymentCommand {
  ...
  if (operation.kind.startsWith('compose.')) {
    ...
    const application = composeId ? new ComposeApplicationStore(store).get(composeId) : operation.resourceId ? new ComposeApplicationStore(store).getByResource(operation.resourceId) : undefined
    if (!application || application.projectId !== operation.projectId || application.environmentId !== operation.environmentId || application.resourceId !== operation.resourceId) throw new Error('Queued Compose application does not match the operation scope')
    ...
    return { id: `queue:${operation.id}`, label: `${action[0]!.toUpperCase()}${action.slice(1)} Compose application`, description: `${action} ${application.name}.`, command, mutates: true, target: { environment: environment.slug, resource: resource?.slug, composeId: application.id } }
  }
...
export function createDeploymentQueueHandlers(input: {
  ...
  const handler: QueueOperationHandler = async (context) => {
    ...
    if (compose && command.target.composeId) compose.transition(command.target.composeId, context.operation.kind === 'compose.delete' ? 'deleting' : context.operation.kind === 'compose.stop' ? 'stopped' : 'deploying', { operationId: context.operation.id })
    ...
    if (!result.ok) {
      ...
      if (compose && command.target.composeId) compose.transition(command.target.composeId, context.operation.kind === 'compose.delete' ? 'degraded' : 'failed', { operationId: context.operation.id, error: result.stderr?.trim() || `Process exited with ${result.exitCode}` })
