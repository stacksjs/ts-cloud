...
describe('durable queue worker', () => {
  it('runs independent targets in parallel up to its lane bound', async () => {
    ...
    for (const resource of [target.first, target.second]) {
      queue.enqueue({ projectId: target.project.id, environmentId: target.environment.id, resourceId: resource.id, kind: 'deployment.create', lockKey: `resource:${resource.id}` })
    }

    let active = 0
    let peak = 0
    const worker = new DurableQueueWorker(queue, {
  ...
  it('relies on persisted resource locks to serialize conflicting work', async () => {
    ...
    for (const kind of ['deployment.create', 'deployment.rollback']) {
      queue.enqueue({ projectId: target.project.id, environmentId: target.environment.id, resourceId: target.first.id, kind, lockKey: `resource:${target.first.id}` })
    }
    ...
    const worker = new DurableQueueWorker(queue, { 'deployment.create': handler, 'deployment.rollback': handler }, { parallelism: 4 })
  ...
  it('polls for newly queued work and stops without claiming more', async () => {
    ...
    let handled = 0
    const worker = new DurableQueueWorker(queue, { 'deployment.create': async () => { handled++ } }, { parallelism: 1, pollIntervalMs: 5 }).start()
