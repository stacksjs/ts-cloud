...
function job(row: Row): OperationJob {
  return {
    ...
    lockKey: optional(row.lock_key),
    providerKey: optional(row.provider_key),
    buildSlot: Number(row.build_slot) === 1,
    maxAttempts: Number(row.max_attempts),
    availableAt: String(row.available_at),
    timeoutSeconds: Number(row.timeout_seconds),
    heartbeatAt: optional(row.heartbeat_at),
    currentStep: optional(row.current_step),
    blockedReason: optional(row.blocked_reason),
    retryClasses: parsedStrings(row.retry_classes),
    resumePolicy: String(row.resume_policy) as OperationJob['resumePolicy'],
    cancellationMode: String(row.cancellation_mode) as OperationJob['cancellationMode'],
...
function logEntry(row: Row): OperationLogEntry {
  return {
    ...
    id: String(row.id),
    operationId: String(row.operation_id),
    stream: String(row.stream) as OperationLogEntry['stream'],
    step: optional(row.step),
...
export class DurableOperationQueue {
  ...
  private leaseExpiry(): string {
    return new Date(this.nowFn().getTime() + this.leaseMs).toISOString()
  }
  ...
  enqueue(input: EnqueueOperationInput): QueueOperationView {
    ...
    this.controlPlane.database.run(
      ...
      [
        operation.id,
        input.lockKey ?? input.resourceId ?? null,
        input.providerKey ?? null,
        input.buildSlot ? 1 : 0,
        maxAttempts,
        availableAt,
        timeoutSeconds,
        JSON.stringify([...new Set(input.retryClasses ?? [])]),
        input.resumePolicy ?? 'fail',
        input.cancellationMode ?? 'cooperative',
        retentionUntil,
        now,
        now,
      ],
  ...
  getJob(operationId: string): OperationJob | undefined {
    const row = this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM operation_jobs WHERE operation_id=?')
      .get(operationId)
  ...
  list(input: { projectId?: string; state?: OperationState; limit?: number } = {}): QueueOperationView[] {
    return this.controlPlane
      .listOperations({ projectId: input.projectId, state: input.state, limit: input.limit ?? 200 })
      .map((operation) => this.view(operation.id))
      .filter((value): value is QueueOperationView => !!value)
  }
  ...
  configureConcurrency(
    ...
    this.controlPlane.appendEvent({
      ...
      type: 'queue.concurrency.updated',
      level: 'warning',
      payload: next,
    })
    return next
  ...
  private blockReason(operation: ControlPlaneOperation, metadata: OperationJob): string | undefined {
    if (metadata.lockKey) {
      const lock = this.controlPlane.database
        .query<Row, [string, string, string]>(
          'SELECT * FROM operation_locks WHERE lock_key=? AND operation_id!=? AND lease_expires_at>?',
        )
        .get(metadata.lockKey, operation.id, this.now())
  ...
  claim(operationId: string): QueueOperationView | undefined {
    return this.controlPlane.transaction(() => {
      ...
      const operation = this.controlPlane.getOperation(operationId)
      const metadata = operation ? this.getJob(operation.id) : undefined
      if (!operation || !metadata || operation.state !== 'queued' || metadata.availableAt > now) return undefined
      if (operation.cancelRequestedAt) {
        this.controlPlane.transitionOperation(operation.id, {
          to: 'cancelled',
          expectedVersion: operation.version,
          output: { cancelledBeforeStart: true },
        })
      ...
      if (blocked) {
        this.controlPlane.database.run(
          'UPDATE operation_jobs SET blocked_reason=?, updated_at=? WHERE operation_id=?',
          [blocked, now, operation.id],
        )
        return undefined
      }
      ...
      if (metadata.lockKey)
        this.controlPlane.database.run(
          'INSERT INTO operation_locks (lock_key, operation_id, lease_owner, lease_expires_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
          [metadata.lockKey, operation.id, this.workerId, expiry, now, now],
        )
      this.controlPlane.database.run(
        'UPDATE operation_jobs SET heartbeat_at=?, blocked_reason=NULL, updated_at=? WHERE operation_id=?',
  ...
  claimNext(kinds?: readonly string[]): QueueOperationView | undefined {
    ...
    const rows = this.controlPlane.database
      .query<Row, SQLQueryBindings[]>(
        `SELECT o.id FROM operations o JOIN operation_jobs j ON j.operation_id=o.id WHERE o.state='queued' AND j.available_at<=? ${kindSql} ORDER BY o.priority DESC, o.created_at ASC, o.rowid ASC LIMIT 100`,
      )
      .all(now, ...(kinds ?? []))
  ...
  heartbeat(operationId: string, step?: string): void {
    const operation = this.controlPlane.getOperation(operationId)
    if (!operation || operation.state !== 'running' || operation.leaseOwner !== this.workerId)
      throw new Error('Operation lease is not owned by this worker')
    const now = this.now()
    ...
    this.controlPlane.database.run(
      'UPDATE operation_jobs SET heartbeat_at=?, current_step=COALESCE(?, current_step), updated_at=? WHERE operation_id=?',
      [now, step ?? null, now, operationId],
    )
    if (metadata?.lockKey)
      this.controlPlane.database.run(
        'UPDATE operation_locks SET lease_expires_at=?, updated_at=? WHERE lock_key=? AND operation_id=? AND lease_owner=?',
        [expiry, now, metadata.lockKey, operationId, this.workerId],
      )
    if (step && step !== metadata?.currentStep)
      this.controlPlane.appendEvent({
  ...
  appendLog(operationId: string, message: string, input: QueueLogInput = {}): OperationLogEntry {
    ...
    for (const secret of input.secrets ?? [])
      if (secret && safe.includes(secret)) {
    ...
    const truncated = bytes.byteLength > MAX_LOG_BYTES
    if (truncated) safe = `${bytes.subarray(0, MAX_LOG_BYTES - 32).toString('utf8')}\n[output truncated]`
    const id = this.idFn()
    const now = this.now()
    this.controlPlane.database.run(
      'INSERT INTO operation_logs (id, operation_id, stream, step, message, redacted, truncated, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, operationId, input.stream ?? 'stdout', input.step ?? null, safe, redacted ? 1 : 0, truncated ? 1 : 0, now],
    )
  ...
  logs(operationId: string, input: { after?: number; limit?: number } = {}): OperationLogEntry[] {
    ...
    return this.controlPlane.database
      .query<Row, [string, number, number]>(
        'SELECT * FROM operation_logs WHERE operation_id=? AND sequence>? ORDER BY sequence ASC LIMIT ?',
      )
      .all(operationId, input.after ?? 0, limit)
      .map(logEntry)
  }

  requestCancellation(operationId: string, actorId?: string): ControlPlaneOperation {
    const current = this.controlPlane.getOperation(operationId)
    ...
    this.controlPlane.appendEvent({
      ...
      type: 'operation.cancellation_requested',
      level: 'warning',
    ...
    this.appendLog(
      ...
      current.state === 'queued'
        ? 'Cancellation accepted before execution.'
        : 'Cancellation requested; waiting for the current provider step.',
      { stream: 'system' },
    )
  ...
  retry(
    operationId: string,
    errorClass: string,
    input: { delayMs?: number; actorId?: string } = {},
  ): ControlPlaneOperation {
    const operation = this.controlPlane.getOperation(operationId)
    const metadata = this.getJob(operationId)
    if (!operation || !metadata || !['failed', 'cancelled', 'timed_out'].includes(operation.state))
      throw new Error('Only failed, cancelled, or timed-out queue jobs can be retried')
    if (operation.attempt >= metadata.maxAttempts)
      throw new Error(`Operation reached its ${metadata.maxAttempts}-attempt limit`)
    if (!metadata.retryClasses.includes(errorClass))
      throw new Error(`Retry class ${errorClass} is not allowed for this operation`)
    const queued = this.controlPlane.transitionOperation(operationId, {
      to: 'queued',
      expectedVersion: operation.version,
      error: `Retry requested after ${errorClass}`,
    })
    const available = new Date(this.nowFn().getTime() + Math.max(0, input.delayMs ?? 0)).toISOString()
    this.controlPlane.database.run('UPDATE operations SET cancel_requested_at=NULL, finished_at=NULL WHERE id=?', [
      operationId,
    ])
    this.controlPlane.database.run(
      'UPDATE operation_jobs SET available_at=?, blocked_reason=NULL, updated_at=? WHERE operation_id=?',
      [available, this.now(), operationId],
    )
    this.controlPlane.appendEvent({
      projectId: operation.projectId,
      operationId,
      ...
      actorId: input.actorId,
      correlationId: operation.correlationId,
      type: 'operation.retry_requested',
  ...
  private finish(
    operationId: string,
    state: Extract<OperationState, 'succeeded' | 'failed' | 'cancelled' | 'timed_out'>,
    output: JsonValue = {},
    error?: string,
  ): ControlPlaneOperation {
    const current = this.controlPlane.getOperation(operationId)
    if (!current) throw new Error(`Operation ${operationId} was not found`)
    const result = this.controlPlane.transitionOperation(operationId, {
      to: state,
  ...
  complete(operationId: string, output: JsonValue = {}): ControlPlaneOperation {
    const operation = this.controlPlane.getOperation(operationId)
    if (!operation || operation.state !== 'running' || operation.leaseOwner !== this.workerId)
      throw new Error('Only the lease owner can complete a running operation')
  ...
  fail(operationId: string, error: string, output: JsonValue = {}): ControlPlaneOperation {
    const operation = this.controlPlane.getOperation(operationId)
    if (!operation || operation.state !== 'running' || operation.leaseOwner !== this.workerId)
      throw new Error('Only the lease owner can fail a running operation')
  ...
  async runOne(handlers: Record<string, QueueOperationHandler>): Promise<QueueRunResult> {
    const claimed = this.claimNext(Object.keys(handlers))
    if (!claimed) return { handled: false }
    const { operation, job: metadata } = claimed
    ...
    const timeoutTimer = setTimeout(() => {
      timeout = true
      controller.abort(new QueueTimeoutError(metadata.timeoutSeconds))
    }, metadata.timeoutSeconds * 1000)
    const heartbeatTimer = setInterval(
      () => {
        try {
          this.heartbeat(operation.id)
        } catch {}
      },
      Math.max(250, Math.floor(this.leaseMs / 3)),
    )
    const cancellationTimer = setInterval(() => {
      if (this.controlPlane.getOperation(operation.id)?.cancelRequestedAt)
        controller.abort(new QueueCancellationError())
    }, 100)
    const context: QueueExecutionContext = {
      operation,
      signal: controller.signal,
      log: (message, input) => this.appendLog(operation.id, message, input),
      checkpoint: (step, message) => {
        this.heartbeat(operation.id, step)
        this.appendLog(operation.id, message ?? step, { stream: 'step', step })
      },
      ...
      throwIfCancellationRequested: () => {
        if (this.controlPlane.getOperation(operation.id)?.cancelRequestedAt) throw new QueueCancellationError()
      },
    }
    try {
      ...
      if (latest.cancelRequestedAt) {
        ...
        this.appendLog(
          operation.id,
          providerCompleted
            ? 'Provider step completed after cancellation was requested; reconciliation is required.'
            : 'Operation cancelled cooperatively.',
          { stream: 'system' },
        )
        const result = this.finish(operation.id, 'cancelled', {
          providerCompleted,
          reconciliationRequired: providerCompleted,
        })
      ...
      if (timeout || error instanceof QueueTimeoutError) {
        ...
        const result = this.finish(
          operation.id,
          'timed_out',
          { reconciliationRequired: metadata.cancellationMode === 'provider_non_cancellable' },
      ...
      if (
        ...
        metadata.retryClasses.includes(error.errorClass) &&
        operation.attempt < metadata.maxAttempts
      ) {
        ...
        this.controlPlane.database.run(
          'UPDATE operation_jobs SET available_at=?, blocked_reason=?, updated_at=? WHERE operation_id=?',
          [available, `retry_backoff:${error.errorClass}`, this.now(), operation.id],
        )
        this.release(operation.id)
        this.appendLog(operation.id, `Retryable ${error.errorClass} failure; next attempt is delayed.`, {
          stream: 'system',
        })
  ...
  recoverExpired(): QueueRecoveryResult {
    const rows = this.controlPlane.database
      .query<Row, [string]>(
        `SELECT o.id FROM operations o JOIN operation_jobs j ON j.operation_id=o.id WHERE o.state='running' AND (o.lease_expires_at IS NULL OR o.lease_expires_at<=?)`,
      )
      .all(this.now())
    const result: QueueRecoveryResult = { requeued: 0, failed: 0, cancelled: 0 }
    for (const row of rows) {
      const operation = this.controlPlane.getOperation(String(row.id))!
      const metadata = this.getJob(operation.id)!
      this.release(operation.id)
      if (operation.cancelRequestedAt) {
        this.finish(operation.id, 'cancelled', { recoveredAfterWorkerLoss: true })
        result.cancelled++
        continue
      }
      if (metadata.resumePolicy === 'requeue' && operation.attempt < metadata.maxAttempts) {
        ...
        this.finish(
          operation.id,
          'failed',
          { recoverable: true, lastCheckpoint: metadata.currentStep ?? null },
          'Worker lease expired; retry explicitly after reconciling provider state.',
        )
        result.failed++
      }
    }
  ...
  clearCompleted(input: { before?: string; actorId?: string; projectId?: string } = {}): number {
    ...
    const project = input.projectId ? 'AND o.project_id=?' : ''
    if (input.projectId) bindings.push(input.projectId)
    const finished = input.before ? 'AND o.finished_at<=?' : ''
    if (input.before) bindings.push(new Date(input.before).toISOString())
    const rows = this.controlPlane.database
      .query<Row, SQLQueryBindings[]>(
        `SELECT o.id, o.project_id, o.correlation_id FROM operations o JOIN operation_jobs j ON j.operation_id=o.id WHERE o.state IN ('succeeded','failed','cancelled','timed_out') AND j.retention_until<=? ${project} ${finished}`,
      )
      .all(...bindings)
    for (const row of rows) {
      this.controlPlane.appendEvent({
        ...
        type: 'queue.history.cleared',
        level: 'warning',
        payload: { operationId: String(row.id), retentionCutoff: now },
      })
