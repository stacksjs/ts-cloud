...
describe('durable operation queue', () => {
  it('survives restart, claims once, and prevents conflicting target work', async () => {
    const root = mkdtempSync(join(tmpdir(), 'ts-cloud-queue-'))
    roots.push(root)
    const path = join(root, 'control-plane.sqlite')
    const firstStore = new ControlPlaneStore({ path })
    const target = scope(firstStore)
    const firstQueue = new DurableOperationQueue(firstStore, { workerId: 'worker-1', limits: { environment: 4 } })
    ...
    const conflicting = firstQueue.enqueue({
      projectId: target.project.id,
      environmentId: target.environment.id,
      resourceId: target.first.id,
    ...
    const restartedStore = new ControlPlaneStore({ path })
    const restarted = new DurableOperationQueue(restartedStore, { workerId: 'worker-1', limits: { environment: 4 } })
    expect(restarted.view(first.operation.id)).toMatchObject({
      operation: { state: 'queued' },
      job: { resumePolicy: 'requeue' },
    })
    const claimed = restarted.claimNext()!
    expect(claimed.operation).toMatchObject({
      id: first.operation.id,
      state: 'running',
      attempt: 1,
      leaseOwner: 'worker-1',
    })
    const duplicateWorker = new DurableOperationQueue(restartedStore, {
      workerId: 'worker-2',
      limits: { environment: 4 },
    })
    expect(duplicateWorker.claimNext()).toBeUndefined()
    expect(restarted.view(conflicting.operation.id)?.job.blockedReason).toBe(
      `resource_lock:resource:${target.first.id}`,
    )
  ...
  it('persists ordered cursor logs with redaction and truncation before storage', () => {
    ...
    const first = queue.appendLog(queued.operation.id, 'authorization=Bearer-secret and exact-value', {
      stream: 'stdout',
      secrets: ['exact-value'],
    })
    ...
    expect(queue.logs(queued.operation.id, { after: first.sequence }).map((entry) => entry.sequence)).toEqual([
      second.sequence,
    ])
    expect(
      store.database
        .query<Record<string, string>, [string]>('SELECT message FROM operation_logs WHERE id=?')
  ...
  it('requeues only allow-listed retry classes with backoff and deterministic attempts', async () => {
    ...
    const first = await queue.runOne({
      'deployment.create': async () => {
        throw new RetryableOperationError('provider is throttling', 'provider_throttled')
      },
    })
    ...
    const second = await queue.runOne({
      'deployment.create': async (context) => {
        context.checkpoint('publish', 'Publishing release')
        return { release: 'r1' }
      },
    })
  ...
  it('cancels queued work and reconciles expired worker leases from checkpoints', () => {
    ...
    const recoverable = queue.enqueue({
      projectId: target.project.id,
      resourceId: target.second.id,
      kind: 'deployment.create',
      resumePolicy: 'requeue',
    ...
    expect(queue.view(recoverable.operation.id)).toMatchObject({
      operation: { state: 'queued' },
      job: { currentStep: 'upload', blockedReason: 'worker_restart' },
    })
    expect(store.listEvents({ operationId: cancelled.operation.id }).map((event) => event.type)).toContain(
  ...
  it('times out bounded handlers and marks non-cancellable provider work for reconciliation', async () => {
    const store = new ControlPlaneStore({ path: ':memory:' })
    const target = scope(store)
    const queue = new DurableOperationQueue(store, { workerId: 'timeout-worker' })
    queue.enqueue({
      projectId: target.project.id,
      resourceId: target.first.id,
      kind: 'provider.deploy',
      timeoutSeconds: 1,
      cancellationMode: 'provider_non_cancellable',
    })
    const result = await queue.runOne({
    ...
    expect(result).toMatchObject({
      ...
      operation: { state: 'timed_out', output: { reconciliationRequired: true } },
    })
    store.close()
  }, 3_000)
})
