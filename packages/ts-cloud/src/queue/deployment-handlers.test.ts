...
describe('deployment queue handlers', () => {
  it('resolves scoped deploy and revision rollback commands without prompts', () => {
    ...
    const rollback = queue.enqueue({
      ...
      input: { revision: 'release-42' },
    })
    expect(resolveQueuedDeploymentCommand(target.store, deploy.operation).command).toEqual([
      'deploy',
      '--env',
      'production',
      '--yes',
    ...
    expect(resolveQueuedDeploymentCommand(target.store, rollback.operation).command).toEqual([
      'deploy:rollback',
      'web',
      '--env',
      'production',
  ...
  it('records checkpoints and classifies transient process failures for queue retry', async () => {
    ...
    const queued = queue.enqueue({
      ...
      kind: 'deployment.create',
      maxAttempts: 2,
      retryClasses: ['provider_throttled'],
    })
    const handlers = createDeploymentQueueHandlers({
      store: target.store,
      execute: async (_command, context: QueueExecutionContext) => {
        context.log('provider returned 429', { stream: 'stderr' })
        return { ok: false, exitCode: 1, stderr: 'provider returned 429: rate limit' }
      },
    })
    expect(await queue.runOne(handlers)).toMatchObject({
      handled: true,
      requeued: true,
      operation: { id: queued.operation.id, state: 'queued' },
    })
    expect(
      queue
        .logs(queued.operation.id)
  ...
  it('persists the successful command result through the generic worker contract', async () => {
    ...
    const handlers = createDeploymentQueueHandlers({
      store: target.store,
      execute: async (command) => ({ ok: true, exitCode: 0, command: `cloud ${command.command.join(' ')}` }),
    })
    const result = await queue.runOne(handlers)
    expect(result).toMatchObject({
      terminalState: 'succeeded',
      operation: { id: queued.operation.id, output: { exitCode: 0, environment: 'production', resource: 'web' } },
    })
  ...
  it('runs isolated preview stacks through create, update, and idempotent destroy states', async () => {
    ...
    const policy = previews.createDefinition({
      projectId: target.project.id,
      resourceId: target.resource.id,
      baseEnvironmentId: target.environment.id,
    ...
    const handlers = createDeploymentQueueHandlers({
      ...
      execute: async (command) => {
        ...
        return { ok: true, exitCode: 0, command: `cloud ${command.command.join(' ')}` }
      },
    })
    expect(await queue.runOne(handlers)).toMatchObject({
      terminalState: 'succeeded',
    ...
    expect(previews.getInstance(preview.id)).toMatchObject({
      status: 'active',
      observedState: { exactCommitSha: 'a'.repeat(40), providerStatus: 'deployed' },
    })
    expect(previews.listResources(preview.id)).toMatchObject([
      { providerResourceId: preview.stackName, deletedAt: undefined },
    ])

    const destroy = queue.enqueue({
      projectId: target.project.id,
      environmentId: target.environment.id,
      resourceId: target.resource.id,
    ...
    expect(await queue.runOne(handlers)).toMatchObject({
      terminalState: 'succeeded',
      operation: { id: destroy.operation.id },
    })
  ...
  it('runs Compose applications through durable deploy, scale, and data-safe delete', async () => {
    const target = setup()
    const applications = new ComposeApplicationStore(target.store)
    const application = applications.import(
      `services:\n  web: { image: acme/web:1.0.0 }\n  worker: { image: acme/worker:1.0.0 }\nvolumes: {}\n`,
      { name: 'Stack', projectId: target.project.id, environmentId: target.environment.id },
    ).application
    const lifecycle = new ComposeApplicationService(target.store)
    lifecycle.enqueue(application, 'deploy')
    const commands: string[][] = []
    ...
    lifecycle.enqueue(applications.get(application.id)!, 'scale', { service: 'worker', replicas: 3 })
    await queue.runOne(handlers)
    ...
    lifecycle.enqueue(applications.get(application.id)!, 'delete', { confirmation: application.slug })
    await queue.runOne(handlers)
    expect(commands[2]).not.toContain('--remove-volumes')
    expect(applications.get(application.id)?.status).toBe('deleted')
  })
})
