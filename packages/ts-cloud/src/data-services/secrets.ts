...
export class EncryptedDataSecretStore implements SecretBackend {
  ...
  constructor(
    ...
    if (!encryptionKey) throw new Error('Data-service encryption key is required.')
    this.key = createHash('sha256').update(`ts-cloud:data-services:${encryptionKey}`).digest()
  }
  ...
  async put(reference: string, value: string): Promise<void> {
    ...
    this.controlPlane.database.run(
      'INSERT INTO data_service_secrets (reference,ciphertext,fingerprint,created_at,updated_at) VALUES (?,?,?,?,?) ON CONFLICT(reference) DO UPDATE SET ciphertext=excluded.ciphertext,fingerprint=excluded.fingerprint,updated_at=excluded.updated_at',
      [reference, this.encrypt(value), createHash('sha256').update(value).digest('hex'), now, now],
    )
  }
  async resolve(reference: string): Promise<string> {
    const row = this.controlPlane.database
      .query<{ ciphertext: string }, [string]>('SELECT ciphertext FROM data_service_secrets WHERE reference = ?')
      .get(reference)
    if (!row) throw new Error('Data-service secret was not found.')
    return this.decrypt(row.ciphertext)
  }
  async remove(reference: string): Promise<void> {
    this.controlPlane.database.run('DELETE FROM data_service_secrets WHERE reference = ?', [reference])
  }
}
