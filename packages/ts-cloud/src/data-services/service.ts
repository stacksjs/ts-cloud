...
import type { DataAction, DataEndpoint, DataService, DataServicePlan } from './model'
...
export interface DataServiceAdapter {
  ...
  apply(service: DataService, credential?: string, context?: QueueExecutionContext): Promise<Record<string, JsonValue>>
  action(
    service: DataService,
    action: DataAction,
...
export class DataServiceLifecycle {
  ...
  async create(
    input: Omit<
      DataService,
      'id' | 'capabilities' | 'credentialRef' | 'version' | 'createdAt' | 'updatedAt' | 'status'
    > & { username?: string },
  ): Promise<{
    service: DataService
    ...
    let credential: OneTimeCredential | undefined, credentialRef: string | undefined
    ...
    try {
      ...
      if (credential) this.store.saveCredential(service.id, credential.username, credential.secretRef)
  ...
  plan(service: DataService, action: DataAction, changes: Record<string, JsonValue> = {}): DataServicePlan {
    const capability = service.capabilities.actions[action]
    const major =
      action === 'version' &&
      String(changes.engineVersion ?? '').split('.')[0] !== String(service.engineVersion ?? '').split('.')[0]
    return {
      service,
      action,
      capability,
      ...
      warnings: [
        ...(capability.downtime !== 'none' ? [`Downtime is ${capability.downtime}.`] : []),
  ...
  enqueue(service: DataService, action: DataAction, input: Record<string, JsonValue>, actorId?: string): string {
    const plan = this.plan(service, action, input)
    if (!plan.capability.supported) throw new Error(plan.capability.explanation)
    if (service.origin === 'adopted' && !service.managementEnabled && action !== 'observe')
      throw new Error('Adopted resources remain read-only until management is explicitly enabled.')
    if (plan.preflight.typedConfirmation && input.confirm !== plan.preflight.typedConfirmation)
      throw new Error(`Type ${plan.preflight.typedConfirmation} to confirm ${action}.`)
    if (plan.preflight.retentionChoiceRequired && !['final_backup', 'retain'].includes(String(input.retention ?? '')))
      throw new Error('Deletion requires retention final_backup or retain.')
    if (plan.preflight.compatibilityRequired && (input.compatibilityReviewed !== true || !input.backupId))
      throw new Error('Major version changes require compatibilityReviewed=true and a backupId.')
    if (action === 'restore') {
      const backupId = String(input.backupId ?? ''),
        targetId = String(input.targetId ?? '')
      if (!backupId || !targetId) throw new Error('Restore requires both backupId and targetId.')
      if (targetId === service.placement) throw new Error('Restore targetId must differ from the source placement.')
      if (!/^[a-z0-9][a-z0-9-]{1,62}$/.test(targetId))
        throw new Error('Restore targetId must be 2-63 lowercase letters, numbers, or dashes.')
    }
  ...
  async rotate(
    ...
    if (!current) throw new Error('Data service credential metadata was not found.')
    const value = password()
    const nextRef = `${current.secretRef}/v${current.version + 1}`
    await this.secrets.put(nextRef, value)
    try {
      await adapter.action(service, 'rotate', { username: current.username, secretRef: nextRef }, value, context)
...
export function createDataServiceQueueHandlers(input: {
  ...
  return {
    'data_service.action': async (context): Promise<Record<string, JsonValue>> => {
      ...
      if (!adapter || adapter.provider !== service.provider || !adapter.engines.includes(service.engine))
        throw new Error(`No ${service.provider}/${service.engine} adapter is configured.`)
      if (action === 'rotate') {
      ...
      const credential = service.credentialRef ? await input.secrets.resolve(service.credentialRef) : undefined
      try {
        const observed =
          action === 'observe'
        ...
        input.store.update(service.id, latest.version, {
          ...
          status: action === 'delete' ? (body.retention === 'retain' ? 'retained' : 'deleting') : 'available',
        })
        ...
        if (/temporar|timeout|unavailable|throttl/i.test(error instanceof Error ? error.message : String(error)))
