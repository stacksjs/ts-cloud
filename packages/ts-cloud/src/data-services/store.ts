...
export class DataServiceStore {
  ...
  create(input: Omit<DataService, 'id' | 'capabilities' | 'version' | 'createdAt' | 'updatedAt'>): DataService {
    if (!/^[a-z0-9][a-z0-9-]{1,62}$/.test(input.name))
      throw new Error('Data service names must be 2-63 lowercase letters, numbers, or dashes.')
    if (input.publicExposure && (!input.allowedCidrs.length || input.allowedCidrs.includes('0.0.0.0/0')))
      throw new Error('Public exposure requires explicit narrow CIDRs; 0.0.0.0/0 is refused.')
  ...
  get(id: string): DataService | undefined {
    const row = this.controlPlane.database.query<Row, [string]>('SELECT * FROM data_services WHERE id=?').get(id)
  ...
  update(
    ...
    if (current.version !== expectedVersion) throw new Error('Data service changed; refresh before retrying.')
    const next = { ...current, ...patch }
    if (next.publicExposure && (!next.allowedCidrs.length || next.allowedCidrs.includes('0.0.0.0/0')))
      throw new Error('Public exposure requires explicit narrow CIDRs; 0.0.0.0/0 is refused.')
    ...
    if (result.changes !== 1) throw new Error('Data service changed; refresh before retrying.')
    return this.get(id)!
  }
  credential(serviceId: string): DataCredential | undefined {
    const row = this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM data_service_credentials WHERE service_id=? ORDER BY version DESC LIMIT 1')
      .get(serviceId)
    return row
      ? {
          id: String(row.id),
  ...
  saveCredential(serviceId: string, username: string, secretRef: string): DataCredential {
    ...
    this.controlPlane.database.run(
      ...
      [id, serviceId, username, secretRef, current?.version ?? 1, current?.createdAt ?? now, current ? now : null],
    )
  ...
  addDependency(input: Omit<DataDependency, 'createdAt'>): DataDependency {
    ...
    this.controlPlane.database.run(
      ...
      [input.serviceId, input.resourceId, input.secretRef, input.requiresRedeploy ? 1 : 0, now],
    )
    return { ...input, createdAt: now }
  }
  dependencies(serviceId: string): DataDependency[] {
    return this.controlPlane.database
      .query<Row, [string]>('SELECT * FROM data_service_dependencies WHERE service_id=? ORDER BY resource_id')
      .all(serviceId)
      .map((row) => ({
        serviceId: String(row.service_id),
  ...
  removeDependency(serviceId: string, resourceId: string): boolean {
    return (
      this.controlPlane.database.run('DELETE FROM data_service_dependencies WHERE service_id=? AND resource_id=?', [
        serviceId,
        resourceId,
      ]).changes === 1
    )
