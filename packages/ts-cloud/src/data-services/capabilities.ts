import type { DataAction, DataActionCapability, DataEngine, DataProvider, DataServiceCapabilities } from './model'
...
export function dataServiceCapabilities(engine: DataEngine, provider: DataProvider): DataServiceCapabilities {
  const managed = ['aws_rds', 'aws_aurora', 'aws_elasticache'].includes(provider),
  ...
  actions.create =
    provider === 'external'
      ...
      : yes(
          managed ? 'Provision through the provider API.' : 'Provision through the owning runtime driver.',
          'possible',
        )
  actions.observe =
    provider === 'external'
  ...
  actions.connect = yes('Connection guidance uses secret references and never returns a password by default.')
  actions.backup =
    sql && (managed || runtime)
      ? yes(managed ? 'Create a provider snapshot.' : 'Create a consistent engine-native dump.', 'possible')
  ...
  actions.restart =
    provider === 'external'
      ...
      : yes(managed ? 'Provider restart/failover semantics apply.' : 'Restart the owning service unit.', 'required')
  actions.resize =
    provider === 'aws_elasticache'
      ? no('ElastiCache resizing requires a replication-group migration runner.')
      : managed
        ? yes('Provider plan/storage modification may enter a maintenance window.', 'possible')
        : runtime
  ...
  actions.rotate =
    provider === 'aws_elasticache' || provider === 'external' || (runtime && !sql && !cache)
      ? no('Credential rotation is not available for this provider/engine path.')
      : yes('Rotate through the secret backend, update the engine, then flag dependent releases.')
  actions.expose = runtime
    ? no('Direct container publishing is refused; use a reviewed firewall and proxy policy.')
    : provider === 'aws_rds'
      ? yes('Exposure is reconciled through security groups; public access defaults off.')
  ...
  actions.slow_queries =
    runtime && sql
      ? yes('Read bounded runtime logs after engine slow-query logging is enabled.')
      : no('A slow-query log runner is not configured for this provider.')
  actions.users =
    runtime && sql
      ? yes('Logical users can be listed; creation requires a referenced secondary credential.')
      : no('User management requires an engine connection runner.')
  actions.databases =
    runtime && sql
      ? yes('Logical databases can be listed, created, and confirmation-gated for deletion.')
      : no('Logical database management requires an engine connection runner.')
  return {
    actions,
    endpointTypes: managed ? ['internal', 'external', 'tunnel'] : ['internal', 'tunnel'],
    metrics: cache
      ? ['connections', 'memory', 'evictions', 'hit_rate', 'latency']
      : ['connections', 'cpu', 'storage', 'read_latency', 'write_latency', 'iops'],
  }
}
