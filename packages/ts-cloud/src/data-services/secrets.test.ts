...
describe('encrypted data-service secret store', () => {
  it('persists only authenticated ciphertext and removes references', async () => {
    const controlPlane = new ControlPlaneStore({ path: ':memory:' }),
      secrets = new EncryptedDataSecretStore(controlPlane, 'dashboard-encryption-key'),
    ...
    expect(Buffer.from(controlPlane.database.serialize()).toString()).not.toContain('not-in-the-database')
    await expect(new EncryptedDataSecretStore(controlPlane, 'wrong-key').resolve(reference)).rejects.toThrow(
