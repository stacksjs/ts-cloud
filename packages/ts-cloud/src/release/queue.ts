...
export function createReleaseQueueHandlers(input: { store: ControlPlaneStore, resolveDriver: ReleaseDriverResolver, resolveHealthGate?: (name: string, release: ReleaseRecord) => Promise<{ healthy: boolean, evidence?: JsonValue, message?: string }> }): Record<string, QueueOperationHandler> {
  ...
  const handler: QueueOperationHandler = async (context: QueueExecutionContext) => {
    ...
    if (release.projectId !== context.operation.projectId || release.environmentId !== context.operation.environmentId || release.resourceId !== context.operation.resourceId) throw new Error('Queued release does not match the operation scope')
    const artifact = releases.releases.getArtifact(release.artifactId)
    if (!artifact || artifact.digest !== releases.releases.getArtifact(release.artifactId)?.digest) throw new Error('Immutable release artifact is unavailable')
    const previousId = typeof operationInput.targetReleaseId === 'string' && operationInput.targetReleaseId ? operationInput.targetReleaseId : release.previousReleaseId
    const previous = previousId ? releases.releases.get(previousId) : undefined
    if (previous && (previous.resourceId !== release.resourceId || previous.artifactId === release.artifactId && previous.id === release.id)) throw new Error('Rollback target does not match the release resource')
    ...
    context.log(`Artifact ${artifact.digest}; source ${release.sourceSha ?? 'not recorded'}; config ${release.configHash}.`, { stream: 'system' })
    context.throwIfCancellationRequested()
    let result
    try {
      result = context.operation.kind === 'release.rollback'
      ...
      if (context.operation.kind === 'release.activate' && releases.releases.get(release.id)?.status === 'activating') releases.completeHealthGate(release.id, { healthy: false, operationId: context.operation.id, message: error instanceof Error ? error.message : String(error) })
      throw error
    }
    for (const step of result.transitions) {
      releases.releases.progress(release.id, { message: `${context.operation.kind === 'release.rollback' ? 'Rollback' : 'Activation'} traffic at ${step.trafficPercent}%.`, trafficPercent: step.trafficPercent, health: step.observed, operationId: context.operation.id })
      context.log(`Traffic ${step.trafficPercent}%: ${step.healthy ? 'healthy' : 'unhealthy'}.`, { stream: step.healthy ? 'step' : 'stderr', step: 'health' })
    }
    ...
    if (context.operation.kind === 'release.rollback') {
      ...
      if (failed.status !== 'rolled_back') releases.releases.transition(failed.id, 'rolled_back', { message: `Traffic restored through ${driver.name}.`, operationId: context.operation.id, trafficPercent: 0, health: { resourceVersions: result.resourceVersions } })
      if (previous && previous.status !== 'active') {
        const activating = previous.status === 'activating' ? previous : releases.releases.transition(previous.id, 'activating', { message: 'Rollback target restoring traffic.', operationId: context.operation.id, trafficPercent: 0 })
        releases.releases.transition(activating.id, 'active', { message: 'Previous immutable release restored.', operationId: context.operation.id, trafficPercent: 100, health: { resourceVersions: result.resourceVersions } })
      }
    }
    else releases.completeHealthGate(release.id, { healthy: true, operationId: context.operation.id, health: { providerVersion: result.providerVersion ?? null, resourceVersions: result.resourceVersions }, message: 'Provider activation and health gates completed.' })
    context.checkpoint('finalize', 'Persisted the terminal release state.')
    return { releaseId: release.id, artifactDigest: artifact.digest, provider: driver.name, providerVersion: result.providerVersion ?? null, resourceVersions: result.resourceVersions, status: releases.releases.get(release.id)!.status }
  }
