...
describe('dashboard jobs integration', () => {
  it('previews, creates, operates, and audits schedules without executing previews', async () => {
    ...
    const preview = await (await call('/api/jobs/preview', { method: 'POST', headers, body: JSON.stringify({ expression: '30 2 * * *', timezone: 'America/Los_Angeles', from: '2026-03-08T08:00:00Z' }) })).json() as any
    ...
    const unsupportedRate = await call('/api/jobs', { method: 'POST', headers, body: JSON.stringify({ name: 'Server rate', provider: 'server', expression: 'rate(5 minutes)', operationId: 'scheduler:run:web' }) })
    ...
    expect(created).toMatchObject({ ok: true, job: { provider: 'server', overlapPolicy: 'replace', missedRunPolicy: 'catch_up' } })

    const runResponse = await call('/api/jobs/action', { method: 'POST', headers, body: JSON.stringify({ id: created.job.id, action: 'run' }) })
    ...
    const history = await (await call(`/api/jobs/history?jobId=${created.job.id}`, { headers: { cookie: session } })).json() as any
    expect(history.executions[0]).toMatchObject({ id: run.execution.id, logs: [{ stream: 'system', message: 'Queued for durable execution.' }] })

    const unconfirmed = await call('/api/jobs/action', { method: 'POST', headers, body: JSON.stringify({ id: created.job.id, action: 'disable' }) })
    expect(unconfirmed.status).toBe(409)
    const disabled = await (await call('/api/jobs/action', { method: 'POST', headers, body: JSON.stringify({ id: created.job.id, action: 'disable', confirm: 'disable' }) })).json() as any
    expect(disabled.job.enabled).toBe(false)

    const configDelete = await call('/api/jobs/action', { method: 'POST', headers, body: JSON.stringify({ id: initial.jobs[0].id, action: 'delete', confirm: 'delete' }) })
    expect(configDelete.status).toBe(409)

    const project = store.getProjectBySlug('acme')!
    expect(store.listEvents({ projectId: project.id }).map(event => event.type)).toEqual(expect.arrayContaining(['job.created', 'job.run_queued', 'job.disabled']))
  }, 10_000)
})
