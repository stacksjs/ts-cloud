...
describe('dashboard application onboarding integration', () => {
  it('detects, resumes, validates, and explicitly applies the same secret-free plan', async () => {
    ...
    const initial = (await (
      await call('/api/onboarding?env=production', { headers: { cookie: session } })
    ).json()) as any
    const environment = initial.environments.find((item: any) => item.slug === 'production')
    const detection = (await (
      await call('/api/onboarding/detect?env=production', {
        method: 'POST',
        headers,
        body: JSON.stringify({
    ...
    const draftInput = {
      ...
      environment: { DATABASE_PASSWORD: { secretRef: 'DATABASE_PASSWORD' } },
      requiredSecretNames: ['DATABASE_PASSWORD'],
      domain: { hostname: 'web.example.com', tls: true },
    }
    const createdResponse = await call('/api/onboarding/drafts?env=production', {
      method: 'POST',
      headers,
      body: JSON.stringify({ draft: draftInput, step: 'environment' }),
    })
    expect(createdResponse.status).toBe(201)
    const created = (await createdResponse.json()) as any
    expect(created.draft).toMatchObject({ status: 'draft', step: 'environment', suppliedSecretNames: [] })
    const planned = (await (
      await call('/api/onboarding/plan?env=production', {
        method: 'POST',
        headers,
    ...
    expect(planned.plan).toMatchObject({
      valid: false,
      missingSecrets: ['DATABASE_PASSWORD'],
      capabilityRequirements: expect.any(Array),
      costDrivers: expect.any(Array),
    })
    const updated = (await (
      await call('/api/onboarding/drafts?env=production', {
        method: 'PATCH',
        headers,
        body: JSON.stringify({
          id: created.draft.id,
    ...
    const rejected = await call('/api/onboarding/apply?env=production', {
      method: 'POST',
      headers,
      body: JSON.stringify({ id: updated.draft.id, version: 2, confirmEnvironment: 'staging' }),
    })
    expect(rejected.status).toBe(422)
    expect(
      ((await (await call('/api/onboarding?env=production', { headers: { cookie: session } })).json()) as any)
        .resources,
    ).toHaveLength(0)
    const appliedResponse = await call('/api/onboarding/apply?env=production', {
      method: 'POST',
      headers,
      body: JSON.stringify({ id: updated.draft.id, version: 2, confirmEnvironment: 'production' }),
    })
    ...
    expect(applied).toMatchObject({
      ...
      plan: { valid: true, serializedManifest: expect.any(String) },
    })
    ...
    expect(queue.operations).toMatchObject([
      { operation: { id: applied.operation.id, state: 'queued' }, job: { maxAttempts: 3 }, target: { slug: 'web' } },
    ])
    const logs = (await (
      await call(`/api/queue/logs?id=${applied.operation.id}`, { headers: { cookie: session } })
    ).json()) as any
    expect(logs.entries).toMatchObject([{ stream: 'system', message: 'Queued for durable execution.' }])
    const cancelled = (await (
      await call('/api/queue/cancel', { method: 'POST', headers, body: JSON.stringify({ id: applied.operation.id }) })
    ).json()) as any
    expect(cancelled.operation).toMatchObject({ id: applied.operation.id, state: 'cancelled' })
    const retried = (await (
      await call('/api/queue/retry', {
        method: 'POST',
        headers,
        body: JSON.stringify({ id: applied.operation.id, errorClass: 'provider_unavailable' }),
      })
    ).json()) as any
    expect(retried.operation).toMatchObject({ id: applied.operation.id, state: 'queued' })

    const artifactResponse = await call('/api/onboarding/artifacts?env=production', {
      method: 'POST',
      headers: {
        origin: base,
        cookie: session,
        'content-type': 'application/octet-stream',
        'x-artifact-filename': 'site.zip',
      },
      body: storedZip('dist/index.html'),
    })
    expect(artifactResponse.status).toBe(201)
    expect(await artifactResponse.json()).toMatchObject({ artifact: { format: 'zip', entryCount: 1 } })
    const registryResponse = await call('/api/onboarding/registries?env=production', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        provider: 'generic',
        ...
        username: 'robot',
        password: 'registry-runtime-secret',
      }),
    })
    expect(registryResponse.status).toBe(201)
    const registry = (await registryResponse.json()) as any
    expect(registry.registry).toMatchObject({ credentialConfigured: true })
    expect(JSON.stringify(registry)).not.toContain('registry-runtime-secret')
    const preview = (await (
      await call('/api/onboarding/registries?env=production', {
        method: 'DELETE',
        headers,
        body: JSON.stringify({ id: registry.registry.id, preview: true }),
      })
    ).json()) as any
    expect(preview).toMatchObject({ preview: true, affectedDrafts: [] })
