...
function validateOptions(options: ExistingStaticFullStackOptions): {
  stackName: string
  region: string
  pathPattern: string
  originId: string
  ...
  if (
    ...
    throw new Error('Expected distribution alias is invalid')
  ...
  if (!/^\/[A-Za-z0-9_.*?~!$&'()+,;=:@%/-]+\*$/.test(pathPattern) || pathPattern === '/*')
    throw new Error('Backend path must be a non-default wildcard pattern')
  ...
  if (options.originVerifySecret && options.originVerifySecret.length < 24)
    throw new Error('Origin verification secret must contain at least 24 characters')
  if (!!options.originDomain !== !!options.certificateArn)
    throw new Error('A custom origin domain and matching ACM certificate ARN must be provided together')
  return { stackName, region: options.region || 'us-east-1', pathPattern, originId, digest }
}

export function createExistingStaticFullStackDependencies(
  options: Pick<ExistingStaticFullStackOptions, 'region' | 'profile'>,
): ExistingStaticFullStackDependencies {
...
export function generateExistingStaticFullStackTemplate(options: ExistingStaticFullStackOptions): Record<string, any> {
  ...
  template.Metadata = {
    ManagedBy: 'ts-cloud',
    ExistingDistributionId: options.distributionId,
    ExistingDistributionAlias: options.expectedAlias,
    BackendPathPattern: resolved.pathPattern,
    ContainerDigest: resolved.digest,
  }
  return template
}

async function waitForHealthyOrigin(
  dependencies: ExistingStaticFullStackDependencies,
  url: string,
  secret?: string,
): Promise<{ url: string; status: number; latencyMs: number; body: unknown }> {
  ...
  for (let attempt = 0; attempt < 30; attempt++) {
    ...
    try {
      const response = await dependencies.fetch(url, {
        redirect: 'manual',
        headers: secret ? { 'x-origin-verify': secret } : undefined,
      })
  ...
  throw new Error(
    `Backend origin did not become healthy: ${lastError instanceof Error ? lastError.message : String(lastError)}`,
  )
}

export async function deployExistingStaticFullStack(
  options: ExistingStaticFullStackOptions,
  injected?: ExistingStaticFullStackDependencies,
): Promise<ExistingStaticFullStackPlan> {
  const resolved = validateOptions(options)
  const dependencies =
    injected || createExistingStaticFullStackDependencies({ region: resolved.region, profile: options.profile })
  const [identity, distribution] = await Promise.all([
  ...
  if (!aliasesOf(distribution).includes(options.expectedAlias))
  ...
  try {
    existingStack = (await dependencies.cloudformation.describeStacks({ stackName: resolved.stackName })).Stacks[0]
  } catch (error) {
  ...
  if (Buffer.byteLength(templateBody) > 51_200)
    throw new Error('Generated template exceeds the direct CloudFormation template body limit')
  const resourceTypes = Object.values(template.Resources || {}).reduce<Record<string, number>>(
    (counts, resource: any) => {
  ...
  const base: ExistingStaticFullStackPlan = {
    ...
    stack: {
      name: resolved.stackName,
      existed: !!existingStack,
      status: existingStack?.StackStatus,
      templateSha256: createHash('sha256').update(templateBody).digest('hex'),
  ...
  if (options.confirm !== confirmation)
    throw new Error(`Pass exact confirmation token ${confirmation} to provision the backend and change live routing`)
  if (existingStack) {
    try {
      await dependencies.cloudformation.updateStack({
        stackName: resolved.stackName,
        templateBody,
        capabilities: ['CAPABILITY_NAMED_IAM'],
        tags: [
          { Key: 'ManagedBy', Value: 'ts-cloud' },
          { Key: 'Application', Value: options.slug },
        ],
      })
      await dependencies.cloudformation.waitForStack(resolved.stackName, 'stack-update-complete')
    } catch (error) {
      if (!String((error as Error).message).includes('No updates are to be performed')) throw error
    }
  } else {
    await dependencies.cloudformation.createStack({
      stackName: resolved.stackName,
      templateBody,
      capabilities: ['CAPABILITY_NAMED_IAM'],
      onFailure: 'ROLLBACK',
  ...
  if (options.originDomain && options.originDomain !== albDomain) {
    if (!dependencies.dns)
      throw new Error(`External DNS provider is required to map ${options.originDomain} to ${albDomain}`)
    const result = await dependencies.dns.upsertRecord(options.originDomain, {
      name: options.originDomain,
      type: 'CNAME',
      content: albDomain,
      ttl: 300,
    })
  ...
  const health = options.skipHealthCheck
    ? undefined
    : await waitForHealthyOrigin(
        dependencies,
        `${protocolPolicy === 'https-only' ? 'https' : 'http'}://${originHost}/api/health`,
        options.originVerifySecret,
      )
  await dependencies.cloudfront.upsertExistingDistributionOrigin(options.distributionId, {
    ...
    pathPattern: resolved.pathPattern,
    protocolPolicy,
    customHeaders: options.originVerifySecret ? { 'X-Origin-Verify': options.originVerifySecret } : undefined,
  })
  return { ...base, outputs, health, cloudFrontApplied: true, dnsApplied, applied: true }
}

export function estimateExistingStaticFullStackMonthlyCost(
  options: { desiredCount?: number; multiAzDatabase?: boolean } = {},
): { monthlyUsd: number; components: Record<string, number>; assumptions: string[] } {
  const hours = 730
  const components = {
  ...
  return {
    ...
    assumptions: [
      'us-east-1 public on-demand rate snapshot, July 2026',
      'one 0.25 vCPU/0.5 GB Linux Fargate task unless desiredCount is changed',
      'one ALB before LCU charges and one NAT gateway before data processing',
      'single-AZ db.t4g.micro PostgreSQL by default and two cache.t4g.micro Redis nodes',
      'excludes storage, backups, CloudFront, data transfer, public IPv4, logs, email, queue requests, taxes, and free-tier credits',
    ],
  }
}
