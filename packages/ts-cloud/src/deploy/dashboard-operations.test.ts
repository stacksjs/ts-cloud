...
describe('buildDashboardOperations', () => {
  ...
  it('omits backup ops when backups are disabled, and restore without a db', () => {
    const noBackups = buildDashboardOperations(config({ infrastructure: { compute: { webServer: 'rpx' } } } as any), data)
    expect(noBackups.some(o => o.group === 'backup')).toBe(false)

    const noDb = buildDashboardOperations(config({ infrastructure: { compute: { webServer: 'rpx', backups: { enabled: true } } } } as any), data)
