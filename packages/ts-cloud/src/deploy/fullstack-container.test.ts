...
function options() {
  return {
    ...
    originDomain: 'origin-api.example.com',
    certificateArn: 'arn:aws:acm:us-east-1:923076644019:certificate/one',
  }
}
    ...
    dependencies: {
      ...
      cloudformation: {
        describeStacks: async () => {
          if (!created)
            throw Object.assign(new Error('Stack with id example-backend does not exist'), { code: 'ValidationError' })
          return {
            Stacks: [
              {
                StackId: 'stack',
                StackName: 'example-backend',
                StackStatus: 'CREATE_COMPLETE',
                CreationTime: 'now',
                Outputs: [
                  { OutputKey: 'AppLoadBalancerDnsName', OutputValue: 'example-alb.us-east-1.elb.amazonaws.com' },
                ],
              },
            ],
          }
        ...
        waitForStack: async () => {
          calls.push('wait-stack')
        },
      },
      dns: {
        upsertRecord: async (_domain, record) => {
          calls.push(`dns:${record.content}`)
          return { success: true }
        },
      },
      fetch: (async () => {
...
describe('existing static full-stack deployment', () => {
  ...
  it('refuses a mutable container tag', async () => {
    const value = fake()
    await expect(
      deployExistingStaticFullStack({ ...options(), imageUri: 'example:latest' }, value.dependencies),
    ).rejects.toThrow('immutable ECR digest URI')
  ...
  it('provisions, maps external DNS, checks health, then changes CloudFront', async () => {
    const value = fake()
    const plan = await deployExistingStaticFullStack(
      { ...options(), apply: true, confirm: 'E123456789AB:/api/*:example-backend' },
      value.dependencies,
    )
    ...
    expect(value.calls).toEqual([
      'create-stack',
      'wait-stack',
      'dns:example-alb.us-east-1.elb.amazonaws.com',
      'health',
      'cloudfront:origin-api.example.com',
    ])
  })

  it('estimates every always-on baseline component explicitly', () => {
    const estimate = estimateExistingStaticFullStackMonthlyCost()
    expect(Object.keys(estimate.components)).toEqual([
      'fargate',
