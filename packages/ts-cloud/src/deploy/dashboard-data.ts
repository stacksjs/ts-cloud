...
async function lambdaMetric(
  cw: CloudWatchClient,
  fn: string,
  name: string,
  kind: 'Sum' | 'Average' | 'Maximum',
  start: Date,
  end: Date,
): Promise<number> {
  const pts = await cw
...
export async function resolveDashboardData(config: CloudConfig, environment: EnvironmentType): Promise<DashboardData> {
  ...
  out.scheduler = {
    enabled: info.scheduler !== 'off',
    expression: info.scheduler === 'sub-minute' ? 'rate(1 minute) · sub-minute' : 'rate(1 minute)',
    lastRun: '—',
  }
  ...
  for (const f of info.functions) {
    const [invocations, errors, throttles, durAvg, cfg, p50, p95, p99, spark] = await Promise.all([
      ...
      cw
        .getMetricSeries({
          ...
          StartTime: new Date(end.getTime() - 12 * 3_600_000),
          EndTime: end,
          Period: 3600,
          Stat: 'Sum',
        })
    ...
    try {
      const { events = [] } = await logs.filterLogEvents({
        logGroupName: `/aws/lambda/${f.name}`,
      ...
      for (const e of events) {
        ...
        if (msg && !/^(?:START|END|REPORT|INIT_START)/.test(msg)) {
          serverlessLogs.push({
            ...
            level: /error|exception|fatal|panic|traceback/i.test(msg)
              ? 'error'
              : /warn|warning|timeout|retr(?:y|ies)|throttl/i.test(msg)
                ? 'warn'
                : 'info',
          })
        }
      ...
      recent = events
        .filter((e) => !/^(?:START|END|REPORT|INIT_START)/.test((e.message ?? '').trim()))
        .slice(-6)
        .map((e) => ({
          ts: new Date(e.timestamp ?? 0).toISOString().slice(11, 19),
          msg: (e.message ?? '').trim().slice(0, 120),
          err: /error|exception|fatal/i.test(e.message ?? ''),
        }))
    } catch {
      /* logs optional */
    }
    ...
    fnsDetail.push({
      ...
      name: f.name,
      runtime: cfg?.Configuration?.Runtime ?? 'provided.al2023',
      arch: (cfg?.Configuration as any)?.Architectures?.[0] ?? 'x86_64',
      memory,
      timeout: cfg?.Configuration?.Timeout ?? 0,
      ephemeral: (cfg?.Configuration as any)?.EphemeralStorage?.Size ?? 512,
      version: f.version,
  ...
  out.serverlessLogs = serverlessLogs
  ...
  const concurrency = Math.round(
    await lambdaMetric(cw, info.functions[0]?.name ?? '', 'ConcurrentExecutions', 'Maximum', start, end),
  )
  out.metrics = {
    invocations: Math.round(totalInvocations),
    errorRatePct,
    p95Ms: Math.round(maxDur),
    coldStartPct: undefined,
    concurrency,
  ...
  try {
    ...
    for (const q of resolvedQueues) {
      ...
      const processed = Math.round(
        await cwMetric(
          cw,
          'AWS/SQS',
          'NumberOfMessagesDeleted',
          [{ Name: 'QueueName', Value: q.name }],
          'Sum',
          start,
          end,
        ),
      ...
      detail.push({
        ...
        processed,
        oldestSec: 0,
        concurrency: q.concurrency ?? app?.queueConcurrency ?? '—',
        tries: app?.queueTries ?? 3,
        visTimeout: (app?.queueTimeout ?? 120) * 6,
        dlq: dlqDepth,
      })
    }
  ...
  try {
    ...
    if (app?.database?.connection === 'aurora-serverless') {
      ...
      out.aurora = {
        id: clusterId,
        engine: 'aurora-mysql',
        minAcu: app?.database?.minCapacity ?? 0.5,
        maxAcu: app?.database?.maxCapacity ?? 4,
        currentAcu: acu ? Number(acu.toFixed(1)) : (app?.database?.minCapacity ?? 0.5),
        database: 'app',
        connections: Math.round(conns),
        status: outputs.DbEndpoint ? 'available' : 'creating',
      }
      if (app?.rdsProxy)
        out.proxy = {
          name: `${info.slug}-${environment}-proxy`,
          endpoint: outputs.DbProxyEndpoint ?? '—',
          pooledConns: Math.round(conns),
          status: 'available',
        }
    }
    if (app?.cache?.driver === 'elasticache') {
      ...
      out.redis = {
        ...
        engine: 'redis',
        hitRate,
        status: outputs.CacheEndpoint ? 'available' : 'creating',
      }
    }
    if (app?.efs) {
      let sizeMb = 0
    ...
    out.assetsInfo = outputs.AssetsCdnDomain
      ? {
          bucket: outputs.AssetsBucketName ?? `${info.slug}-${environment}-assets`,
          cdn: outputs.AssetsCdnDomain,
          customDomain: app?.assetDomain ?? '—',
  ...
  if (out.assetsInfo) {
    try {
      ...
      out.assetsRecent = objs.slice(0, 8).map((o) => ({
        path: `/${o.Key}`,
        size: `${Math.max(1, Math.round((o.Size ?? 0) / 1024))} KB`,
        type: o.Key.split('.').pop() ?? '',
      }))
    } catch {
      /* s3 optional */
    }
  ...
  try {
    if (app?.secrets) {
      const list = Array.isArray(app.secrets)
        ? app.secrets.map((s) => ({
            key: s
              .split('/')
              .pop()!
        ...
        : Object.entries(app.secrets).map(([k, v]) => ({ key: k, source: String(v) }))
  ...
  try {
    ...
    const hist = await s3
      .getObjectJson<{ records: any[] }>(bucket, `deployments/${info.slug}/${environment}/history.json`)
      .catch(() => null)
    if (hist?.records?.length) {
      out.deploymentsDetail = hist.records
        .slice(-12)
        .reverse()
        .map((r: any) => ({
          sha: (r.sha ?? '').slice(0, 7),
  ...
  out.metricsTotals = {
    invocations: out.metrics.invocations,
    errors: Math.round(totalErrors),
    errorRatePct,
    p95Ms: out.metrics.p95Ms,
    throttles: fnsDetail.reduce((n, f) => n + f.throttles, 0),
    coldStartPct: undefined,
    estCostUsd: undefined,
    concurrency,
  }
  out.metricsPerFn = fnsDetail.map((f) => ({
    key: f.key,
  ...
  try {
    out.invSpark = await cw.getMetricSeries({
      Namespace: 'AWS/Lambda',
      MetricName: 'Invocations',
      Dimensions: [{ Name: 'FunctionName', Value: info.functions.find((f) => f.mode === 'http')?.name ?? '' }],
      StartTime: new Date(end.getTime() - 24 * 3_600_000),
      EndTime: end,
      Period: 3600,
      Stat: 'Sum',
    })
  ...
  try {
    ...
    const [thisMonth, lastMonth] = await Promise.all([
      ...
      ce
        .getCostByService({ start: isoDate(prevStart), end: isoDate(monthStart), granularity: 'MONTHLY' })
        .catch(() => []),
    ])
    if (thisMonth.length) {
      ...
      out.costSummary = {
        monthToDateUsd: Number(mtd.toFixed(2)),
        projectedUsd: Number(((mtd / dayOfMonth) * daysInMonth).toFixed(2)),
        lastMonthUsd: Number(lastTotal.toFixed(2)),
        dailyAvgUsd: Number((mtd / dayOfMonth).toFixed(2)),
      }
      out.costServices = thisMonth
        .filter((s) => s.amount > 0)
        .sort((a, b) => b.amount - a.amount)
        .slice(0, 12)
        .map((s) => ({ name: s.service, usd: Number(s.amount.toFixed(2)), note: '' }))
      ...
      out.costPerFn = fnsDetail.map((f) => ({
        key: f.key,
        requests: f.invocations,
        gbSec: Math.round(gbSec(f)),
        usd: Number((lambdaCost * (gbSec(f) / totalGbSec)).toFixed(2)),
      }))
    }
  } catch {
    /* cost optional */
  ...
  if (app?.firewall) {
    try {
      ...
      out.waf = {
        enabled: true,
        scope: 'REGIONAL',
        acl: `${info.slug}-${environment}-waf`,
        allowed24h: Math.round(allowed.reduce((n, p) => n + (p.Sum ?? 0), 0)),
        blocked24h: Math.round(blocked.reduce((n, p) => n + (p.Sum ?? 0), 0)),
      }
      ...
      try {
        ...
        if (acl?.Name && acl.Id) {
          ...
          if (rules.length)
            out.wafRules = rules.map((r) => ({
              name: r.Name ?? '—',
              detail: `priority ${r.Priority ?? 0}`,
              action: r.Action ?? 'count',
              blocked24h: 0,
            }))
        }
      } catch {
  ...
  if (out.aurora)
    dataServices.push({
      ...
      detail: `${out.aurora.id} · ${out.aurora.minAcu}-${out.aurora.maxAcu} ACU`,
      status: out.aurora.status,
    })
  if (out.proxy) dataServices.push({ name: 'RDS Proxy', detail: out.proxy.name, status: out.proxy.status })
  if (out.redis)
    dataServices.push({
      name: `ElastiCache (${out.redis.engine})`,
      detail: `${out.redis.node} · ${out.redis.hitRate}% hit rate`,
      status: out.redis.status,
    })
  if (out.efs)
    dataServices.push({ name: 'EFS', detail: `${out.efs.mount} · ${out.efs.sizeMb} MB`, status: out.efs.status })
  if (dataServices.length) out.dataServices = dataServices
    ...
    try {
      ...
      for (const line of (out2 || '').split('\n')) {
        ...
        if (m && /[*\d]/.test(m[1]) && m[1].trim().split(/\s+/).length === 5)
          tasks.push({
            command: m[2].trim().replace(/^php artisan /, ''),
            cron: m[1].trim(),
            desc: '',
            lastRun: '—',
            lastStatus: 'ok',
