...
describe('dashboard security posture integration', () => {
  it('blocks a critical production finding, honors an expiring waiver, and allows remediation', async () => {
    ...
    expect(
      (await dashboardFetch('/security?env=production', { headers: { ...headers, accept: 'text/html' } })).status,
    ).toBe(200)
    ...
    const waiver = await dashboardFetch('/api/security/waivers?env=production', {
      ...
      body: JSON.stringify({
        ...
        policyId: posture.policies[0].id,
        reason: 'Emergency remediation window with an assigned rotation owner.',
