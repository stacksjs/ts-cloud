...
import { collectPhpAppEntries, generateAppImageDockerfile, packageServerlessApp, phpRuntimeLayerAssets, runPhpBuildHooks, sha256 } from '@ts-cloud/core'
...
function writeEntries(
  baseDir: string,
  entries: Array<{ name: string; data: Buffer | Uint8Array | string; mode?: number }>,
): void {
...
export async function buildAndPushServerlessImage(opts: BuildImageOptions): Promise<BuiltImage> {
  ...
  try {
    ...
    if (isPhp) {
      ...
      writeEntries(
        join(stage, 'runtime'),
        phpRuntimeLayerAssets().map((a) => ({ name: a.path, data: a.contents, mode: a.mode })),
      )
    ...
    if (!repoUri) throw new Error(`could not resolve ECR repository URI for ${repository}`)
    ...
    execFileSync(
      'docker',
      ['build', '--platform', platform, '--provenance=false', '--sbom=false', '-t', imageUri, stage],
      { stdio: 'inherit' },
    )
    ...
    if (!data?.authorizationToken || !data.proxyEndpoint) throw new Error('failed to get an ECR authorization token')
    ...
    writeFileSync(
      ...
      JSON.stringify({ auths: { [registry]: { auth: data.authorizationToken } } }),
    )
