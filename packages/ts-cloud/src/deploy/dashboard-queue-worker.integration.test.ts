...
describe('dashboard durable queue worker integration', () => {
  it('claims queued deployment work, streams logs, and persists success', async () => {
    const root = mkdtempSync(join(tmpdir(), 'ts-cloud-dashboard-queue-'))
    roots.push(root)
    ...
    while (
      !['succeeded', 'failed'].includes(queue.view(queued.operation.id)?.operation.state ?? '') &&
      Date.now() < deadline
    )
      await Bun.sleep(25)

    expect(queue.view(queued.operation.id)).toMatchObject({
      operation: { state: 'succeeded', output: { exitCode: 0, environment: 'production', resource: 'web' } },
      job: { currentStep: 'finalize' },
    })
