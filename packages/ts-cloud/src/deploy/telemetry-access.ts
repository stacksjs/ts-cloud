export interface TelemetryResourceReference { id: string, slug: string }

export function resolveTelemetryResourceIds(resources: TelemetryResourceReference[], environmentWide: boolean, value: unknown): string[] | undefined {
  ...
  const resolved = requested.map(item => resources.find(resource => resource.id === item || resource.slug === item)?.id).filter((item): item is string => !!item)
