...
describe('dashboard release integration', () => {
  it('shows immutable identity, comparison, pinning, and an exact rollback preview target', async () => {
    ...
    const organization = store.getOrganizationBySlug('acme')!
    const environment = store.getEnvironmentBySlug(project.id, 'production')!
    const resource = store.listResources(project.id, environment.id).find((item) => item.slug === 'web')!
    const releases = new ReleaseStore(store)
    const artifact1 = releases.registerArtifact({
      organizationId: organization.id,
      digest: `sha256:${'1'.repeat(64)}`,
      kind: 'static',
      uri: 's3://immutable/web-1',
      size: 10,
      provenance: { builder: 'ci' },
    })
    const first = releases.create({
      organizationId: organization.id,
      ...
      config: { PUBLIC_URL: 'one' },
      manifest: { domains: ['example.com'] },
      strategy: 'atomic',
    })
    releases.transition(first.id, 'activating', { message: 'switch' })
    releases.transition(first.id, 'active', { message: 'healthy' })
    const artifact2 = releases.registerArtifact({
      organizationId: organization.id,
      digest: `sha256:${'2'.repeat(64)}`,
      kind: 'static',
      uri: 's3://immutable/web-2',
      size: 11,
      provenance: { builder: 'ci' },
    })
    const current = releases.create({
      organizationId: organization.id,
      projectId: project.id,
      environmentId: environment.id,
      resourceId: resource.id,
      ...
      config: { PUBLIC_URL: 'two' },
      manifest: { domains: ['example.com'] },
      strategy: 'atomic',
    })
    releases.transition(current.id, 'activating', { message: 'switch' })
    releases.transition(current.id, 'active', { message: 'healthy' })
    ...
    expect(listed.releases[0]).toMatchObject({
      id: current.id,
      status: 'active',
      artifact: { digest: artifact2.digest },
      previous: { id: first.id },
      comparison: { artifactChanged: true, sourceChanged: true, configChanged: true },
      capabilities: expect.any(Array),
    })
    expect(
      await (
        await call('/api/releases/action', {
          method: 'POST',
          headers,
          body: JSON.stringify({ id: current.id, action: 'pin', pinned: true, reason: 'incident baseline' }),
        })
      ).json(),
    ).toMatchObject({ release: { pinned: true, pinReason: 'incident baseline' } })
    const rejected = await call('/api/releases/action', {
      method: 'POST',
      headers,
      body: JSON.stringify({ id: current.id, action: 'rollback', targetReleaseId: first.id, confirm: 'wrong' }),
    })
    ...
    expect(
      await (
        await call('/api/releases/action', {
          method: 'POST',
          headers,
          body: JSON.stringify({ id: current.id, action: 'rollback', targetReleaseId: first.id, confirm: 'web' }),
        })
      ).json(),
    ).toMatchObject({ operation: { kind: 'release.rollback', input: { targetReleaseId: first.id }, state: 'queued' } })
  }, 10_000)
})
