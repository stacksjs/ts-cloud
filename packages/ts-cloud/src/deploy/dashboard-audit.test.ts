...
describe('dashboard mutation audit envelope', () => {
  it('records action, target, outcome, and bounded timing', () => {
    expect(
      dashboardMutationAuditPayload(
        'delete',
        '/api/sites',
        { site: 'acme.example', confirm: 'acme.example' },
        200,
        12.8,
      ),
    ).toEqual({
      ...
      outcome: 'succeeded',
      durationMs: 13,
      input: { site: 'acme.example' },
    })
  })

  it('omits secret values, passwords, confirmations, and unknown request fields', () => {
    const payload = dashboardMutationAuditPayload(
      'post',
      '/api/serverless/secrets',
      { secretId: 'prod/stripe', value: 'must-not-leak', password: 'hidden', confirm: 'prod/stripe', arbitrary: 'no' },
      409,
      2,
    )
