...
function isAlreadyExists(error: unknown): boolean {
  return ['EntityAlreadyExists', 'ResourceAlreadyExistsException', 'ResourceConflictException', '409'].includes(errorCode(error))
}
...
function validateOptions(options: StaticApiOriginOptions): Required<Pick<StaticApiOriginOptions, 'region' | 'pathPattern' | 'originId' | 'roleName' | 'memorySize' | 'timeout' | 'logRetentionDays'>> {
  if (!/^[A-Z0-9]{8,32}$/.test(options.distributionId)) throw new Error('CloudFront distribution ID is invalid')
  if (!/^(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z0-9-]{2,63}$/.test(options.expectedAlias)) throw new Error('Expected CloudFront alias is invalid')
  ...
  if (pathPattern === '/' || pathPattern === '/*' || !pathPattern.endsWith('*')) throw new Error('API path must be a non-default wildcard pattern such as /api/*')
  ...
  return { region: options.region || 'us-east-1', pathPattern, originId, roleName, memorySize, timeout, logRetentionDays }
}

export function createStaticApiOriginDependencies(options: Pick<StaticApiOriginOptions, 'profile' | 'region'>): StaticApiOriginDependencies {
...
export async function deployStaticApiOrigin(options: StaticApiOriginOptions, injected?: StaticApiOriginDependencies): Promise<StaticApiOriginPlan> {
  const resolved = validateOptions(options)
  const dependencies = injected || createStaticApiOriginDependencies({ profile: options.profile, region: resolved.region })
  ...
  const aliases = aliasesOf(distribution).length ? aliasesOf(distribution) : aliasesOf(distributionConfig.DistributionConfig)
  if (!aliases.includes(options.expectedAlias)) throw new Error(`Distribution ${options.distributionId} does not contain expected alias ${options.expectedAlias}`)

  let role: { Arn: string } | undefined
  let roleExisted = true
  try { role = await dependencies.iam.getRole({ RoleName: resolved.roleName }) }
  catch (error) { if (!isMissing(error)) throw error; roleExisted = false }
  ...
  const accessControlExisted = oacs.some(value => value.Name === oacName && value.OriginAccessControlOriginType === 'lambda')
  ...
  const changes = [
    ...(!roleExisted ? [`create IAM execution role ${resolved.roleName}`] : [`reconcile IAM execution policy ${resolved.roleName}`]),
    ...(functionExisted ? [`update Lambda ${options.functionName} code and configuration`] : [`create Lambda ${options.functionName}`]),
  ...
  const base: StaticApiOriginPlan = {
    ...
    distribution: { id: distribution.Id, arn: distribution.ARN, alias: options.expectedAlias, domainName: distribution.DomainName, status: distribution.Status },
    function: { name: options.functionName, existed: functionExisted, runtime: 'nodejs22.x', urlExisted: !!functionUrl, url: functionUrl?.FunctionUrl },
    role: { name: resolved.roleName, existed: roleExisted, arn: expectedRoleArn },
    origin: { id: resolved.originId, pathPattern: resolved.pathPattern, existed: originExisted, behaviorExisted, accessControlExisted },
    changes,
    rollback,
    applied: false,
  }
  ...
  if (options.confirm !== confirmation) throw new Error(`Pass exact confirmation token ${confirmation} to mutate the live distribution`)
  ...
  await dependencies.iam.putRolePolicy({
    ...
    PolicyDocument: JSON.stringify({ Version: '2012-10-17', Statement: [{ Effect: 'Allow', Action: ['logs:CreateLogStream', 'logs:PutLogEvents'], Resource: `arn:aws:logs:${resolved.region}:${identity.Account}:log-group:/aws/lambda/${options.functionName}:*` }] }),
  })
  try { await dependencies.logs.createLogGroup(`/aws/lambda/${options.functionName}`, { ManagedBy: 'ts-cloud' }) }
  ...
  if (functionExisted) {
    ...
    await dependencies.lambda.updateFunctionConfiguration({ FunctionName: options.functionName, Runtime: 'nodejs22.x', Role: role.Arn, Handler: 'index.handler', Description: `Private API origin for ${options.expectedAlias}`, Timeout: resolved.timeout, MemorySize: resolved.memorySize })
  }
  else {
    let lastError: unknown
    for (let attempt = 0; attempt < 8; attempt++) {
      try {
        await dependencies.lambda.createFunctionWithCode({ FunctionName: options.functionName, Runtime: 'nodejs22.x', Role: role.Arn, Handler: 'index.handler', Code: code, Filename: 'index.mjs', Description: `Private API origin for ${options.expectedAlias}`, Timeout: resolved.timeout, MemorySize: resolved.memorySize })
        lastError = undefined
        break
      }
      catch (error) {
        lastError = error
        if (!String((error as Error).message).toLowerCase().includes('role')) throw error
  ...
  await dependencies.lambda.waitForFunctionActive(options.functionName)
  const url = functionUrl || await dependencies.lambda.createFunctionUrl({ FunctionName: options.functionName, AuthType: 'AWS_IAM', InvokeMode: 'BUFFERED' })
  ...
  const permissions = [
    { FunctionName: options.functionName, StatementId: 'AllowCloudFrontFunctionUrl', Action: 'lambda:InvokeFunctionUrl', Principal: 'cloudfront.amazonaws.com', SourceArn: sourceArn, FunctionUrlAuthType: 'AWS_IAM' as const },
    { FunctionName: options.functionName, StatementId: 'AllowCloudFrontInvoke', Action: 'lambda:InvokeFunction', Principal: 'cloudfront.amazonaws.com', SourceArn: sourceArn, InvokedViaFunctionUrl: true },
  ]
  for (const permission of permissions) {
    try { await dependencies.lambda.addPermission(permission) }
    catch (error) { if (!isAlreadyExists(error)) throw error }
  }
  await dependencies.cloudfront.upsertExistingDistributionOrigin(options.distributionId, { id: resolved.originId, domainName: urlHost, pathPattern: resolved.pathPattern, originAccessControlId: oac.Id })
  return { ...base, function: { ...base.function, url: url.FunctionUrl }, applied: true }
}

export async function verifyStaticApiOrigin(options: { alias: string, expectedFrontendSha256?: string, logs?: Pick<CloudWatchLogsClient, 'filterLogEvents'>, functionName?: string, now?: () => Date }): Promise<StaticApiOriginVerification> {
  ...
  if (options.logs && options.functionName) {
    const result = await options.logs.filterLogEvents({ logGroupName: `/aws/lambda/${options.functionName}`, startTime: startedAt - 60_000, filterPattern: '"Init Duration"', limit: 25 })
  ...
  return {
    frontend: { url: frontendUrl, status: frontendResponse.status, sha256: frontendSha256, unchanged: options.expectedFrontendSha256 ? frontendSha256 === options.expectedFrontendSha256 : undefined, bytes: frontendBytes.byteLength },
...
export function estimateStaticApiOriginMonthlyCost(options: { requests: number, averageDurationMs: number, memoryMb?: number, includeFreeTier?: boolean }): { lambdaUsd: number, alwaysOnFargateAndAlbUsd: number, assumptions: string[] } {
  ...
  const billableGbSeconds = Math.max(0, options.requests * (options.averageDurationMs / 1000) * memoryGb - (options.includeFreeTier === false ? 0 : 400_000))
  const lambdaUsd = billableRequests / 1_000_000 * 0.20 + billableGbSeconds * 0.0000166667
  const fargateCompute = 730 * (0.25 * 0.04048 + 0.5 * 0.004445)
  const alb = 730 * 0.0225
  return { lambdaUsd: Number(lambdaUsd.toFixed(2)), alwaysOnFargateAndAlbUsd: Number((fargateCompute + alb).toFixed(2)), assumptions: ['us-east-1 public list rates snapshot, July 2026', 'Lambda x86 request and duration pricing with optional monthly free tier', 'Fargate comparison uses one always-on 0.25 vCPU/0.5 GB task plus one ALB before LCU charges', 'excludes CloudFront, data transfer, logs, taxes, and application data services'] }
}
