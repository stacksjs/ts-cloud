...
describe('dashboard configuration integration', () => {
  test('imports, previews, masks, reveals, rotates, exports, and deletes scoped configuration', async () => {
    ...
    const imported = await call('/api/configuration/import?env=production', {
      method: 'POST',
      headers: { ...headers, 'idempotency-key': 'import-1' },
      body: JSON.stringify({
        scopeType: 'project',
    ...
    const listResponse = await call('/api/configuration?env=production&scopeType=project', {
        headers: { cookie: session },
      }),
      listed = (await listResponse.json()) as any
    expect(listResponse.headers.get('cache-control')).toBe('no-store')
    expect(listed.entries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ key: 'PUBLIC_URL', kind: 'variable', value: 'https://example.test' }),
        expect.objectContaining({ key: 'TOKEN', kind: 'secret', backend: 'local_encrypted' }),
      ]),
    )
    expect(JSON.stringify(listed)).not.toContain('secret-value')
    const token = listed.entries.find((entry: any) => entry.key === 'TOKEN')

    const plan = await call('/api/configuration/plan?env=production', {
        method: 'POST',
        headers,
        body: JSON.stringify({
          scopeType: 'project',
          values: { PUBLIC_URL: 'https://new.test', TOKEN: 'secret-value', ADDED: 'yes' },
        }),
      }),
      planBody = (await plan.json()) as any
    ...
    const reveal = await call('/api/configuration/reveal?env=production', {
      method: 'POST',
      headers,
    ...
    const unconfirmed = await call('/api/configuration?env=production', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        scopeType: 'project',
    ...
    const rotated = await call('/api/configuration?env=production', {
        method: 'POST',
        headers: { ...headers, 'idempotency-key': 'rotate-1' },
        body: JSON.stringify({
          scopeType: 'project',
          key: 'TOKEN',
          kind: 'secret',
      ...
      rotatedBody = (await rotated.json()) as any
    expect(rotatedBody).toMatchObject({
      ok: true,
      entry: { key: 'TOKEN', version: 2, backendVersion: '2' },
      mutation: { changed: ['TOKEN'] },
    })
    expect(JSON.stringify(rotatedBody)).not.toContain('rotated-value')

    const exported = await call('/api/configuration/export?env=production&scopeType=project', {
      headers: { cookie: session },
    })
    expect(await exported.text()).toBe('PUBLIC_URL="https://example.test"\n')

    const removed = await call('/api/configuration?env=production', {
      method: 'DELETE',
      headers,
