...
describe('buildServerlessOperations', () => {
  ...
  it('offers db:scale only with an Aurora database, carrying min/max inputs', () => {
    ...
    const scale = buildServerlessOperations(
      config({ database: { connection: 'aurora-serverless', minCapacity: 1, maxCapacity: 8 } }),
      'production',
      {},
  ...
  it('offers assets:invalidate only when the app ships assets', () => {
    ...
    expect(
      buildServerlessOperations(config({ assets: true }), 'production', {}).some((o) => o.id === 'assets:invalidate'),
    ).toBe(true)
    expect(
      buildServerlessOperations(config(), 'production', { assetsInfo: { bucket: 'x' } }).some(
        (o) => o.id === 'assets:invalidate',
      ),
    ).toBe(true)
  })

  it('offers a purge op per queue and drops unsafe names', () => {
    const ops = buildServerlessOperations(config(), 'production', {
      queues: [{ name: 'default' }, { name: 'emails' }, { name: 'bad; rm -rf /' }],
    })
...
describe('updateFunctionConfig validation', () => {
  it('rejects unknown modes and out-of-range values before any AWS call', async () => {
    expect((await updateFunctionConfig(config(), 'production', 'bogus', { memory: 512 })).error).toMatch(
      /Unknown function mode/,
    )
