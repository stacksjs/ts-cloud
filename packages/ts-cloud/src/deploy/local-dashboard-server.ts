...
import { AwsDatabaseBackupSource, AwsInfrastructureBackupSource, BackupCoordinator, backupCredentialStatus, BackupStore, ControlPlaneBackupSource, createBackupQueueHandlers, DockerVolumeBackupSource, FilesystemBackupSource, LogicalDatabaseBackupSource, S3BackupDestinationAdapter } from '../backups'
import { buildComposeLogsCommand, buildComposeShellCommand, ComposeApplicationService, listComposeTemplates } from '../compose'
import { loadCloudConfig } from '../config'
import { AwsSecretsManagerConfigurationBackend, AwsSsmConfigurationBackend, ConfigurationService, ConfigurationStore, ExternalConfigurationBackend, LocalEncryptedConfigurationBackend, synchronizeConfiguredConfiguration } from '../configuration'
...
import { createJobQueueHandlers, jobProviderCapability, JobService, JobStore, previewSchedule, synchronizeConfiguredJobs } from '../jobs'
import { ControlPlaneCleanupDriver, createMaintenanceQueueHandlers, MaintenanceService, MaintenanceStore } from '../maintenance'
...
import { resolveRuntimeInventory, RuntimeOperationService, RuntimeStreamRegistry } from '../runtime'
import { ensureDefaultSecurityPolicies, productionChangeReview, recordDashboardHostPosture, SecretFindingScanner, SecurityPostureStore, SecurityScannerRunner, securityScope } from '../security'
...
import { loadTelemetryPolicy, saveTelemetryPolicy, telemetryCursor, telemetryEstimatedMonthlyCost, TelemetryStore } from '../telemetry'
...
export function pruneDashboardTempRoots(
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith(DASHBOARD_TEMP_PREFIX)) continue
...
function runtimeEventStream(
  registry: RuntimeStreamRegistry,
  session: RuntimeStreamSnapshot,
  after: number,
  signal: AbortSignal,
): Response {
  ...
  const body = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      unsubscribe = registry.subscribe(session.id, session.workloadId, (snapshot) =>
        send(registry.read(snapshot.id, snapshot.workloadId, cursor) ?? snapshot),
      )
...
const DASHBOARD_AUDIT_FIELDS = new Set([
  'action',
  'operation',
  'site',
  'name',
...
export function dashboardMutationAuditPayload(
  method: string,
  path: string,
  input: unknown,
  status: number,
  ...
  const target = String(
    sanitized.resourceId || sanitized.site || sanitized.secretId || sanitized.database || sanitized.name || path,
  )
  return {
...
export function resolveOidcDashboardOrigin(
  host: string,
  port: number,
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const configured =
    env.TS_CLOUD_DASHBOARD_ORIGIN?.trim() ||
  ...
  const raw =
    configured ||
    (fallbackHost && ['127.0.0.1', 'localhost', '::1'].includes(fallbackHost)
      ? `http://${fallbackHost.includes(':') ? `[${fallbackHost}]` : fallbackHost}:${port}`
      : '')
...
function selectedEnvironment(config: CloudConfig, requested?: EnvironmentType): EnvironmentType {
  if (requested && Object.hasOwn(config.environments ?? {}, requested)) return requested
...
function parseConfigurationScope(
  ...
  defaults: { projectId: string; environmentId?: string },
): ConfigurationScope {
  const type = String(input.scopeType ?? 'environment') as ConfigurationScope['type']
  if (!['project', 'environment', 'service', 'function', 'preview'].includes(type))
    throw new Error('Configuration scopeType must be project, environment, service, function, or preview.')
  const id = String(
    input.scopeId ??
      (type === 'project' ? defaults.projectId : type === 'environment' ? (defaults.environmentId ?? '') : ''),
  )
  ...
  if (type === 'service' || type === 'function')
    return {
      type,
      id,
      environmentId: String(input.environmentId ?? defaults.environmentId ?? '') || undefined,
      resourceId: id,
    }
  return {
    type,
    id,
    environmentId: String(input.environmentId ?? defaults.environmentId ?? '') || undefined,
    resourceId: String(input.resourceId ?? '') || undefined,
    previewId: id,
  }
}
...
function authorizationScope(body: Record<string, any>): AuthorizationScope | undefined {
  ...
  if (!AUTHORIZATION_SCOPES.has(type)) return undefined
...
function resolveUiSourceDir(cwd: string): string | null {
  ...
  for (const dir of candidates) {
    if (existsSync(join(dir, 'pages')) && existsSync(join(dir, 'package.json'))) return dir
  }
  return null
}
...
export function sanitizeCloudConfig(config: CloudConfig): Record<string, any> {
  return {
    ...
    sites: Object.fromEntries(
      Object.entries(config.sites ?? {}).map(([name, site]: [string, any]) => [
        name,
        {
          domain: site.domain,
          path: site.path,
...
export async function runDashboardAction(
  ...
  options: Required<Pick<LocalDashboardServerOptions, 'cwd' | 'cliEntry'>> & {
    signal?: AbortSignal
    onOutput?: (kind: 'stdout' | 'stderr', chunk: string) => void
  ...
  return Object.entries(process.env)
    .filter(
      ([name, value]) =>
        !!value &&
        value.length >= 8 &&
        /(?:secret|token|password|passwd|private|credential|api.?key|access.?key)/i.test(name),
    )
    .map(([, value]) => value!)
}
...
export function isBoxOnlyPage(pathname: string): boolean {
  ...
  if (clean === '/') return false // handled earlier by the member redirect
...
export function canOpenDashboardPage(
  pathname: string,
  user: DashboardUser & { capabilities?: AuthorizationCapability[]; organizationSource?: string },
): boolean {
  if (user.role === 'admin') return true
  const extension = extname(pathname)
  if (extension && extension !== '.html') return true
  if (user.organizationSource === 'legacy') return !isBoxOnlyPage(pathname)
  const clean = pathname.replace(/\.html$/, '').replace(/\/+$/, '') || '/'
  if (clean === '/access-denied') return true
...
function staticPath(uiRoot: string, pathname: string): string | null {
  ...
  if (!extname(base) && existsSync(`${base}.html`)) return `${base}.html`
  if (!extname(base) && existsSync(join(base, 'index.html'))) return join(base, 'index.html')
  return null
}
...
async function serveStatic(uiRoot: string, pathname: string): Promise<Response> {
  ...
  if (statSync(file).isDirectory()) return text('Not found', 404)
...
export async function startLocalDashboardServer(
  ...
  if (controlPlane.reconciliation.failed > 0) {
    console.warn(
      `  ts-cloud dashboard: marked ${controlPlane.reconciliation.failed} orphaned operation(s) as failed after restart.`,
    )
  }
  ...
  function dashboardViewUser(
    _user: DashboardUser,
    _environment: EnvironmentType,
  ): DashboardUser & {
    organizationRole?: OrganizationRoleTemplate
    organizationSource?: string
    capabilities?: AuthorizationCapability[]
  } {
    if (
      _user.role === 'admin' &&
      !controlPlane.store.getActorByExternalId('user', `dashboard:${_user.username.toLowerCase()}`)
    )
      return _user
    const actor = controlPlane.store.getActorByExternalId('user', `dashboard:${_user.username.toLowerCase()}`)
    const membership = actor
      ? controlPlane.store.getMembershipForActor(controlPlane.organization.id, actor.id)
      : undefined
    if (!membership || membership.status !== 'active') return { ..._user, role: 'member', sites: {}, capabilities: [] }
    const grants = controlPlane.store.listGrants(membership.id)
    const scopeTarget =
      controlPlane.store.resolveAuthorizationTarget(controlPlane.organization.id, membership.scope) ??
      controlPlane.store.resolveAuthorizationTarget(controlPlane.organization.id, {
        type: 'project',
        id: controlPlane.project.id,
      })!
    ...
    if (membership.roleTemplate === 'owner' || membership.roleTemplate === 'admin')
      return {
        ...
        role: 'admin',
        sites: {},
        organizationRole: membership.roleTemplate,
        organizationSource: membership.source,
        capabilities,
      }
    const environmentRecord = controlPlane.environments.get(_environment)
    const sites: DashboardUser['sites'] = {}
    for (const resource of controlPlane.store
      .listResources(controlPlane.project.id, environmentRecord?.id)
      ...
      if (!target || !authorizeOrganization({ membership, grants, capability: 'project:read', target }).allowed)
        continue
      sites[resource.slug] = authorizeOrganization({ membership, grants, capability: 'config:write', target }).allowed
        ? 'owner'
        : 'collaborator'
    }
    return {
  ...
  function clearUiCache(): void {
    uiGeneration += 1
    uiCache.clear()
    for (const root of ownedUiRoots) rmSync(root, { recursive: true, force: true })
    ownedUiRoots.clear()
  }

  const scopeKey = (
    user: DashboardUser & { capabilities?: AuthorizationCapability[] },
    environment: EnvironmentType,
  ): string =>
    `${environment}:${
      user.role === 'admin'
        ? 'admin'
        : `member:${Object.entries(user.sites)
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([site, role]) => `${site}=${role}`)
            .join(',')}`
  ...
  async function uiRootFor(user: DashboardUser, environment: EnvironmentType): Promise<string | undefined> {
    ...
    const build = (async (): Promise<string | undefined> => {
      ...
      const latestData =
        latestDataByEnvironment.get(environment) ?? (await resolveLiveDashboardData(config as CloudConfig, environment))
      ...
      const scoped = {
        ...
        viewer: {
          role: viewUser.role,
          organizationRole: viewUser.organizationRole,
          organizationSource: viewUser.organizationSource,
          capabilities: viewUser.capabilities ?? [],
        },
      }
      ...
      if (liveRoot) ownedUiRoots.add(liveRoot)
      const root = liveRoot ?? packagedUi?.uiRoot
      if (root) uiCache.set(key, root)
      return root
    })()
  ...
  if (authDisabled && options.box)
    throw new Error(
      'TS_CLOUD_DASHBOARD_AUTH=0 is refused in box mode: the dashboard is internet-facing there, and disabling auth would expose a root shell.',
    )
  ...
  const sourceConnections = new SourceConnectionStore(controlPlane.store, {
    encryptionKey: resolveAuthEncryptionKey(cwd),
  })
  ...
  const registryConnections = new RegistryConnectionStore(controlPlane.store, {
    encryptionKey: resolveAuthEncryptionKey(cwd),
  })
  ...
  const dataAdapters = {
    ...
    aws_elasticache: new AwsElastiCacheDataAdapter(
      new AwsElastiCacheTransport(new ElastiCacheClient(dataServiceRegion)),
    ),
  ...
  const resolveBackupSource = (policy: BackupPolicy) => backupSources[policy.resourceKind as keyof typeof backupSources]
  ...
  const fleetService = new FleetService(
    fleetStore,
    [new SshFleetDriver('aws'), new SshFleetDriver('hetzner'), new SshFleetDriver('ssh')],
    operationQueue,
  )
  ...
  const cleanupDrivers = (
    ...
    (kind) =>
      new ControlPlaneCleanupDriver(kind, controlPlane.store, controlPlane.project.id, controlPlane.organization.id),
  )
  ...
  for (const environmentRecord of controlPlane.environments.values())
    synchronizeConfiguredJobs(jobStore, config as CloudConfig, {
      ...
      resources: controlPlane.store.listResources(controlPlane.project.id, environmentRecord.id),
    })
  ...
  const evaluateAlerts = async (environmentId?: string) => {
    ...
    const deliveries = []
    for (const evaluation of evaluations.filter((item) => item.notify && item.alert))
      deliveries.push(
        ...(await notificationRouter.deliverAll(
          notificationRouter.enqueue(
            controlPlane.organization.id,
            evaluation.alert!,
            evaluation.transition as 'firing' | 'resolved',
          ),
        )),
      )
    deliveries.push(
      ...(await notificationRouter.deliverAll(
        notificationRouter.enqueueRemindersAndEscalations(
          controlPlane.organization.id,
          alertStore.listAlerts(controlPlane.project.id, { environmentId, states: ['firing'] }),
        ),
      )),
    )
  ...
  const evaluateHealthChecks = async () => {
    ...
    try {
      for (const environmentRecord of controlPlane.environments.values())
        for (const check of alertStore
          .listHealthChecks(controlPlane.project.id, environmentRecord.id)
          ...
          for (const evaluation of outcome.evaluations.filter((item) => item.notify && item.alert))
            await notificationRouter.deliverAll(
              notificationRouter.enqueue(
                controlPlane.organization.id,
                evaluation.alert!,
  ...
  const alertEvaluationSweep = setInterval(() => {
    for (const record of controlPlane.environments.values())
      void evaluateAlerts(record.id).catch((error) => console.error('ts-cloud alert evaluation failed:', error))
  }, 60_000)
  alertEvaluationSweep.unref?.()
  const healthCheckSweep = setInterval(() => {
    void evaluateHealthChecks().catch((error) => console.error('ts-cloud health check evaluation failed:', error))
  }, 30_000)
  healthCheckSweep.unref?.()
  const jobScheduleSweep = setInterval(() => {
    try {
      jobService.tick()
    } catch (error) {
      console.error('ts-cloud scheduled job evaluation failed:', error)
    }
  }, 30_000)
  ...
  const backupScheduleSweep = setInterval(() => {
    try {
      ...
      console.error('ts-cloud backup policy evaluation failed:', error)
    }
  }, 30_000)
  ...
  const previewCleanupSweep = setInterval(
    () => {
      try {
        ...
        console.error('ts-cloud preview cleanup failed:', error)
      }
    },
    60 * 60 * 1000,
  )
  ...
  const queueWorker = queueWorkerEnabled
    ? new DurableQueueWorker(
        operationQueue,
        {
            ...
            execute: async (command, context) => {
              const operationInput =
                ...
                !Array.isArray(context.operation.input)
                  ? (context.operation.input as Record<string, any>)
                  : {}
              const source =
                operationInput.source && typeof operationInput.source === 'object'
                  ? (operationInput.source as Record<string, any>)
                  : undefined
              let actionCwd = cwd
              let checkoutRoot: string | undefined
              let deploymentConfiguration: Awaited<ReturnType<ConfigurationService['resolve']>> | undefined
              let publishStatus:
                | ((_state: 'pending' | 'success' | 'failure' | 'error', _description: string) => Promise<void>)
                | undefined
              try {
                if (source?.bindingId && source?.connectionId && source?.commitSha) {
                  ...
                  if (!binding || !connection || binding.connectionId !== connection.id)
                    throw new Error('Queued source binding is no longer available')
                  const repository = binding.repositoryId
                    ? sourceConnections.getRepository(binding.repositoryId)
                    : sourceConnections
                        .listRepositories(connection.id)
                        .find((item) => item.fullName.toLowerCase() === binding.repositoryFullName.toLowerCase())
                  let remote = repository?.cloneUrl
                  if (!remote && connection.provider === 'generic_ssh')
                    remote = `git@${new URL(connection.host).hostname}:${binding.repositoryFullName}.git`
                  if (!remote)
                    remote = new URL(
                      `/${binding.repositoryFullName}.git`,
                      connection.host.endsWith('/') ? connection.host : `${connection.host}/`,
                    ).href
                  const deployKeyRecord = binding.deployKeyId
                    ? sourceConnections.getDeployKey(binding.deployKeyId)
                    : undefined
                  const deployKey =
                    deployKeyRecord && binding.deployKeyId
                      ? { ...deployKeyRecord, privateKey: sourceConnections.getDeployPrivateKey(binding.deployKeyId) }
                      : undefined
                  if (
                    ['github', 'gitlab', 'bitbucket', 'gitea'].includes(connection.provider) &&
                    ...
                    publishStatus = async (state, description) => {
                      try {
                        await adapter.setCommitStatus(binding.repositoryFullName, String(source.commitSha), {
                          state,
                          url: preview?.url,
                          description,
                        })
                        ...
                        context.log(
                          `Could not publish source status: ${error instanceof Error ? error.message : String(error)}`,
                          { stream: 'stderr' },
                        )
                      }
                    }
                    await publishStatus(
                      'pending',
                      `Preview deployment started for ${String(source.commitSha).slice(0, 12)}`,
                    )
                  }
                  ...
                  const cloned = await cloneSourceBinding(
                    {
                      remote,
                      binding,
                      destination: join(checkoutRoot, 'checkout'),
                ...
                if (context.operation.kind !== 'preview.destroy') {
                  const preview = command.target.previewId
                    ? previewService.previews.getInstance(command.target.previewId)
                    : undefined
                  const definition = preview ? previewService.previews.getDefinition(preview.definitionId) : undefined
                  const targetResource = context.operation.resourceId
                    ? controlPlane.store.getResource(context.operation.resourceId)
                    : undefined
                  deploymentConfiguration = await configurationService.resolve({
                    projectId: controlPlane.project.id,
                    environmentId: context.operation.environmentId,
                    resourceId: context.operation.resourceId,
                    functionId: targetResource?.kind === 'function' ? targetResource.id : undefined,
                    previewId: preview?.id,
                    trustedPreview: !!preview && !preview.fork,
                    allowedPreviewSecrets: definition?.inheritedSecrets,
                  })
                  if (deploymentConfiguration.warnings.some((warning) => warning.code === 'missing_required'))
                    throw new Error(
                      `Required deployment configuration is unavailable: ${deploymentConfiguration.warnings
                        .filter((warning) => warning.code === 'missing_required')
                        .map((warning) => warning.key)
                        .join(', ')}`,
                    )
                  if (context.operation.resourceId)
                    for (const item of Object.values(deploymentConfiguration.entries))
                      configurationStore.setDependency({
                        entryId: item.id,
                        resourceId: context.operation.resourceId,
                  ...
                  context.log(
                    `Resolved configuration ${deploymentConfiguration.configurationHash.slice(0, 18)} for ${Object.keys(deploymentConfiguration.values).length} keys.`,
                    { stream: 'system' },
                  )
                }
                const result = (await runDashboardAction(command, {
                  ...
                  onOutput: (stream, chunk) =>
                    context.log(chunk, {
                      ...
                      secrets: [
                        ...queueSecrets,
                        ...Object.entries(deploymentConfiguration?.entries ?? {})
                          .filter(([, item]) => item.kind === 'secret')
                          .map(([key]) => deploymentConfiguration!.values[key]!)
                          .filter(Boolean),
                      ],
                    }),
                })) as { ok: boolean; exitCode: number; command?: string; stderr?: string }
                if (result.ok && context.operation.resourceId && deploymentConfiguration)
                  for (const item of Object.values(deploymentConfiguration.entries))
                    configurationStore.setDependency({
                      entryId: item.id,
                      resourceId: context.operation.resourceId,
                      ...
                      lastDeployedVersion: item.version,
                    })
                await publishStatus?.(
                  result.ok ? 'success' : 'failure',
                  result.ok
                    ? `Preview is ready at ${String(source?.commitSha).slice(0, 12)}`
                    : `Preview deployment failed at ${String(source?.commitSha).slice(0, 12)}`,
                )
                return result
              } catch (error) {
                await publishStatus?.(
                  'error',
                  `Preview deployment could not complete at ${String(source?.commitSha).slice(0, 12)}`,
                )
                throw error
              } finally {
          ...
          ...createJobQueueHandlers({ store: jobStore, executor: executeScheduledJob }),
          ...createDataServiceQueueHandlers({
            store: dataServiceStore,
            secrets: dataServiceSecrets,
            resolveAdapter: resolveDataAdapter,
          }),
          ...createFleetQueueHandlers(fleetStore, fleetService.drivers),
          ...createRegionQueueHandlers(regionStore, [
            new UnavailableRegionalDriver(
              'aws',
              'AWS regional execution requires an injected AwsRegionalTransport; topology and plans were preserved without provider mutation.',
            ),
          ]),
          ...createMaintenanceQueueHandlers({ store: maintenanceStore, cleanup: cleanupDrivers }),
          ...createVolumeQueueHandlers(volumeStore, volumeDrivers),
          ...createBackupQueueHandlers({
            store: backupStore,
            queue: operationQueue,
            resolveSource: resolveBackupSource,
            resolveDestination: resolveBackupDestination,
            validateHealth: async (policy, target): Promise<Record<string, JsonValue>> => {
              if (!policy.healthCheckId) return { healthy: true, mode: 'adapter' }
              const check = alertStore.getHealthCheck(policy.healthCheckId)
              if (!check || check.projectId !== policy.projectId)
                return { healthy: false, error: 'Configured restore health check was not found.' }
              const outcome = await new HealthCheckRunner(alertStore).runAndEvaluate(check)
              return {
                healthy: outcome.result.status === 'healthy',
                healthCheckId: check.id,
                healthResultId: outcome.result.id,
                target,
              }
            },
          }),
          ...createReleaseQueueHandlers({
            store: controlPlane.store,
            resolveDriver:
              options.releaseDriver ??
              ((release) => ({
                name: `${controlPlane.store.getResource(release.resourceId)?.provider ?? 'provider'} release driver`,
                capability: () => ({
                  strategy: release.strategy,
                  supported: false,
                  explanation:
                    'This dashboard process has no immutable activation driver configured for the target provider.',
                  capacityMultiplier: 1,
                  costImpact: 'none',
                  rollback: 'The previous release remains preserved; configure a release driver before retrying.',
                }),
                activate: async () => {
                  throw new Error('Immutable activation driver is not configured')
                },
                rollback: async () => {
                  throw new Error('Immutable rollback driver is not configured')
                },
              })),
            resolveHealthGate: async (name, release) => {
              const check = alertStore
                .listHealthChecks(release.projectId, release.environmentId)
                .find(
                  (item) =>
                    item.enabled && item.name === name && (!item.resourceId || item.resourceId === release.resourceId),
                )
              if (!check)
                return { healthy: false, message: `Named health gate ${name} was not found for this release.` }
              const outcome = await new HealthCheckRunner(alertStore).runAndEvaluate(check)
              return {
                healthy: outcome.result.status === 'healthy',
                evidence: {
  ...
  if (authEnabled) {
    ...
    console.warn(
      '  ts-cloud dashboard: authentication is DISABLED (TS_CLOUD_DASHBOARD_AUTH=0). Every request runs as an admin.',
    )
  }

  // Cookies are marked Secure unless we're serving plain http on loopback.
  const cookieSecure = host !== '127.0.0.1' && host !== 'localhost'
  const networkHint = (address: string): string =>
    createHash('sha256').update(`${secret}:${address}`).digest('hex').slice(0, 16)
  ...
  const userAgentLabel = (req: Request): string | undefined =>
    req.headers.get('user-agent')?.trim().slice(0, 256) || undefined
  const issueSession = (identityId: string, req: Request, address: string, authMethod: 'local' | 'oidc' = 'local') =>
  ...
  const mayAccessOperation = (
    ...
    const scope: AuthorizationScope = operation.resourceId
  ...
  const server = Bun.serve({
    ...
    websocket: {
      open(ws) {
        ...
        ws.send(
          `Connected to ${host === '127.0.0.1' ? 'localhost' : host} shell. This is a line-oriented shell (no full-screen apps).\r\n`,
        )
      },
      message(ws, message) {
    ...
    async fetch(req, runtimeServer) {
      ...
      const handleRequest = async (): Promise<Response | undefined> => {
        ...
        try {
          ...
          if (sourceWebhookRoute && req.method === 'POST') {
            const result = await processSourceWebhook({
              sources: sourceConnections,
              controlPlane: controlPlane.store,
              endpointToken: sourceWebhookRoute[1]!,
              headers: req.headers,
            ...
            if (action === 'start') {
              try {
                const started = await beginOidcAuthorization(
                  ...
                  providerSlug,
                  oidcOrigin,
                  url.searchParams.get('return') ?? undefined,
                  options.oidcFetch,
                )
                ...
                return new Response(null, {
                  status: 302,
                  headers: { location: '/login?sso_error=start', 'cache-control': 'no-store' },
                })
              }
            }
            try {
              if (url.searchParams.has('error')) {
                ...
                if (provider && state) authentication.consumeOidcTransaction(provider.id, state)
              ...
              const issued = issueSession(
                ...
                req,
                activeServer.requestIP(req)?.address ?? 'unknown',
                'oidc',
              )
              clearUiCache()
              controlPlane.store.appendEvent({
                ...
                payload: { method: 'oidc', provider: providerSlug, provisioned: resolved.provisioned },
              })
              return new Response(null, {
                status: 302,
                headers: {
                  ...
                  'set-cookie': serializeSessionCookie(issued.token, {
                    secure: cookieSecure,
                    maxAgeMs: AUTH_SESSION_ABSOLUTE_TTL_MS,
                  }),
                },
              })
            } catch (error) {
              controlPlane.store.appendEvent({
                organizationId: controlPlane.organization.id,
                type: 'auth.oidc.callback_failed',
                level: 'warning',
                payload: {
                  provider: providerSlug,
                  reason: error instanceof Error ? error.message.slice(0, 200) : 'unknown',
              ...
              return new Response(null, {
                status: 302,
                headers: { location: '/login?sso_error=callback', 'cache-control': 'no-store' },
              })
            }
          }
          ...
          if (url.pathname === '/api/invitations/accept' && req.method === 'POST') {
            ...
            if (!invitationToken || !isValidUsername(username) || password.length < 12)
              return json(
                {
                  ok: false,
                  error: 'Invitation token, valid username, and a password of at least 12 characters are required.',
                },
                422,
              )
            ...
            if (!invitation || invitation.state !== 'pending')
              return json(
                { ok: false, error: invitation ? `Invitation is ${invitation.state}` : 'Invitation is invalid' },
                invitation ? 409 : 404,
              )
            ...
            if (existing && !verifyPassword(password, existingIdentity?.passwordHash ?? existing.passwordHash))
              return json(
                { ok: false, error: 'That username already exists; enter its current password to continue.' },
                401,
              )
              ...
              return json(
                {
                  ok: false,
                  error:
                    'This invitation belongs to an existing account. Sign in with that account or ask an owner to resend it.',
                },
                409,
              )
            ...
            try {
              ...
              if (!existing) removeUser(cwd, result.user.username)
              const message = error instanceof Error ? error.message : 'Invitation could not be accepted.'
              return json(
                { ok: false, error: message },
                message.includes('expired') || message.includes('revoked') || message.includes('accepted') ? 409 : 404,
              )
            }
          }
          ...
          if (url.pathname === '/api/auth/password-reset/request' && req.method === 'POST') {
            ...
            if (gate.allowed) recoveryThrottle.recordFailure(identifier, address)
          ...
          if (url.pathname === '/api/auth/password-reset/complete' && req.method === 'POST') {
            ...
            if (!token || password.length < 12)
              return json(
                { ok: false, error: 'A valid reset token and password of at least 12 characters are required.' },
                422,
              )
            try {
              ...
              return json({ ok: true, message: 'Password changed. Sign in again.' }, 200, {
                'set-cookie': clearSessionCookie({ secure: cookieSecure }),
              })
            } catch (error) {
              const message = error instanceof Error ? error.message : 'Password reset failed.'
              return json(
                { ok: false, error: message.replace('Action token', 'Reset link') },
                message.includes('consumed') || message.includes('expired') ? 409 : 404,
              )
            }
          }
          ...
          if (url.pathname === '/api/login' && req.method === 'POST') {
            ...
            if (!gate.allowed) {
              return json(
                {
                  ok: false,
                  error: `Too many failed attempts. Try again in ${Math.ceil((gate.retryAfterSeconds ?? 0) / 60)} minute(s).`,
                },
                429,
            ...
            const passwordUpgraded =
              currentIdentity.requiresPasswordUpgrade || passwordNeedsRehash(currentIdentity.passwordHash)
            if (passwordUpgraded) {
              const passwordHash = hashPassword(password)
            ...
            if (authentication.getMfaFactor(currentIdentity.id)?.state === 'active') {
              ...
              controlPlane.store.appendEvent({
                organizationId: controlPlane.organization.id,
                actorId: currentIdentity.actorId,
                type: 'auth.mfa.challenge.created',
          ...
          if (url.pathname === '/api/auth/mfa/complete' && req.method === 'POST') {
            ...
            if (!gate.allowed)
              return json({ ok: false, error: 'Too many MFA attempts. Try again later.' }, 429, {
                'retry-after': String(gate.retryAfterSeconds ?? 60),
              })
            try {
              ...
              controlPlane.store.appendEvent({
                organizationId: controlPlane.organization.id,
                actorId: completed.identity.actorId,
                type: 'auth.mfa.succeeded',
                payload: { purpose: 'login', method: completed.method },
              })
          ...
          if (url.pathname === '/api/logout' && req.method === 'POST') {
            ...
            if (session) authentication.revokeSession(session.identityId, session.id)
          ...
          if (url.pathname === '/login') {
            const providers = authentication
              .listOidcProviders(controlPlane.organization.id)
          ...
          if (url.pathname === '/accept-invitation') return serveStatic(adminUiRoot, url.pathname)
          ...
          if (url.pathname === '/api/me') {
            ...
            const membership = actor
              ...
              : undefined
            const target =
              controlPlane.store.resolveAuthorizationTarget(
                controlPlane.organization.id,
                membership?.scope ?? { type: 'project', id: controlPlane.project.id },
              ) ??
              controlPlane.store.resolveAuthorizationTarget(controlPlane.organization.id, {
                type: 'project',
                id: controlPlane.project.id,
            ...
            return json({
              ...
              capabilities: effectiveCapabilities({
                membership,
                grants: membership ? controlPlane.store.listGrants(membership.id) : [],
                target,
              }),
            })
          }
          ...
          if (url.pathname.startsWith('/api/')) {
            ...
            if (!decision.ok) return json({ ok: false, error: decision.error }, decision.status ?? 403)
            if (authEnabled && RECENT_AUTH_MUTATIONS.has(`${req.method.toUpperCase()} ${url.pathname}`)) {
              ...
              if (!session || !authentication.isRecentlyAuthenticated(session))
                return json(
                  { ok: false, error: 'Recent authentication is required for this change.', stepUpRequired: true },
                  401,
                )
            }
          }
          ...
          if (url.pathname === '/api/auth/oidc/providers' && req.method === 'POST') {
            ...
            if (enforceSso && (!identity?.emailVerifiedAt || authentication.remainingRecoveryCodes(identity.id) < 1)) {
              return json(
                { ok: false, error: 'Verify the owner email and generate MFA recovery codes before enforcing SSO.' },
                409,
              )
            }
            try {
              ...
              const provider = authentication.upsertOidcProvider({
                ...
                clientSecret:
                  typeof body.clientSecret === 'string' && body.clientSecret ? body.clientSecret : undefined,
              ...
              controlPlane.store.appendEvent({
                ...
                type: 'auth.oidc.provider.configured',
                payload: {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'OIDC provider could not be saved.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/auth/oidc/providers' && req.method === 'PATCH') {
            ...
            controlPlane.store.appendEvent({
              organizationId: controlPlane.organization.id,
              actorId: principal.actor?.id,
              type: updated.enabled ? 'auth.oidc.provider.enabled' : 'auth.oidc.provider.disabled',
          ...
          if (url.pathname === '/api/automation' && req.method === 'GET') {
            const accounts = automationIdentities
              .listServiceAccounts(controlPlane.organization.id, { includeDisabled: true })
              .map((account) => {
                const membership = controlPlane.store.getMembershipForActor(
                  controlPlane.organization.id,
                  account.actorId,
                )
                const createdBy = account.createdByActorId
                  ? controlPlane.store.getActor(account.createdByActorId)
                  : undefined
                const tokens = automationIdentities.listTokens(account.id, { includeInactive: true }).map((token) => ({
                  ...token,
                  createdBy: token.createdByActorId ? controlPlane.store.getActor(token.createdByActorId) : undefined,
                }))
            ...
            return json({
              ...
              openApiUrl: `${url.origin}/api/v1/openapi.json`,
              tokenEnvironmentVariable: 'TS_CLOUD_API_TOKEN',
            })
          }
          ...
          if (url.pathname === '/api/automation/service-accounts' && req.method === 'POST') {
            ...
            try {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Service account could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/automation/service-accounts' && req.method === 'PATCH') {
            ...
            return json({
              ok: true,
              account: automationIdentities.disableServiceAccount(account.id, principal.actor?.id),
            })
          }
          ...
          if (url.pathname === '/api/automation/tokens' && req.method === 'POST') {
            ...
            if (
              !principal.actor ||
              !account ||
              account.organizationId !== controlPlane.organization.id ||
              !scope ||
              !Array.isArray(body.capabilities)
            )
              return json(
                { ok: false, error: 'Service account, explicit capabilities, and valid scope are required.' },
                422,
              )
            try {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'API token could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/automation/tokens/rotate' && req.method === 'POST') {
            ...
            try {
              ...
              return json(
                { ok: true, token: issued.token, secret: issued.secret, revealOnce: true, overlap: true },
                201,
              )
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'API token could not be rotated.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/onboarding' && req.method === 'GET') {
            ...
            return json({
              ...
              project: {
                id: controlPlane.project.id,
                slug: controlPlane.project.slug,
                name: controlPlane.project.name,
              },
              environments: [...controlPlane.environments.values()].map((item) => ({
                id: item.id,
                slug: item.slug,
                name: item.name,
                kind: item.kind,
              ...
              resources: controlPlane.store
                .listResources(controlPlane.project.id)
                .filter((resource) => resource.kind === 'application')
                .map((resource) => ({
                  id: resource.id,
                  slug: resource.slug,
          ...
          if (url.pathname === '/api/onboarding/detect' && req.method === 'POST') {
            ...
            try {
              if (body.source?.kind === 'git') {
                const connection = sourceConnections.getConnection(String(body.source.connectionId ?? ''))
                const repository = sourceConnections.getRepository(String(body.source.repositoryId ?? ''))
                if (
                  !connection ||
                  connection.organizationId !== controlPlane.organization.id ||
                  !repository ||
                  repository.connectionId !== connection.id
                )
                  throw new Error('Authorized Git repository was not found')
                const deployKey =
                  typeof body.source.deployKeyId === 'string'
                    ? sourceConnections.getDeployKey(body.source.deployKeyId)
                    : undefined
                ...
                try {
                  const cloned = await cloneSourceBinding(
                    {
                      ...
                      destination: join(temporary, 'checkout'),
                      ref: String(body.source.ref ?? repository.defaultBranch),
                      sparsePaths: Array.isArray(body.source.sparsePaths)
                        ? body.source.sparsePaths.map(String)
                        : undefined,
                      binding: {
                        id: 'detection',
                        projectId: controlPlane.project.id,
                        ...
                        monorepoRoot: String(body.source.monorepoRoot ?? '.'),
                        includePaths: [],
                        excludePaths: [],
                        submodules: body.source.submodules === true,
                        cloneDepth: 1,
                    ...
                    {
                      credential: sourceConnections.getCredential(connection.id),
                      deployKey: deployKey
                        ? { ...deployKey, privateKey: sourceConnections.getDeployPrivateKey(deployKey.id) }
                        : undefined,
                      timeoutMs: 120_000,
                    },
                  )
                  return json({
                    ...
                    candidates: detectApplication(scanApplicationDirectory(cloned.directory)),
                  })
                } finally {
              ...
              if (Array.isArray(body.files)) {
                ...
                const files = body.files.map((item: any) => ({
                  path: String(item.path ?? ''),
                  size: Number(item.size ?? 0),
                  content: typeof item.content === 'string' ? item.content.slice(0, 256 * 1024) : undefined,
                }))
                return json({ ok: true, candidates: detectApplication(files) })
              }
              const root = resolve(cwd, String(body.root ?? '.'))
              const rootRelative = relative(cwd, root)
              if (rootRelative === '..' || rootRelative.startsWith(`..${sep}`))
                throw new Error('Detection root must stay inside the project')
              return json({ ok: true, candidates: detectApplication(scanApplicationDirectory(root)) })
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Application detection failed.' },
                422,
              )
            }
          }
            ...
            try {
              return json({
                ...
                plan: planApplication(
                  body.draft,
                  Array.isArray(body.suppliedSecretNames) ? body.suppliedSecretNames.map(String) : [],
                ),
              })
            } catch (error) {
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Application plan could not be generated.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/onboarding/drafts' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  ok: true,
                  draft: applicationDrafts.create({
                    organizationId: controlPlane.organization.id,
                    projectId: controlPlane.project.id,
                    name: String(body.name ?? body.draft?.name ?? 'Application draft'),
                    draft: { ...body.draft, projectId: controlPlane.project.id },
                    step: body.step,
                    suppliedSecretNames: Array.isArray(body.suppliedSecretNames)
                      ? body.suppliedSecretNames.map(String)
                      : [],
                    actorId: principal.actor?.id,
                  }),
                },
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Application draft could not be saved.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/onboarding/drafts' && req.method === 'PATCH') {
            const body = await readJsonBody(req)
            const principal = organizationPrincipal(user)
            const current = applicationDrafts.get(String(body.id ?? ''))
            if (!current || current.projectId !== controlPlane.project.id)
              return json({ ok: false, error: 'Application draft was not found.' }, 404)
            try {
              return json({
                ok: true,
                draft: applicationDrafts.update(current.id, Number(body.version), {
                  draft: { ...body.draft, projectId: controlPlane.project.id },
                  step: body.step,
                  suppliedSecretNames: Array.isArray(body.suppliedSecretNames)
                    ? body.suppliedSecretNames.map(String)
              ...
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Application draft could not be updated.',
                },
                409,
              )
            }
          ...
          if (url.pathname === '/api/onboarding/artifacts' && req.method === 'POST') {
            ...
            if (declared > 100 * 1024 * 1024)
              return json({ ok: false, error: 'Artifact exceeds the 100 MB upload limit.' }, 413)
            try {
              const bytes = new Uint8Array(await req.arrayBuffer())
              return json(
                {
                  ok: true,
                  artifact: applicationArtifacts.create({
                    organizationId: controlPlane.organization.id,
                    projectId: controlPlane.project.id,
                    filename: req.headers.get('x-artifact-filename') ?? 'application.zip',
                    bytes,
                    actorId: principal.actor?.id,
                  }),
                },
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Artifact could not be inspected.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/onboarding/registries' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  ok: true,
                  registry: registryConnections.create({
                    organizationId: controlPlane.organization.id,
                    provider: body.provider,
                    name: String(body.name ?? 'Container registry'),
                    ...
                    credential:
                      body.token || body.password
                        ...
                        : undefined,
                    credentialExpiresAt:
                      typeof body.credentialExpiresAt === 'string' ? body.credentialExpiresAt : undefined,
                    actorId: principal.actor?.id,
                  }),
                },
                201,
              ...
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Registry connection could not be created.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/onboarding/registries' && req.method === 'PATCH') {
            const body = await readJsonBody(req)
            const principal = organizationPrincipal(user)
            const connection = registryConnections.get(String(body.id ?? ''))
            if (!connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Registry connection was not found.' }, 404)
            try {
              if (body.action === 'test')
                return json({
                  ok: true,
                  registry: await registryConnections.test(connection.id, {
              ...
              if (body.action === 'rotate')
                return json({
                  ok: true,
                  registry: registryConnections.rotate(
                    connection.id,
                    {
                      username: typeof body.username === 'string' ? body.username : undefined,
                      password: typeof body.password === 'string' ? body.password : undefined,
              ...
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Registry connection could not be updated.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/onboarding/registries' && req.method === 'DELETE') {
            ...
            if (!connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Registry connection was not found.' }, 404)
            const affectedDrafts = applicationDrafts
              .list(controlPlane.project.id)
              .filter(
                (draft) =>
                  (draft.input.source.kind === 'image' && draft.input.source.registryConnectionId === connection.id) ||
                  (draft.input.build.kind === 'prebuilt_image' &&
                    draft.input.build.registryConnectionId === connection.id),
              )
            if (body.preview === true) return json({ ok: true, preview: true, affectedDrafts })
            return json({
              ok: true,
              registry: registryConnections.disconnect(connection.id, principal.actor?.id),
              affectedDrafts,
            })
          }
          ...
          if (url.pathname === '/api/onboarding/apply' && req.method === 'POST') {
            ...
            const principal = organizationPrincipal(user)
            try {
              const result = applyApplicationDraft({
                controlPlane: controlPlane.store,
                drafts: applicationDrafts,
                draftId: String(body.id ?? ''),
                expectedVersion: Number(body.version),
                confirmEnvironment: String(body.confirmEnvironment ?? ''),
                actorId: principal.actor?.id,
              })
              return json({ ok: true, resource: result.resource, operation: result.operation, plan: result.plan }, 202)
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Application plan could not be applied.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/sources' && req.method === 'GET') {
            ...
            return json({
              ...
              webhooks: connections
                .flatMap((connection) => sourceConnections.listWebhooks(connection.id))
                .map((webhook) => ({ ...webhook, deliveries: sourceConnections.listDeliveries(webhook.id, 20) })),
              ...
              resources: controlPlane.store
                .listResources(controlPlane.project.id, securityScope(controlPlane, String(environment)).environmentId)
                .filter((resource) => resource.kind === 'application')
                .map((resource) => ({ id: resource.id, slug: resource.slug, name: resource.name })),
            })
          }
          ...
          if (url.pathname === '/api/sources/connections' && req.method === 'POST') {
            ...
            if (!['github', 'gitlab', 'bitbucket', 'gitea', 'generic_https', 'generic_ssh'].includes(provider))
              return json({ ok: false, error: 'A supported Git provider is required.' }, 422)
            try {
              const credential =
                provider === 'github' && (body.privateKey || body.appId)
                  ? {
              ...
              controlPlane.store.database.transaction(() => {
                connection = sourceConnections.createConnection({
                  organizationId: controlPlane.organization.id,
                  provider: provider as any,
                  name: String(body.name ?? provider),
                  host: String(body.host ?? defaults[provider] ?? ''),
                ...
                if (typeof body.repositoryUrl === 'string' && typeof body.repositoryFullName === 'string')
                  repository = sourceConnections.upsertRepository({
                    connectionId: connection.id,
                    providerRepositoryId: `manual:${body.repositoryFullName}`,
                    fullName: body.repositoryFullName,
                    cloneUrl: body.repositoryUrl,
                    defaultBranch: String(body.defaultBranch ?? 'main'),
                    visibility: 'unknown',
                    archived: false,
                    metadata: { source: 'manual' },
                  })
                if (provider === 'generic_ssh')
                  deployKey = sourceConnections.createDeployKey({
                    connectionId: connection.id,
                    name: String(body.deployKeyName ?? 'Repository deploy key'),
                    publicKey: String(body.publicKey ?? ''),
                    privateKey: String(body.deployPrivateKey ?? ''),
                    host: String(body.sshHost ?? ''),
                    hostKey: String(body.hostKey ?? ''),
                    actorId: principal.actor?.id,
                  })
              })()
              if (body.test === true)
                connection = await testSourceConnection(sourceConnections, connection.id, repository?.id)
              return json({ ok: true, connection, repository, deployKey }, 201)
            } catch (error) {
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Source connection could not be created.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/sources/connections' && req.method === 'PATCH') {
            ...
            if (!connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Source connection was not found.' }, 404)
            try {
              if (body.action === 'test')
                return json({
                  ok: true,
                  connection: await testSourceConnection(
                    sourceConnections,
                    connection.id,
              ...
              if (body.action === 'rotate')
                return json({
                  ok: true,
                  connection: sourceConnections.rotateCredential(
                    connection.id,
                    {
                      token: typeof body.token === 'string' ? body.token : undefined,
                      username: typeof body.username === 'string' ? body.username : undefined,
              ...
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Source connection could not be updated.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/sources/deploy-keys' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  ...
                  deployKey: sourceConnections.createDeployKey({
                    connectionId: connection.id,
                    name: String(body.name ?? 'Repository deploy key'),
                    publicKey: String(body.publicKey ?? ''),
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Deploy key could not be stored.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/sources/deploy-keys' && req.method === 'DELETE') {
            ...
            if (!deployKey || !connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Deploy key was not found.' }, 404)
            const affectedBindings = sourceConnections
              .listBindings({ connectionId: connection.id })
              .filter((binding) => binding.deployKeyId === deployKey.id)
            if (body.preview === true) return json({ ok: true, preview: true, affectedBindings })
          ...
          if (url.pathname === '/api/sources/repositories/sync' && req.method === 'POST') {
            ...
            try {
              return json({
                ok: true,
                repositories: await syncSourceRepositories(sourceConnections, String(body.connectionId ?? ''), {
              ...
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Repositories could not be synchronized.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/sources/references' && req.method === 'GET') {
            ...
            try {
              return json(
                await listSourceReferences(sourceConnections, {
                  connectionId: String(url.searchParams.get('connectionId') ?? ''),
                  repository: String(url.searchParams.get('repository') ?? ''),
                  repositoryId: url.searchParams.get('repositoryId') ?? undefined,
                  deployKeyId: url.searchParams.get('deployKeyId') ?? undefined,
                  type: url.searchParams.get('type') === 'tags' ? 'tags' : 'branches',
                  cursor: url.searchParams.get('cursor') ?? undefined,
                }),
              )
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'References could not be listed.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/sources/bindings' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  ok: true,
                  binding: sourceConnections.createBinding({
                    projectId: controlPlane.project.id,
                    ...
                    repositoryFullName: String(body.repositoryFullName ?? ''),
                    defaultBranch: String(body.defaultBranch ?? 'main'),
                    branchRule: typeof body.branchRule === 'string' ? body.branchRule : undefined,
                    tagRule: typeof body.tagRule === 'string' ? body.tagRule : undefined,
                    monorepoRoot: String(body.monorepoRoot ?? '.'),
                    includePaths: Array.isArray(body.includePaths) ? body.includePaths.map(String) : [],
                    excludePaths: Array.isArray(body.excludePaths) ? body.excludePaths.map(String) : [],
                    submodules: body.submodules === true,
                    cloneDepth: body.cloneDepth ? Number(body.cloneDepth) : undefined,
                    deployKeyId:
                      typeof body.deployKeyId === 'string' && body.deployKeyId ? body.deployKeyId : undefined,
                    autoDeploy: body.autoDeploy !== false,
                    pullRequestPreviews: body.pullRequestPreviews !== false,
                    actorId: principal.actor?.id,
                  }),
                },
                201,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Source binding could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/sources/bindings' && req.method === 'PATCH') {
            ...
            if (!binding || binding.projectId !== controlPlane.project.id)
              return json({ ok: false, error: 'Source binding was not found.' }, 404)
            try {
              return json({
                ok: true,
                binding: sourceConnections.updateBinding(binding.id, Number(body.version), {
                  defaultBranch: body.defaultBranch,
                  branchRule: body.branchRule,
                  tagRule: body.tagRule,
                  monorepoRoot: body.monorepoRoot,
                  includePaths: body.includePaths,
                  excludePaths: body.excludePaths,
                  submodules: body.submodules,
                  cloneDepth: body.cloneDepth,
                  deployKeyId: body.deployKeyId,
                  autoDeploy: body.autoDeploy,
                  pullRequestPreviews: body.pullRequestPreviews,
                  status: body.status,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Source binding could not be updated.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/sources/webhooks' && req.method === 'POST') {
            ...
            if (!connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Source connection was not found.' }, 404)
            try {
              const created = sourceConnections.createWebhook({
                connectionId: String(body.connectionId ?? ''),
                repositoryId: typeof body.repositoryId === 'string' ? body.repositoryId : undefined,
                repositoryFullName: String(body.repositoryFullName ?? ''),
              ...
              const webhook =
                body.reconcile !== false && baseUrl
                  ? await reconcileSourceWebhook(sourceConnections, created.webhook.id, baseUrl)
                  : created.webhook
              return json({ ok: true, webhook, endpoint, endpointRevealOnce: true }, 201)
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Source webhook could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/sources/webhooks' && req.method === 'PATCH') {
            ...
            if (!sourceWebhook || !connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Source webhook was not found.' }, 404)
            if (!baseUrl)
              return json({ ok: false, error: 'Set TS_CLOUD_WEBHOOK_BASE_URL to reconcile provider webhooks.' }, 422)
            try {
              return json({
                ok: true,
                webhook: await reconcileSourceWebhook(sourceConnections, String(body.id ?? ''), baseUrl),
              })
            } catch (error) {
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Source webhook could not be reconciled.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/sources/webhooks' && req.method === 'DELETE') {
            ...
            if (!sourceWebhook || !connection || connection.organizationId !== controlPlane.organization.id)
              return json({ ok: false, error: 'Source webhook was not found.' }, 404)
            try {
              return json({ ok: true, webhook: await removeSourceWebhook(sourceConnections, String(body.id ?? '')) })
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Source webhook could not be removed.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/security/posture' && req.method === 'GET') {
            ...
            return json({
              ...
              policies: securityPosture
                .listPolicies(scope.organizationId)
                .filter((policy) => !policy.environmentId || policy.environmentId === scope.environmentId),
              ...
              actors: controlPlane.store
                .listMemberships(controlPlane.organization.id)
                .map((membership) => controlPlane.store.getActor(membership.actorId))
                .filter(Boolean),
            })
          }
          ...
          if (url.pathname === '/api/security/scan' && req.method === 'POST') {
            const scope = securityScope(controlPlane, String(environment))
            const source = await new SecurityScannerRunner(securityPosture).run(new SecretFindingScanner(), {
              ...scope,
              artifactRoot: cwd,
            })
          ...
          if (url.pathname === '/api/security/review' && req.method === 'POST') {
            try {
              ...
              return json({
                ok: true,
                ...productionChangeReview(securityPosture, {
                  scope,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Security change review failed.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/security/policies' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const principal = organizationPrincipal(user)
            if (!principal.actor) return json({ ok: false, error: 'An organization actor is required.' }, 403)
            try {
              const policy = securityPosture.createPolicy({
                ...
                environmentId:
                  body.environmentId === null
                    ? undefined
                    : String(body.environmentId ?? securityScope(controlPlane, String(environment)).environmentId),
                name: String(body.name ?? ''),
                ...
                rules: Array.isArray(body.rules)
                  ? (body.rules.map((rule: Record<string, unknown>) => ({
                      minimumSeverity: String(rule.minimumSeverity),
                      action: String(rule.action),
                      scannerId: rule.scannerId ? String(rule.scannerId) : undefined,
                    })) as any)
                  : [],
                actorId: principal.actor.id,
              })
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Security policy could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/security/policies' && req.method === 'PATCH') {
            ...
            try {
              return json({
                ok: true,
                policy: securityPosture.updatePolicy(policy.id, Number(body.version), {
                  name: typeof body.name === 'string' ? body.name : undefined,
                  scannerFailMode:
                    body.scannerFailMode === 'open' || body.scannerFailMode === 'closed'
                      ? body.scannerFailMode
                      : undefined,
                  requiredScanners: Array.isArray(body.requiredScanners)
                    ? body.requiredScanners.map(String)
                    : undefined,
                  rules: Array.isArray(body.rules)
                    ? (body.rules.map((rule: Record<string, unknown>) => ({
                        minimumSeverity: String(rule.minimumSeverity),
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Security policy could not be updated.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/security/waivers' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  ok: true,
                  waiver: securityPosture.createWaiver({
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Security waiver could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/security/waivers' && req.method === 'DELETE') {
            ...
            if (!principal.actor) return json({ ok: false, error: 'An organization actor is required.' }, 403)
            try {
              return json({ ok: true, waiver: securityPosture.revokeWaiver(String(body.id ?? ''), principal.actor.id) })
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Security waiver could not be revoked.' },
                404,
              )
            }
          }
            ...
            try {
              const updated =
                body.action === 'acknowledge'
                  ...
                  : securityPosture.assignFinding(
                      finding.id,
                      typeof body.ownerActorId === 'string' && body.ownerActorId ? body.ownerActorId : undefined,
                      principal.actor.id,
                    )
              return json({ ok: true, finding: updated })
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Security finding could not be updated.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/security/comments' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  ok: true,
                  comment: securityPosture.addComment({
                    findingId: String(body.findingId ?? ''),
                    actorId: principal.actor.id,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Comment could not be saved.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/auth/sessions' && req.method === 'GET') {
            ...
            return json({
              currentSessionId: session?.id,
              sessions: session ? authentication.listSessions(session.identityId, { includeInactive: true }) : [],
            })
          }
          ...
          if (url.pathname === '/api/auth/sessions' && req.method === 'DELETE') {
            ...
            if (!current) return json({ ok: false, error: 'Sign in again to manage sessions.' }, 401)
            const sessionId = String(body.id ?? '')
            const revoked = authentication.revokeSession(current.identityId, sessionId)
            const headers =
              sessionId === current.id && revoked
                ? { 'set-cookie': clearSessionCookie({ secure: cookieSecure }) }
                : undefined
          ...
          if (url.pathname === '/api/auth/mfa/enroll' && req.method === 'POST') {
            ...
            const principal = organizationPrincipal(user)
            const identity = principal.actor ? authentication.getIdentityByActor(principal.actor.id) : undefined
            if (!current || !identity) return json({ ok: false, error: 'Sign in again before enabling MFA.' }, 401)
            if (!verifyPassword(String(body.password ?? ''), identity.passwordHash))
              return json({ ok: false, error: 'Current password is incorrect.' }, 401)
            try {
              const enrollment = authentication.beginTotpEnrollment(identity.id, {
                issuer: controlPlane.organization.name,
              })
              controlPlane.store.appendEvent({
                organizationId: controlPlane.organization.id,
                actorId: identity.actorId,
          ...
          if (url.pathname === '/api/auth/mfa/verify' && req.method === 'POST') {
            ...
            try {
              ...
              controlPlane.store.appendEvent({
                ...
                payload: { recoveryCodeCount: verified.recoveryCodes.length },
              })
          ...
          if (url.pathname === '/api/control-plane/operations') {
            const state = url.searchParams.get('state')
            const validState =
              state && ['queued', 'running', 'succeeded', 'failed', 'cancelled', 'timed_out'].includes(state)
          ...
          if (url.pathname === '/api/queue' && req.method === 'GET') {
            ...
            const queueState =
              requestedState &&
              ['queued', 'running', 'succeeded', 'failed', 'cancelled', 'timed_out'].includes(requestedState)
                ? (requestedState as OperationState)
                : undefined
            const operations = operationQueue
              ...
              .map((item) => {
                const resource = item.operation.resourceId
                  ? controlPlane.store.getResource(item.operation.resourceId)
                  : undefined
                const targetEnvironment = item.operation.environmentId
                  ? controlPlane.store
                      .listEnvironments(controlPlane.project.id)
                      .find((candidate) => candidate.id === item.operation.environmentId)
                  : undefined
                ...
                return {
                  ...
                  actor: actor ? { id: actor.id, name: actor.displayName, kind: actor.kind } : undefined,
                  commit: operationInput.revision ?? operationInput.source?.commitSha ?? null,
                }
              })
          ...
          if (url.pathname === '/api/queue/logs' && req.method === 'GET') {
            ...
            if (!mayAccessOperation(user, operationId, 'deployments:read'))
              return json({ ok: false, error: 'Operation was not found in this scope.' }, 404)
            const entries = operationQueue.logs(operationId, {
              after: Number(url.searchParams.get('after')) || 0,
              limit: Number(url.searchParams.get('limit')) || 500,
            })
            return json({
              entries,
              cursor: entries.at(-1)?.sequence ?? (Number(url.searchParams.get('after')) || 0),
              operation: controlPlane.store.getOperation(operationId),
            })
          }
            ...
            const stream = new ReadableStream<Uint8Array>({
              start(controller) {
                const flush = () => {
                  ...
                  if (!currentUser || !mayAccessOperation(currentUser, operationId, 'deployments:read')) {
                    closed = true
                  ...
                  for (const entry of entries) {
                    ...
                    controller.enqueue(
                      encoder.encode(`id: ${entry.sequence}\nevent: log\ndata: ${JSON.stringify(entry)}\n\n`),
                    )
                  }
                  const operation = controlPlane.store.getOperation(operationId)
                  if (
                    ...
                    operationQueue.logs(operationId, { after: cursor, limit: 1 }).length === 0
                  ) {
                    closed = true
                    if (timer) clearInterval(timer)
                    controller.enqueue(
                ...
                flush()
                if (!closed) timer = setInterval(flush, 500)
          ...
          if (url.pathname === '/api/queue/cancel' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const operationId = String(body.id ?? '')
            if (!mayAccessOperation(user, operationId, 'deployments:cancel'))
              return json({ ok: false, error: 'Operation was not found or cannot be cancelled in this scope.' }, 404)
            const actor = ensureDashboardActor(controlPlane.store, user)
            try {
              return json({ ok: true, operation: operationQueue.requestCancellation(operationId, actor.id) })
            } catch (error) {
              return json({ ok: false, error: error instanceof Error ? error.message : 'Cancellation failed.' }, 409)
            }
          }
          ...
          if (url.pathname === '/api/queue/retry' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const operationId = String(body.id ?? '')
            if (!mayAccessOperation(user, operationId, 'deployments:create'))
              return json({ ok: false, error: 'Operation was not found or cannot be retried in this scope.' }, 404)
            const actor = ensureDashboardActor(controlPlane.store, user)
            try {
              return json({
                ok: true,
                operation: operationQueue.retry(operationId, String(body.errorClass ?? 'manual'), {
                  delayMs: Number(body.delayMs) || 0,
                  actorId: actor.id,
                }),
              })
            } catch (error) {
              return json({ ok: false, error: error instanceof Error ? error.message : 'Retry failed.' }, 409)
            }
          }

          if (url.pathname === '/api/queue/settings' && req.method === 'GET')
            return json({ concurrency: operationQueue.limits })
          if (url.pathname === '/api/queue/settings' && req.method === 'PATCH') {
            const body = await readJsonBody(req)
            if (body.confirm !== 'update queue limits')
              return json(
                { ok: false, error: 'Type "update queue limits" to confirm production concurrency changes.' },
                409,
              )
            const actor = ensureDashboardActor(controlPlane.store, user)
            return json({
              ok: true,
              concurrency: operationQueue.configureConcurrency(body.concurrency ?? {}, {
                organizationId: controlPlane.organization.id,
                actorId: actor.id,
              }),
            })
          ...
          if (url.pathname === '/api/queue/history' && req.method === 'DELETE') {
            ...
            if (body.confirm !== 'clear completed')
              return json({ ok: false, error: 'Type "clear completed" to confirm retained history cleanup.' }, 409)
            const actor = ensureDashboardActor(controlPlane.store, user)
            return json({
              ok: true,
              deleted: operationQueue.clearCompleted({
                projectId: controlPlane.project.id,
          ...
          if (url.pathname === '/api/previews' && req.method === 'GET') {
            const previews = previewService.previews
              .listInstances({ projectId: controlPlane.project.id })
              .filter((item) => mayAccessResource(user, item.resourceId, 'deployments:read'))
            const definitions = previewService.previews
              .listDefinitions(controlPlane.project.id)
              .filter((item) => mayAccessResource(user, item.resourceId, 'project:read'))
            const resources = controlPlane.store
              .listResources(controlPlane.project.id)
              .filter((item) => item.kind === 'application' && mayAccessResource(user, item.id, 'project:read'))
              .map((item) => ({ id: item.id, slug: item.slug, name: item.name, environmentId: item.environmentId }))
          ...
          if (url.pathname === '/api/releases' && req.method === 'GET') {
            ...
            const resources = controlPlane.store
              .listResources(controlPlane.project.id)
              .filter((item) => mayAccessResource(user, item.id, 'deployments:read'))
            const visible = new Set(resources.map((item) => item.id))
            const releases = releaseService.releases
              .list({ projectId: controlPlane.project.id })
              .filter((item) => visible.has(item.resourceId))
              .map((item) => {
                ...
                const previous = item.previousReleaseId
                  ? releaseService.releases.get(item.previousReleaseId)
                  : undefined
                return {
                  ...item,
                  resource: resource
                    ? { id: resource.id, name: resource.name, slug: resource.slug, provider: resource.provider }
                    : undefined,
                  environment: environment
                    ? { id: environment.id, name: environment.name, slug: environment.slug, kind: environment.kind }
                    : undefined,
                  artifact: releaseService.releases.getArtifact(item.artifactId),
                  transitions: releaseService.releases.transitions(item.id),
                  approvals: releaseService.releases.approvals(item.id),
                  previous,
                  comparison: previous ? releaseService.releases.compare(previous.id, item.id) : undefined,
                  capabilities: releaseStrategyCapabilities({
                    kind: item.kind,
                    provider: resource?.provider,
                    hasHealthGate: !!item.healthGate,
                    replicas: Number((item.manifest as any)?.replicas) || 1,
                  }),
                }
              })
            return json({
          ...
          if (url.pathname === '/api/releases/action' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const release = releaseService.releases.get(String(body.id ?? ''))
            const action = String(body.action ?? '')
            if (!release || !mayAccessResource(user, release.resourceId, 'deployments:create'))
              return json({ ok: false, error: 'Release was not found in this scope.' }, 404)
            const actor = ensureDashboardActor(controlPlane.store, user)
            try {
              if (action === 'activate')
                return json({ ok: true, operation: releaseService.enqueueActivation(release, { actorId: actor.id }) })
              if (action === 'rollback') {
                const resource = controlPlane.store.getResource(release.resourceId)
                if (body.confirm !== resource?.slug)
                  return json({ ok: false, error: `Type "${resource?.slug ?? ''}" to confirm rollback.` }, 409)
                return json({
                  ok: true,
                  operation: releaseService.enqueueRollback(release, {
                    actorId: actor.id,
                    targetReleaseId: typeof body.targetReleaseId === 'string' ? body.targetReleaseId : undefined,
                  }),
                })
              }
              if (action === 'approve') {
                const environment = controlPlane.store
                  .listEnvironments(release.projectId)
                  .find((item) => item.id === release.environmentId)
                if (body.confirm !== environment?.slug)
                  return json(
                    { ok: false, error: `Type "${environment?.slug ?? ''}" to confirm the environment gate.` },
                    409,
                  )
                return json({
                  ok: true,
                  release: releaseService.releases.approve(release.id, {
                    actorId: actor.id,
              ...
              if (action === 'pin')
                return json({
                  ok: true,
                  release: releaseService.releases.pin(
                    release.id,
                    body.pinned !== false,
              ...
              if (action === 'promote') {
                const targetResource = controlPlane.store.getResource(String(body.targetResourceId ?? ''))
                const targetEnvironment = targetResource
                  ? controlPlane.store
                      .listEnvironments(release.projectId)
                      .find((item) => item.id === targetResource.environmentId)
                  : undefined
                if (
                  !targetResource ||
                  !targetEnvironment ||
                  !mayAccessResource(user, targetResource.id, 'deployments:create')
                )
                  return json({ ok: false, error: 'Promotion target was not found in this scope.' }, 404)
                if (body.confirm !== targetEnvironment.slug)
                  return json({ ok: false, error: `Type "${targetEnvironment.slug}" to confirm promotion.` }, 409)
                return json({
                  ok: true,
                  release: releaseService.releases.promote(release.id, {
                    targetEnvironmentId: targetEnvironment.id,
                    targetResourceId: targetResource.id,
                    config: body.config && typeof body.config === 'object' ? body.config : {},
                    strategy: typeof body.strategy === 'string' ? (body.strategy as any) : undefined,
                    actorId: actor.id,
              ...
              return json({ ok: false, error: error instanceof Error ? error.message : 'Release action failed.' }, 409)
            }
          }
          if (url.pathname === '/api/compose' && req.method === 'GET') {
            const applications = composeService.applications
              .list({ projectId: controlPlane.project.id })
              .filter((item) => mayAccessResource(user, item.resourceId, 'applications:read'))
              .map((item) => ({ ...item, services: composeService.applications.services(item.id) }))
            return json({
              applications,
              templates: listComposeTemplates(),
              environments: [...controlPlane.environments.values()],
            })
          }
          if (url.pathname === '/api/compose/preview' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const environmentId = String(body.environmentId ?? '')
            if (!mayAccessEnvironment(user, environmentId, 'applications:manage'))
              return json({ ok: false, error: 'Environment was not found in this scope.' }, 404)
            try {
              return json({
                ok: true,
                result: composeService.applications.preview(String(body.source ?? ''), {
                  name: String(body.name ?? ''),
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Compose could not be parsed.' },
                422,
              )
            }
          }
          if (
            (url.pathname === '/api/compose/import' || url.pathname === '/api/compose/template') &&
            req.method === 'POST'
          ) {
            const body = await readJsonBody(req)
            const environmentId = String(body.environmentId ?? '')
            ...
            const actor = ensureDashboardActor(controlPlane.store, user)
            try {
              const result = url.pathname.endsWith('/template')
                ? composeService.applications.fromTemplate(
                    ...
                    body.inputs && typeof body.inputs === 'object' ? body.inputs : {},
                    {
                      ...
                      projectId: controlPlane.project.id,
                      environmentId,
                      version: typeof body.templateVersion === 'string' ? body.templateVersion : undefined,
                      createdByActorId: actor.id,
                    },
                  )
                : composeService.applications.import(String(body.source ?? ''), {
                    name: String(body.name ?? ''),
                    slug: typeof body.slug === 'string' ? body.slug : undefined,
                    projectId: controlPlane.project.id,
                    environmentId,
              ...
              return json({ ok: true, application: result.application, diagnostics: result.parsed.diagnostics })
            } catch (error) {
              return json(
                {
                  ok: false,
                  error: error instanceof Error ? error.message : 'Compose application could not be saved.',
                },
                422,
              )
            }
          ...
          if (url.pathname === '/api/compose/action' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const application = composeService.applications.get(String(body.id ?? ''))
            const action = String(body.action ?? '') as 'deploy' | 'redeploy' | 'start' | 'stop' | 'scale' | 'delete'
            const capability: AuthorizationCapability =
              action === 'delete' || action === 'stop' ? 'deployments:cancel' : 'deployments:create'
            if (!application || !mayAccessResource(user, application.resourceId, capability))
              return json({ ok: false, error: 'Compose application was not found in this scope.' }, 404)
            try {
              const actor = ensureDashboardActor(controlPlane.store, user)
              return json({
                ok: true,
                operation: composeService.enqueue(application, action, {
                  actorId: actor.id,
                  service: typeof body.service === 'string' ? body.service : undefined,
                  replicas: Number.isFinite(Number(body.replicas)) ? Number(body.replicas) : undefined,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Compose operation could not be queued.' },
                409,
              )
            }
          }
          if (
            (url.pathname === '/api/compose/logs' || url.pathname === '/api/compose/shell') &&
            req.method === 'POST'
          ) {
            const body = await readJsonBody(req)
            const application = composeService.applications.get(String(body.id ?? ''))
            const shell = url.pathname.endsWith('/shell')
            const capability: AuthorizationCapability = shell ? 'runtime:terminal' : 'runtime:logs'
            if (!application || !mayAccessResource(user, application.resourceId, capability))
              return json({ ok: false, error: 'Compose service was not found in this scope.' }, 404)
            const serviceName = String(body.service ?? '')
            if (!application.manifest.spec.services[serviceName])
              return json({ ok: false, error: 'Compose service was not found.' }, 404)
            if (shell && body.confirm !== serviceName)
              return json({ ok: false, error: `Type "${serviceName}" to run a service command.` }, 409)
            const environmentSlug = controlPlane.store
              .listEnvironments(application.projectId)
              .find((item) => item.id === application.environmentId)?.slug as EnvironmentType | undefined
            if (!environmentSlug) return json({ ok: false, error: 'Compose environment was not found.' }, 404)
            const command = shell
              ? buildComposeShellCommand(
                  application.manifest,
                  serviceName,
                  Array.isArray(body.command) ? body.command.map(String) : ['sh'],
                )
              : buildComposeLogsCommand(application.manifest, serviceName, Number(body.lines) || 200)
          ...
          if (url.pathname === '/api/previews/definitions' && req.method === 'POST') {
            ...
            if (!mayAccessResource(user, resourceId, 'config:write'))
              return json({ ok: false, error: 'Application was not found in this scope.' }, 404)
            const actor = ensureDashboardActor(controlPlane.store, user)
            try {
              return json({
                ok: true,
                definition: previewService.previews.createDefinition({
                  ...body,
                  projectId: controlPlane.project.id,
                  resourceId,
                  baseEnvironmentId: String(body.baseEnvironmentId ?? ''),
                  domainPattern: String(body.domainPattern ?? ''),
                  createdByActorId: actor.id,
                }),
              })
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Preview policy could not be created.' },
                409,
              )
            }
          }
          if (url.pathname === '/api/previews/deploy' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const policy = previewService.previews.getDefinition(String(body.definitionId ?? ''))
            if (!policy || !mayAccessResource(user, policy.resourceId, 'deployments:create'))
              return json({ ok: false, error: 'Preview policy was not found in this scope.' }, 404)
            const actor = ensureDashboardActor(controlPlane.store, user)
            try {
              const persisted = previewService.previews.upsert({
                definitionId: policy.id,
                sourceProvider: 'dashboard',
              ...
              return json({
                ...
                operation: previewService.enqueueDeploy(persisted.preview, {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Preview could not be queued.' },
                409,
              )
            }
          }
          if (url.pathname === '/api/previews/destroy' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const preview = previewService.previews.getInstance(String(body.id ?? ''))
            if (!preview || !mayAccessResource(user, preview.resourceId, 'deployments:cancel'))
              return json({ ok: false, error: 'Preview was not found in this scope.' }, 404)
            if (body.confirm !== preview.name)
              return json({ ok: false, error: `Type "${preview.name}" to confirm tagged teardown.` }, 409)
            const actor = ensureDashboardActor(controlPlane.store, user)
            return json({ ok: true, operation: previewService.enqueueDestroy(preview, 'dashboard', actor.id) })
          }
          if (url.pathname === '/api/previews/extend' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const preview = previewService.previews.getInstance(String(body.id ?? ''))
            if (!preview || !mayAccessResource(user, preview.resourceId, 'deployments:create'))
              return json({ ok: false, error: 'Preview was not found in this scope.' }, 404)
          ...
          if (url.pathname === '/api/previews/rebuild' && req.method === 'POST') {
            ...
            return json({
              ok: true,
              operation: previewService.enqueueDeploy(preview, { actorId: actor.id, reason: 'manual_rebuild' }),
            })
          }
          if (url.pathname === '/api/previews/cleanup' && req.method === 'POST') {
            const body = await readJsonBody(req)
            if (!body.dryRun && body.confirm !== 'cleanup previews')
              return json({ ok: false, error: 'Type "cleanup previews" to confirm teardown.' }, 409)
            const actor = ensureDashboardActor(controlPlane.store, user)
            return json({
              ok: true,
              ...previewService.cleanup({
                dryRun: body.dryRun === true,
                maxAgeHours: Number(body.maxAgeHours) || undefined,
                keepCount: Number(body.keepCount) || undefined,
                actorId: actor.id,
              }),
            })
          }
          ...
          if (url.pathname === '/api/search') {
            const query = String(url.searchParams.get('q') ?? '')
              .trim()
              .slice(0, 128)
            const allowedResourceSlugs =
              scopedUser.role === 'member' ? new Set(Object.keys(scopedUser.sites)) : undefined
            const results = query
          ...
          if (url.pathname === '/api/search/preferences' && req.method === 'POST') {
            ...
            if (action === 'save-filter') {
              return json({
                savedFilter: controlPlane.store.saveFilter(
                  actorKey,
                  String(body.name ?? ''),
                  String(body.routeId ?? ''),
            ...
            if (action === 'favorite') {
              return json({
                navigation: controlPlane.store.setFavorite(
                  actorKey,
            ...
            if (action === 'visit') {
              return json({
                navigation: controlPlane.store.recordNavigation(
                  actorKey,
                  String(body.entityType ?? ''),
                  String(body.entityId ?? ''),
                ),
          ...
          if (url.pathname === '/api/organization' && req.method === 'GET') {
            const memberships = controlPlane.store
              .listMemberships(controlPlane.organization.id, { includeRevoked: true })
              .map((membership) => {
          ...
          if (url.pathname === '/api/organization/invitations' && req.method === 'POST') {
            ...
            if (
              ...
              (!verifyPassword(String(body.password ?? ''), user.passwordHash) ||
                principal.membership.roleTemplate !== 'owner')
            )
            ...
            try {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Invitation could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/organization/invitations' && req.method === 'DELETE') {
            ...
            try {
              return json({
                invitation: controlPlane.store.revokeInvitation(String(body.id ?? ''), principal.actor?.id),
              })
            } catch (error) {
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Invitation could not be revoked.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/organization/invitations/resend' && req.method === 'POST') {
            ...
            try {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Invitation could not be resent.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/organization/memberships' && req.method === 'PATCH') {
            ...
            const comparisonTarget =
              controlPlane.store.resolveAuthorizationTarget(controlPlane.organization.id, scope) ??
              controlPlane.store.resolveAuthorizationTarget(controlPlane.organization.id, {
                type: 'project',
                id: controlPlane.project.id,
              })!
            ...
            const accessDiff = {
              gained: after.filter((capability) => !before.includes(capability)),
              lost: before.filter((capability) => !after.includes(capability)),
            }
            if (body.preview === true) return json({ membership, proposed: { roleTemplate, scope }, accessDiff })
            if (String(body.confirm ?? '') !== membership.id)
              return json(
                { ok: false, error: `Confirm this change with membership ID ${membership.id}.`, accessDiff },
                409,
              )
            try {
              return json({
                membership: controlPlane.store.updateMembership({
                  id: membership.id,
                  roleTemplate,
                  scope,
                  actorId: principal.actor?.id,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Membership could not be updated.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/organization/memberships' && req.method === 'DELETE') {
            ...
            const principal = organizationPrincipal(user)
            if (
              membership.roleTemplate === 'owner' &&
              (principal.membership?.roleTemplate !== 'owner' ||
                !verifyPassword(String(body.password ?? ''), user.passwordHash))
            )
              return json(
                { ok: false, error: 'Removing an owner requires an owner to re-enter their current password.' },
                401,
              )
            try {
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Membership could not be removed.' },
                409,
              )
            }
          }
          ...
          if (url.pathname === '/api/organization/grants' && req.method === 'POST') {
            ...
            try {
              return json(
                {
                  grant: controlPlane.store.upsertGrant({
                    organizationId: controlPlane.organization.id,
              ...
              return json(
                { ok: false, error: error instanceof Error ? error.message : 'Grant could not be created.' },
                422,
              )
            }
          }
          ...
          if (url.pathname === '/api/tags' && req.method === 'POST') {
            const body = await readJsonBody(req)
            const tag = controlPlane.store.upsertTag(
              controlPlane.project.id,
              String(body.name ?? ''),
              String(body.color ?? '#5a8be0'),
            )
            if (body.resourceId) controlPlane.store.assignTag(String(body.resourceId), tag.id)
            return json({ tag }, 201)
          }
          ...
          if (url.pathname === '/' && !canOpenDashboardPage(url.pathname, scopedUser))
            return new Response(null, {
              status: 302,
              headers: { location: `/server/sites?env=${encodeURIComponent(environment)}` },
            })
          ...
          if (url.pathname === '/' && latestData?.mode === 'serverless')
            return new Response(null, {
              status: 302,
              headers: { location: `/serverless?env=${encodeURIComponent(environment)}` },
            })
          ...
          if (legacyRoute)
            return new Response(null, {
              status: 308,
              headers: { location: `${legacyRoute}?env=${encodeURIComponent(environment)}` },
            })
          ...
          if (url.pathname === '/api/env' && req.method === 'POST') {
            ...
            if (!availableEnvironments.includes(requested))
              return json(
                { ok: false, error: `Unknown environment '${requested}'.`, environments: availableEnvironments },
                404,
              )
            return json({
              ok: true,
              ...
              environments: availableEnvironments,
              href: `?env=${encodeURIComponent(requested)}`,
            })
          }

          if (url.pathname === '/api/config') {
            const sanitized = {
              ...sanitizeCloudConfig(config as CloudConfig),
              environment,
              environments: availableEnvironments,
            }
          ...
          if (url.pathname === '/api/ssh-keys' && req.method === 'POST') {
            if (!configPath) return json({ ok: false, error: 'No cloud config file was found in this checkout.' }, 404)
            const body = await readJsonBody(req)
            ...
            replaceComputeSshKeys(config as CloudConfig, [
              ...currentKeys,
              {
          ...
          if (url.pathname === '/api/ssh-keys' && req.method === 'DELETE') {
            ...
            await writeFile(configPath, after)
            replaceComputeSshKeys(
              config as CloudConfig,
              currentKeys.filter((key) => key.name !== name.trim()),
            )
            return json({ ok: true, configPath, keys: describeSshKeys(computeSshKeys(config as CloudConfig)) })
          }
          ...
          if (url.pathname === '/api/firewall' && req.method === 'POST') {
            ...
            const apply =
              body.apply === false
                ? null
                : await runServerShellCommand(config as CloudConfig, environment, `ufw allow ${port}/tcp`).catch(
          ...
          if (url.pathname === '/api/firewall' && req.method === 'DELETE') {
            ...
            const apply =
              body.apply === false
                ? null
                : await runServerShellCommand(config as CloudConfig, environment, `ufw delete allow ${port}/tcp`).catch(
                    (e: any) => ({ ok: false, error: String(e?.message ?? e) }),
                  )
            return json({ ok: true, configPath, ports, apply })
          }
          ...
          if (url.pathname === '/api/sites' && req.method === 'POST') {
            ...
            if (!siteName || !root) return json({ ok: false, error: 'Site name and root are required.' }, 422)
            const site = {
            ...
            replaceSiteConfig(
              config as CloudConfig,
              siteName,
              Object.fromEntries(Object.entries(site).filter(([, value]) => value !== undefined)),
            )
            latestData = await resolveLiveDashboardData(config as CloudConfig, environment)
            latestDataByEnvironment.set(environment, latestData)
          ...
          if (url.pathname === '/api/sites' && req.method === 'PATCH') {
            ...
            if (!name || !existing) return json({ ok: false, error: `Site '${name}' was not found.` }, 404)
            ...
            if (scopedUser.role === 'member') {
              ...
              if (!fields.ok) return json({ ok: false, error: fields.error }, 403)
              ...
              if (!conflict.ok) return json({ ok: false, error: conflict.error }, 409)
            }
            ...
            if (
              ...
              body.port !== null &&
              body.port !== '' &&
              (!Number.isInteger(Number(body.port)) || Number(body.port) < 1 || Number(body.port) > 65_535)
            )
              return json({ ok: false, error: 'Port must be a number between 1 and 65535.' }, 422)
          ...
          if (url.pathname === '/api/sites/deploy' && req.method === 'POST') {
            ...
            if (!name || !(config.sites as any)?.[name])
              return json({ ok: false, error: `Site '${name}' was not found.` }, 404)
            if (body.confirm !== name) return json({ ok: false, error: `Type "${name}" to deploy this site.` }, 409)
            const postureScope = securityScope(controlPlane, String(environment))
            await new SecurityScannerRunner(securityPosture).run(new SecretFindingScanner(), {
              ...postureScope,
              artifactRoot: cwd,
            })
            recordDashboardHostPosture(securityPosture, postureScope, latestData ?? {})
            const securityReview = productionChangeReview(securityPosture, {
              scope: postureScope,
              desiredConfigHash: controlPlane.project.desiredConfigHash,
            })
            if (securityReview.decision.outcome === 'block') {
              return json(
                {
                  ok: false,
                  error: `Deployment blocked by security policy: ${securityReview.decision.explanation}`,
                  securityReview,
                },
                409,
              )
            ...
            const tracked = await trackDashboardOperation({
              ...
              input: { site: name },
              execute: () =>
                runDashboardAction(action, { cwd, cliEntry }) as Promise<{ ok: boolean } & Record<string, any>>,
            })
            return json({ ...tracked.result, controlPlaneOperation: tracked.operation, securityReview })
          }

          if (url.pathname === '/api/actions') return json(dashboardActions(environment))
          ...
          if (url.pathname === '/api/runtime/logs' && req.method === 'GET') {
            ...
            if (!workloadId) return json({ ok: false, error: 'A workload query parameter is required.' }, 422)
            const limit = Math.min(1000, Math.max(1, Number(url.searchParams.get('limit')) || 200))
            try {
              return json(
                await new RuntimeOperationService(config as CloudConfig, environment).logs(workloadId, { limit }),
              )
            } catch (error: any) {
          ...
          if (url.pathname === '/api/runtime/log-sessions' && req.method === 'POST') {
            ...
            if (!workload.capabilities.logs.supported)
              return json(
                { ok: false, error: workload.capabilities.logs.reason ?? 'Logs are unsupported for this workload.' },
                409,
              )
            const service = new RuntimeOperationService(config as CloudConfig, environment, {
            ...
            void (async () => {
              ...
              try {
                for (let attempt = 0; attempt < 200 && !runtimeStreams.signal(session.id)?.aborted; attempt++) {
                  const result = await service.logs(workloadId, {
                  ...
                  for (const line of result.lines) {
                    ...
                    if (line.timestamp)
                      since = new Date(Math.max(since.getTime(), new Date(line.timestamp).getTime() + 1))
                  }
            ...
            return json(
              {
                ok: true,
                session,
                streamUrl: `/api/runtime/log-stream?id=${encodeURIComponent(session.id)}&workload=${encodeURIComponent(workloadId)}`,
              },
              201,
            )
          }
          ...
          if (
            (url.pathname === '/api/runtime/log-stream' || url.pathname === '/api/runtime/exec-stream') &&
            req.method === 'GET'
          ) {
            ...
            if (!session || session.kind !== expectedKind)
              return json({ ok: false, error: 'Runtime stream was not found in this workload scope.' }, 404)
            return runtimeEventStream(runtimeStreams, session, after, req.signal)
          }

          if (
            (url.pathname === '/api/runtime/log-sessions' || url.pathname === '/api/runtime/exec-sessions') &&
            req.method === 'DELETE'
          ) {
            const body = await readJsonBody(req)
          ...
          if (url.pathname === '/api/runtime/exec-sessions' && req.method === 'POST') {
            ...
            void trackDashboardOperation({
              ...
              input: {
                ...
                freeForm: !!String(body.command ?? '').trim(),
              },
              execute: () =>
                service.exec({
                  workloadId,
                  preset: ['process', 'sockets', 'filesystem'].includes(String(body.preset))
                    ? (String(body.preset) as 'process' | 'sockets' | 'filesystem')
                    : undefined,
                  command: typeof body.command === 'string' ? body.command : undefined,
                  confirm: String(body.confirm ?? ''),
                  recentAuth:
                    !authEnabled || (!!sessionRecord && authentication.isRecentlyAuthenticated(sessionRecord)),
                }),
            })
            ...
            return json(
              {
                ok: true,
                session: stream,
                streamUrl: `/api/runtime/exec-stream?id=${encodeURIComponent(stream.id)}&workload=${encodeURIComponent(workloadId)}`,
              },
              202,
            )
          }
          ...
          if (url.pathname === '/api/runtime/files/read' && req.method === 'POST') {
            ...
            const inventory = await resolveRuntimeInventory(config as CloudConfig, environment)
            const sessionRecord = guard.resolveSession(req)
            const result = await new RuntimeOperationService(config as CloudConfig, environment, {
              inventory: async () => inventory,
            }).readFile({
          ...
          if (url.pathname === '/api/runtime/files/write' && req.method === 'POST') {
            ...
            const tracked = await trackDashboardOperation({
              ...
              input: {
                ...
                provider: workload.provider,
                path: String(body.path ?? ''),
                bytes: Buffer.from(String(body.contentBase64 ?? ''), 'base64').byteLength,
              },
              execute: () =>
                service.writeFile({
                  workloadId,
          ...
          if (url.pathname === '/api/runtime/operations' && req.method === 'POST') {
            ...
            const tracked = await trackDashboardOperation({
              ...
              input: {
                ...
                provider: workload.provider,
                action,
                replicas: body.replicas == null ? null : Number(body.replicas),
              },
              execute: () =>
                service.run({
                  workloadId,
          ...
          if (
            ...
            const readableResources =
              user.role === 'admin' ||
              (!!environmentRecord && mayAccessEnvironment(user, environmentRecord.id, 'runtime:read'))
                ? resources
                : resources.filter((resource) => mayAccessResource(user, resource.id, 'runtime:read'))
            const visibleResource = (resourceId?: string) =>
              !resourceId || readableResources.some((resource) => resource.id === resourceId)
            const resolveResource = (value: unknown): string | undefined => {
              if (!value) return undefined
              const resource = resources.find((item) => item.id === value || item.slug === value)
              if (!resource) throw new Error('Alert resource was not found.')
              if (!visibleResource(resource.id)) throw new Error('Alert resource is outside your authorized scope.')
              return resource.id
            }
            const actor = organizationPrincipal(user).actor

            if (url.pathname === '/api/health/checks' && req.method === 'GET')
              return json({
                ok: true,
                checks: alertStore
                  .listHealthChecks(controlPlane.project.id, environmentRecord?.id)
                  .filter((check) => visibleResource(check.resourceId))
                  .map((check) => ({ ...check, results: alertStore.listHealthResults(check.id, 20) })),
              })
            if (url.pathname === '/api/health/checks' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const kind = String(body.kind ?? 'http') as import('../alerts').HealthCheckKind
              if (kind === 'command' && user.role !== 'admin')
                return json({ ok: false, error: 'Command health checks require an administrator.' }, 403)
              const config = {
                method: String(body.config?.method ?? 'GET')
                  .toUpperCase()
                  .slice(0, 10),
                path: body.config?.path ? String(body.config.path).slice(0, 500) : undefined,
                port: body.config?.port == null ? undefined : Number(body.config.port),
                command: Array.isArray(body.config?.command) ? body.config.command.map(String).slice(0, 20) : undefined,
                expectedStatuses: Array.isArray(body.config?.expectedStatuses)
                  ? body.config.expectedStatuses.map(Number).filter(Number.isInteger).slice(0, 20)
                  : undefined,
                expectedBody: body.config?.expectedBody ? String(body.config.expectedBody).slice(0, 500) : undefined,
              }
              return json(
                {
                  ok: true,
                  check: alertStore.createHealthCheck({
                    projectId: controlPlane.project.id,
                    environmentId: environmentRecord?.id,
                    resourceId: resolveResource(body.resourceId ?? body.resource),
                    name: String(body.name ?? ''),
                    kind,
                    target: String(body.target ?? ''),
                    config,
                    intervalSeconds: Number(body.intervalSeconds) || 60,
                    timeoutSeconds: Number(body.timeoutSeconds) || 10,
                    failureThreshold: Number(body.failureThreshold) || 3,
                    recoveryThreshold: Number(body.recoveryThreshold) || 2,
                    regions: Array.isArray(body.regions) ? body.regions.map(String) : ['local'],
                    enabled: body.enabled !== false,
                  }),
                },
                201,
            ...
            if (url.pathname === '/api/health/checks' && req.method === 'PATCH') {
              const body = await readJsonBody(req)
              const current = alertStore.getHealthCheck(String(body.id ?? ''))
              if (!current || !visibleResource(current.resourceId))
                return json({ ok: false, error: 'Health check was not found.' }, 404)
              return json({ ok: true, check: alertStore.setHealthCheckEnabled(current.id, body.enabled !== false) })
            }
            if (url.pathname === '/api/health/checks/test' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const check = alertStore.getHealthCheck(String(body.id ?? ''))
              if (!check || !visibleResource(check.resourceId))
                return json({ ok: false, error: 'Health check was not found.' }, 404)
              return json({
                ok: true,
                productionIncidentCreated: false,
                result: await new HealthCheckRunner(alertStore).probe(check),
              })
            }
            if (url.pathname === '/api/health/checks/run' && req.method === 'POST') {
              ...
              const deliveries = []
              for (const evaluation of outcome.evaluations.filter((item) => item.notify && item.alert)) {
                const queued = notificationRouter.enqueue(
                  controlPlane.organization.id,
                  evaluation.alert!,
                  evaluation.transition as 'firing' | 'resolved',
                )
                deliveries.push(...(await notificationRouter.deliverAll(queued)))
              }
            ...
            if (url.pathname === '/api/alerts/rules' && req.method === 'GET')
              return json({
                ok: true,
                rules: alertStore
                  .listRules(controlPlane.project.id, environmentRecord?.id)
                  .filter((rule) => visibleResource(rule.resourceId)),
              })
            if (url.pathname === '/api/alerts/rules' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const healthCheck = body.healthCheckId ? alertStore.getHealthCheck(String(body.healthCheckId)) : undefined
              if (body.healthCheckId && (!healthCheck || !visibleResource(healthCheck.resourceId)))
                return json({ ok: false, error: 'Health check was not found.' }, 404)
              return json(
                {
                  ok: true,
                  rule: alertStore.createRule({
                    projectId: controlPlane.project.id,
                    environmentId: environmentRecord?.id,
                    resourceId: resolveResource(body.resourceId ?? body.resource) ?? healthCheck?.resourceId,
                    healthCheckId: healthCheck?.id,
                    name: String(body.name ?? ''),
                    signal: String(body.signal ?? (healthCheck ? 'health' : '')),
                    operator: String(body.operator ?? (healthCheck ? 'unhealthy' : 'gt')) as any,
                    threshold: body.threshold == null ? undefined : Number(body.threshold),
                    recoveryThreshold: body.recoveryThreshold == null ? undefined : Number(body.recoveryThreshold),
                    windowMs: Number(body.windowMs) || 300_000,
                    consecutive: Number(body.consecutive) || healthCheck?.failureThreshold || 3,
                    recoveryConsecutive: Number(body.recoveryConsecutive) || healthCheck?.recoveryThreshold || 2,
                    noDataPolicy: ['ignore', 'pending', 'firing'].includes(String(body.noDataPolicy))
                      ? body.noDataPolicy
                      : 'pending',
                    severity: ['info', 'warning', 'critical'].includes(String(body.severity))
                      ? body.severity
                      : 'warning',
            ...
            if (url.pathname === '/api/alerts/rules' && req.method === 'PATCH') {
              const body = await readJsonBody(req)
              const current = alertStore.getRule(String(body.id ?? ''))
              if (!current || !visibleResource(current.resourceId))
                return json({ ok: false, error: 'Alert rule was not found.' }, 404)
              return json({ ok: true, rule: alertStore.setRuleEnabled(current.id, body.enabled !== false) })
            }
            if (url.pathname === '/api/alerts/evaluate' && req.method === 'POST')
              return json({ ok: true, ...(await evaluateAlerts(environmentRecord?.id)) })
            if (url.pathname === '/api/alerts' && req.method === 'GET')
              return json({
                ok: true,
                alerts: alertStore
                  .listAlerts(controlPlane.project.id, {
                    environmentId: environmentRecord?.id,
                    states: url.searchParams.getAll('state').length ? url.searchParams.getAll('state') : undefined,
                    limit: Number(url.searchParams.get('limit')) || 200,
                  })
                  .filter((alert) => visibleResource(alert.resourceId))
                  .map((alert) => ({
                    ...alert,
                    deliveries: alertStore.listDeliveries({ alertId: alert.id, limit: 50 }),
                  })),
              })
            if (url.pathname === '/api/alerts/action' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const current = alertStore.getAlert(String(body.id ?? ''))
              if (!current || !visibleResource(current.resourceId))
                return json({ ok: false, error: 'Alert was not found.' }, 404)
              const action = String(body.action ?? '')
              if (action === 'acknowledge')
                return json({ ok: true, alert: alertStore.acknowledge(current.id, actor?.id ?? '') })
              if (action === 'assign')
                return json({
                  ok: true,
                  alert: alertStore.assign(
                    current.id,
                    body.ownerActorId === 'me' ? actor?.id : body.ownerActorId ? String(body.ownerActorId) : undefined,
                    actor?.id,
                  ),
                })
              if (action === 'silence') {
                if (user.role !== 'admin')
                  return json({ ok: false, error: 'Silencing production alerts requires an administrator.' }, 403)
                const session = guard.resolveSession(req)
                if (authEnabled && (!session || !authentication.isRecentlyAuthenticated(session)))
                  return json(
                    { ok: false, error: 'Recent authentication is required to silence alerts.', stepUpRequired: true },
                    401,
                  )
                return json({
                  ok: true,
                  alert: alertStore.silenceAlert(current.id, String(body.until ?? ''), actor?.id),
                })
              }
              return json({ ok: false, error: 'Action must be acknowledge, assign, or silence.' }, 422)
            }
            if (url.pathname === '/api/alerts/silences' && req.method === 'POST') {
              const body = await readJsonBody(req)
              return json(
                {
                  ok: true,
                  id: alertStore.createSilence({
                    projectId: controlPlane.project.id,
                    environmentId: environmentRecord?.id,
                    resourceId: resolveResource(body.resourceId ?? body.resource),
            ...
            if (url.pathname === '/api/notifications/channels' && req.method === 'GET')
              return json({ ok: true, channels: alertStore.listChannels(controlPlane.organization.id) })
            if (url.pathname === '/api/notifications/channels' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const kind = String(body.kind ?? '') as import('../alerts').NotificationChannelKind
              const allowedConfig =
                kind === 'email'
                  ? { to: body.config?.to, from: body.config?.from }
                  : kind === 'telegram'
                    ? { chatId: String(body.config?.chatId ?? '') }
                    : {}
              return json(
                {
                  ok: true,
                  channel: alertStore.createChannel({
                    organizationId: controlPlane.organization.id,
                    name: String(body.name ?? ''),
                    kind,
                    config: allowedConfig,
                    credential: body.credential ? String(body.credential) : undefined,
                    actorId: actor?.id,
                  }),
                },
                201,
            ...
            if (url.pathname === '/api/notifications/channels' && req.method === 'PATCH') {
              const body = await readJsonBody(req)
              const id = String(body.id ?? '')
              if (body.credential)
                return json({ ok: true, channel: alertStore.rotateChannel(id, String(body.credential)) })
              const status = String(body.status ?? 'paused')
              if (!['active', 'paused', 'disabled'].includes(status))
                return json({ ok: false, error: 'Channel status must be active, paused, or disabled.' }, 422)
              return json({ ok: true, channel: alertStore.setChannelStatus(id, status as any) })
            }
            if (url.pathname === '/api/notifications/channels/test' && req.method === 'POST') {
              const body = await readJsonBody(req)
              return json({
                ok: true,
                productionIncidentCreated: false,
                result: await notificationRouter.testChannel(String(body.id ?? '')),
              })
            }
            if (url.pathname === '/api/notifications/routes' && req.method === 'GET')
              return json({ ok: true, routes: alertStore.listRoutes(controlPlane.organization.id) })
            if (url.pathname === '/api/notifications/routes' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const channelIds = Array.isArray(body.channelIds) ? body.channelIds.map(String) : []
              if (channelIds.some((id) => alertStore.getChannel(id)?.organizationId !== controlPlane.organization.id))
                return json({ ok: false, error: 'Route contains an unavailable channel.' }, 422)
              return json(
                {
                  ok: true,
                  route: alertStore.createRoute({
                    organizationId: controlPlane.organization.id,
                    name: String(body.name ?? ''),
                    priority: Number(body.priority) || 0,
                    matcher: body.matcher && typeof body.matcher === 'object' ? body.matcher : {},
                    channelIds,
                    quietHours: body.quietHours,
                    groupWaitSeconds: Number(body.groupWaitSeconds) || 30,
                    reminderSeconds: body.reminderSeconds == null ? undefined : Number(body.reminderSeconds),
                    escalation: Array.isArray(body.escalation) ? body.escalation : [],
            ...
            if (url.pathname === '/api/notifications/routes' && req.method === 'PATCH') {
              const body = await readJsonBody(req)
              const route = alertStore.getRoute(String(body.id ?? ''))
              if (!route || route.organizationId !== controlPlane.organization.id)
                return json({ ok: false, error: 'Notification route was not found.' }, 404)
              return json({ ok: true, route: alertStore.setRouteEnabled(route.id, body.enabled !== false) })
            }
            if (url.pathname === '/api/notifications/routes/preview' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const existing = body.alertId ? alertStore.getAlert(String(body.alertId)) : undefined
              const preview =
                existing ??
                ({
                  id: 'preview',
                  ...
                  resourceId: resolveResource(body.resourceId ?? body.resource),
                  dedupKey: 'preview',
                  groupKey: String(body.groupKey ?? 'default'),
                  state: 'firing',
                  severity: ['info', 'warning', 'critical'].includes(String(body.severity)) ? body.severity : 'warning',
                  title: String(body.title ?? 'Rule preview'),
                  evidence: {},
                  failureCount: 1,
                  recoveryCount: 0,
                  occurrenceCount: 1,
                  ...
                  updatedAt: new Date().toISOString(),
                } as import('../alerts').Alert)
              return json({
                ok: true,
                productionIncidentCreated: false,
                matches: notificationRouter
                  .preview(controlPlane.organization.id, preview, String(body.eventType ?? 'firing'))
                  .map((item) => ({ route: item.route, channels: item.channels, quiet: item.quiet })),
              })
            }
            if (url.pathname === '/api/notifications/deliveries' && req.method === 'GET')
              return json({
                ok: true,
                deliveries: alertStore.listDeliveries({
                  alertId: url.searchParams.get('alertId') ?? undefined,
                  states: url.searchParams.getAll('state'),
                  limit: Number(url.searchParams.get('limit')) || 200,
                }),
              })
            if (url.pathname === '/api/notifications/deliveries/retry' && req.method === 'POST') {
              const body = await readJsonBody(req)
              const current = alertStore.getDelivery(String(body.id ?? ''))
              if (!current) return json({ ok: false, error: 'Delivery was not found.' }, 404)
              alertStore.updateDelivery(current.id, {
                state: 'retrying',
                attempt: Math.max(0, current.attempt - 1),
                nextAttemptAt: new Date().toISOString(),
              })
              return json({ ok: true, delivery: await notificationRouter.deliver(current.id) })
            }
          }
          ...
          if (url.pathname.startsWith('/api/jobs') || url.pathname.startsWith('/api/workers')) {
            const environmentRecord = controlPlane.environments.get(environment)
            const resources = controlPlane.store.listResources(controlPlane.project.id, environmentRecord?.id)
            const environmentWide =
              user.role === 'admin' ||
              (!!environmentRecord && mayAccessEnvironment(user, environmentRecord.id, 'automation:read'))
            const readableResources = environmentWide
              ? resources
              : resources.filter((resource) => mayAccessResource(user, resource.id, 'automation:read'))
            const visibleResource = (resourceId?: string) =>
              !resourceId || readableResources.some((resource) => resource.id === resourceId)
            const actor = organizationPrincipal(user).actor
            ...
            const schedulableOperations = operations.filter(
              (item) => !item.danger && ['scheduler', 'backup', 'worker'].includes(item.group),
            )
            ...
            const audit = (type: string, payload: Record<string, JsonValue>, resourceId?: string) =>
              controlPlane.store.appendEvent({
                organizationId: controlPlane.organization.id,
                projectId: controlPlane.project.id,
                resourceId,
              ...
              try {
                return json({
                  ...
                  preview: previewSchedule(
                    ...
                    new Date(body.from ? String(body.from) : Date.now()),
                    Number(body.count) || 5,
                  ),
                })
              } catch (error) {
                return json(
                  {
                    ok: false,
                    error: error instanceof Error ? error.message : 'Schedule preview could not be generated.',
                    productionExecutionCreated: false,
                  },
                  422,
                )
            ...
            if (url.pathname === '/api/jobs' && req.method === 'GET') {
              return json({
                ok: true,
                jobs: jobStore
                  .list(controlPlane.project.id, { environmentId: environmentRecord?.id })
                  .filter((item) => visibleResource(item.resourceId))
                  .map((item) => ({
                    ...item,
                    preview: previewSchedule(item.normalizedExpression, item.timezone, new Date(), 5),
                    capability: jobProviderCapability(item),
                    lastExecution: jobStore.listExecutions(item.id, 1)[0],
                  })),
              })
            }
            ...
            if (url.pathname === '/api/jobs' && req.method === 'POST') {
              ...
              const provider = String(
                body.provider ?? (deploymentMode === 'serverless' ? 'eventbridge' : 'server'),
              ) as import('../jobs').JobProvider
              const expectedProvider = deploymentMode === 'serverless' ? 'eventbridge' : 'server'
              if (provider !== expectedProvider)
                return json(
                  {
                    ok: false,
                    error: `${deploymentMode} environments accept ${expectedProvider} schedules through this adapter.`,
                  },
                  422,
                )
              ...
              if (
                provider === 'server' &&
                (!operationId || !schedulableOperations.some((item) => item.id === operationId))
              )
                return json({ ok: false, error: 'Server jobs require an allowlisted operation target.' }, 422)
              const providerCapability = jobProviderCapability({
                provider,
                ...
                overlapPolicy: ['allow', 'forbid', 'replace'].includes(String(body.overlapPolicy))
                  ? body.overlapPolicy
                  : 'forbid',
              })
              if (!providerCapability.supported)
                return json({ ok: false, error: providerCapability.notes.join(' ') }, 422)
              const resource = body.resourceId
                ? resources.find((item) => item.id === body.resourceId || item.slug === body.resourceId)
                : undefined
              ...
              try {
                ...
                const created = jobStore.create({
                  ...
                  retryPolicy: {
                    ...
                    backoffSeconds: Number(body.backoffSeconds) || 30,
                    deadLetterRef: body.deadLetterRef ? String(body.deadLetterRef) : undefined,
                  },
                ...
                audit(
                  'created',
                  { jobId: created.id, provider: created.provider, expression: created.normalizedExpression },
                  created.resourceId,
                )
                return json({ ok: true, job: created, capability: providerCapability }, 201)
              } catch (error) {
                return json(
                  { ok: false, error: error instanceof Error ? error.message : 'Scheduled job could not be created.' },
                  422,
                )
              }
            }
            ...
            if (url.pathname === '/api/jobs' && req.method === 'PATCH') {
              ...
              if (!current || !visibleResource(current.resourceId))
                return json({ ok: false, error: 'Scheduled job was not found.' }, 404)
              if (current.origin === 'config')
                return json(
                  { ok: false, error: 'Config-defined schedules must be edited in cloud.config.ts and reconciled.' },
                  409,
                )
              ...
              if (missedRunPolicy && !['skip', 'catch_up'].includes(missedRunPolicy))
                return json({ ok: false, error: 'Missed-run policy must be skip or catch_up.' }, 422)
              if (overlapPolicy && !['allow', 'forbid', 'replace'].includes(overlapPolicy))
                return json({ ok: false, error: 'Overlap policy must be allow, forbid, or replace.' }, 422)
              try {
                ...
                audit(
                  'updated',
                  { jobId: updated.id, expression: updated.normalizedExpression, version: updated.version },
                  updated.resourceId,
                )
                return json({ ok: true, job: updated })
              } catch (error) {
                return json(
                  { ok: false, error: error instanceof Error ? error.message : 'Scheduled job could not be updated.' },
                  422,
                )
              }
            }
            ...
            if (url.pathname === '/api/jobs/action' && req.method === 'POST') {
              ...
              if (['disable', 'delete'].includes(action) && environmentRecord?.kind === 'production') {
                if (String(body.confirm ?? '') !== action)
                  return json({ ok: false, error: `Type "${action}" to confirm this production change.` }, 409)
                if (!recent())
                  return json(
                    {
                      ok: false,
                      error: 'Recent authentication is required for this production change.',
                      stepUpRequired: true,
                    },
                    401,
                  )
              ...
              if (action === 'delete') {
                try {
                  ...
                  return json(
                    {
                      ok: false,
                      error: error instanceof Error ? error.message : 'Scheduled job could not be deleted.',
                    },
                    409,
                  )
                }
            ...
            if (url.pathname === '/api/jobs/history' && req.method === 'GET') {
              ...
              return json({
                ok: true,
                executions: jobStore
                  .listExecutions(current.id, Number(url.searchParams.get('limit')) || 100)
                  .map((item) => ({
                    ...item,
                    logs: item.operationId ? operationQueue.logs(item.operationId, { limit: 500 }) : [],
                  })),
              })
            }
            ...
            if (url.pathname === '/api/jobs/reconcile' && req.method === 'POST') {
              ...
              const reconciliation = synchronizeConfiguredJobs(jobStore, config as CloudConfig, {
                organization: controlPlane.organization,
                project: controlPlane.project,
                environment: environmentRecord,
                resources,
            ...
            if (url.pathname === '/api/workers' && req.method === 'GET') {
              const observedWorkers = Array.isArray(currentData.workersDetail ?? currentData.workers)
                ? (currentData.workersDetail ?? currentData.workers)
                : []
              return json({
                ok: true,
                workers: jobStore
                  .listWorkers(controlPlane.project.id, environmentRecord?.id)
                  .filter((item) => visibleResource(item.resourceId))
                  .map((item) => {
                    const observed = observedWorkers.find(
                      (candidate: any) => candidate.name === item.name || candidate.site === item.target.site,
                    )
                    return observed
                      ? {
                          ...item,
                          observedState: {
                            ...
                            currentJob: observed.currentJob ?? null,
                            failures: Number(observed.failed ?? observed.failures) || 0,
                            processed: Number(observed.processed) || 0,
                          },
                        }
                      : item
                  }),
            ...
            if (url.pathname === '/api/workers/action' && req.method === 'POST') {
              ...
              if (!current || !visibleResource(current.resourceId))
                return json({ ok: false, error: 'Worker was not found.' }, 404)
              const action = String(body.action ?? '')
              if (action === 'restart') {
                if (String(body.confirm ?? '') !== current.name)
                  return json({ ok: false, error: `Type "${current.name}" to restart this worker.` }, 409)
                const operation = operations.find((item) => item.id === current.target.operationId)
                if (!operation)
                  return json({ ok: false, error: 'The allowlisted worker restart operation is unavailable.' }, 409)
                const result = await runDashboardOperation(config as CloudConfig, environment, operation)
                const worker = jobStore.reconcileWorker(current.id, result.ok ? 'in_sync' : 'unavailable', {
                ...
                return json(
                  {
                    ok: false,
                    error:
                      current.provider === 'systemd'
                        ? 'Systemd worker scale is config-managed; edit the worker definition and reconcile.'
                        : 'This provider does not expose safe worker scaling through the current adapter.',
                  },
                  422,
                )
          ...
          if (url.pathname.startsWith('/api/telemetry/')) {
            ...
            const telemetryReadCapability: AuthorizationCapability = [
              '/api/telemetry/query',
              '/api/telemetry/tail',
              '/api/telemetry/export',
              '/api/telemetry/saved-queries',
            ...
            const environmentWide =
              user.role === 'admin' ||
              (!!environmentRecord && mayAccessEnvironment(user, environmentRecord.id, telemetryReadCapability))
            const readableResources = environmentWide
              ? allResources
              : allResources.filter((resource) => mayAccessResource(user, resource.id, telemetryReadCapability))
            const resourceIds = (value: unknown): string[] | undefined =>
              resolveTelemetryResourceIds(readableResources, environmentWide, value)
            const queryFrom = (input: Record<string, any>): TelemetryQuery => {
              ...
              const from = String(
                input.from ??
                  new Date(
                    new Date(to).getTime() - (ranges[String(input.range ?? '1h')] ?? ranges['1h']),
                  ).toISOString(),
              )
              ...
              const kinds = list(input.kinds)?.filter((kind) =>
                ['metric', 'log', 'trace', 'request', 'event'].includes(kind),
              ) as TelemetryKind[] | undefined
              return {
                projectId: controlPlane.project.id,
                environmentId: environmentRecord?.id,
                resourceIds: resourceIds(input.resources),
                kinds,
                ...
                from,
                to,
                text: String(input.text ?? '') || undefined,
                traceId: String(input.traceId ?? '') || undefined,
                requestId: String(input.requestId ?? '') || undefined,
                deploymentId: String(input.deploymentId ?? '') || undefined,
                releaseId: String(input.releaseId ?? '') || undefined,
                workloadId: String(input.workloadId ?? '') || undefined,
                cursor: String(input.cursor ?? '') || undefined,
            ...
            if (url.pathname === '/api/telemetry/status' && req.method === 'GET') {
              ...
              const scopedStatuses = environmentWide
                ...
                : telemetry.status(
                    ...
                    collection.policy.rawDays,
                    collection.policy.samplingRate,
                    readableResources.map((resource) => resource.id),
                  )
              const estimatedMonthlyBytes = scopedStatuses.reduce(
                (sum, source) => sum + source.estimatedDailyBytes * 30,
                0,
              )
              return json({
                ok: true,
                ...
                sources: scopedStatuses,
                errors: environmentWide ? collection.errors : [],
                policy: collection.policy,
                retention: {
                  rawDays: collection.policy.rawDays,
                  downsampleAfterDays: collection.policy.downsampleAfterDays,
                  downsampleBucketMs: collection.policy.downsampleBucketMs,
                  maxRecords: collection.policy.maxRecords,
                },
                estimatedMonthlyBytes,
                estimatedMonthlyCostUsd: telemetryEstimatedMonthlyCost(estimatedMonthlyBytes, collection.policy),
                costEstimateConfigured: collection.policy.estimatedStorageUsdPerGbMonth > 0,
              })
            }
            if (url.pathname === '/api/telemetry/settings' && req.method === 'GET')
              return json({ ok: true, policy: loadTelemetryPolicy(controlPlane.store, controlPlane.project.id) })
            ...
            if (url.pathname === '/api/telemetry/query' && req.method === 'GET') {
              ...
              return json({
                ...
                ...telemetry.query(queryFrom(urlInput)),
              })
            }
            if (url.pathname === '/api/telemetry/tail' && req.method === 'GET') {
              ...
              const baseQuery = queryFrom({
                ...urlInput,
                kinds: urlInput.kinds || 'log,request,event,trace',
                from: urlInput.from || new Date(initialTo.getTime() - 120_000).toISOString(),
                to: initialTo.toISOString(),
                limit: Math.min(1_000, Number(urlInput.limit) || 200),
                cursor: undefined,
              })
              ...
              const stream = new ReadableStream<Uint8Array>({
                start(controller) {
                  ...
                  const poll = async () => {
                    ...
                    polling = true
                    try {
                      const result = telemetry.tail(
                        { ...baseQuery, to: new Date(Date.now() + 1_000).toISOString() },
                        position,
                      )
            ...
            if (url.pathname === '/api/telemetry/series' && req.method === 'GET') {
              ...
              const aggregation = (
                ['sum', 'avg', 'min', 'max', 'count', 'p50', 'p90', 'p95', 'p99'].includes(String(urlInput.aggregation))
                  ...
                  : 'avg'
              ) as TelemetryAggregation
              const query = queryFrom({ ...urlInput, kinds: 'metric', limit: 5_000 })
              const series = telemetry.series({
                ...query,
                bucketMs: Number(urlInput.bucketMs) || 60_000,
                aggregation,
                timezone: String(urlInput.timezone ?? 'UTC'),
                compareFrom: urlInput.compareFrom,
                compareTo: urlInput.compareTo,
              })
              return json({
                ok: true,
                collection: { generatedAt: collection.generatedAt, cached: collection.cached },
            ...
            if (url.pathname === '/api/telemetry/refresh' && req.method === 'POST')
              return json({ ok: true, ...(await collect(true)) })
            if (url.pathname === '/api/telemetry/export' && req.method === 'POST') {
              const body = await readJsonBody(req)
              await collect(false)
              const result = telemetry.query({
                ...queryFrom(body),
                limit: Math.min(5_000, Number(body.limit) || 5_000),
                cursor: undefined,
              })
              const format = body.format === 'csv' ? 'csv' : 'json'
              if (format === 'json')
                return new Response(
                  JSON.stringify({ exportedAt: new Date().toISOString(), query: queryFrom(body), ...result }, null, 2),
                  {
                    headers: {
                      'content-disposition': 'attachment; filename="ts-cloud-telemetry.json"',
              ...
              return new Response(csv, {
                headers: {
                  'content-disposition': 'attachment; filename="ts-cloud-telemetry.csv"',
            ...
            if (url.pathname === '/api/telemetry/saved-queries' && req.method === 'GET') {
              ...
              return json({
                ok: true,
                queries: telemetry.listSavedQueries(controlPlane.project.id, principal.actor?.id),
              })
            }
            if (url.pathname === '/api/telemetry/saved-queries' && req.method === 'POST') {
              ...
              return json(
                {
                  ok: true,
                  query: telemetry.saveQuery(
                    controlPlane.project.id,
                    principal.actor?.id,
                    String(body.name ?? ''),
                    queryFrom(body.query ?? body),
                  ),
                },
                201,
              )
            ...
            if (url.pathname === '/api/telemetry/saved-queries' && req.method === 'DELETE') {
              const body = await readJsonBody(req)
              const principal = organizationPrincipal(user)
              const removed = telemetry.deleteSavedQuery(
                controlPlane.project.id,
                principal.actor?.id,
                String(body.id ?? ''),
              )
          ...
          if (url.pathname.startsWith('/api/configuration')) {
            ...
            if (url.pathname === '/api/configuration' && req.method === 'GET') {
              ...
              const kind = ['variable', 'secret'].includes(String(query.kind))
                ? (query.kind as 'variable' | 'secret')
                : undefined
              const preview = scope?.type === 'preview' ? previewService.previews.getInstance(scope.id) : undefined
              const definition = preview ? previewService.previews.getDefinition(preview.definitionId) : undefined
              const effective = scope
                ? await configurationService.resolve({
                    projectId: controlPlane.project.id,
                    environmentId: scope.environmentId,
                    resourceId: scope.resourceId,
                    functionId: scope.type === 'function' ? scope.id : undefined,
                    previewId: scope.previewId,
                    trustedPreview: !!preview && !preview.fork,
                    allowedPreviewSecrets: definition?.inheritedSecrets,
                    nativeReferences: true,
              ...
              return json(
                {
                  ok: true,
                  entries: configurationService.list({
                    projectId: controlPlane.project.id,
                    scope,
                    kind,
                    search: query.search,
                  ...
                  effectiveEntries: effective
                    ? Object.values(effective.entries)
                        .filter((entry) => !kind || entry.kind === kind)
                        .filter(
                          (entry) =>
                            !query.search || entry.key.toLowerCase().includes(String(query.search).toLowerCase()),
                        )
                    : [],
                  configurationHash: effective?.configurationHash,
                  warnings: effective?.warnings ?? [],
                  scopes: {
                    project: controlPlane.project,
                    environments: controlPlane.store.listEnvironments(controlPlane.project.id),
                    resources: allResources.filter((item) => item.kind !== 'function'),
                    functions: allResources.filter((item) => item.kind === 'function'),
                    previews: previewService.previews
                      .listInstances({ projectId: controlPlane.project.id })
                      .filter((item) => item.status !== 'destroyed')
                      .map((item) => ({
                        id: item.id,
                        name: item.name,
                        environmentId: item.baseEnvironmentId,
            ...
            if (url.pathname === '/api/configuration/export' && req.method === 'GET') {
              ...
              return new Response(configurationService.exportVariables({ projectId: controlPlane.project.id, scope }), {
                headers: {
                  ...noStore,
                  'content-disposition': 'attachment; filename="ts-cloud.env"',
            ...
            if (url.pathname === '/api/configuration/plan' && req.method === 'POST') {
              const body = await readJsonBody(req),
                scope = parseConfigurationScope(body, defaults)
              const values =
                body.values && typeof body.values === 'object' && !Array.isArray(body.values)
                  ? Object.fromEntries(Object.entries(body.values).map(([key, value]) => [key, String(value)]))
                  : {}
              return json(
                {
                  ok: true,
                  plan: configurationService.plan({
                    projectId: controlPlane.project.id,
                    scope,
                    values,
            ...
            if (url.pathname === '/api/configuration' && req.method === 'POST') {
              const body = await readJsonBody(req),
                scope = parseConfigurationScope(body, defaults),
                kind = body.kind === 'secret' ? ('secret' as const) : ('variable' as const),
                key = String(body.key ?? '')
              if (kind === 'secret' && !canWriteSecrets)
                return json({ ok: false, error: 'Secret write permission is required.' }, 403, noStore)
              try {
                const result = await configurationService.set({
                  ...
                  confirmed: String(body.confirm ?? '') === key,
                ...
                return json(
                  {
                    ok: true,
                    entry: configurationService
                      .list({ projectId: controlPlane.project.id, scope, canReadSecretMetadata })
                      .find((entry) => entry.id === result.entry.id),
                    mutation: result.mutation,
                    warnings: result.warnings,
                  },
                  result.mutation.added.length ? 201 : 200,
                  noStore,
                )
              } catch (error) {
                return json({ ok: false, error: error instanceof Error ? error.message : String(error) }, 409, noStore)
              }
            }
            if (url.pathname === '/api/configuration/import' && req.method === 'POST') {
              const body = await readJsonBody(req),
                scope = parseConfigurationScope(body, defaults),
                secretKeys = Array.isArray(body.secretKeys) ? body.secretKeys.map(String) : []
              if (secretKeys.length && !canWriteSecrets)
                return json({ ok: false, error: 'Secret write permission is required.' }, 403, noStore)
              try {
                const result = await configurationService.importDotenv({
                  organizationId: controlPlane.organization.id,
                  projectId: controlPlane.project.id,
                  scope,
                  ...
                  secretKeys,
                  backend: body.backend,
                  confirmed: body.confirm === 'import',
                  idempotencyKey: req.headers.get('idempotency-key') ?? body.idempotencyKey,
                  actorId: organizationPrincipal(user).actor?.id,
                })
                return json(
                  {
                    ok: result.document.valid,
                    diagnostics: result.document.diagnostics,
                    mutation: result.mutation,
                    warnings: result.warnings,
                  },
                  result.document.valid ? 200 : 422,
                  noStore,
                )
              } catch (error) {
                return json({ ok: false, error: error instanceof Error ? error.message : String(error) }, 409, noStore)
            ...
            if (url.pathname === '/api/configuration/dependencies' && req.method === 'POST') {
              const body = await readJsonBody(req),
                entry = configurationStore.get(String(body.entryId ?? '')),
                resource = controlPlane.store.getResource(String(body.resourceId ?? ''))
              if (
                !entry ||
                entry.projectId !== controlPlane.project.id ||
                !resource ||
                resource.projectId !== controlPlane.project.id
              )
                return json(
                  { ok: false, error: 'Configuration entry or dependent resource was not found.' },
                  404,
                  noStore,
                )
              return json(
                {
                  ok: true,
                  dependency: configurationStore.setDependency({
                    entryId: entry.id,
                    resourceId: resource.id,
                    injectionTarget: ['native_reference', 'file'].includes(String(body.injectionTarget))
                      ? body.injectionTarget
                      : 'environment',
                    required: body.required !== false,
                    requiresRedeploy: body.requiresRedeploy !== false,
                    lastDeployedVersion:
                      body.lastDeployedVersion == null ? undefined : Number(body.lastDeployedVersion),
                  }),
                },
                200,
                noStore,
            ...
            if (url.pathname === '/api/configuration/transfer' && req.method === 'POST') {
              const body = await readJsonBody(req),
                source = configurationStore.get(String(body.id ?? '')),
                targetScope = parseConfigurationScope(
                  { ...body, scopeType: body.targetScopeType, scopeId: body.targetScopeId },
                  defaults,
                )
              if (!source || source.projectId !== controlPlane.project.id)
                return json({ ok: false, error: 'Configuration entry was not found.' }, 404, noStore)
              if (source.kind === 'secret' && !canWriteSecrets)
                return json({ ok: false, error: 'Secret write permission is required.' }, 403, noStore)
              try {
                const result = await configurationService.transfer({
                  entryId: source.id,
                  targetScope,
                  mode: body.mode === 'move' ? 'move' : 'copy',
                  confirmed: body.confirm === source.key,
                  actorId: organizationPrincipal(user).actor?.id,
                  idempotencyKey: req.headers.get('idempotency-key') ?? body.idempotencyKey,
                })
                return json(
                  {
                    ok: true,
                    entry: configurationService
                      .list({ projectId: controlPlane.project.id, scope: targetScope, canReadSecretMetadata })
                      .find((entry) => entry.id === result.entry.id),
                    mutation: result.mutation,
                  },
                  200,
            ...
            if (url.pathname === '/api/configuration/bulk' && req.method === 'POST') {
              const body = await readJsonBody(req),
                ids = Array.isArray(body.ids) ? body.ids.map(String).slice(0, 500) : [],
                selected = ids
                  .map((id) => configurationStore.get(id))
                  .filter(
                    (entry): entry is NonNullable<typeof entry> =>
                      !!entry && entry.projectId === controlPlane.project.id,
                  )
              if (selected.length !== ids.length)
                return json({ ok: false, error: 'One or more configuration entries were not found.' }, 404, noStore)
              if (selected.some((entry) => entry.kind === 'secret') && !canWriteSecrets)
                return json({ ok: false, error: 'Secret write permission is required.' }, 403, noStore)
              if (body.action !== 'delete' || body.confirm !== 'bulk')
                return json(
                  { ok: false, error: 'Bulk deletion requires action delete and confirm bulk.' },
                  409,
                  noStore,
                )
              const removed = []
              try {
                ...
                return json(
                  {
                    ok: true,
                    removed: removed.flatMap((result) => result.removed),
                    affectedResourceIds: [...new Set(removed.flatMap((result) => result.affectedResourceIds))].sort(),
                  },
                  200,
                  noStore,
                )
                ...
                return json(
                  {
                    ...
                    partial: removed.flatMap((result) => result.removed),
                  },
                  409,
                  noStore,
                )
            ...
            if (url.pathname === '/api/configuration/reveal' && req.method === 'POST') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    value: await configurationService.reveal({
                      entryId: String(body.id ?? ''),
                      canRevealSecrets: canReadSecretMetadata,
                      recentlyAuthenticated:
                        !authEnabled || (!!session && authentication.isRecentlyAuthenticated(session)),
                      actorId: organizationPrincipal(user).actor?.id,
                    }),
                  },
                  200,
                ...
                return json({ ok: false, error: error instanceof Error ? error.message : String(error) }, 403, noStore)
              }
            }
            if (url.pathname === '/api/configuration' && req.method === 'DELETE') {
              const body = await readJsonBody(req),
                entry = configurationStore.get(String(body.id ?? ''))
              if (entry?.kind === 'secret' && !canWriteSecrets)
                return json({ ok: false, error: 'Secret write permission is required.' }, 403, noStore)
              if (!entry || entry.projectId !== controlPlane.project.id)
                return json({ ok: false, error: 'Configuration entry was not found.' }, 404, noStore)
              try {
                return json(
                  {
                    ok: true,
                    mutation: await configurationService.remove({
                      entryId: entry.id,
                      expectedVersion: Number(body.expectedVersion),
                      confirmed: String(body.confirm ?? '') === entry.key,
                      actorId: organizationPrincipal(user).actor?.id,
                    }),
                  },
                  200,
          ...
          if (url.pathname.startsWith('/api/fleet')) {
            const actorId = organizationPrincipal(user).actor?.id,
              fail = (error: unknown, status = 409) =>
                json({ ok: false, error: error instanceof Error ? error.message : String(error) }, status, {
                  'cache-control': 'no-store',
                })
            if (url.pathname === '/api/fleet' && req.method === 'GET')
              return json(
                {
                  ok: true,
                  servers: fleetStore.list(controlPlane.project.id),
                  resources: controlPlane.store.listResources(controlPlane.project.id),
                  operations: operationQueue
                    .list({ projectId: controlPlane.project.id, limit: 200 })
                    .filter((item) => item.operation.kind.startsWith('fleet.')),
                },
                200,
                { 'cache-control': 'no-store' },
              )
            if (url.pathname === '/api/fleet' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    server: fleetService.enroll({
                      organizationId: controlPlane.organization.id,
                      projectId: controlPlane.project.id,
                      name: String(body.name ?? ''),
                      provider: ['aws', 'hetzner'].includes(String(body.provider)) ? body.provider : 'ssh',
                      providerId: body.providerId ? String(body.providerId) : undefined,
                      region: body.region ? String(body.region) : undefined,
                      zone: body.zone ? String(body.zone) : undefined,
                      endpoint: String(body.endpoint ?? ''),
                      sshUser: String(body.sshUser ?? 'deploy'),
            ...
            if (url.pathname === '/api/fleet/action' && req.method === 'POST') {
              const body = await readJsonBody(req),
                id = String(body.id ?? ''),
                action = String(body.action ?? '')
              try {
                if (action === 'test') return json({ ok: true, server: await fleetService.test(id) })
                if (action === 'validate') return json({ ok: true, server: await fleetService.validate(id) })
                if (action === 'review_host_key')
                  return json({ ok: true, server: fleetService.reviewHostKey(id, String(body.fingerprint ?? '')) })
                if (action === 'bootstrap')
                  return json(
                    { ok: true, ...fleetService.bootstrap(id, body.confirmed === true) },
                    body.confirmed === true ? 202 : 200,
                  )
                if (action === 'heartbeat')
                  return json({
                    ok: true,
                    server: fleetService.heartbeat(id, {
                      usage: body.usage && typeof body.usage === 'object' ? body.usage : {},
                    }),
                  })
                if (action === 'drain')
                  return json({ ok: true, server: fleetService.drain(id, body.completed === true) })
                if (action === 'uncordon') return json({ ok: true, server: fleetService.uncordon(id) })
                if (action === 'archive')
                  return json({ ok: true, server: fleetService.archive(id, String(body.confirmation ?? '')) })
                return fail('Unsupported fleet action.', 422)
              } catch (error) {
                return fail(error)
              }
            }
          ...
          if (url.pathname.startsWith('/api/capacity')) {
            const capacityFail = (error: unknown, status = 409) =>
              json({ ok: false, error: error instanceof Error ? error.message : String(error) }, status, {
                'cache-control': 'no-store',
              })
            if (url.pathname === '/api/capacity' && req.method === 'GET')
              return json(
                {
                  ok: true,
                  pools: placementStore.listPools(controlPlane.project.id),
                  placements: placementStore.listPlacements(controlPlane.project.id),
                  servers: fleetStore.list(controlPlane.project.id),
                },
                200,
                { 'cache-control': 'no-store' },
              )
            if (url.pathname === '/api/capacity' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    pool: placementStore.createPool({
                      organizationId: controlPlane.organization.id,
                      projectId: controlPlane.project.id,
                      name: String(body.name ?? ''),
                      purpose: String(body.purpose ?? 'application') as any,
                      backend: String(body.backend ?? 'server') as any,
                      region: body.region ? String(body.region) : undefined,
                      architecture: body.architecture ? String(body.architecture) : undefined,
                      labels: body.labels && typeof body.labels === 'object' ? body.labels : {},
                      requiredServerLabels:
                        body.requiredServerLabels && typeof body.requiredServerLabels === 'object'
                          ? body.requiredServerLabels
                          : {},
                      toleratedTaints: Array.isArray(body.toleratedTaints) ? body.toleratedTaints.map(String) : [],
                      capacity: body.capacity && typeof body.capacity === 'object' ? body.capacity : {},
                      maxWorkloads: Number(body.maxWorkloads ?? 10),
                      costWeight: Number(body.costWeight ?? 1),
                      spreadKey: body.spreadKey ? String(body.spreadKey) : undefined,
                ...
                return capacityFail(error, 422)
              }
            }
            if (url.pathname === '/api/capacity/member' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                placementStore.addMember(String(body.poolId ?? ''), String(body.serverId ?? ''))
                return json({ ok: true, pool: placementStore.getPool(String(body.poolId ?? '')) })
              } catch (error) {
                return capacityFail(error, 422)
              }
            }
            if (url.pathname === '/api/capacity/explain' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json({
                  ok: true,
                  decisions: placementService.explain(controlPlane.project.id, {
                    purpose: String(body.purpose ?? 'application') as any,
            ...
            if (url.pathname === '/api/capacity/action' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                if (body.action === 'drain')
                  return json({ ok: true, ...placementService.drainPool(String(body.poolId ?? '')) })
                if (body.action === 'enable')
                  return json({
                    ok: true,
                    pool: placementStore.updatePool(String(body.poolId ?? ''), { status: 'active' }),
                  })
                if (body.action === 'disable')
                  return json({
                    ok: true,
                    pool: placementStore.updatePool(String(body.poolId ?? ''), { status: 'disabled' }),
                  })
                return capacityFail('Unsupported capacity action.', 422)
              } catch (error) {
                return capacityFail(error, 422)
              }
            }
          ...
          if (url.pathname.startsWith('/api/regions')) {
            const regionFail = (error: unknown, status = 409) =>
              json({ ok: false, error: error instanceof Error ? error.message : String(error) }, status, {
                'cache-control': 'no-store',
              })
            if (url.pathname === '/api/regions' && req.method === 'GET')
              return json(
                {
                  ok: true,
                  topologies: regionStore.list(controlPlane.project.id).map((topology) => ({
                    topology,
                    targets: regionStore.targets(topology.id),
                    channels: regionStore.channels(topology.id),
                    route: regionStore.route(topology.id),
                    executions: regionStore.executions(topology.id),
                  })),
                },
                200,
                { 'cache-control': 'no-store' },
              )
            if (url.pathname === '/api/regions' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    topology: regionService.create({
                      organizationId: controlPlane.organization.id,
                      projectId: controlPlane.project.id,
                      environmentId: controlPlane.environments.get(environment)?.id,
                      name: String(body.name ?? ''),
                      ...
                      trafficPolicy:
                        body.trafficPolicy === 'weighted' || body.trafficPolicy === 'latency'
                          ...
                          : 'active_passive',
                      dataPolicy: {
                        replicate: Array.isArray(body.replicate) ? body.replicate : ['s3', 'dynamodb', 'secrets'],
                        maxLagSeconds: Number(body.maxLagSeconds ?? 30),
                        retainOnDestroy: body.retainOnDestroy !== false,
                      },
                      dnsProvider: 'route53',
                      cdnEnabled: body.cdnEnabled === true,
                      wafEnabled: body.wafEnabled === true,
                    }),
                  },
                  201,
                )
              } catch (error) {
                return regionFail(error, 422)
              }
            }
            if (url.pathname === '/api/regions/action' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    ...regionService.enqueue({
                      topologyId: String(body.topologyId ?? ''),
                      kind: String(body.action ?? '') as any,
                      targetRegion: body.targetRegion ? String(body.targetRegion) : undefined,
                      revision: body.revision ? String(body.revision) : undefined,
                      manifest: body.manifest && typeof body.manifest === 'object' ? body.manifest : {},
                      confirmation: body.confirmation ? String(body.confirmation) : undefined,
          ...
          if (url.pathname.startsWith('/api/maintenance')) {
            const maintenanceFail = (error: unknown, status = 409) =>
              json({ ok: false, error: error instanceof Error ? error.message : String(error) }, status, {
                'cache-control': 'no-store',
              })
            if (url.pathname === '/api/maintenance' && req.method === 'GET')
              return json(
                {
                  ok: true,
                  manifests: maintenanceStore.manifests(controlPlane.organization.id),
                  windows: maintenanceStore.windows(controlPlane.project.id),
                  campaigns: maintenanceStore
                    .campaigns(controlPlane.project.id)
                    .map((campaign) => ({ campaign, targets: maintenanceStore.targets(campaign.id) })),
                  cleanups: maintenanceStore.cleanups(controlPlane.project.id),
                  drills: maintenanceStore.drills(controlPlane.project.id),
                },
                200,
                { 'cache-control': 'no-store' },
              )
            if (url.pathname === '/api/maintenance/cleanup-preview' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    plan: await maintenanceService.previewCleanup(
                      {
                        projectId: controlPlane.project.id,
                        kind: String(body.kind ?? 'operations') as CleanupPlan['kind'],
                        criteria: { olderThanDays: Number(body.olderThanDays ?? 30) },
                      },
                      cleanupDrivers,
                    ),
                  },
                ...
                return maintenanceFail(error, 422)
              }
            }
            if (url.pathname === '/api/maintenance/cleanup-apply' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    ...maintenanceService.enqueueCleanup(String(body.planId ?? ''), String(body.confirmation ?? '')),
                  },
                  202,
                )
              } catch (error) {
            ...
            if (url.pathname === '/api/maintenance/drill' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json(
                  {
                    ok: true,
                    drill: maintenanceStore.createDrill({
                      projectId: controlPlane.project.id,
                      scenario: String(body.scenario ?? 'control_plane') as any,
                      isolatedTarget: String(body.isolatedTarget ?? ''),
                      expectedRpoMinutes: Number(body.expectedRpoMinutes ?? 60),
                      expectedRtoMinutes: Number(body.expectedRtoMinutes ?? 30),
                      recoveryPointId: body.recoveryPointId ? String(body.recoveryPointId) : undefined,
            ...
            if (url.pathname === '/api/maintenance/drill-run' && req.method === 'POST') {
              const body = await readJsonBody(req)
              try {
                return json({ ok: true, ...maintenanceService.enqueueDrill(String(body.drillId ?? '')) }, 202)
              } catch (error) {
                return maintenanceFail(error, 422)
              }
            }
          ...
          if (url.pathname.startsWith('/api/volumes')) {
            const environmentRecord = controlPlane.environments.get(environment)
            const actorId = organizationPrincipal(user).actor?.id
            const fail = (error: unknown, status = 409) =>
              json({ ok: false, error: error instanceof Error ? error.message : String(error) }, status, {
                'cache-control': 'no-store',
              })
              ...
              return json(
                {
                  ...
                  resources,
                  operations: operationQueue
                    .list({ projectId: controlPlane.project.id, limit: 100 })
                    .filter((item) => item.operation.kind.startsWith('volume.')),
                  recoveryPoints: backupStore
                    .listRecoveryPoints(controlPlane.project.id)
                    .filter((item) => item.kind === 'volume'),
                  providers: volumeDrivers.map((driver) => ({
                ...
                return json(
                  {
                    ...
                    ...volumeService.create({
                      ...
                      type: body.type === 'docker' ? 'docker' : body.type,
                      capacityBytes: body.capacityBytes == null ? undefined : Number(body.capacityBytes),
                      filesystem: body.filesystem ? String(body.filesystem) : undefined,
                      encrypted: body.encrypted === true,
                      desiredState: body.desiredState && typeof body.desiredState === 'object' ? body.desiredState : {},
                      actorId,
                    }),
                  },
                  202,
            ...
            if (url.pathname === '/api/volumes/attach' && req.method === 'POST') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    ...volumeService.attach(String(body.volumeId ?? ''), {
                      resourceId: String(body.resourceId ?? ''),
                      targetPath: String(body.targetPath ?? ''),
                      readOnly: body.readOnly === true,
                      uid: body.uid == null ? undefined : Number(body.uid),
                      gid: body.gid == null ? undefined : Number(body.gid),
                      mode: body.mode ? String(body.mode) : undefined,
                      propagation: body.propagation,
                      driverOptions:
                        body.driverOptions && typeof body.driverOptions === 'object' ? body.driverOptions : {},
                      actorId,
                    }),
                  },
                  202,
            ...
            if (url.pathname === '/api/volumes/detach' && req.method === 'POST') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    ...volumeService.detach(String(body.volumeId ?? ''), String(body.attachmentId ?? ''), {
                      force: body.force === true,
                      drained: body.drained === true,
                      unmounted: body.unmounted === true,
                      confirmation: body.confirmation ? String(body.confirmation) : undefined,
                      actorId,
                    }),
                  },
            ...
            if (url.pathname === '/api/volumes/resize' && req.method === 'POST') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    ...volumeService.resize(String(body.volumeId ?? ''), Number(body.capacityBytes), {
                      drained: body.drained === true,
                      actorId,
                    }),
                  },
            ...
            if (url.pathname === '/api/volumes/snapshot' && req.method === 'POST') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    ...volumeService.snapshot(String(body.volumeId ?? ''), {
            ...
            if (url.pathname === '/api/volumes/restore' && req.method === 'POST') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    ...volumeService.restore(String(body.snapshotId ?? ''), { name: String(body.name ?? ''), actorId }),
                  },
                  202,
                )
              } catch (error) {
            ...
            if (url.pathname === '/api/volumes/discover' && req.method === 'POST') {
              try {
                return json({
                  ok: true,
                  volumes: await volumeService.discover({
                    organizationId: controlPlane.organization.id,
                    projectId: controlPlane.project.id,
                    environmentId: environmentRecord?.id,
                    actorId,
            ...
            if (url.pathname === '/api/volumes/adopt' && req.method === 'POST') {
              ...
              try {
                return json({
                  ok: true,
                  volume: volumeService.adopt(String(body.volumeId ?? ''), { name: String(body.name ?? ''), actorId }),
                })
              } catch (error) {
                return fail(error, 422)
              }
            ...
            if (url.pathname === '/api/volumes' && req.method === 'DELETE') {
              ...
              try {
                return json(
                  {
                    ok: true,
                    ...volumeService.delete(String(body.volumeId ?? ''), {
                      recentAuthAt:
                        !authEnabled || (session && authentication.isRecentlyAuthenticated(session))
                          ? new Date().toISOString()
                          : undefined,
                      confirmation: body.confirmation ? String(body.confirmation) : undefined,
                      backupOverride: body.backupOverride === true,
                      backupOverrideConfirmation: body.backupOverrideConfirmation
                        ? String(body.backupOverrideConfirmation)
                        : undefined,
                      actorId,
                    }),
                  },
          ...
          if (url.pathname === '/api/backups' && req.method === 'GET') {
            const environmentRecord = controlPlane.environments.get(environment),
              ...
              recoveryPoints = backupStore
                .listRecoveryPoints(controlPlane.project.id)
                .filter((item) => !item.policyId || policyIds.has(item.policyId)),
              jobs = backupStore
            ...
            const destinations = await Promise.all(
              backupStore.listDestinations(controlPlane.project.id).map(async (destination) => ({
            ...
            return json({
              ...
              resources: environmentRecord
                ? controlPlane.store
                    .listResources(controlPlane.project.id, environmentRecord.id)
                    .map((item) => ({ id: item.id, name: item.name, slug: item.slug, kind: item.kind }))
                : [],
            })
          }
          ...
          if (url.pathname === '/api/backups/destinations' && req.method === 'POST') {
            ...
            try {
              ...
              if (encryption !== 'provider') {
                ...
                if (key.length < 32) throw new Error('Client-side encryption keys must contain at least 32 characters.')
              ...
              const destination = backupStore.createDestination({
                ...
                encryptionKeyRef,
                immutability:
                  body.immutability && typeof body.immutability === 'object' && !Array.isArray(body.immutability)
                    ? body.immutability
                    : {},
                status: 'untested',
              })
          ...
          if (url.pathname === '/api/backups/destinations/test' && req.method === 'POST') {
            ...
            if (destination.provider === 'aws_backup')
              return json(
                { ok: false, error: 'AWS Backup destinations are verified by creating a provider recovery point.' },
                422,
              )
            try {
          ...
          if (url.pathname === '/api/backups/policies' && req.method === 'POST') {
            ...
            if (!destination || destination.projectId !== controlPlane.project.id)
              return json({ ok: false, error: 'Backup destination was not found.' }, 404)
            if (
              !['managed_database', 'logical_database', 'volume', 'files', 'control_plane', 'infrastructure'].includes(
                resourceKind,
              )
            )
            ...
            if (
              dataServiceId &&
              (!service ||
                service.projectId !== controlPlane.project.id ||
                service.environmentId !== environmentRecord.id)
            )
            ...
            if (
              ...
              (!resource ||
                resource.projectId !== controlPlane.project.id ||
                resource.environmentId !== environmentRecord.id)
            )
            ...
            if (
              ...
              (!service || !['container', 'server'].includes(service.provider))
            )
              return json({ ok: false, error: 'Logical database policies require an on-box data service.' }, 422)
            if (
              resourceKind === 'volume' &&
              (!resource || !/^[A-Za-z0-9][A-Za-z0-9_.-]{1,127}$/.test(String(body.volumeName ?? '')))
            )
              return json({ ok: false, error: 'Volume policies require a scoped resource and valid volumeName.' }, 422)
            const includePatterns = Array.isArray(body.includePatterns)
              ? body.includePatterns.map(String).filter(Boolean)
              : []
            if (resourceKind === 'files' && (!resource || !includePatterns.length))
              return json(
                {
                  ok: false,
                  error: 'File policies require a scoped resource and at least one project-relative include path.',
                },
                422,
              )
            if (resourceKind === 'control_plane' && destination.encryption === 'provider')
              return json(
                { ok: false, error: 'Control-plane policies require a destination with client-side encryption.' },
                422,
              )
            if (
              resourceKind === 'infrastructure' &&
              (destination.provider !== 'aws_backup' ||
                !includePatterns.some((item) => item.startsWith('resource:arn:')) ||
                !includePatterns.some((item) => item.startsWith('role:arn:')))
            )
              return json(
                {
                  ok: false,
                  error:
                    'Infrastructure policies require an AWS Backup destination plus resource:<arn> and role:<arn> include values.',
                },
                422,
              )
            try {
              const policy = backupStore.createPolicy({
                ...
                retention:
                  body.retention && typeof body.retention === 'object' && !Array.isArray(body.retention)
                    ? body.retention
                    : { keepLast: 7, expireAfterDays: 30 },
                compression: ['none', 'gzip', 'zstd'].includes(String(body.compression)) ? body.compression : 'gzip',
                encryption: ['destination', 'client_side', 'both'].includes(String(body.encryption))
                  ? body.encryption
                  : 'destination',
                includePatterns:
                  resourceKind === 'volume' ? [String(body.volumeName), ...includePatterns] : includePatterns,
          ...
          if (url.pathname === '/api/backups/run' && req.method === 'POST') {
            ...
            if (
              !policy ||
              policy.projectId !== controlPlane.project.id ||
              policy.environmentId !== environmentRecord?.id
            )
              return json({ ok: false, error: 'Backup policy was not found in this environment.' }, 404)
            try {
              const job = backupCoordinator.enqueueBackup(
                policy,
                new Date().toISOString(),
                organizationPrincipal(user).actor?.id,
              )
          ...
          if (url.pathname === '/api/backups/restore' && req.method === 'POST') {
            const body = await readJsonBody(req),
              ...
              target =
                body.target && typeof body.target === 'object' && !Array.isArray(body.target)
                  ? { ...body.target, inPlace: mode === 'in_place' }
            ...
            try {
              ...
              const job = backupCoordinator.enqueueRestore(point, {
                ...restoreInput,
          ...
          if (url.pathname === '/api/backups/recovery-points' && req.method === 'POST') {
            ...
            return json({
              ok: true,
              recoveryPoint: backupStore.updateRecoveryPoint(point.id, {
          ...
          if (url.pathname === '/api/data-services/capabilities' && req.method === 'GET') {
            ...
            if (
              !['postgres', 'mysql', 'mariadb', 'redis', 'mongodb', 'libsql'].includes(engine) ||
              !['aws_rds', 'aws_aurora', 'aws_elasticache', 'server', 'container', 'external'].includes(provider)
            )
            ...
            const services = dataServiceStore.list(controlPlane.project.id, environmentRecord?.id).map((service) => {
              ...
              return {
                ...
                credential: credential
                  ...
                  : undefined,
                dependencies: dataServiceStore
                  .dependencies(service.id)
                  .map((item) => ({ resourceId: item.resourceId, requiresRedeploy: item.requiresRedeploy })),
              }
            })
          ...
          if (url.pathname === '/api/data-services/preview' && req.method === 'POST') {
            ...
            if (body.id) {
              ...
              try {
                return json({
                  ok: true,
                  plan: dataServiceLifecycle.plan(service, action as DataAction, body.changes ?? {}),
                })
              } catch (error) {
                return json({ ok: false, error: error instanceof Error ? error.message : String(error) }, 422)
              }
            ...
            return json({
              ok: true,
              capabilities: dataServiceCapabilities(engine, provider),
              defaults: { publicExposure: false, backupRetentionDays: 7, deletionProtection: true },
              productionExecutionCreated: false,
            })
          }
          ...
          if (url.pathname === '/api/data-services' && req.method === 'POST') {
            ...
            const input = {
              ...
              desiredState:
                body.desiredState && typeof body.desiredState === 'object' && !Array.isArray(body.desiredState)
                  ? body.desiredState
                  : {},
              observedState: {},
            ...
            try {
              ...
              if (!resolveDataAdapter({ provider, engine } as DataService))
                return json(
                  {
                    ok: false,
                    error: `No ${provider}/${engine} provisioning adapter is configured; adopt it read-only or choose a supported provider.`,
                  },
                  422,
                )
              ...
              return json(
                {
                  ...
                  oneTimeCredential: created.credential,
                },
                201,
              )
            } catch (error) {
          ...
          if (url.pathname === '/api/data-services/connections' && req.method === 'GET') {
            ...
            if (
              ...
              return json({ ok: false, error: 'Data service was not found in this environment.' }, 404)
            const endpoint =
                typeof service.observedState.endpoint === 'string' ? service.observedState.endpoint : undefined,
              reader =
                typeof service.observedState.readerEndpoint === 'string'
                  ? service.observedState.readerEndpoint
                  : undefined,
              port =
                Number(service.observedState.port) ||
                  ...
                  : service.engine === 'redis'
                    ...
                    : service.engine === 'mongodb'
                      ...
                      : service.engine === 'libsql'
                        ...
                        : 3306),
              database =
                typeof service.observedState.database === 'string' ? service.observedState.database : undefined,
              tls =
                typeof service.observedState.tls === 'boolean'
          ...
          if (url.pathname === '/api/data-services/action' && req.method === 'POST') {
            ...
            if (!DATA_SERVICE_ACTIONS.includes(action as any))
              return json({ ok: false, error: `Action must be one of: ${DATA_SERVICE_ACTIONS.join(', ')}.` }, 422)
            const changes =
              body.changes && typeof body.changes === 'object' && !Array.isArray(body.changes) ? body.changes : {}
            try {
              ...
              const operationId = dataServiceLifecycle.enqueue(
                service,
          ...
          if (url.pathname === '/api/data-services/dependencies' && req.method === 'POST') {
            ...
            dataServiceStore.addDependency({
              serviceId: service.id,
              resourceId: resource.id,
              secretRef: service.credentialRef ?? `secret://data-services/${service.projectId}/${service.name}`,
              requiresRedeploy: body.requiresRedeploy !== false,
            })
            return json({
              ok: true,
          ...
          if (url.pathname === '/api/data-services/management' && req.method === 'POST') {
            ...
            if (service.origin !== 'adopted' || body.reviewed !== true || body.confirm !== service.name)
              return json(
                { ok: false, error: `Enabling management requires reviewed=true and typing ${service.name}.` },
                409,
              )
            return json({
              ok: true,
              service: dataServiceStore.update(service.id, service.version, { managementEnabled: true }),
            })
          }
            ...
            return json(
              {
                ...
                credential: {
                  username: credential.username,
                  password: await dataServiceSecrets.resolve(credential.secretRef),
                  version: credential.version,
                },
              },
              200,
              ...
              return json(
                {
                  ok: false,
                  error:
                    'Database name must be a valid identifier (letters, numbers, underscore; not starting with a digit).',
                },
                422,
              )
          ...
          if (url.pathname === '/api/databases/users' && req.method === 'POST') {
            ...
            if (!password) return json({ ok: false, error: 'Password is required.' }, 422)
            if (database && !isValidDbIdentifier(database))
              return json({ ok: false, error: 'Database must be a valid identifier.' }, 422)
            return json({
              ...(await createDatabaseUser(config as CloudConfig, environment, {
                username,
                password,
                database,
                access,
          ...
          if (url.pathname === '/api/users' && req.method === 'POST') {
            ...
            if (!isValidUsername(username))
              return json(
                { ok: false, error: 'Username must be 2-32 characters: letters, numbers, dot, dash or underscore.' },
                422,
              )
            ...
            for (const [site, role] of Object.entries(requested)) {
              if (!known.has(site)) return json({ ok: false, error: `Site '${site}' was not found.` }, 404)
            ...
            const password =
              typeof body.password === 'string' && body.password.trim() ? body.password.trim() : undefined
          ...
          if (url.pathname === '/api/actions/run' && req.method === 'POST') {
            ...
            if (!action) return json({ ok: false, error: 'Unknown dashboard action.' }, 404)
          ...
          if (url.pathname === '/api/server/operations/run' && req.method === 'POST') {
            ...
            const operation = body.operation
              ? resolveDashboardOperation(body.operation, config as CloudConfig, latestData)
              : undefined
            if (!operation) return json({ ok: false, error: 'Unknown or unavailable server operation.' }, 404)
            ...
            if (securityReview.decision.outcome === 'block')
              return json(
                {
                  ok: false,
                  error: `Operation blocked by security policy: ${securityReview.decision.explanation}`,
                  securityReview,
                },
                409,
              )
          ...
          if (url.pathname === '/api/serverless/operations/run' && req.method === 'POST') {
            ...
            const operation = body.operation
              ? resolveServerlessOperation(body.operation, config as CloudConfig, environment, latestData)
              : undefined
            if (!operation) return json({ ok: false, error: 'Unknown or unavailable serverless operation.' }, 404)
            if (operation.mutates && body.confirm !== operation.confirm)
              return json({ ok: false, error: `Type "${operation.confirm}" to run this operation.` }, 409)
            const tracked = await trackDashboardOperation({
              controlPlane,
              ...
              execute: () =>
                runServerlessOperation(config as CloudConfig, environment, operation, { min: body.min, max: body.max }),
            })
            latestData = await resolveLiveDashboardData(config as CloudConfig, environment)
            latestDataByEnvironment.set(environment, latestData)
            return json({ ...tracked.result, controlPlaneOperation: tracked.operation })
          ...
          if (url.pathname === '/api/serverless/command' && req.method === 'POST') {
            const body = (await req.json().catch(() => ({}))) as { command?: string; confirm?: string }
            if (body.confirm !== 'run') return json({ ok: false, error: 'Type "run" to execute this command.' }, 409)
          ...
          if (url.pathname === '/api/serverless/dlq/redrive' && req.method === 'POST') {
            ...
            return json(
              await redriveDlq(config as CloudConfig, environment, { max: body.max, targetQueue: body.targetQueue }),
            )
          }
          ...
          if (url.pathname === '/api/serverless/secrets' && req.method === 'POST') {
            ...
            if (!secretId) return json({ ok: false, error: 'A secret id is required.' }, 422)
          ...
          if (url.pathname === '/api/serverless/functions/config' && req.method === 'POST') {
            ...
            if (body.confirm !== mode) return json({ ok: false, error: `Type "${mode}" to update this function.` }, 409)
            return json(
              await updateFunctionConfig(config as CloudConfig, environment, mode, {
                memory: body.memory,
          ...
          if (url.pathname === '/api/serverless/alarms' && req.method === 'POST') {
            ...
            if (!preset) return json({ ok: false, error: 'An alarm metric is required.' }, 422)
            if (!Number.isFinite(threshold)) return json({ ok: false, error: 'A numeric threshold is required.' }, 422)
          ...
          if (url.pathname === '/api/serverless/alarms' && req.method === 'DELETE') {
            ...
            const name = String(body.name ?? '').trim()
            if (!name) return json({ ok: false, error: 'An alarm name is required.' }, 422)
            if (body.confirm !== name) return json({ ok: false, error: `Type "${name}" to delete this alarm.` }, 409)
          ...
          if (!canOpenDashboardPage(url.pathname, scopedUser))
            return new Response(null, {
              status: 302,
              headers: { location: `/access-denied?env=${encodeURIComponent(environment)}` },
            })

          const uiRoot = await uiRootFor(user, environment)
          if (!uiRoot) return text('ts-cloud dashboard UI is unavailable.', 503)
      ...
      if (auditMutation && (url.pathname.startsWith('/api/') || url.pathname.startsWith('/auth/'))) {
        ...
        controlPlane.store.appendEvent({
          ...
          payload: dashboardMutationAuditPayload(
            req.method,
            url.pathname,
            input,
            response.status,
