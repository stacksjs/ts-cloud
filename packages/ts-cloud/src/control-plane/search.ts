...
const SAFE_METADATA_KEYS: ReadonlySet<string> = new Set(['domain', 'domains', 'hostname', 'hostnames', 'url', 'repository', 'repo', 'image', 'label', 'labels'])
...
function searchableValues(value: unknown, allowedKeys: ReadonlySet<string>): string[] {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (typeof child === 'string')
      ...
      values.push(...child.filter(item => typeof item === 'string') as string[])
  }
  return values
}
...
function rank(query: string, title: string, text: string): number {
  ...
  if (normalizedTitle === wanted)
    return 1_000
  if (normalizedTitle.startsWith(wanted))
    return 800 - Math.min(100, normalizedTitle.length - wanted.length)
  if (normalizedTitle.includes(wanted))
    return 650 - Math.min(100, normalizedTitle.indexOf(wanted))
  const tokens = wanted.split(/\s+/).filter(Boolean)
  if (tokens.every(token => normalizedText.includes(token)))
    return 400 + tokens.length * 10
  let position = 0
...
function resourceResult(resource: { kind: string, slug: string }, environment: string): Pick<ControlPlaneSearchResult, 'type' | 'href'> {
  ...
  if (kind.includes('database') || ['mysql', 'postgres', 'postgresql', 'sqlite', 'redis'].includes(kind))
    return { type: 'database', href: `/server/database?env=${encodeURIComponent(environment)}&database=${encodeURIComponent(resource.slug)}` }
  return { type: 'service', href: `/server/sites?env=${encodeURIComponent(environment)}&service=${encodeURIComponent(resource.slug)}` }
}

export function searchControlPlane(store: ControlPlaneStore, options: ControlPlaneSearchOptions): ControlPlaneSearchResult[] {
  ...
  if (!restricted) {
    results.push({
      type: 'project', id: project.id, title: project.name, subtitle: project.slug,
      href: `/?project=${encodeURIComponent(project.slug)}`, tags: [], score: 0,
    })
    for (const environment of environments) {
      results.push({
        type: 'environment', id: environment.id, title: environment.name, subtitle: `${project.name} · ${environment.kind}`,
        href: `/?env=${encodeURIComponent(environment.slug)}`, environment: environment.slug, tags: [], score: 0,
      })
    }
  }

  const resources = store.listResources(project.id).filter(resource => !options.allowedResourceSlugs || options.allowedResourceSlugs.has(resource.slug))
  for (const resource of resources) {
    ...
    const text = [resource.name, resource.slug, resource.kind, resource.provider, resource.providerId, environment?.slug, ...metadata, ...tags.map(tag => tag.name)].filter(Boolean).join(' ')
    const destination = resourceResult(resource, environment?.slug ?? environments[0]?.slug ?? '')
    results.push({
      type: destination.type, id: resource.id, title: resource.name, subtitle: [resource.kind, ...metadata.slice(0, 2)].join(' · '),
      href: destination.href,
      environment: environment?.slug, provider: resource.provider, tags, score: rank(options.query, resource.name, text),
    })
  }
  ...
  if (!restricted) {
    for (const operation of store.listOperations({ projectId: project.id, limit: 500 })) {
      ...
      results.push({
        type: deployment ? 'deployment' : 'operation', id: operation.id, title,
        ...
        environment: environment?.slug, status: operation.state, tags: [], score: rank(options.query, title, text),
      })
    }
  }
  ...
  for (const result of results) {
    if (result.score === 0)
      result.score = rank(options.query, result.title, `${result.title} ${result.subtitle} ${result.environment ?? ''} ${result.provider ?? ''}`)
  }
  const typeOrder: Record<ControlPlaneSearchResultType, number> = { service: 0, server: 1, database: 2, deployment: 3, environment: 4, project: 5, operation: 6 }
  return results
    .filter(result => result.score > 0)
    .sort((left, right) => right.score - left.score || typeOrder[left.type] - typeOrder[right.type] || left.title.localeCompare(right.title) || left.id.localeCompare(right.id))
