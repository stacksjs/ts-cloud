...
const SENSITIVE_KEY =
  /(?:^|_)(?:authorization|cookie|credential|password|passwd|secret|token|api_?key|private_?key|access_?key)(?:$|_)/i
const SENSITIVE_TEXT =
  /(?:authorization|password|secret|token|api[_-]?key|private[_-]?key|access[_-]?key)\s*[=:]\s*[^\s,;]+/gi
...
function redactValue(value: unknown, key?: string): JsonValue {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const [childKey, child] of Object.entries(value)) result[childKey] = redactValue(child, childKey)
    return result
  }
  return String(value ?? '')
}
...
export function sanitizeControlPlaneValue(value: unknown, maxBytes: number = MAX_CONTROL_PLANE_JSON_BYTES): JsonValue {
  ...
  return {
    ...
    preview: encoded.slice(0, Math.max(0, maxBytes - 128)),
  }
}
...
function mapProject(row: Row): ControlPlaneProject {
  return {
    ...
    desiredConfigHash: optionalString(row.desired_config_hash),
...
function mapEnvironment(row: Row): ControlPlaneEnvironment {
  return {
    ...
    desiredState: parseJson(row.desired_state),
    discoveredState: parseJson(row.discovered_state),
    version: Number(row.version),
    createdAt: String(row.created_at),
    updatedAt: String(row.updated_at),
  }
...
function mapActor(row: Row): ControlPlaneActor {
  return {
    id: String(row.id),
    kind: String(row.kind) as ControlPlaneActor['kind'],
...
function mapEvent(row: Row): ControlPlaneEvent {
  return {
    ...
    level: String(row.level) as ControlPlaneEvent['level'],
...
function mapSavedFilter(row: Row): SavedFilter {
  return {
    ...
    query: parseJson(row.query) as Record<string, JsonValue>,
    createdAt: String(row.created_at),
    updatedAt: String(row.updated_at),
  }
}
...
function mapMembership(row: Row): OrganizationMembership {
  return {
    ...
    roleTemplate: String(row.role_template) as OrganizationMembership['roleTemplate'],
    scope: mapScope(row),
    source: String(row.source ?? 'manual') as OrganizationMembership['source'],
    status: String(row.status) as OrganizationMembership['status'],
    sessionVersion: Number(row.session_version),
    lastActiveAt: optionalString(row.last_active_at),
    createdAt: String(row.created_at),
    updatedAt: String(row.updated_at),
  }
}
...
function mapInvitation(row: Row, now: string): OrganizationInvitation {
  ...
  const state: OrganizationInvitation['state'] = revokedAt
    ? 'revoked'
    : acceptedAt
      ? 'accepted'
      : expiresAt <= now
  ...
  return {
    ...
    roleTemplate: String(row.role_template) as OrganizationInvitation['roleTemplate'],
    scope: mapScope(row),
    invitedByActorId: optionalString(row.invited_by_actor_id),
    acceptedByActorId: optionalString(row.accepted_by_actor_id),
    expiresAt,
    acceptedAt,
    revokedAt,
    state,
...
function mapGrant(row: Row): AuthorizationGrant {
  return {
    ...
    effect: String(row.effect) as AuthorizationGrant['effect'],
    capability: String(row.capability) as AuthorizationGrant['capability'],
    scope: mapScope(row),
    source: String(row.source ?? 'manual') as AuthorizationGrant['source'],
    createdAt: String(row.created_at),
    updatedAt: String(row.updated_at),
  }
}
...
export class ControlPlaneStore {
  ...
  constructor(options: ControlPlaneStoreOptions = {}) {
    ...
    this.migrate()
    if (!inMemory) this.secureDatabaseFiles()
  }
  ...
  private migrate(): void {
    ...
    if (current === CONTROL_PLANE_SCHEMA_VERSION) return
    ...
    try {
      const apply = this.database.transaction(() => {
        for (const migration of controlPlaneMigrations) {
          ...
          run(this.database, 'INSERT INTO schema_migrations (version, name, applied_at) VALUES (?, ?, ?)', [
            migration.version,
            migration.name,
            this.now(),
          ])
      ...
      throw new Error(
        `Control-plane migration from schema ${current} failed.${restore} ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  }

  close(): void {
    if (this.path !== ':memory:') this.secureDatabaseFiles()
    this.database.close()
  }
  ...
  updateProject(id: string, expectedVersion: number, input: UpdateProjectInput): ControlPlaneProject {
    ...
    if (result.changes !== 1) throw new OptimisticConcurrencyError('Project', id, expectedVersion)
    return this.getProject(id)!
  }
  ...
  listEnvironments(projectId: string): ControlPlaneEnvironment[] {
    return this.database
      .query<Row, [string]>('SELECT * FROM environments WHERE project_id = ? ORDER BY name COLLATE NOCASE, id')
      .all(projectId)
      .map(mapEnvironment)
  }
  ...
  getEnvironmentBySlug(projectId: string, slug: string): ControlPlaneEnvironment | undefined {
    const row = this.database
      .query<Row, [string, string]>('SELECT * FROM environments WHERE project_id = ? AND slug = ?')
      .get(projectId, slug)
    ...
    const rows = environmentId
      ? this.database
          .query<Row, [string, string]>(
            'SELECT * FROM resources WHERE project_id = ? AND environment_id = ? ORDER BY kind, name COLLATE NOCASE',
          )
          .all(projectId, environmentId)
      : this.database
          .query<Row, [string]>('SELECT * FROM resources WHERE project_id = ? ORDER BY kind, name COLLATE NOCASE')
          .all(projectId)
  ...
  updateResource(id: string, expectedVersion: number, input: UpdateResourceInput): ControlPlaneResource {
    ...
    if (result.changes !== 1) throw new OptimisticConcurrencyError('Resource', id, expectedVersion)
  ...
  getActorByExternalId(kind: ControlPlaneActor['kind'], externalId: string): ControlPlaneActor | undefined {
    const row = this.database
      .query<Row, [ControlPlaneActor['kind'], string]>('SELECT * FROM actors WHERE kind = ? AND external_id = ?')
      .get(kind, externalId)
  ...
  createOrganization(input: CreateOrganizationInput): ControlPlaneOrganization {
    ...
    if (!name || name.length > 100) throw new Error('Organization names must contain 1-100 characters')
    const id = input.id ?? this.idFn()
    const now = this.now()
  ...
  getOrganizationBySlug(slug: string): ControlPlaneOrganization | undefined {
    const row = this.database
      .query<Row, [string]>('SELECT * FROM organizations WHERE slug = ?')
  ...
  private normalizeAuthorizationScope(
    ...
    scope: AuthorizationScope = { type: 'organization' },
  ): AuthorizationScope {
    ...
    if (!scope.id) throw new Error(`${scope.type} scopes require an ID`)
    const target = this.resolveAuthorizationTarget(organizationId, scope)
    if (!target) throw new Error('Authorization scope was not found in this organization')
  ...
  resolveAuthorizationTarget(organizationId: string, scope: AuthorizationScope): AuthorizationTarget | undefined {
    if (!this.getOrganization(organizationId)) return undefined
    if (scope.type === 'organization') return { organizationId }
    if (!scope.id) return undefined
    if (scope.type === 'project') {
      const project = this.database
        .query<Row, [string, string]>('SELECT id FROM projects WHERE id = ? AND organization_id = ?')
    ...
    if (scope.type === 'environment') {
      const environment = this.database
        ...
        .get(scope.id, organizationId)
      return environment
        ? { organizationId, projectId: String(environment.project_id), environmentId: String(environment.id) }
        : undefined
    }
    ...
    return resource
      ? {
          ...
          environmentId: optionalString(resource.environment_id),
  ...
  createMembership(input: CreateMembershipInput): OrganizationMembership {
    if (!this.getActor(input.actorId)) throw new Error('Membership actor was not found')
  ...
  getMembershipForActor(organizationId: string, actorId: string): OrganizationMembership | undefined {
    const row = this.database
      .query<Row, [string, string]>('SELECT * FROM organization_memberships WHERE organization_id = ? AND actor_id = ?')
      .get(organizationId, actorId)
    return row ? mapMembership(row) : undefined
  }
  ...
  updateMembership(input: {
    ...
    if (input.roleTemplate === 'owner' && scope.type !== 'organization')
      throw new Error('Owners must be scoped to the organization')
    if (
      current.roleTemplate === 'owner' &&
      current.status === 'active' &&
      (input.roleTemplate !== 'owner' || scope.type !== 'organization')
    )
    ...
    this.appendEvent({
      ...
      actorId: input.actorId,
      type: 'organization.membership.updated',
      payload: {
        membershipId: current.id,
        from: { roleTemplate: current.roleTemplate, scope: scopePayload(current.scope) },
        to: { roleTemplate: input.roleTemplate, scope: scopePayload(scope) },
      },
    })
  ...
  revokeMembership(id: string, actorId?: string): OrganizationMembership {
    ...
    if (current.roleTemplate === 'owner') this.assertAnotherOwner(current)
    run(
      this.database,
  ...
  touchMembership(id: string): OrganizationMembership {
    ...
    run(
      this.database,
      'UPDATE organization_memberships SET last_active_at = ?, updated_at = ? WHERE id = ? AND status = ?',
      [now, now, id, 'active'],
    )
    const membership = this.getMembership(id)
    if (!membership) throw new Error('Membership was not found')
    return membership
  }
  ...
  private assertAnotherOwner(current: OrganizationMembership): void {
    ...
    if (owners <= 1) throw new Error('Cannot remove or demote the last organization owner')
  }
  ...
  createInvitation(input: CreateInvitationInput): { invitation: OrganizationInvitation; token: string } {
    ...
    const expiresAt = new Date(
      this.nowFn().getTime() + Math.min(30 * 86_400_000, Math.max(60_000, input.expiresInMs ?? 7 * 86_400_000)),
    ).toISOString()
    run(
      this.database,
    ...
    this.appendEvent({
      ...
      payload: { invitationId: id, email, roleTemplate: input.roleTemplate, scope: scopePayload(scope), expiresAt },
    })
  ...
  listInvitations(organizationId: string): OrganizationInvitation[] {
    return this.database
      .query<Row, [string]>('SELECT * FROM organization_invitations WHERE organization_id = ? ORDER BY created_at DESC')
      .all(organizationId)
      .map((row) => mapInvitation(row, this.now()))
  }
  ...
  inspectInvitationToken(token: string): OrganizationInvitation | undefined {
    const tokenHash = createHash('sha256').update(token).digest('hex')
    const row = this.database
      .query<Row, [string]>('SELECT * FROM organization_invitations WHERE token_hash = ?')
      .get(tokenHash)
    return row ? mapInvitation(row, this.now()) : undefined
  }
  ...
  acceptInvitation(
    ...
    return this.transaction(() => {
      const row = this.database
        .query<Row, [string]>('SELECT * FROM organization_invitations WHERE token_hash = ?')
        .get(tokenHash)
      if (!row) throw new Error('Invitation is invalid')
      const invitation = mapInvitation(row, this.now())
      if (invitation.state !== 'pending') throw new Error(`Invitation is ${invitation.state}`)
      if (!this.getActor(actorId)) throw new Error('Invitation actor was not found')
      const changed = run(
        this.database,
      ...
      if (changed.changes !== 1) throw new Error('Invitation is no longer available')
      ...
      this.appendEvent({
        ...
        payload: { invitationId: invitation.id, membershipId: membership.id },
      })
  ...
  revokeInvitation(id: string, actorId?: string): OrganizationInvitation {
    ...
    if (!invitation.revokedAt)
      run(this.database, 'UPDATE organization_invitations SET revoked_at = ?, updated_at = ? WHERE id = ?', [
        this.now(),
        this.now(),
        id,
      ])
  ...
  reissueInvitation(id: string, actorId?: string): { invitation: OrganizationInvitation; token: string } {
    ...
    if (!previous) throw new Error('Invitation was not found')
    if (previous.state === 'accepted') throw new Error('Accepted invitations cannot be reissued')
  ...
  upsertGrant(input: CreateGrantInput): AuthorizationGrant {
    if (!AUTHORIZATION_CAPABILITIES.includes(input.capability)) throw new Error('Unknown authorization capability')
    ...
    const existing = this.database
      .query<Row, [string, string, string, string, string | null]>(
    ...
    run(
      this.database,
      'UPDATE organization_memberships SET session_version = session_version + 1, updated_at = ? WHERE id = ?',
      [now, input.membershipId],
    )
    this.appendEvent({
      organizationId: input.organizationId,
      actorId: input.actorId,
      type: 'organization.grant.created',
      payload: {
        grantId: id,
  ...
  listGrants(membershipId: string): AuthorizationGrant[] {
    return this.database
      .query<Row, [string]>(
        'SELECT * FROM authorization_grants WHERE membership_id = ? ORDER BY effect DESC, capability, scope_type, scope_id',
      )
      .all(membershipId)
      .map(mapGrant)
  }
  ...
  removeGrant(id: string, actorId?: string): boolean {
    ...
    if (removed) {
      run(
        this.database,
        'UPDATE organization_memberships SET session_version = session_version + 1, updated_at = ? WHERE id = ?',
        [this.now(), grant.membershipId],
      )
      this.appendEvent({
        organizationId: grant.organizationId,
        actorId,
        type: 'organization.grant.removed',
        payload: { grantId: id, membershipId: grant.membershipId },
      })
    }
    return removed
  }
  ...
  createOperation(input: CreateOperationInput): ControlPlaneOperation {
    if (input.idempotencyKey) {
      const existing = this.database
        .query<Row, [string]>('SELECT * FROM operations WHERE idempotency_key = ?')
  ...
  listOperations(options: OperationListOptions = {}): ControlPlaneOperation[] {
    ...
    return this.database
      .query<Row, SQLQueryBindings[]>(`SELECT * FROM operations ${where} ORDER BY created_at DESC, id DESC LIMIT ?`)
      .all(...bindings)
      .map(mapOperation)
  }
  ...
  claimNextOperation(
  ...
  ): ControlPlaneOperation | undefined {
    const now = this.now()
    ...
    return this.transaction(() => {
      ...
      this.appendEvent({
        ...
        payload: { attempt: operation.attempt + 1, workerId },
      })
  ...
  claimOperation(id: string, workerId: string, leaseMs: number = 60_000): ControlPlaneOperation | undefined {
    ...
    return this.transaction(() => {
      ...
      if (!operation || operation.state !== 'queued') return undefined
      const result = run(
        this.database,
        `UPDATE operations SET state = 'running', attempt = attempt + 1, lease_owner = ?, lease_expires_at = ?,
        started_at = COALESCE(started_at, ?), updated_at = ?, version = version + 1 WHERE id = ? AND state = 'queued' AND version = ?`,
  ...
  transitionOperation(id: string, input: TransitionOperationInput): ControlPlaneOperation {
    return this.transaction(() => {
      ...
      if (current.version !== expectedVersion) throw new OptimisticConcurrencyError('Operation', id, expectedVersion)
      ...
      const result = run(
        ...
        [
          input.to,
          json(input.output ?? current.output),
          clampError(input.error) ?? null,
      ...
      if (result.changes !== 1) throw new OptimisticConcurrencyError('Operation', id, expectedVersion)
      this.appendEvent({
        ...
        type: `operation.${input.to}`,
        level: input.to === 'failed' ? 'error' : 'info',
  ...
  requestCancellation(id: string): ControlPlaneOperation {
    ...
    run(
      this.database,
      `UPDATE operations SET cancel_requested_at = ?, updated_at = ?, version = version + 1 WHERE id = ? AND state IN ('queued', 'running')`,
      [now, now, id],
    )
    const operation = this.getOperation(id)
  ...
  reconcileOrphanedOperations(options: { policy?: 'requeue' | 'fail'; now?: Date } = {}): ReconcileResult {
    ...
    for (const operation of rows) {
      if (policy === 'requeue') {
        this.transitionOperation(operation.id, {
          to: 'queued',
          expectedVersion: operation.version,
          error: 'Worker lease expired; operation requeued after restart.',
        })
        requeued++
      } else {
        this.transitionOperation(operation.id, {
          to: 'failed',
          expectedVersion: operation.version,
          error: 'Worker lease expired or dashboard restarted before completion.',
        })
        failed++
      }
    }
  ...
  appendEvent(input: AppendEventInput): ControlPlaneEvent {
    ...
    const organizationId =
      ...
      (input.projectId
        ? optionalString(
            this.database.query<Row, [string]>('SELECT organization_id FROM projects WHERE id = ?').get(input.projectId)
              ?.organization_id,
          )
        : undefined)
    run(
  ...
  listEvents(options: EventListOptions = {}): ControlPlaneEvent[] {
    ...
    if (options.organizationId) {
      ...
      bindings.push(options.organizationId)
    }
    if (options.projectId) {
      filters.push('project_id = ?')
      bindings.push(options.projectId)
    ...
    if (options.operationId) {
      ...
      bindings.push(options.operationId)
    }
    ...
    const where = filters.length ? `WHERE ${filters.join(' AND ')}` : ''
    return this.database
      .query<Row, SQLQueryBindings[]>(`SELECT * FROM events ${where} ORDER BY sequence ASC LIMIT ?`)
      .all(...bindings)
      .map(mapEvent)
  }
  ...
  upsertTag(projectId: string, name: string, color: string = '#5a8be0'): ControlPlaneTag {
    ...
    if (!/^#[0-9a-f]{6}$/i.test(color)) throw new Error('Tag colors must be six-digit hex colors')
    const now = this.now()
    const existing = this.database
      .query<Row, [string, string]>('SELECT * FROM tags WHERE project_id = ? AND normalized_name = ?')
      .get(projectId, normalizedName)
    if (existing) {
      run(this.database, 'UPDATE tags SET name = ?, color = ?, updated_at = ? WHERE id = ?', [
        name.trim(),
        color.toLowerCase(),
        now,
        String(existing.id),
  ...
  listTags(projectId: string): ControlPlaneTag[] {
    return this.database
      .query<Row, [string]>('SELECT * FROM tags WHERE project_id = ? ORDER BY normalized_name')
      .all(projectId)
      .map(mapTag)
  }

  assignTag(resourceId: string, tagId: string): void {
    const tag = this.database
      .query<Row, [string, string]>(
    ...
    if (!tag) throw new Error('The tag and resource must belong to the same project')
    run(this.database, 'INSERT OR IGNORE INTO resource_tags (resource_id, tag_id, created_at) VALUES (?, ?, ?)', [
      resourceId,
      tagId,
      this.now(),
    ])
  ...
  saveFilter(actorKey: string, name: string, routeId: string, query: Record<string, JsonValue>): SavedFilter {
    ...
    if (!trimmed || trimmed.length > 64) throw new Error('Saved filter names must contain 1-64 characters')
    ...
    if (serializedQuery.length > 8192) throw new Error('Saved filter queries must be smaller than 8 KB')
    const existing = this.database
      .query<Row, [string, string]>('SELECT * FROM saved_filters WHERE actor_key = ? AND name = ?')
      .get(actorKey, trimmed)
    const now = this.now()
    if (existing) {
      run(this.database, 'UPDATE saved_filters SET route_id = ?, query = ?, updated_at = ? WHERE id = ?', [
        routeId,
        serializedQuery,
        now,
        String(existing.id),
      ...
      return mapSavedFilter(
        this.database.query<Row, [string]>('SELECT * FROM saved_filters WHERE id = ?').get(String(existing.id))!,
      )
    }
    const id = this.idFn()
    run(
      this.database,
      'INSERT INTO saved_filters (id, actor_key, name, route_id, query, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, actorKey, trimmed, routeId, serializedQuery, now, now],
    )
  ...
  listSavedFilters(actorKey: string): SavedFilter[] {
    return this.database
      .query<Row, [string]>('SELECT * FROM saved_filters WHERE actor_key = ? ORDER BY updated_at DESC, name')
      .all(actorKey)
      .map(mapSavedFilter)
  }
  ...
  recordNavigation(actorKey: string, entityType: string, entityId: string): NavigationPreference {
    ...
    return mapNavigationPreference(
      this.database
        .query<Row, [string, string, string]>(
          'SELECT * FROM navigation_items WHERE actor_key = ? AND entity_type = ? AND entity_id = ?',
        )
        .get(actorKey, entityType, entityId)!,
    )
  }
  ...
  listNavigation(actorKey: string, options: { favoritesOnly?: boolean; limit?: number } = {}): NavigationPreference[] {
    ...
    return this.database
      ...
      .all(actorKey, limit)
      .map(mapNavigationPreference)
  }

  compact(
    options: { eventRetentionDays?: number; operationRetentionDays?: number; vacuum?: boolean } = {},
  ): CompactResult {
    const eventCutoff = new Date(this.nowFn().getTime() - (options.eventRetentionDays ?? 90) * 86_400_000).toISOString()
    const operationCutoff = new Date(
      this.nowFn().getTime() - (options.operationRetentionDays ?? 365) * 86_400_000,
    ).toISOString()
    ...
    if (options.vacuum !== false) this.database.run('VACUUM')
  ...
  health(): ControlPlaneHealth {
    const operations = Object.fromEntries(
      (['queued', 'running', 'succeeded', 'failed', 'cancelled', 'timed_out'] as OperationState[]).map((state) => [
        state,
        0,
      ]),
    ) as Record<OperationState, number>
    for (const row of this.database
      .query<Row, []>('SELECT state, COUNT(*) AS count FROM operations GROUP BY state')
      .all())
    ...
    const lastBackupAt =
      backup && !Array.isArray(backup) && typeof backup === 'object' && typeof backup.createdAt === 'string'
        ? backup.createdAt
        : undefined
    return {
      path: this.path,
      ...
      databaseBytes:
        this.path !== ':memory:' && existsSync(this.path)
          ? statSync(this.path).size
          : this.database.serialize().byteLength,
      lastBackupAt,
      operations,
  ...
  exportSnapshot(): ControlPlaneSnapshot {
    ...
    return {
      ...
      memberships: this.database
        .query<Row, []>('SELECT * FROM organization_memberships ORDER BY id')
        .all()
        .map(mapMembership),
      invitations: this.database
        .query<Row, []>('SELECT * FROM organization_invitations ORDER BY id')
        .all()
        .map((row) => ({ ...mapInvitation(row, this.now()), tokenHash: String(row.token_hash) })),
      ...
      resourceTags: this.database
        .query<Row, []>('SELECT * FROM resource_tags ORDER BY resource_id, tag_id')
        .all()
        .map((row) => ({
      ...
      navigationItems: this.database
        .query<Row, []>('SELECT * FROM navigation_items ORDER BY actor_key, entity_type, entity_id')
        .all()
        .map(mapNavigationPreference),
    }
  }
  ...
  importSnapshot(snapshot: ControlPlaneSnapshot, options: ImportSnapshotOptions = {}): void {
    if (snapshot.format !== 'ts-cloud-control-plane') throw new Error('Unsupported control-plane snapshot format')
    ...
    this.transaction(() => {
      const populated =
        Number(this.database.query<Row, []>('SELECT COUNT(*) AS count FROM projects').get()?.count ?? 0) > 0 ||
      ...
      for (const item of snapshot.environments) {
        run(
          ...
          [
            ...
            item.kind,
            item.region ?? null,
            json(item.desiredState),
            json(item.discoveredState),
            item.version,
            item.createdAt,
            item.updatedAt,
          ],
      ...
      for (const item of snapshot.invitations ?? []) {
        const tokenHash =
          item.tokenHash ?? createHash('sha256').update(`revoked:${item.id}:${this.idFn()}`).digest('hex')
        run(
          this.database,
          ...
          [
            ...
            item.scope.id ?? null,
            tokenHash,
            item.invitedByActorId ?? null,
            item.acceptedByActorId ?? null,
            item.expiresAt,
            item.acceptedAt ?? null,
            item.tokenHash ? (item.revokedAt ?? null) : (item.revokedAt ?? this.now()),
            item.createdAt,
            item.updatedAt,
          ],
        )
      ...
      for (const [key, value] of Object.entries(snapshot.settings))
        run(this.database, 'INSERT INTO settings (key, value, updated_at) VALUES (?, ?, ?)', [
          key,
          json(value),
          this.now(),
        ])
      ...
      for (const item of snapshot.resourceTags ?? [])
        run(this.database, 'INSERT INTO resource_tags (resource_id, tag_id, created_at) VALUES (?, ?, ?)', [
          item.resourceId,
          item.tagId,
          item.createdAt,
        ])
      ...
      for (const item of snapshot.navigationItems ?? []) {
        run(
          this.database,
          'INSERT INTO navigation_items (actor_key, entity_type, entity_id, favorite, last_visited_at, visit_count) VALUES (?, ?, ?, ?, ?, ?)',
