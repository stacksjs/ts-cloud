...
function membership(
  ...
  scope: OrganizationMembership['scope'] = { type: 'organization' },
...
describe('organization authorization', () => {
  ...
  it('inherits a role only through its assigned resource ancestry', () => {
    ...
    expect(
      authorizeOrganization({ membership: deployer, grants: [], capability: 'deployments:create', target }).allowed,
    ).toBe(true)
    expect(
  ...
  it('applies explicit deny before role and explicit allow grants', () => {
    const grants: AuthorizationGrant[] = [
      {
        ...
        scope: { type: 'resource', id: 'resource-1' },
        source: 'manual',
        createdAt: '',
        updatedAt: '',
      },
    ...
    expect(
      authorizeOrganization({ membership: membership('owner'), grants, capability: 'secrets:read', target }),
    ...
    ).toMatchObject({ allowed: true, reason: 'explicit-allow' })
  })

  it('denies inactive and cross-organization memberships before evaluating grants', () => {
    expect(
      authorizeOrganization({
        membership: { ...membership('owner'), status: 'revoked' },
        grants: [],
...
describe('organization membership and invitations', () => {
  it('stores only invitation hashes and rejects expiry and replay', () => {
    ...
    const expiring = store.createInvitation({
      organizationId: organization.id,
  ...
  it('protects the last owner and invalidates membership sessions on changes', () => {
    ...
    const first = store.createMembership({
      organizationId: organization.id,
  ...
  it('adds an invitation scope to an existing member without replacing prior access', () => {
    ...
    const firstEnvironment = store.createEnvironment({
      projectId: project.id,
      slug: 'staging',
      name: 'Staging',
      kind: 'staging',
    })
    const secondEnvironment = store.createEnvironment({
      projectId: project.id,
      slug: 'production',
      name: 'Production',
      kind: 'production',
    ...
    const membership = store.createMembership({
      ...
      actorId: actor.id,
      roleTemplate: 'viewer',
      scope: { type: 'environment', id: firstEnvironment.id },
    })
    const created = store.createInvitation({
      organizationId: organization.id,
      email: 'dev@acme.test',
      roleTemplate: 'deployer',
      scope: { type: 'environment', id: secondEnvironment.id },
    })
  ...
  it('rejects guessed scopes from another organization and isolates audit events', () => {
    ...
    expect(store.listEvents({ organizationId: first.id }).every((event) => event.organizationId === first.id)).toBe(
      true,
    )
    expect(store.listEvents({ organizationId: first.id }).map((event) => event.payload)).not.toEqual(
      store.listEvents({ organizationId: second.id }).map((event) => event.payload),
    )
    store.close()
  })
})
