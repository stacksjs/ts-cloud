...
export interface PlatformUpdateDocument {
  ...
  artifacts: Array<{ platform: string; architecture: string; url: string; sha256: string; sizeBytes: number }>
  migrations: Array<{ id: string; reversible: boolean; description: string }>
  releaseNotes?: string
}
...
export interface UpgradeCampaign {
  ...
  status:
    'planned' | 'queued' | 'running' | 'paused' | 'rolling_back' | 'succeeded' | 'failed' | 'rolled_back' | 'cancelled'
  currentStage?: string
  backupId?: string
  approvedBy?: string
  createdAt: string
...
export interface DisasterRecoveryDriver {
  ...
  restore(input: {
    ...
    signal: AbortSignal
  }): Promise<{ target: string; recoveryPointAt: string; evidence: Record<string, JsonValue> }>
  verify(input: {
    drill: DisasterRecoveryDrill
    target: string
  }): Promise<{ healthy: boolean; evidence: Record<string, JsonValue> }>
