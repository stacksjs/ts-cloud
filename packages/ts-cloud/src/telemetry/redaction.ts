...
const SENSITIVE_KEY = /(?:authorization|cookie|password|passwd|secret|token|api[_-]?key|private[_-]?key|access[_-]?key|session)/i
const SENSITIVE_TEXT = /\b(?:authorization|cookie|password|passwd|secret|token|api[_-]?key|private[_-]?key|access[_-]?key)\s*[=:]\s*[^\s,;&\[]+/gi
...
export function redactTelemetryValue(value: unknown, options: TelemetryRedactionOptions = {}, key?: string): JsonValue {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const [childKey, child] of Object.entries(value)) result[childKey] = redactTelemetryValue(child, options, childKey)
    return result
  }
  return String(value ?? '')
}
