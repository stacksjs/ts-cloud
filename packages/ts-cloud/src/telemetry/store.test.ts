...
describe('telemetry safety and persistence', () => {
  it('redacts query values, credentials, email, and sensitive attributes before persistence', () => {
    ...
    const record = telemetry.append({
      ...
      message: 'GET /users?email=person@example.com authorization=Bearer-secret',
      attributes: { cookie: 'session=secret', nested: { email: 'person@example.com' } },
    })
    ...
    expect(redactTelemetryText('https://x.test/a?token=one&safe=two')).toBe(
      'https://x.test/a?token=[REDACTED]&safe=[REDACTED]',
    )
  ...
  it('enforces project/resource scope and resumes cursor pagination without overlap', () => {
    ...
    for (let minute = 0; minute < 3; minute++)
      telemetry.append({
        ...
        source: 'journald',
        name: 'api.log',
        timestamp: `2026-07-21T11:0${minute}:00Z`,
        message: `line ${minute}`,
      })
    const first = telemetry.query({
      projectId: project.id,
      environmentId: environment.id,
...
describe('telemetry aggregation and time semantics', () => {
  it('uses nearest-rank percentiles and emits explicit empty buckets', () => {
    ...
    for (const [second, value] of [
      ...
      telemetry.append({
        ...
        unit: 'ms',
        timestamp: `2026-07-21T11:00:${second.toString().padStart(2, '0')}Z`,
        value,
      })
    const [series] = telemetry.series({
      ...
      from: '2026-07-21T11:00:00Z',
      to: '2026-07-21T11:03:00Z',
      bucketMs: 60_000,
      aggregation: 'p95',
      timezone: 'UTC',
    })
    expect(telemetryPercentile([10, 20, 30, 40, 100], 95)).toBe(100)
    expect(series.points.map((point) => ({ value: point.value, gap: point.gap }))).toEqual([
      { value: 100, gap: false },
      { value: undefined, gap: true },
      { value: undefined, gap: true },
    ])
  })
  ...
  it('aligns a comparison period to the current bucket sequence', () => {
    ...
    const [series] = telemetry.series({
      projectId: project.id,
      environmentId: environment.id,
      names: ['traffic.requests'],
      from: '2026-07-21T11:00:00Z',
      to: '2026-07-21T11:01:00Z',
      compareFrom: '2026-07-21T10:00:00Z',
      compareTo: '2026-07-21T10:01:00Z',
      bucketMs: 60_000,
      aggregation: 'sum',
    })
  ...
  it('bounds ranges and enforces age/count retention', () => {
    ...
    expect(() =>
      telemetry.query({ projectId: project.id, from: '2026-01-01T00:00:00Z', to: '2026-07-21T00:00:00Z' }),
    ).toThrow('31 days')
    expect(
      telemetry.enforceRetention(
        { rawDays: 30, downsampleAfterDays: 7, downsampleBucketMs: 3_600_000, maxRecords: 1 },
        project.id,
      ).deleted,
    ).toBe(1)
  })
  ...
  it('downsamples older raw metrics with count/min/max/p95 provenance', () => {
    ...
    expect(
      telemetry.downsample(
        { rawDays: 30, downsampleAfterDays: 7, downsampleBucketMs: 3_600_000, maxRecords: 100 },
        project.id,
      ),
    ).toEqual({ compacted: 2, rollups: 1 })
    const result = telemetry.query({ projectId: project.id, from: '2026-07-11T10:00:00Z', to: '2026-07-11T11:00:00Z' })
    expect(result.records[0]).toMatchObject({
      source: 'host:rollup',
      value: 20,
      attributes: { count: 2, min: 10, max: 30, p95: 30 },
    })
  })
})
