...
export function exportCompose(manifest: ComposeApplicationManifest): string {
  const services = Object.fromEntries(Object.entries(manifest.spec.services).map(([name, service]) => [name, {
    ...(service.image ? { image: service.image } : {}), ...(service.build ? { build: { context: service.build.context, dockerfile: service.build.dockerfile, ...(service.build.target ? { target: service.build.target } : {}), ...(Object.keys(service.build.args).length ? { args: Object.fromEntries(Object.entries(service.build.args).map(([key, value]) => [key, typeof value === 'string' ? value : `\${${value.secretRef}}`])) } : {}) } } : {}),
    ...(service.command ? { command: service.command } : {}), ...(service.entrypoint ? { entrypoint: service.entrypoint } : {}), ...(Object.keys(service.environment).length ? { environment: Object.fromEntries(Object.entries(service.environment).map(([key, value]) => [key, typeof value === 'string' ? value : `\${${value.secretRef}}`])) } : {}),
