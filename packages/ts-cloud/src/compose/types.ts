...
export interface ComposeBuild {
  ...
  dockerfile: string
  target?: string
  args: Record<string, string | { secretRef: string }>
}
...
export interface ComposeCatalogResult {
  valid: boolean
  source: string
  templates: Array<{ template: ComposeTemplate; parsed: ComposeParseResult }>
  diagnostics: ComposeDiagnostic[]
}
export interface ComposeTemplateUpgradePlan {
  template: ComposeTemplate
  parsed: ComposeParseResult
  templateChanges: Array<{ path: string; before?: unknown; after?: unknown }>
  userChanges: Array<{ path: string; before?: unknown; after?: unknown }>
}
