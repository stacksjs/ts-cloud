...
describe('Compose subset conversion', () => {
  it('normalizes and round-trips web, worker, Postgres, and Redis without secrets', () => {
    ...
    expect(parsed.manifest.spec.services.web).toMatchObject({
      ports: [{ published: 8080, target: 3000 }],
      environment: { DATABASE_URL: { secretRef: 'DATABASE_URL' } },
  ...
  it('orders health dependencies, reports cycles, and emits inspectable diffs', () => {
    ...
    const first = parseCompose(representative, target).manifest
    const next = structuredClone(first)
    next.spec.services.web.replicas = 3
...
describe('versioned Compose templates', () => {
  it('exposes verified metadata and resolves to a normal editable manifest', () => {
    expect(listComposeTemplates()).toEqual(
      expect.arrayContaining([
  ...
  it('validates checksum-pinned local catalogs and separates upgrade from user changes', () => {
    ...
    const catalog = parseComposeCatalog(
      JSON.stringify({
        apiVersion: 'ts-cloud.dev/compose-catalog/v1',
        source: 'file:./catalog.json',
        templates: [{ id: 'custom-app', version: '1.0.0', name: 'Custom app', compose, checksum }],
      }),
      target,
    )
    expect(catalog).toMatchObject({
      valid: true,
      templates: [{ template: { id: 'custom-app', builtin: false, checksum }, parsed: { valid: true } }],
    })
    const previous = renderComposeTemplate('wordpress', { domain: 'cms.example.com' }, target, '1.0.0').manifest
    const current = structuredClone(previous)
    current.spec.services.wordpress.replicas = 2
    const plan = planComposeTemplateUpgrade(
      current,
      previous,
      'wordpress',
    ...
    expect(plan.templateChanges).toEqual([])
    expect(plan.userChanges).toContainEqual({ path: 'spec.services.wordpress.replicas', before: 1, after: 2 })
  })
})
