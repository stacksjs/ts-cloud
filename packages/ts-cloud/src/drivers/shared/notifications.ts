 ...
 */
import type { NotificationsConfig, NotifyEvent } from '@ts-cloud/core'
...
export function buildNotifierScript(config: NotificationsConfig | undefined): string[] {
  ...
  if (config.slack?.webhookUrl)
    curls.push(
      `curl -fsS -X POST -H 'Content-Type: application/json' -d "{\\"text\\":\\"$MSG\\"}" ${config.slack.webhookUrl} || true`,
    )
  if (config.discord?.webhookUrl)
    curls.push(
      `curl -fsS -X POST -H 'Content-Type: application/json' -d "{\\"content\\":\\"$MSG\\"}" ${config.discord.webhookUrl} || true`,
    )
  if (config.telegram?.botToken && config.telegram.chatId)
    curls.push(
      `curl -fsS -X POST "https://api.telegram.org/bot${config.telegram.botToken}/sendMessage" --data-urlencode "chat_id=${config.telegram.chatId}" --data-urlencode "text=$MSG" || true`,
    )
  if (config.webhook?.url)
    curls.push(
      `curl -fsS -X POST -H 'Content-Type: application/json' -d "{\\"message\\":\\"$MSG\\"}" ${config.webhook.url} || true`,
    )
