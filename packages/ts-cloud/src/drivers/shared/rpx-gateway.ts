...
export function buildCertManagementCommands(options: BuildRpxProvisionOptions): string[] {
  ...
  const renewService = [
    '[Unit]',
    'Description=Issue/renew rpx gateway TLS certs (tlsx http-01)',
    `After=network-online.target ${RPX_SERVICE_NAME}`,
    'Wants=network-online.target',
    '',
    '[Service]',
    'Type=oneshot',
    `ExecStart=${RPX_CERT_RENEW_SCRIPT}`,
  ].join('\n')

  const renewTimer = [
    '[Unit]',
    'Description=Daily rpx gateway TLS cert issuance/renewal',
    '',
    '[Timer]',
    'OnCalendar=*-*-* 03:30:00',
  ...
  return [
    ...
    ...writeFileHeredoc(RPX_CERT_RENEW_SCRIPT, renewScript, 'TS_CLOUD_RENEW_EOF'),
    `chmod +x ${RPX_CERT_RENEW_SCRIPT}`,
    ...writeFileHeredoc(`/etc/systemd/system/${RPX_CERT_RENEW_SERVICE}`, renewService, 'TS_CLOUD_RENEW_SVC_EOF'),
    ...writeFileHeredoc(`/etc/systemd/system/${RPX_CERT_RENEW_TIMER}`, renewTimer, 'TS_CLOUD_RENEW_TIMER_EOF'),
    'systemctl daemon-reload',
    `systemctl enable --now ${RPX_CERT_RENEW_TIMER} || true`,
    // Initial issuance now (gateway is already up to answer the challenge).
    `${RPX_CERT_RENEW_SCRIPT} || true`,
  ]
}
