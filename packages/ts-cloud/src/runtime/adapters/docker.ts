...
export interface DockerInspectRecord {
  ...
  NetworkSettings?: {
    Networks?: Record<string, { NetworkID?: string; IPAddress?: string; GlobalIPv6Address?: string }>
...
function dockerNetworks(record: DockerInspectRecord): RuntimeNetwork[] {
  const ports = Object.entries(record.NetworkSettings?.Ports ?? {}).flatMap(([target, bindings]) => {
    ...
    return (bindings ?? []).map((binding) => ({
      ...
      host: Number(binding.HostPort) || undefined,
      protocol,
...
export function dockerWorkloads(
  records: DockerInspectRecord[],
  context: RuntimeDiscoveryContext,
  sourceId: string = context.server ?? 'local',
): RuntimeWorkload[] {
  const now = context.now ?? new Date()
  return records.map((record) => {
    const rawStatus = record.State?.OOMKilled
      ? 'oomkilled'
    ...
    return {
      id: runtimeId('docker', sourceId, record.Id),
      provider: 'docker',
      kind: 'container',
      name,
      status,
      ...
      image: record.Config?.Image,
      runtime: 'docker',
      ageSeconds: ageSeconds(record.State?.StartedAt, now),
      restartCount: record.RestartCount ?? 0,
      tags: labels,
      links: {
        project: context.project,
        environment: context.environment,
        server: context.server ?? sourceId.replace(/^docker:/, ''),
        service: labels['com.docker.compose.service'] ?? labels['ts-cloud.service'] ?? name,
        release: labels['ts-cloud.release'],
        providerId: record.Id,
      },
      resources: container.resources,
      replicas: [
        {
          id: runtimeId('docker', sourceId, `${record.Id}:0`),
          name,
          status,
          rawStatus,
          startedAt: record.State?.StartedAt,
          stoppedAt: record.State?.FinishedAt,
      ...
      capabilities: capabilities(
        ['start', 'stop', 'restart', 'logs', 'exec', 'inspect', 'files'],
        'Use the owning Compose service for redeploy and scale',
      ),
      config: redactRuntimeConfig({
        image: record.Config?.Image,
        labels,
        environmentKeys: (record.Config?.Env ?? []).map((value) => value.split('=', 1)[0]),
...
export class DockerDiscoveryAdapter implements RuntimeDiscoveryAdapter {
  ...
  constructor(
    private readonly inspect: (context: RuntimeDiscoveryContext) => Promise<string>,
    id = 'docker:local',
  ) {
    this.id = id
  }
  async discover(context: RuntimeDiscoveryContext): Promise<RuntimeWorkload[]> {
    return dockerWorkloads(parseDockerInspect(await this.inspect(context)), context, this.id)
  }
}
