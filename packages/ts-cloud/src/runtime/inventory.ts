...
export async function discoverRuntimeInventory(
  ...
  const settled = await Promise.all(adapters.map(async (adapter): Promise<{ workloads: RuntimeWorkload[], source: RuntimeSourceStatus }> => {
    try {
      const workloads = await Promise.race([
        adapter.discover({ ...context, now }),
        new Promise<never>((_, reject) => setTimeout(() => reject(new Error(`Runtime source ${adapter.id} timed out after ${timeoutMs}ms`)), timeoutMs)),
      ])
      return { workloads, source: { id: adapter.id, provider: adapter.provider, status: 'fresh', discoveredAt: now.toISOString(), staleAfterSeconds, itemCount: workloads.length } }
    }
    catch (error: any) {
      return { workloads: [], source: { id: adapter.id, provider: adapter.provider, status: errorStatus(error), discoveredAt: now.toISOString(), staleAfterSeconds, itemCount: 0, message: String(error?.message ?? error).slice(0, 500) } }
    }
  }))
  const sources = settled.map(result => result.source)
  return {
    generatedAt: now.toISOString(),
    workloads: settled.flatMap(result => result.workloads).sort((a, b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id)),
    sources,
