...
function bounded(value: string): string {
  ...
  return encoded.byteLength <= MAX_OUTPUT_BYTES
    ? value
    : `${encoded.subarray(0, MAX_OUTPUT_BYTES - 24).toString('utf8')}\n[output truncated]`
}
...
function remoteTarget(targets: ComputeTarget[], workload: RuntimeWorkload): ComputeTarget | undefined {
  return (
    targets.find((target) => target.id === workload.links.server) ?? (targets.length === 1 ? targets[0] : undefined)
  )
}
 ...
 */
export function authorizeRuntimePath(
  _workload: RuntimeWorkload,
  _requested: string,
): { ok: true; path: string } | { ok: false; error: string } {
  if (!_requested.startsWith('/') || _requested.includes('\0'))
  ...
  if (SENSITIVE_PATHS.some((root) => normalized === root || normalized.startsWith(`${root}/`)))
    return { ok: false, error: 'Sensitive host paths are not available through runtime file access.' }
  ...
  if (!allowed.some((root) => normalized === root || normalized.startsWith(`${root.replace(/\/$/, '')}/`)))
    return { ok: false, error: 'Path is outside this service’s owned roots.' }
...
function runtimePathRoot(workload: RuntimeWorkload, requested: string): string | undefined {
  ...
  return [
    ...
    .find((root) => requested === root || requested.startsWith(`${root.replace(/\/$/, '')}/`))
}
...
export class RuntimeOperationService {
  ...
  constructor(
    ...
    private readonly dependencies: RuntimeOperationDependencies = {},
  ) {
  ...
  async inventory(): Promise<RuntimeInventory> {
    return (
      ...
      resolveRuntimeInventory(this.config, this.environment, { driver: this.dependencies.driver })
    )
  }
  ...
  private async remote(workload: RuntimeWorkload, commands: string[], timeoutSeconds: number, comment: string) {
    ...
    return driver.runRemoteDeploy({
      targets: [target],
      commands,
      timeoutSeconds,
      comment,
      tags: { Project: this.config.project.slug, Environment: this.environment, Role: 'app' },
    })
  }
  ...
  async run(input: RuntimeOperationInput): Promise<RuntimeOperationResult> {
    ...
    if (!workload)
      return { ...base, ok: false, error: 'Workload was not found in the current project and environment.' }
    const capability = workload.capabilities[input.action]
    if (!capability.supported)
      return {
        ...base,
        ok: false,
        error: capability.reason ?? `${input.action} is unsupported for ${workload.provider}.`,
      }
    if (capability.requiresConfirmation && input.confirm !== workload.name)
      return { ...base, ok: false, error: `Type "${workload.name}" to ${input.action} this workload.` }
    if (capability.requiresRecentAuth && !input.recentAuth)
      return { ...base, ok: false, error: 'Sign in again before using elevated runtime access.' }
    ...
    if (workload.provider === 'systemd' || workload.provider === 'docker') {
      ...
      const result = await this.remote(
        workload,
        ['set -uo pipefail', ...commands],
        60,
        `ts-cloud runtime:${input.action}`,
      ).catch(remoteFailure)
      return {
        ...base,
        ok: result.success,
        command: commands[0],
        stdout: bounded(result.perInstance?.[0]?.output ?? ''),
        stderr: bounded(result.perInstance?.[0]?.error ?? result.error ?? ''),
        error: result.success ? undefined : (result.error ?? 'Runtime operation failed.'),
      }
    }
  ...
  async exec(input: RuntimeExecInput): Promise<RuntimeExecResult> {
    ...
    if (!workload)
      return { ...base, ok: false, error: 'Workload was not found in the current project and environment.' }
    if (process.env.TS_CLOUD_RUNTIME_EXEC === '0')
      return { ...base, ok: false, error: 'Runtime exec is disabled by TS_CLOUD_RUNTIME_EXEC.' }
    if (!workload.capabilities.exec.supported)
      return {
        ...base,
        ok: false,
        error: workload.capabilities.exec.reason ?? 'Exec is unsupported for this workload.',
      }
    if (input.confirm !== workload.name)
      return { ...base, ok: false, error: `Type "${workload.name}" to open exec access.` }
    ...
    if (!preset && !input.command?.trim())
      return { ...base, ok: false, error: 'Choose a diagnostic preset or provide a command.' }
    if (input.command && workload.provider !== 'docker')
      return {
        ...base,
        ok: false,
        error: 'Free-form exec is restricted to isolated containers; use a diagnostic preset for system services.',
      }
    ...
    const result = await this.remote(workload, ['set -uo pipefail', command], 60, 'ts-cloud runtime:exec').catch(
      remoteFailure,
    )
    return {
      ...base,
      ...
      stdout: bounded(result.perInstance?.[0]?.output ?? ''),
      stderr: bounded(result.perInstance?.[0]?.error ?? result.error ?? ''),
      error: result.success ? undefined : (result.error ?? 'Exec command failed.'),
    }
  }
  ...
  async readFile(input: RuntimeFileInput): Promise<RuntimeFileReadResult> {
    ...
    const script =
      'resolved=$(realpath -m "$1"); root=$(realpath -m "$3"); case "$resolved" in "$root"|"$root"/*) ;; *) exit 73;; esac; test -f "$resolved"; size=$(wc -c < "$resolved"); printf "__TSCLOUD_SIZE__=%s\\n" "$size"; head -c "$2" "$resolved" | base64'
    ...
    if (!result.success)
      return { ...base, ok: false, error: result.error ?? result.perInstance?.[0]?.error ?? 'File read failed.' }
    ...
    if (Buffer.from(contentBase64, 'base64').byteLength > MAX_FILE_BYTES)
      return { ...base, ok: false, error: 'The remote file exceeded the transfer limit.' }
  ...
  async writeFile(input: RuntimeFileWriteInput): Promise<RuntimeFileReadResult> {
    ...
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(input.contentBase64))
      return { ...base, ok: false, error: 'File content must be valid base64.' }
    const content = Buffer.from(input.contentBase64, 'base64')
    if (content.toString('base64') !== input.contentBase64)
      return { ...base, ok: false, error: 'File content must be canonical base64.' }
    if (content.byteLength > MAX_FILE_BYTES)
      return { ...base, ok: false, error: `File uploads are limited to ${MAX_FILE_BYTES} bytes.` }
    const { workload, path, root, providerId } = target
    const script =
      'resolved=$(realpath -m "$1"); root=$(realpath -m "$2"); case "$resolved" in "$root"|"$root"/*) ;; *) exit 73;; esac; mkdir -p "$(dirname "$resolved")"; tmp="$resolved.ts-cloud-$$"; base64 -d > "$tmp"; chmod 600 "$tmp"; mv "$tmp" "$resolved"'
    const inner = `sh -c ${shellQuote(script)} sh ${shellQuote(path)} ${shellQuote(root)}`
    const command =
      workload.provider === 'docker'
    ...
    return {
      ...
      truncated: false,
      error: result.success ? undefined : (result.error ?? result.perInstance?.[0]?.error ?? 'File write failed.'),
    }
  }

  private async fileTarget(
    input: RuntimeFileInput,
  ): Promise<{ workload: RuntimeWorkload; path: string; root: string; providerId: string } | { error: string }> {
    const workload = await this.workload(input.workloadId)
    if (!workload) return { error: 'Workload was not found in the current project and environment.' }
    if (process.env.TS_CLOUD_RUNTIME_FILE_ACCESS === '0')
      return { error: 'Runtime file access is disabled by TS_CLOUD_RUNTIME_FILE_ACCESS.' }
    if (!workload.capabilities.files.supported)
      return { error: workload.capabilities.files.reason ?? 'File access is unsupported for this workload.' }
    ...
    if (
      !root ||
      (workload.provider === 'systemd' && !SAFE_SYSTEMD.test(providerId)) ||
      (workload.provider === 'docker' && !SAFE_CONTAINER.test(providerId))
    )
      return { error: 'File target identity is invalid or unsupported.' }
  ...
  async logs(workloadId: string, input: { limit?: number; since?: Date } = {}): Promise<RuntimeLogResult> {
    ...
    if (!workload.capabilities.logs.supported)
      throw new Error(workload.capabilities.logs.reason ?? 'Logs are unsupported for this workload.')
    ...
    if (workload.provider === 'lambda' || workload.provider === 'ecs') {
      ...
      const logGroupName =
        workload.provider === 'lambda'
          ? `/aws/lambda/${workload.name}`
          : Array.isArray(configuredGroups) && typeof configuredGroups[0] === 'string'
            ? configuredGroups[0]
            : undefined
      ...
      const lines = (result.events ?? []).slice(0, limit).map((event) => ({
        timestamp: event.timestamp == null ? undefined : new Date(event.timestamp).toISOString(),
        message: bounded(String(event.message ?? '')),
      }))
      return {
        workloadId,
        ...
        truncated: (result.events?.length ?? 0) >= limit,
      }
    }
    const providerId = workload.links.providerId ?? ''
    ...
    return {
      ...
      provider: workload.provider,
      lines,
      truncated: raw.includes('[output truncated]') || lines.length >= limit,
    }
  }
}
