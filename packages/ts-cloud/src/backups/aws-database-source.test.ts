...
function fixture(provider: 'aws_rds' | 'aws_aurora') {
  ...
  const organization = control.createOrganization({ slug: 'acme', name: 'Acme' }),
    ...
    data = new DataServiceStore(control),
    service = data.create({
      organizationId: organization.id,
      projectId: project.id,
      environmentId: environment.id,
      name: provider === 'aws_rds' ? 'orders-db' : 'orders-aurora',
      engine: 'postgres',
      provider,
      placement: provider === 'aws_rds' ? 'orders-db' : 'orders-aurora',
      engineVersion: '16.3',
      plan: provider === 'aws_rds' ? 'db.t4g.small' : 'db.serverless',
      highAvailability: provider === 'aws_aurora',
    ...
    policy = backups.createPolicy({
      ...
      name: `${service.name}-daily`,
      resourceKind: 'managed_database',
      schedule: 'daily',
      timezone: 'UTC',
      retention: { expireAfterDays: 30 },
      compression: 'none',
...
describe('AWS managed database backup source', () => {
  it('creates, verifies, restores, and deletes RDS snapshots through protected targets', async () => {
    const target = fixture('aws_rds'),
      calls: Array<[string, any]> = [],
      client = {
        createDBSnapshot: async (input: any) => {
          calls.push(['snapshot', input])
          return {}
        },
        describeDBSnapshots: async (input: any) => {
          calls.push(['verify', input])
          return { DBSnapshots: [{ DBSnapshotStatus: 'available', Encrypted: true }] }
        },
        ...
        deleteDBInstance: async (input: any) => {
          calls.push(['delete-target', input])
          return {}
        },
        deleteDBSnapshot: async (id: string) => {
          calls.push(['delete-snapshot', id])
          return {}
        },
      } as unknown as RDSClient,
      source = new AwsDatabaseBackupSource(target.data, client),
      created = await source.create(target.policy, context)
    expect(created).toMatchObject({
      mode: 'external',
      manifest: { provider: 'aws_rds', sourceId: 'orders-db' },
    ...
    const point = target.backups.createRecoveryPoint({
      ...
      dataServiceId: target.service.id,
      kind: 'managed_database',
      pointInTime: new Date().toISOString(),
      uri: created.mode === 'external' ? created.uri : '',
      sizeBytes: 0,
      checksum: created.mode === 'external' ? created.checksum : '',
    ...
    expect(await source.restore(point, undefined, { targetId: 'orders-drill' }, context)).toMatchObject({
      targetId: 'orders-drill',
      isolated: true,
    })
    expect(await source.validateHealth({ targetId: 'orders-drill', provider: 'aws_rds' }, context)).toMatchObject({
      healthy: true,
      status: 'available',
    })
    await source.cleanup({ targetId: 'orders-drill', provider: 'aws_rds' }, context)
    await source.deleteExternal(point, context)
    expect(calls.find(([kind]) => kind === 'restore')?.[1]).toMatchObject({
  ...
  it('restores Aurora snapshots with a private writer and cleans the isolated cluster', async () => {
    const target = fixture('aws_aurora'),
      calls: Array<[string, any]> = [],
      client = {
        createDBClusterSnapshot: async (source: string, snapshot: string) => {
          calls.push(['snapshot', { source, snapshot }])
          return {}
        },
        describeDBClusterSnapshots: async () => ({
          DBClusterSnapshots: [{ Status: 'available', StorageEncrypted: true }],
        }),
        ...
        deleteDBInstance: async (input: any) => {
          calls.push(['delete-writer', input])
          return {}
        },
        modifyDBCluster: async (input: any) => {
          calls.push(['modify', input])
          return {}
        },
        deleteDBCluster: async (id: string) => {
          calls.push(['delete-cluster', id])
          return {}
        },
        deleteDBClusterSnapshot: async (id: string) => {
          calls.push(['delete-snapshot', id])
        },
      } as unknown as RDSClient,
      source = new AwsDatabaseBackupSource(target.data, client),
    ...
    expect(await source.validateHealth({ targetId: 'aurora-drill', provider: 'aws_aurora' }, context)).toMatchObject({
      healthy: true,
      status: 'available',
    })
