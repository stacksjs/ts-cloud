  ...
  it('creates, verifies, restores, and expires provider recovery points', async () => {
    const calls: Array<[string, unknown]> = [],
      client: InfrastructureBackupClient = {
        startBackupJob: async (input) => {
          calls.push(['start-backup', input])
          return { BackupJobId: 'backup-job' }
        },
        describeBackupJob: async (id) => {
          calls.push(['describe-backup', id])
          return {
            State: 'COMPLETED',
            RecoveryPointArn: 'arn:aws:backup:us-east-1:123:recovery-point:one',
            BackupSizeInBytes: 42,
          }
        },
        startRestoreJob: async (input) => {
          calls.push(['start-restore', input])
          return { RestoreJobId: 'restore-job' }
        },
        describeRestoreJob: async (id) => {
          calls.push(['describe-restore', id])
          return { Status: 'COMPLETED', CreatedResourceArn: 'arn:aws:ec2:us-east-1:123:volume/vol-restored' }
        },
        deleteRecoveryPoint: async (vault, arn) => {
          calls.push(['delete', { vault, arn }])
        },
      },
      source = new AwsInfrastructureBackupSource(client),
      created = await source.create(
        {
          projectId: 'project',
          includePatterns: [
            'resource:arn:aws:ec2:us-east-1:123:volume/vol-source',
            'role:arn:aws:iam::123:role/backup',
            'vault:production',
          ],
        } as any,
        context(),
    ...
    expect(created).toMatchObject({
      mode: 'external',
      uri: 'arn:aws:backup:us-east-1:123:recovery-point:one',
      sizeBytes: 42,
      manifest: { vaultName: 'production', providerBackupJobId: 'backup-job' },
    })
    const point = {
    ...
    expect(restored).toMatchObject({
      createdResourceArn: 'arn:aws:ec2:us-east-1:123:volume/vol-restored',
      isolated: true,
    })
