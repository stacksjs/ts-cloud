...
class MemoryClient implements BackupObjectClient {
  ...
  async createMultipartUpload() {
    this.uploads.set('upload-1', new Map())
  ...
  async uploadPart(_bucket: string, _key: string, uploadId: string, partNumber: number, body: Uint8Array | Buffer) {
    ...
    this.uploads.get(uploadId)!.set(partNumber, Buffer.from(body))
  ...
  async abortMultipartUpload(_bucket: string, _key: string, uploadId: string) {
...
const destination = (encryption: BackupDestination['encryption'] = 'provider'): BackupDestination => ({
  ...
  region: 'us-east-1',
  forcePathStyle: true,
  credentialRef: 'secret://credentials',
  encryption,
  encryptionKeyRef: encryption === 'provider' ? undefined : 'secret://encryption',
  immutability: {},
  status: 'healthy',
  version: 1,
  createdAt: '',
...
describe('S3 backup destination adapter', () => {
  it('uses referenced credentials and health-tests custom endpoints without leaking keys', async () => {
    const client = new MemoryClient(),
      seen: unknown[] = [],
      adapter = new S3BackupDestinationAdapter(
        {
          resolve: async (ref) =>
            ref === 'secret://credentials'
  ...
  it('encrypts client-side, checks ciphertext and plaintext integrity, and detects corruption', async () => {
    const client = new MemoryClient(),
      ...
      body = Buffer.from('critical recovery data'),
      stored = await adapter.upload(destination('both'), { key: 'point.bin', body })
  ...
  it('resumes multipart uploads from persisted checkpoints and cleans abandoned uploads', async () => {
    ...
    const checkpoints: MultipartCheckpoint[] = [],
      stored = await adapter.upload(destination(), {
        key: 'large.bin',
    ...
    await adapter.abortPartial(destination(), {
      uploadId: 'abandoned',
      key: 'production/partial',
      parts: [],
      bytesUploaded: 0,
    })
  ...
  it('resumes encrypted multipart uploads with the same authenticated envelope', async () => {
    const client = new MemoryClient(),
      ...
      body = Buffer.from('abcdefghij'),
      checkpoints: MultipartCheckpoint[] = []
    client.failPartOnce = 2
    await expect(
      adapter.upload(destination('both'), {
        key: 'encrypted.bin',
        body,
        checkpoint: (value) => checkpoints.push(structuredClone(value)),
      }),
  ...
  it('reports expiring credentials and rejects expired credentials before use', async () => {
    ...
    expect(await backupCredentialStatus(destination(), secrets, new Date('2026-07-21T00:00:00.000Z'))).toEqual({
      status: 'expiring',
      expiresAt,
    })
    const expired = new S3BackupDestinationAdapter(
      {
        resolve: async () =>
          JSON.stringify({ accessKeyId: 'access', secretAccessKey: 'secret', expiresAt: '2000-01-01T00:00:00.000Z' }),
      },
      () => new MemoryClient(),
    )
