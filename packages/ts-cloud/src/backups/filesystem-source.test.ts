...
describe('filesystem backup source', () => {
  it('archives scoped files, restores an isolated target, validates it, and cleans it', async () => {
    const target = await fixture(), created = await target.source.create(target.policy, { operation: { id: 'operation-1' } } as any)
    expect(created).toMatchObject({ mode: 'object', key: 'project/files/operation1.tar.zst', manifest: { sourcePaths: ['shared'], entries: 2, compression: 'zstd' } })
    if (created.mode !== 'object') throw new Error('Expected an object backup.')
    const restored = await target.source.restore({ manifest: created.manifest } as any, created.body, { targetId: 'recovery-one', restoreMode: 'isolated' }, {} as any)
  ...
  it('atomically replaces declared paths for an explicitly in-place restore', async () => {
    const target = await fixture(), created = await target.source.create(target.policy, { operation: { id: 'operation-2' } } as any)
    if (created.mode !== 'object') throw new Error('Expected an object backup.')
    await writeFile(resolve(target.root, 'shared', 'avatar.txt'), 'changed')
    await target.source.restore({ manifest: created.manifest } as any, created.body, { targetId: 'replace-one', restoreMode: 'in_place' }, {} as any)
  ...
  it('refuses source paths outside the project root', async () => {
    ...
    await expect(target.source.create({ ...target.policy, includePatterns: ['../secrets'] }, {} as any)).rejects.toThrow('outside')
  })
})
