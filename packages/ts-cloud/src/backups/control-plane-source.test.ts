...
describe('control-plane backup source', () => {
  it('serializes a consistent database and config into an isolated verified target', async () => {
    ...
    const source = new ControlPlaneBackupSource(control, root),
      created = await source.create(
        { projectId: 'project', includePatterns: [], compression: 'gzip' } as any,
        { operation: { id: 'control-operation' } } as any,
      )
    expect(created).toMatchObject({
      mode: 'object',
      key: 'project/control-plane/controloperation.json.gz',
      manifest: { sensitive: true, configFiles: ['cloud.config.ts'], compression: 'gzip' },
    })
    ...
    const restored = await source.restore(
      ...
      created.body,
      { targetId: 'drill-one', restoreMode: 'isolated' },
      {} as any,
    )
    expect(await source.verify({ manifest: created.manifest } as any, created.body, {} as any)).toMatchObject({
      integrity: 'ok',
      configFiles: 1,
    })
  ...
  it('refuses to replace the running control-plane database in place', async () => {
    ...
    controls.push(control)
    const source = new ControlPlaneBackupSource(control, root),
      created = await source.create({ projectId: 'project', includePatterns: [] } as any, {} as any)
    if (created.mode !== 'object') throw new Error('Expected an object backup.')
    await expect(
      source.restore({} as any, created.body, { targetId: 'live-one', restoreMode: 'in_place' }, {} as any),
    ).rejects.toThrow('live control plane')
  })
})
