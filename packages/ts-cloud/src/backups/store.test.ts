...
describe('backup recovery control plane', () => {
  it('validates explicit custom endpoint and encryption security policy', () => {
    const base = {
      organizationId: 'org', projectId: 'project', name: 'offsite', provider: 's3_compatible' as const,
      endpoint: 'https://minio.internal/', endpointPolicy: 'public_https' as const, bucket: 'backups', prefix: '', region: 'us-east-1', forcePathStyle: true,
      credentialRef: 'secret://backup/offsite', encryption: 'both' as const, encryptionKeyRef: 'secret://backup/key', immutability: {}, status: 'untested' as const,
    }
  ...
  it('schedules policies, detects missed RPO, tracks verification, and enforces safe retention', () => {
    const target = fixture(),
      destination = target.backups.createDestination({
        organizationId: target.organization.id, projectId: target.project.id, name: 'archive', provider: 'aws_s3', endpointPolicy: 'public_https', bucket: 'acme-backups', prefix: 'production', region: 'us-west-2', forcePathStyle: false,
        encryption: 'provider', immutability: { objectLock: true, defaultRetentionDays: 30 }, status: 'untested',
      }),
    ...
    expect(target.backups.coverage(target.project.id)).toMatchObject([{ missedRpo: true, unverified: 0, destinationHealthy: true }])

    const backupJob = target.backups.createJob({ projectId: target.project.id, policyId: policy.id, kind: 'backup', status: 'succeeded', idempotencyKey: `${policy.id}:2026-07-21T11:30Z`, target: {}, cancellability: 'safe', progress: {}, startedAt: '2026-07-21T11:29:00.000Z', finishedAt: '2026-07-21T11:30:00.000Z' }),
      point = target.backups.createRecoveryPoint({
        projectId: target.project.id, policyId: policy.id, destinationId: destination.id, resourceId: target.resource.id, backupJobId: backupJob.id, kind: 'volume', pointInTime: '2026-07-21T11:30:00.000Z', uri: 's3://acme-backups/production/uploads.tar.zst', sizeBytes: 42, checksum: `sha256:${'a'.repeat(64)}`, manifest: { files: 3 }, expiresAt: '2026-07-21T11:59:00.000Z', held: false, pinned: false, status: 'available', verificationState: 'unverified',
      })
  ...
  it('deduplicates jobs and protects recovery points during restores', () => {
    const target = fixture(),
      destination = target.backups.createDestination({ organizationId: target.organization.id, projectId: target.project.id, name: 'archive', provider: 'aws_s3', endpointPolicy: 'public_https', bucket: 'acme', prefix: '', forcePathStyle: false, encryption: 'provider', immutability: {}, status: 'healthy' }),
      first = target.backups.createJob({ projectId: target.project.id, kind: 'backup', status: 'queued', idempotencyKey: 'stable-key', target: {}, cancellability: 'safe', progress: {} }),
      replay = target.backups.createJob({ projectId: target.project.id, kind: 'backup', status: 'queued', idempotencyKey: 'stable-key', target: { changed: true }, cancellability: 'safe', progress: {} })
    expect(replay.id).toBe(first.id)
    const point = target.backups.createRecoveryPoint({ projectId: target.project.id, destinationId: destination.id, kind: 'files', pointInTime: '2026-07-20T00:00:00.000Z', uri: 's3://acme/files.tar', sizeBytes: 1, checksum: `sha256:${'b'.repeat(64)}`, manifest: {}, expiresAt: '2026-07-20T01:00:00.000Z', held: false, pinned: false, status: 'available', verificationState: 'verified' })
    target.backups.createJob({ projectId: target.project.id, recoveryPointId: point.id, kind: 'restore', status: 'running', idempotencyKey: 'restore-1', target: { path: '/isolated' }, restoreMode: 'isolated', cancellability: 'checkpoint_only', progress: {} })
    expect(target.backups.retentionCandidates()).toEqual([])
  })
})
