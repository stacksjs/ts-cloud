...
export async function discoverOidcProvider(provider: AuthOidcProvider, fetchFn: OidcFetch = fetch): Promise<OidcDiscoveryDocument> {
  ...
  const document = await readJson(await fetchFn(discoveryUrl, {
...
export async function beginOidcAuthorization(
  ...
  authorizationUrl.searchParams.set('code_challenge', createHash('sha256').update(transaction.verifier).digest('base64url'))
...
function parseJwt(token: string): { header: Record<string, unknown>, claims: Record<string, unknown>, signingInput: string, signature: Buffer } {
  if (token.length > 32 * 1024)
    throw new Error('OIDC ID token is too large')
...
async function verifyIdTokenSignature(token: string, discovery: OidcDiscoveryDocument, fetchFn: OidcFetch): Promise<Record<string, unknown>> {
  ...
  if (!SUPPORTED_ALGORITHMS.has(algorithm) || !keyId)
    throw new Error('OIDC ID token uses an unsupported signing key')
  if (discovery.id_token_signing_alg_values_supported && !discovery.id_token_signing_alg_values_supported.includes(algorithm))
    throw new Error('OIDC ID-token algorithm was not advertised by the provider')
  const jwks = await readJson(await fetchFn(discovery.jwks_uri, {
    headers: { accept: 'application/json' },
    redirect: 'error',
    signal: AbortSignal.timeout(10_000),
  ...
  if (!jwk)
    throw new Error('OIDC ID-token signing key was not found')
  const key = createPublicKey({ key: jwk, format: 'jwk' })
  const verifier = createVerify('SHA256').update(parsed.signingInput).end()
  const valid = algorithm === 'PS256'
...
function verifyIdTokenClaims(claims: Record<string, unknown>, provider: AuthOidcProvider, nonce: string, now: Date): VerifiedOidcIdentity {
  if (claims.iss !== provider.issuer)
    throw new Error('OIDC ID-token issuer is invalid')
  const audience = typeof claims.aud === 'string'
    ? [claims.aud]
    : Array.isArray(claims.aud) ? claims.aud.filter(value => typeof value === 'string') : []
...
export async function completeOidcAuthorization(
  ...
  const tokens = await readJson(await fetchFn(discovery.token_endpoint, {
    method: 'POST',
    headers,
    body,
    redirect: 'error',
  ...
  return { identity: verifyIdTokenClaims(claims, provider, consumed.nonce, input.now ?? new Date()), returnPath: consumed.transaction.returnPath }
}
