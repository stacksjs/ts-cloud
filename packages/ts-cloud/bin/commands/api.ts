...
async function getApiGatewayClient(region: string) {
  const { AwsClient } = await import('../../src/aws/client')

  class ApiGatewayV2Client extends AwsClient {
    constructor(region: string) {
      super(region, 'apigateway')
    }

    // Override to use API Gateway v2 endpoint
    protected getEndpoint(): string {
      return `https://apigateway.${this.region}.amazonaws.com`
    }

    async getApis() {
      return this.request('GET', '/v2/apis')
    }

    async getApi(apiId: string) {
      return this.request('GET', `/v2/apis/${apiId}`)
    }

    async getStages(apiId: string) {
      return this.request('GET', `/v2/apis/${apiId}/stages`)
    }

    async getStage(apiId: string, stageName: string) {
      return this.request('GET', `/v2/apis/${apiId}/stages/${stageName}`)
    }

    async getRoutes(apiId: string) {
      return this.request('GET', `/v2/apis/${apiId}/routes`)
    }

    async getIntegrations(apiId: string) {
      return this.request('GET', `/v2/apis/${apiId}/integrations`)
    }

    async getDeployments(apiId: string) {
      return this.request('GET', `/v2/apis/${apiId}/deployments`)
    }

    async createDeployment(apiId: string, stageName?: string, description?: string) {
      return this.request('POST', `/v2/apis/${apiId}/deployments`, {
        StageName: stageName,
    ...
    async getDomainNames() {
      return this.request('GET', '/v2/domainnames')
    }
    ...
    async createApi(params: {
      ...
      return this.request('POST', '/v2/apis', params)
    }

    async deleteApi(apiId: string) {
      return this.request('DELETE', `/v2/apis/${apiId}`)
    }

    async createStage(apiId: string, params: {
      StageName: string
      DeploymentId?: string
      Description?: string
      AutoDeploy?: boolean
    }) {
      return this.request('POST', `/v2/apis/${apiId}/stages`, params)
    }

    async deleteStage(apiId: string, stageName: string) {
      return this.request('DELETE', `/v2/apis/${apiId}/stages/${stageName}`)
    }

    async updateStage(apiId: string, stageName: string, params: any) {
      return this.request('PATCH', `/v2/apis/${apiId}/stages/${stageName}`, params)
    }

    // REST API methods wrapper
    ...
    private async signedFetch(url: string, options: RequestInit): Promise<Response> {
      // Use AWS SDK v4 signing
      const { getCredentials, signRequest } = await import('../../src/aws/client')
      const credentials = await getCredentials()

      const signedOptions = await signRequest({
        url,
  ...
  return new ApiGatewayV2Client(region)
}
  ...
  app
    ...
    .action(async (options: { region?: string }) => {
      ...
      try {
        ...
        class SimpleApiGateway extends AwsClient {
          constructor(region: string) {
            super(region, 'apigateway')
          }
          ...
          async getRestApis() {
            const response = await fetch(
              ...
              await this.getSignedRequest('GET', '/restapis'),
            )
            return response.json()
          }

          private async getSignedRequest(method: string, path: string) {
            // Simple unsigned request for now
            return {
              method,
              headers: {
                'Content-Type': 'application/json',
        ...
        // For now, show a simplified view using CloudFormation
