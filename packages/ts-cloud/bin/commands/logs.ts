...
export function registerLogsCommands(app: CLI): void {
  ...
  app
    ...
    .action(async (options?: { env?: string }) => {
      try {
        ...
        const environment = (options?.env || 'production') as EnvironmentType
        const { region, functions } = resolveServerlessFunctions(config, environment)
        const metricsSpec = encodeURIComponent(
          JSON.stringify({
  ...
  app
    ...
    .action(async (options?: { prefix?: string; region?: string }) => {
      ...
      if (!region) {
        try {
          region = (await loadValidatedConfig()).project.region
        } catch {
          /* default below */
        }
      }
      try {
        ...
        cli.table(
          ...
          alarms.map((a) => [
            ...
            a.StateValue ?? '-',
            a.MetricName ?? '-',
            `${a.ComparisonOperator ?? ''} ${a.Threshold ?? ''}`.trim(),
          ]),
        )
      } catch (error: any) {
  ...
  app
    ...
    .option('--comparison <op>', 'GreaterThanThreshold | GreaterThanOrEqualToThreshold | LessThanThreshold', {
