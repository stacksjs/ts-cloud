...
export function registerEnvironmentCommands(app: CLI): void {
  app.command('env:list', 'List configured environments and their deployed stack status').action(async () => {
        ...
        if (envCfg?.app) {
          try {
            ...
            const { Stacks } = await cf.describeStacks({
              stackName: resolveServerlessAppStackName(config, env as EnvironmentType),
            })
  ...
  app
    ...
    .action(async (env1: string, env2: string) => {
      ...
      try {
        ...
        if (!a || !b) {
          cli.error(
            `Both environments must exist in config (have: ${Object.keys(config.environments ?? {}).join(', ') || 'none'}).`,
          )
          process.exitCode = 1
          return
        }
        ...
        const rows = keys.filter((k) => fa[k] !== fb[k]).map((k) => [k, fa[k] ?? '—', fb[k] ?? '—'])
        if (!rows.length) {
  ...
  app
    ...
    .action(async (options?: { env?: string; function?: string; file?: string; replace?: boolean }) => {
      try {
        ...
        const targets =
          which === 'all' ? (['http', 'queue', 'cli'] as const) : ([which] as Array<'http' | 'queue' | 'cli'>)
        ...
        for (const mode of targets) {
          ...
          cli.success(
            `Pushed ${Object.keys(fileVars).length} var(s) → ${name}${options?.replace ? ' (replaced)' : ' (merged)'}${usesPc ? ' (published + flipped live alias)' : ''}`,
          )
        }
  ...
  app.command('env:create <name>', 'Create new environment').action(async (name: string) => {
    notImplemented(
      `env:create ${name}`,
      'Add an `environments.<name>` block to your cloud.config and run `cloud deploy:serverless --env <name>`.',
    )
  })

  app.command('env:switch <name>', 'Switch active environment').action(async (name: string) => {
    notImplemented(
      `env:switch ${name}`,
      'ts-cloud has no persistent "active" environment — pass --env <name> to each command.',
    )
  })
  ...
  for (const verb of ['clone', 'promote', 'sync'] as const) {
    app
      ...
      .action(async (source: string, target: string) => {
        notImplemented(
          `env:${verb} ${source} ${target}`,
          `Copy the \`environments.${source}\` block to \`environments.${target}\` in cloud.config, then \`cloud deploy:serverless --env ${target}\`.`,
        )
      })
  }
  ...
  app
    ...
    .action(
      async (
        ...
        try {
          const config = await loadValidatedConfig()
          const base = options.env || 'production'
          const site = options.site || Object.keys(config.sites ?? {})[0]
          ...
          const environment = controlPlane.environments.get(base)
          const resource = environment
            ? controlPlane.store
                .listResources(controlPlane.project.id, environment.id)
                .find((item) => item.kind === 'application' && item.slug === site)
            : undefined
          if (!environment || !resource)
            throw new Error(`Application ${site} was not found in base environment ${base}`)
          ...
          if (!policy) {
            ...
            if (!domainPattern)
              throw new Error('Pass --domain with an HTTPS pattern containing {name} to configure previews')
            policy = service.previews.createDefinition({
              projectId: controlPlane.project.id,
              resourceId: resource.id,
              baseEnvironmentId: environment.id,
              domainPattern,
              ttlHours: Number(options.ttl) || undefined,
            })
          }
          const pr = Number(options.pr) || undefined
          const preview = pr
            ? service.previews.findForPullRequest(policy.id, 'local', pr)
            : service.previews.findForBranch(policy.id, 'local', branch)
          if (options.getUrl) {
            if (!preview) throw new Error('Preview was not found')
            cli.info(preview.url ?? 'URL unavailable')
            return
          }
          if (options.extend) {
            if (!preview) throw new Error('Preview was not found')
            const extended = service.previews.extend(preview.id, Number(options.extend))
            cli.success(`Extended ${extended.name} until ${extended.expiresAt}`)
            return
          }
          if (options.destroy) {
            if (!preview) throw new Error('Preview was not found')
            if (!options.yes && !(await cli.confirm(`Destroy ${preview.name} and only its tagged resources?`, false)))
              return
            const operation = service.enqueueDestroy(preview, 'manual')
            cli.success(`Preview teardown queued: ${operation.id}`)
            return
          }
          if (options.rebuild) {
            if (!preview) throw new Error('Preview was not found')
            const operation = service.enqueueDeploy(preview, { reason: 'manual_rebuild' })
            cli.success(`Preview rebuild queued: ${operation.id}`)
            return
          }
          if (!options.sha) throw new Error('Pass --sha with the exact immutable commit to deploy')
          const persisted = service.previews.upsert({
    ...
    .action(async (options: { dryRun?: boolean; maxAge?: string; keep?: string }) => {
      ...
      try {
        ...
        const result = service.cleanup({
          ...
          maxAgeHours: Number(options.maxAge) || undefined,
          keepCount: Number(options.keep) || undefined,
        })
        cli.table(
          ['Preview', 'Status', 'Expires', 'Reason'],
        ...
        cli.success(
          options.dryRun
            ? `${result.candidates.length} cleanup candidate(s)`
            : `${result.operations.length} teardown job(s) queued`,
        )
      } catch (error) {
        cli.error(error instanceof Error ? error.message : String(error))
        process.exitCode = 1
  ...
  app
    .command('env:previews', 'List persistent preview environments')
    .option('--site <name>', 'Filter application site')
    .action(async (options: { site?: string }) => {
      ...
      try {
        ...
        controlPlane = initializeDashboardControlPlane(process.cwd(), config)
        const service = new PreviewEnvironmentService(controlPlane.store)
        const resourceIds = options.site
          ? new Set(
              controlPlane.store
                .listResources(controlPlane.project.id)
                .filter((item) => item.slug === options.site)
                .map((item) => item.id),
            )
          : undefined
        const values = service.previews
          .listInstances({ projectId: controlPlane.project.id })
          .filter((item) => !resourceIds || resourceIds.has(item.resourceId))
        cli.table(
          ['ID', 'Name', 'Status', 'Branch / PR', 'Commit', 'URL', 'Expires', 'Cost'],
          values.map((item) => [
            item.id,
            item.name,
            item.status,
            item.pullRequestNumber ? `PR #${item.pullRequestNumber}` : item.branch,
            item.commitSha.slice(0, 12),
            item.url ?? '—',
            item.expiresAt,
            item.costEstimate == null ? '—' : `$${item.costEstimate.toFixed(2)}`,
          ]),
        )
      } catch (error) {
        cli.error(error instanceof Error ? error.message : String(error))
