...
export function registerStackCommands(app: CLI): void {
  app.command('stack:list', 'List all CloudFormation stacks').action(async () => {
  ...
  app.command('stack:describe STACK_NAME', 'Describe a CloudFormation stack').action(async (stackName: string) => {
  ...
  app.command('stack:outputs STACK_NAME', 'Show stack outputs').action(async (stackName: string) => {
