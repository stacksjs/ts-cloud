...
import { listSourceReferences, reconcileSourceWebhook, removeSourceWebhook, SourceConnectionStore, syncSourceRepositories, webhookEndpoint } from '../../src/source'
...
export function inferSourceRepository(
  value: string,
  input: { provider?: SourceProvider; host?: string; cloneUrl?: string } = {},
): { provider: SourceProvider; host: string; fullName: string; cloneUrl?: string } {
  if (
    input.provider &&
    !['github', 'gitlab', 'bitbucket', 'gitea', 'generic_https', 'generic_ssh'].includes(input.provider)
  )
    throw new Error(`Unsupported Git provider: ${input.provider}`)
  ...
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(fullName))
    throw new Error('Repository must use owner/name or a supported clone URL')
  const provider =
    input.provider ??
...
async function sourceContext(): Promise<SourceContext> {
  ...
  return {
    config,
    controlPlane,
    sources: new SourceConnectionStore(controlPlane.store, { encryptionKey: resolveAuthEncryptionKey(process.cwd()) }),
  }
}
...
async function withSources<T>(callback: (context: SourceContext) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    context.controlPlane.store.close()
  }
}
...
async function resolveRepository(
  ...
  inferred: ReturnType<typeof inferSourceRepository>,
  ...
  if (['github', 'gitlab', 'bitbucket', 'gitea'].includes(connection.provider)) {
    ...
    const synchronized = await syncSourceRepositories(sources, connection.id, {
      search: inferred.fullName,
      maxPages: 20,
    })
  ...
  return sources.upsertRepository({
    connectionId: connection.id,
    providerRepositoryId: `manual:${inferred.fullName}`,
...
async function connectRepository(
  ...
  value: string,
  options: GitAddOptions,
): Promise<{ connection: SourceConnection; repository: SourceRepository }> {
  const inferred = inferSourceRepository(value, {
  ...
  if (!connection) {
    ...
    if (['github', 'gitlab', 'bitbucket', 'gitea'].includes(inferred.provider) && !token)
      throw new Error(`Set ${tokenEnv} or pass --token-env <name>; secrets are never accepted as command arguments`)
    const sshKey =
      inferred.provider === 'generic_ssh'
        ? {
            publicKey: readSecretFile(options.publicKey, 'Public key'),
            privateKey: readSecretFile(options.privateKey, 'Private key'),
            hostKey: options.hostKey ?? '',
          }
        : undefined
    context.controlPlane.store.database.transaction(() => {
      connection = context.sources.createConnection({
        organizationId: context.controlPlane.organization.id,
        provider: inferred.provider,
        name: options.name ?? `${inferred.provider.replace('_', ' ')} · ${new URL(inferred.host).host}`,
        host: inferred.host,
        owner: inferred.fullName.split('/')[0],
        authKind: inferred.provider === 'generic_ssh' ? 'deploy_key' : token ? 'access_token' : 'none',
        credential: token ? { token } : undefined,
      })
      if (sshKey)
        context.sources.createDeployKey({
          connectionId: connection!.id,
          name: options.deployKey ?? `${inferred.fullName} deploy key`,
          ...sshKey,
          host: options.sshHost ?? new URL(inferred.host).hostname,
          actorId: undefined,
        })
    })()
  }
  ...
  if (options.site) {
    const environment =
      context.controlPlane.environments.get('production') ?? [...context.controlPlane.environments.values()][0]
    const resource = context.controlPlane.store
      .listResources(context.controlPlane.project.id, environment?.id)
      .find((item) => item.kind === 'application' && item.slug === options.site)
    if (!resource) throw new Error(`Application ${options.site} was not found`)
    const deployKey =
      connection.provider === 'generic_ssh' ? context.sources.listDeployKeys(connection.id)[0] : undefined
    context.sources.createBinding({
      projectId: context.controlPlane.project.id,
      ...
      branchRule: options.tag ? undefined : (options.branch ?? repository.defaultBranch),
      tagRule: options.tag,
      ...
      cloneDepth: options.fullClone ? undefined : 20,
...
export function registerGitCommands(app: CLI): void {
  app.command('git:connections', 'List encrypted Git provider connections').action(async () => {
    try {
      await withSources(async ({ sources, controlPlane }) => {
        ...
        cli.table(
          ['ID', 'Name', 'Provider', 'Status', 'Credential', 'Last sync'],
          values.map((item) => [
            item.id,
            item.name,
            item.provider,
            item.status,
            item.credentialConfigured ? 'encrypted' : 'none',
            item.lastSyncedAt ?? '-',
          ]),
        )
      })
  ...
  app
    .command('git:repositories', 'List repositories granted to a connection')
    ...
    .action(async (options: { connection?: string }) => {
      try {
        await withSources(async ({ sources, controlPlane }) => {
          ...
          cli.table(
            ['ID', 'Repository', 'Default branch', 'Visibility', 'Archived'],
            values.map((item) => [
              item.id,
              item.fullName,
              item.defaultBranch,
  ...
  app
    .command('git:add <repo>', 'Connect and optionally bind a Git repository')
    .option('--branch <branch>', 'Default branch to deploy', { default: 'main' })
    .option('--connection <id>', 'Reuse a connection ID or name')
    .option('--provider <provider>', 'github, gitlab, bitbucket, gitea, generic_https, or generic_ssh')
    .option('--host <url>', 'Provider API host')
    .option('--name <name>', 'Connection name')
    .option('--token-env <name>', 'Environment variable containing the access token')
    .option('--url <clone-url>', 'Credential-free clone URL')
    .option('--site <slug>', 'Bind the repository to an application')
    .option('--root <path>', 'Monorepo root', { default: '.' })
    .option('--include <globs>', 'Comma-separated watched paths')
    .option('--exclude <globs>', 'Comma-separated ignored paths')
    .option('--tag <glob>', 'Deploy matching tags instead of branch pushes')
    .option('--full-clone', 'Use full history instead of a shallow clone')
    .option('--submodules', 'Initialize repository submodules')
    .option('--deploy-key <name>', 'Deploy-key display name')
    .option('--public-key <path>', 'SSH public-key file')
    .option('--private-key <path>', 'SSH private-key file')
    .option('--host-key <key>', 'Pinned SSH host key')
    .option('--ssh-host <host>', 'SSH clone host')
  ...
  app
    .command('git:import', 'Import site.repository entries from cloud.config.ts')
    .option('--connection <id>', 'Reuse a connection ID or name')
    .option('--token-env <name>', 'Environment variable containing provider token')
    .option('--yes', 'Skip confirmation')
    .action(async (options: { connection?: string; tokenEnv?: string; yes?: boolean }) => {
      try {
        await withSources(async (context) => {
          ...
          if (
            !options.yes &&
            !(await cli.confirm(
              `Import ${entries.length} repository configuration${entries.length === 1 ? '' : 's'}?`,
              true,
            ))
          )
            return
          for (const [site, value] of entries) {
            ...
            await connectRepository(context, repository.url, {
              connection: options.connection,
              tokenEnv: options.tokenEnv,
              site,
              provider,
              branch: repository.branch ?? 'main',
              tag: repository.strategy === 'tag' ? (repository.tag ?? repository.tagPattern ?? 'v*') : undefined,
              url: repository.url,
            })
  ...
  app
    .command('git:branches <repo>', 'List branches or tags visible to a connection')
    .option('--connection <id>', 'Connection ID or name')
    .option('--tags', 'List tags instead of branches')
    .option('--deploy-key <id>', 'SSH deploy-key ID')
    .action(async (repo: string, options: { connection?: string; tags?: boolean; deployKey?: string }) => {
      try {
        await withSources(async ({ sources, controlPlane }) => {
          const connection = findConnection(sources, controlPlane.organization.id, options.connection)
          if (!connection) throw new Error('Choose a connection with --connection <id>')
          const repository = sources
            .listRepositories(connection.id)
            .find((item) => item.id === repo || item.fullName.toLowerCase() === repo.toLowerCase())
          if (!repository) throw new Error(`Repository ${repo} was not found`)
          const result = await listSourceReferences(sources, {
            connectionId: connection.id,
            repository: repository.fullName,
            repositoryId: repository.id,
            type: options.tags ? 'tags' : 'branches',
            deployKeyId: options.deployKey ?? sources.listDeployKeys(connection.id)[0]?.id,
          })
          cli.table(
            [options.tags ? 'Tag' : 'Branch', 'Commit', 'Protected'],
            result.refs.map((item) => [item.name, item.commitSha, item.protected ? 'yes' : 'no']),
          )
        })
      } catch (error) {
        printError(error)
  ...
  app
    .command('git:deploy <branch>', 'Queue an idempotent deployment from a Git branch')
    .option('--binding <id>', 'Source binding ID')
    .option('--repo <owner/name>', 'Bound repository')
    .option('--env <environment>', 'Target environment', { default: 'production' })
    .action(async (branch: string, options: { binding?: string; repo?: string; env?: string }) => {
      try {
        await withSources(async ({ sources, controlPlane }) => {
          ...
          const binding = sources
            .listBindings({ projectId: controlPlane.project.id, status: 'active' })
            .find(
              (item) =>
                (!options.binding || item.id === options.binding) &&
                (!options.repo || item.repositoryFullName.toLowerCase() === options.repo.toLowerCase()) &&
                (!item.environmentId || item.environmentId === environment.id),
            )
          if (!binding) throw new Error('No active source binding matched; use --binding or --repo')
          const refs = await listSourceReferences(sources, {
            connectionId: binding.connectionId,
            repository: binding.repositoryFullName,
            repositoryId: binding.repositoryId,
            type: 'branches',
            deployKeyId: binding.deployKeyId,
          })
          ...
          const operation = controlPlane.store.createOperation({
            ...
            kind: 'deploy.source',
            idempotencyKey: `cli-source:${binding.id}:${ref.commitSha}`,
            correlationId: `cli-source:${binding.id}:${ref.commitSha}`,
            input: {
              source: {
                connectionId: binding.connectionId,
  ...
  app
    .command('git:webhook:add <repo>', 'Create and reconcile a signed auto-deploy webhook')
    .option('--connection <id>', 'Connection ID or name')
    .option('--base-url <url>', 'Public HTTPS dashboard URL')
    .action(async (repo: string, options: { connection?: string; baseUrl?: string }) => {
      try {
        await withSources(async ({ sources, controlPlane }) => {
          const connection = findConnection(sources, controlPlane.organization.id, options.connection)
          if (!connection) throw new Error('Choose a connection with --connection <id>')
          ...
          const created = sources.createWebhook({
            connectionId: connection.id,
            repositoryId: repository.id,
            repositoryFullName: repository.fullName,
          })
  ...
  app
    .command('git:webhook:remove <webhook>', 'Disable and remove a provider webhook')
    .option('--yes', 'Skip confirmation')
    .action(async (webhook: string, options: { yes?: boolean }) => {
      try {
        await withSources(async ({ sources, controlPlane }) => {
          const candidate = sources
            .listConnections(controlPlane.organization.id)
            .flatMap((item) => sources.listWebhooks(item.id))
            .find((item) => item.id === webhook || item.repositoryFullName.toLowerCase() === webhook.toLowerCase())
  ...
  app
    .command('git:disconnect <connection>', 'Preview and disconnect a Git provider connection')
    .option('--yes', 'Skip confirmation')
      ...
      try {
        await withSources(async ({ sources, controlPlane }) => {
          ...
          for (const binding of affected)
            cli.info(
              `  ${binding.repositoryFullName} · ${binding.branchRule ?? binding.defaultBranch} · ${binding.monorepoRoot}`,
            )
