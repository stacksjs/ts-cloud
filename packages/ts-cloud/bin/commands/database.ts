...
const ENGINES: readonly DataEngine[] = ['postgres', 'mysql', 'mariadb', 'redis', 'mongodb', 'libsql'],
  PROVIDERS: readonly DataProvider[] = ['aws_rds', 'aws_aurora', 'aws_elasticache', 'server', 'container', 'external'],
...
async function context(environment?: string, serviceHint?: string) {
  const config = await loadValidatedConfig(),
    ...
    hintedService =
      !environment && serviceHint
        ? (store.get(serviceHint) ?? store.list(controlPlane.project.id).find((item) => item.name === serviceHint))
        : undefined,
  ...
  const queue = new DurableOperationQueue(controlPlane.store, {
    ...
    adapters = {
      ...
      aws_elasticache: new AwsElastiCacheDataAdapter(new AwsElastiCacheTransport(new ElastiCacheClient(region))),
  ...
  return {
    ...
    lifecycle,
    actor,
    resolveAdapter: (service: Pick<DataService, 'provider'>) => adapters[service.provider as keyof typeof adapters],
  }
}
...
function engine(value: string | undefined): DataEngine {
  if (!ENGINES.includes(value as DataEngine)) throw new Error(`--engine must be ${ENGINES.join(', ')}.`)
  return value as DataEngine
}

function provider(value: string | undefined): DataProvider {
  if (!PROVIDERS.includes(value as DataProvider)) throw new Error(`--provider must be ${PROVIDERS.join(', ')}.`)
...
function findService(value: DataContext, idOrName: string): DataService {
  const services = value.store.list(value.controlPlane.project.id, value.environmentRecord.id),
    service = value.store.get(idOrName) ?? services.find((item) => item.name === idOrName)
...
export function dataActionChanges(options: DataCommandOptions): Record<string, JsonValue> {
  ...
  if (options.minCapacity != null) changes.minCapacity = Number(options.minCapacity)
  if (options.maxCapacity != null) changes.maxCapacity = Number(options.maxCapacity)
...
function endpoints(service: DataService) {
  const host = typeof service.observedState.endpoint === 'string' ? service.observedState.endpoint : service.placement,
    port =
...
function createOptions(command: ReturnType<CLI['command']>): void {
  command
    ...
    .option('--provider <provider>', 'aws_rds, aws_aurora, aws_elasticache, server, container, or external', {
      default: 'aws_rds',
    })
...
export function registerDatabaseCommands(app: CLI): void {
  ...
  app
    ...
    .action((engineName: string, providerName: string) =>
      run(async () => {
        output.info(JSON.stringify(dataServiceCapabilities(engine(engineName), provider(providerName)), null, 2))
      }),
    )

  const create = app.command('data:create <name>', 'Provision an encrypted, private-by-default data service')
  ...
  create.action((name: string, options: DataCommandOptions) =>
    run(() =>
      withContext(options.env, async (value) => {
        ...
        output.success(`Queued ${created.service.name} as operation ${created.operationId}.`)
  ...
  const adopt = app.command('data:adopt <name>', 'Adopt an existing provider resource as read-only inventory')
  createOptions(adopt)
    ...
    .action((id: string, actionName: string, options: DataCommandOptions) =>
      run(() =>
        withContext(
          options.env,
          async (value) => {
            if (!ACTIONS.includes(actionName as DataAction)) throw new Error(`Action must be ${ACTIONS.join(', ')}.`)
            const service = findService(value, id),
            ...
            const operationId = value.lifecycle.enqueue(service, action, changes, value.actor.id)
      ...
      run(() =>
        withContext(
          ...
          async (value) => {
            ...
            if (options.remove) {
              output.info(
                value.store.removeDependency(service.id, resource.id)
                  ? 'Dependency removed.'
