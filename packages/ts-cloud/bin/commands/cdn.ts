...
export function registerCdnCommands(app: CLI): void {
  app
    .command(
      'cdn:api:deploy <distributionId> <alias>',
      'Plan or deploy a private Lambda API behind an existing static distribution',
    )
    ...
    .action(
      async (
        ...
        try {
          ...
          if (plan.applied)
            cli.success('Private API origin submitted to CloudFront. Run cdn:api:verify after propagation.')
  ...
  app
    .command(
      'cdn:api:verify <alias>',
      'Verify static frontend integrity, API health, latency, and Lambda initialization',
    )
    .option('--function <name>', 'Lambda function name', { default: 'ts-cloud-static-api' })
    ...
    .action(
      async (
        alias: string,
        options: { function?: string; frontendSha256?: string; region?: string; profile?: string },
      ) => {
        try {
          const logs = new CloudWatchLogsClient(options.region || 'us-east-1', options.profile)
          const result = await verifyStaticApiOrigin({
            alias,
            expectedFrontendSha256: options.frontendSha256,
            logs,
            functionName: options.function || 'ts-cloud-static-api',
          })
          cli.info(JSON.stringify(result, null, 2))
          if (!result.api.healthy || result.frontend.unchanged === false)
            throw new Error('Static API verification failed')
  ...
  app
    .command('cdn:api:cost <requests>', 'Compare a low-volume Lambda API with one always-on Fargate task and ALB')
    ...
    .action((requests: string, options: { duration?: string; memory?: string; freeTier?: boolean }) => {
      const estimate = estimateStaticApiOriginMonthlyCost({
        requests: Number(requests),
        averageDurationMs: Number(options.duration),
        memoryMb: Number(options.memory),
        includeFreeTier: options.freeTier !== false,
      })
  ...
  app
    .command(
      'cdn:origin:add <distributionId> <domain>',
      'Safely add a backend origin and path to an existing distribution',
    )
    .option('--id <origin>', 'Stable origin ID', { default: 'api-backend' })
    .option('--path <pattern>', 'Path behavior', { default: '/api/*' })
    .option('--profile <name>', 'AWS credential profile')
    .option('--replace', 'Replace a reviewed path/origin collision')
    .option('--apply', 'Apply the ETag-protected patch')
    .option('--confirm <text>', 'Exact distribution:path confirmation')
    .action(
      async (
        distributionId: string,
        domain: string,
        options: { id?: string; path?: string; profile?: string; replace?: boolean; apply?: boolean; confirm?: string },
      ) => {
        try {
          const cloudfront = new CloudFrontClient(options.profile)
          const request = {
            id: options.id ?? 'api-backend',
            domainName: domain,
            pathPattern: options.path ?? '/api/*',
            replaceExisting: !!options.replace,
          }
          const preview = await cloudfront.upsertExistingDistributionOrigin(distributionId, {
            ...request,
            dryRun: true,
          })
          cli.info(JSON.stringify(preview, null, 2))
          if (!options.apply) {
            cli.info('Dry run only. Re-run with --apply and the exact --confirm token.')
            return
          }
          const token = `${distributionId}:${preview.pathPattern}`
          if (options.confirm !== token)
            throw new Error(`Pass --confirm "${token}" to apply this live distribution change`)
          const result = await cloudfront.upsertExistingDistributionOrigin(distributionId, request)
          cli.success(result.changed ? 'Backend origin patch submitted.' : 'Backend origin already matches.')
          cli.info(JSON.stringify(result, null, 2))
        } catch (error) {
          cli.error(error instanceof Error ? error.message : String(error))
          process.exitCode = 1
  ...
  app
    .command(
      'cdn:origin:remove <distributionId> <domain>',
      'Remove one exact backend behavior and its unreferenced origin',
    )
    .option('--id <origin>', 'Origin ID', { default: 'api-backend' })
    .option('--path <pattern>', 'Path behavior', { default: '/api/*' })
    .option('--profile <name>', 'AWS credential profile')
    .option('--apply', 'Apply the ETag-protected rollback')
    .option('--confirm <text>', 'Exact remove:distribution:path confirmation')
    .action(
      async (
        distributionId: string,
        domain: string,
        options: { id?: string; path?: string; profile?: string; apply?: boolean; confirm?: string },
      ) => {
        try {
          const cloudfront = new CloudFrontClient(options.profile)
          const request = { id: options.id ?? 'api-backend', domainName: domain, pathPattern: options.path ?? '/api/*' }
          const preview = await cloudfront.removeExistingDistributionOrigin(distributionId, {
            ...request,
            dryRun: true,
          })
          cli.info(JSON.stringify(preview, null, 2))
          ...
          const token = `remove:${distributionId}:${preview.pathPattern}`
          if (options.confirm !== token)
            throw new Error(`Pass --confirm "${token}" to apply this live distribution rollback`)
          const result = await cloudfront.removeExistingDistributionOrigin(distributionId, request)
          cli.success(result.changed ? 'Backend behavior rollback submitted.' : 'Backend behavior was already absent.')
          cli.info(JSON.stringify(result, null, 2))
        } catch (error) {
          cli.error(error instanceof Error ? error.message : String(error))
          process.exitCode = 1
  ...
  app.command('cdn:list', 'List all CloudFront distributions').action(async () => {
  ...
  app
    ...
    .action(
      async (options: { bucket?: string; region: string; alias?: string; certificate?: string; comment?: string }) => {
