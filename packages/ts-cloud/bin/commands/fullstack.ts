...
export function registerFullStackCommands(app: CLI): void {
  app.command('container:artifact <repository>', 'Build, scan, push, and resolve an immutable ECR container artifact')
    ...
    .action(async (repository: string, options: { context?: string, dockerfile?: string, platform?: 'linux/amd64' | 'linux/arm64', region?: string, profile?: string }) => {
      try {
        const result = await buildAndPushContainerImage({ repository, context: options.context || '.', dockerfile: options.dockerfile, platform: options.platform, region: options.region, profile: options.profile })
  ...
  app.command('deploy:fullstack <distributionId> <alias>', 'Plan or deploy an ECS/RDS/Redis/SQS/SES backend behind an existing static site')
    ...
    .option('--origin-secret-env <name>', 'Environment variable holding the CloudFront-to-ALB secret', { default: 'TS_CLOUD_ORIGIN_SECRET' })
    ...
    .action(async (distributionId: string, alias: string, options: { name?: string, slug?: string, image?: string, stack?: string, path?: string, originId?: string, originDomain?: string, certificateArn?: string, dnsProvider?: string, originSecretEnv?: string, desiredCount?: string, database?: boolean, cache?: boolean, queue?: boolean, region?: string, profile?: string, skipHealthCheck?: boolean, apply?: boolean, confirm?: string }) => {
      try {
        ...
        if (options.apply && !originVerifySecret) throw new Error(`${secretVariable} must contain a stable random CloudFront-to-ALB secret before apply`)
        const baseDependencies = createExistingStaticFullStackDependencies({ region: options.region, profile: options.profile })
        const dependencies = options.originDomain && options.apply ? { ...baseDependencies, dns: getDnsProvider(options.dnsProvider) } : baseDependencies
        ...
        output.info(JSON.stringify(result, null, 2))
        if (result.applied) output.success('Full-stack backend is healthy and the CloudFront path patch was submitted.')
        else output.info(`Plan only. Apply with --confirm '${distributionId}:${result.distribution.pathPattern}:${result.stack.name}'.`)
      }
      catch (error) { output.error(error instanceof Error ? error.message : String(error)); process.exitCode = 1 }
    })

  app.command('deploy:fullstack:cost', 'Estimate the always-on full-stack backend baseline')
    ...
    .action((options: { desiredCount?: string, multiAzDatabase?: boolean }) => output.info(JSON.stringify(estimateExistingStaticFullStackMonthlyCost({ desiredCount: Number(options.desiredCount) || 1, multiAzDatabase: !!options.multiAzDatabase }), null, 2)))
}
