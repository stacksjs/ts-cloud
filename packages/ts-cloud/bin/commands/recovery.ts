...
export function registerRecoveryCommands(app: CLI): void {
  ...
  app.command('recovery:policy:add <name>', 'Create a scheduled recovery policy')
    ...
    .action(async (name: string, options: { env?: string, destination?: string, kind?: string, dataService?: string, resource?: string, volume?: string, include?: string | string[], exclude?: string | string[], schedule?: string, timezone?: string, rpo?: string, rto?: string, keep?: string, expire?: string, compression?: string, disabled?: boolean }) => run(async () => withContext(options.env, async value => {
      if (!options.destination) throw new Error('--destination is required.'); if (!['managed_database','logical_database','volume','files','control_plane','infrastructure'].includes(options.kind ?? '')) throw new Error('--kind must be managed_database, logical_database, volume, files, control_plane, or infrastructure.')
      ...
      if (options.kind === 'infrastructure' && (!includes.some(item => item.startsWith('resource:arn:')) || !includes.some(item => item.startsWith('role:arn:')))) throw new Error('Infrastructure policies require --include resource:<arn> and --include role:<arn>.')
  ...
  app.command('recovery:restore <point> <target>', 'Validate or queue an isolated, in-place, or drill restore')
    .option('--env <environment>', 'Target environment').option('--mode <mode>', 'isolated or in_place', { default: 'isolated' }).option('--drill', 'Validate health and clean the isolated target').option('--execute', 'Queue the validated restore').option('--confirm <target>', 'Exact in-place target confirmation').option('--ack-downtime', 'Acknowledge downtime and data replacement').option('--safety <point>', 'Distinct verified safety recovery point').option('--metadata <json>', 'Provider restore metadata JSON')
    .action(async (id: string, target: string, options: { env?: string, mode?: string, drill?: boolean, execute?: boolean, confirm?: string, ackDowntime?: boolean, safety?: string, metadata?: string }) => run(async () => withContext(options.env, async value => {
