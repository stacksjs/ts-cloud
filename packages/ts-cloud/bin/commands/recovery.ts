...
export function registerRecoveryCommands(app: CLI): void {
  app.command('recovery:list', 'Show backup coverage, destination health, and recent recovery points')
    ...
    .action(async (options: { env?: string, json?: boolean }) => run(async () => withContext(options.env, async value => {
      const policies = value.store.listPolicies(value.controlPlane.project.id, value.environmentRecord.id), ids = new Set(policies.map(item => item.id)), points = value.store.listRecoveryPoints(value.controlPlane.project.id).filter(item => !item.policyId || ids.has(item.policyId)), result = { coverage: value.store.coverage(value.controlPlane.project.id).filter(item => ids.has(item.policy.id)), destinations: value.store.listDestinations(value.controlPlane.project.id).map(item => ({ ...item, credentialRef: undefined, encryptionKeyRef: undefined, credentialsConfigured: !!item.credentialRef, clientEncryptionConfigured: !!item.encryptionKeyRef })), policies, recoveryPoints: points, jobs: value.store.listJobs(value.controlPlane.project.id).filter(item => !item.policyId || ids.has(item.policyId)) }
  ...
  app.command('recovery:jobs', 'List durable backup, verification, restore, drill, and cleanup jobs')
    .option('--env <environment>', 'Target environment').option('--json', 'Print structured JSON')
    .action(async (options: { env?: string, json?: boolean }) => run(async () => withContext(options.env, async value => {
      const policyIds = new Set(value.store.listPolicies(value.controlPlane.project.id, value.environmentRecord.id).map(item => item.id)), jobs = value.store.listJobs(value.controlPlane.project.id).filter(item => !item.policyId || policyIds.has(item.policyId))
  ...
  app.command('recovery:policy:add <name>', 'Create a scheduled recovery policy')
    .option('--env <environment>', 'Target environment').option('--destination <id>', 'Destination ID or name').option('--kind <kind>', 'managed_database, logical_database, or volume').option('--data-service <id>', 'Data service ID or name').option('--resource <id>', 'Resource ID or slug').option('--volume <name>', 'Named Docker volume').option('--schedule <expression>', 'Cron, rate, or preset', { default: 'daily' }).option('--timezone <timezone>', 'IANA timezone', { default: 'UTC' }).option('--rpo <minutes>', 'Recovery point objective', { default: '1440' }).option('--rto <minutes>', 'Recovery time objective', { default: '120' }).option('--keep <count>', 'Minimum latest points', { default: '7' }).option('--expire <days>', 'Expiry days', { default: '30' }).option('--compression <kind>', 'none, gzip, or zstd', { default: 'gzip' }).option('--disabled', 'Create disabled')
    .action(async (name: string, options: { env?: string, destination?: string, kind?: string, dataService?: string, resource?: string, volume?: string, schedule?: string, timezone?: string, rpo?: string, rto?: string, keep?: string, expire?: string, compression?: string, disabled?: boolean }) => run(async () => withContext(options.env, async value => {
      if (!options.destination) throw new Error('--destination is required.'); if (!['managed_database','logical_database','volume'].includes(options.kind ?? '')) throw new Error('--kind must be managed_database, logical_database, or volume.')
      const targetDestination = destination(value, options.destination), services = value.dataServices.list(value.controlPlane.project.id, value.environmentRecord.id), service = options.dataService ? value.dataServices.get(options.dataService) ?? services.find(item => item.name === options.dataService) : undefined, resources = value.controlPlane.store.listResources(value.controlPlane.project.id, value.environmentRecord.id), resource = options.resource ? resources.find(item => item.id === options.resource || item.slug === options.resource) : undefined
      if (options.kind !== 'volume' && !service) throw new Error('--data-service must identify a service in this environment.'); if (options.kind === 'volume' && (!resource || !options.volume)) throw new Error('Volume policies require --resource and --volume.')
      const item = value.store.createPolicy({ organizationId: value.controlPlane.organization.id, projectId: value.controlPlane.project.id, environmentId: value.environmentRecord.id, resourceId: resource?.id, dataServiceId: service?.id, destinationId: targetDestination.id, name, resourceKind: options.kind as BackupPolicy['resourceKind'], schedule: options.schedule ?? 'daily', timezone: options.timezone ?? 'UTC', retention: { keepLast: Number(options.keep) || 7, expireAfterDays: Number(options.expire) || 30 }, compression: options.compression as BackupPolicy['compression'], encryption: 'destination', includePatterns: options.volume ? [options.volume] : [], excludePatterns: [], expectedRpoMinutes: Number(options.rpo) || 1440, expectedRtoMinutes: Number(options.rto) || 120, enabled: !options.disabled })
