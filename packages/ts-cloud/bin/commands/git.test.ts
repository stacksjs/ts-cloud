...
describe('Git command repository inference', () => {
  it('defaults owner/name input to GitHub', () => {
    expect(inferSourceRepository('acme/web')).toEqual({
      provider: 'github',
  ...
  it('recognizes hosted HTTPS clone URLs', () => {
    expect(inferSourceRepository('https://gitlab.com/acme/web.git')).toEqual({
      provider: 'gitlab',
  ...
  it('recognizes generic SSH clone URLs and explicit providers', () => {
    expect(inferSourceRepository('git@git.example:acme/web.git')).toEqual({
      provider: 'generic_ssh',
      host: 'https://git.example',
      fullName: 'acme/web',
      cloneUrl: 'git@git.example:acme/web.git',
    })
    expect(inferSourceRepository('acme/web', { provider: 'gitea', host: 'https://code.example' })).toMatchObject({
      provider: 'gitea',
