...
async function context(environment?: string, jobId?: string) {
  ...
  const hintedEnvironment = hintedJob?.environmentId
    ? controlPlane.store.listEnvironments(controlPlane.project.id).find((item) => item.id === hintedJob.environmentId)
        ?.slug
    : undefined
  ...
  synchronizeConfiguredJobs(store, config, {
...
async function withContext<T>(
  ...
  callback: (value: JobContext) => Promise<T>,
  jobId?: string,
): Promise<T> {
...
function findJob(value: JobContext, idOrName: string): ScheduledJob {
  ...
  if (!exact || exact.environmentId !== value.environmentRecord.id)
    throw new Error(`Scheduled job ${idOrName} was not found in ${value.env}`)
  return exact
}
...
function requireProductionConfirmation(value: JobContext, action: 'disable' | 'delete', confirmation?: string): void {
  if (value.environmentRecord.kind === 'production' && confirmation !== action)
    throw new Error(`Production ${action} requires --confirm ${action}`)
}

function payloadReferences(input?: string): Record<string, JsonValue> {
  if (!input) return {}
  if (!/^(?:secret|ssm|arn|env):\/\//.test(input))
    throw new Error('Payloads must use a secret://, ssm://, arn://, or env:// reference')
  return { input }
}
...
export function registerJobCommands(app: CLI): void {
  app
    .command('jobs:list', 'List server cron, EventBridge, and recurring control-plane jobs')
      ...
      run(async () =>
        withContext(options.env, async (value) => {
          ...
          if (options.json) output.info(JSON.stringify(jobs, null, 2))
            ...
            output.table(
              ['ID', 'State', 'Provider', 'Name', 'Expression', 'Timezone', 'Next run', 'Reconciliation'],
              scheduleRows(jobs),
            )
        }),
      ),
  ...
  app
    .command('jobs:preview <expression>', 'Preview the next runs without creating production work')
    ...
    .action(async (expression: string, options: { timezone?: string; from?: string; count?: string }) =>
      run(async () => {
        const preview = previewSchedule(
          expression,
          options.timezone ?? 'UTC',
          options.from ? new Date(options.from) : new Date(),
          Number(options.count) || 5,
        )
  ...
  app
    .command('jobs:add <name> <expression>', 'Create a managed recurring job')
    .option('--env <environment>', 'Target environment')
    .option('--timezone <timezone>', 'IANA timezone', { default: 'UTC' })
    ...
    .action(
      async (
        ...
        run(async () =>
          withContext(options.env, async (value) => {
            if (!['skip', 'catch_up'].includes(options.missed ?? 'skip'))
              throw new Error('--missed must be skip or catch_up')
            if (!['allow', 'forbid', 'replace'].includes(options.overlap ?? 'forbid'))
              throw new Error('--overlap must be allow, forbid, or replace')
            ...
            if (mode === 'server') {
              const safe = buildDashboardOperations(value.config, operationData).filter(
                (item) => !item.danger && ['scheduler', 'backup', 'worker'].includes(item.group),
              )
              const operation = options.operation
                ? safe.find((item) => item.id === options.operation)
                : safe.length === 1
                  ? safe[0]
                  : undefined
              if (!operation)
                throw new Error(
                  `Choose a safe target with --operation. Available: ${safe.map((item) => item.id).join(', ') || 'none'}`,
                )
            ...
            const capability = jobProviderCapability({
              provider,
              expression,
            ...
            const resource = options.resource
              ? value.resources.find((item) => item.id === options.resource || item.slug === options.resource)
              : undefined
            if (options.resource && !resource)
              throw new Error(`Resource ${options.resource} was not found in ${value.env}`)
            const job = value.store.create({
              ...
              retryPolicy: {
                ...
                backoffSeconds: Number(options.backoff) || 30,
                deadLetterRef: options.deadLetter,
              },
            ...
            audit(
              value,
              'created',
              { jobId: job.id, provider: job.provider, expression: job.normalizedExpression },
              job.resourceId,
            )
  ...
  app
    .command('jobs:update <job>', 'Update a managed job schedule and policies')
    .option('--env <environment>', 'Target environment')
        ...
        run(async () =>
          withContext(options.env, async (value) => {
            ...
            if (current.origin === 'config')
              throw new Error('Config-defined schedules must be edited in cloud.config.ts and reconciled')
            if (options.missed && !['skip', 'catch_up'].includes(options.missed))
              throw new Error('--missed must be skip or catch_up')
            if (options.overlap && !['allow', 'forbid', 'replace'].includes(options.overlap))
              throw new Error('--overlap must be allow, forbid, or replace')
            const updated = value.store.updateSchedule(current.id, {
              expression: options.expression,
              timezone: options.timezone,
              missedRunPolicy: options.missed as ScheduledJob['missedRunPolicy'] | undefined,
              overlapPolicy: options.overlap as ScheduledJob['overlapPolicy'] | undefined,
              timeoutSeconds: options.timeout == null ? undefined : Number(options.timeout),
            })
            audit(
              value,
              'updated',
  ...
  app
    .command('jobs:run <job>', 'Queue a manual run through the durable operation worker')
    .option('--env <environment>', 'Target environment')
  ...
  app
    .command('jobs:dispatch <job>', 'Dispatch an externally triggered schedule into the durable queue')
    .option('--env <environment>', 'Target environment')
    .option('--scheduled-for <date>', 'Provider scheduled timestamp')
    .option('--scheduled', 'Mark this as a provider schedule trigger')
    .action(async (jobId: string, options: { env?: string; scheduledFor?: string; scheduled?: boolean }) =>
      run(async () =>
        withContext(
          options.env,
          async (value) => {
            ...
            const execution = value.service.enqueue(job, {
              trigger: options.scheduled ? 'scheduled' : 'external',
              scheduledFor,
              actorId: value.actor.id,
            })
  ...
  for (const action of ['enable', 'disable'] as const) {
    app
      .command(`jobs:${action} <job>`, `${action === 'enable' ? 'Enable' : 'Disable'} a scheduled job`)
      .option('--env <environment>', 'Target environment')
      .option('--confirm <text>', `Production ${action} confirmation`)
      .action(async (jobId: string, options: { env?: string; confirm?: string }) =>
        run(async () =>
          withContext(options.env, async (value) => {
            const job = findJob(value, jobId)
  ...
  app
    .command('jobs:delete <job>', 'Delete a managed schedule')
    .option('--env <environment>', 'Target environment')
  ...
  app
    .command('jobs:history <job>', 'Show execution attempts, output, errors, and redacted logs')
    .option('--env <environment>', 'Target environment')
    .option('--limit <count>', 'Maximum executions', { default: '100' })
    .option('--logs', 'Print execution logs')
    .option('--json', 'Print structured JSON')
    .action(async (jobId: string, options: { env?: string; limit?: string; logs?: boolean; json?: boolean }) =>
      run(async () =>
        withContext(options.env, async (value) => {
          const job = findJob(value, jobId)
          const executions = value.store.listExecutions(job.id, Number(options.limit) || 100).map((item) => ({
            ...item,
            logs: options.logs && item.operationId ? value.queue.logs(item.operationId, { limit: 500 }) : [],
          }))
          if (options.json) output.info(JSON.stringify(executions, null, 2))
          else {
            output.table(
              ['ID', 'Status', 'Trigger', 'Scheduled for', 'Attempt', 'Started', 'Finished', 'Error'],
              executions.map((item) => [
                item.id,
                item.status,
                item.trigger,
            ...
            if (options.logs)
              for (const execution of executions)
                for (const entry of execution.logs)
                  output.info(`${execution.id} ${entry.sequence} ${entry.stream.toUpperCase()} ${entry.message}`)
          }
        }),
      ),
    )
  ...
  app
    .command('jobs:reconcile', 'Import declarative schedules and workers without destructive deletion')
    .option('--env <environment>', 'Target environment')
    .action(async (options: { env?: string }) =>
      run(async () =>
        withContext(options.env, async (value) => {
          const result = synchronizeConfiguredJobs(value.store, value.config, {
            organization: value.controlPlane.organization,
            project: value.controlPlane.project,
            environment: value.environmentRecord,
            resources: value.resources,
          })
          audit(value, 'reconciled', result)
          output.success(
            `Reconciled ${result.jobs} schedules and ${result.workers} workers; retained ${result.drifted} missing definitions as drifted`,
          )
        }),
      ),
    )
  ...
  app
    .command('workers:list', 'List configured systemd, ECS, and Lambda workers')
    .option('--env <environment>', 'Target environment')
    .option('--json', 'Print structured JSON')
    .action(async (options: { env?: string; json?: boolean }) =>
      run(async () =>
        withContext(options.env, async (value) => {
          ...
          if (options.json) output.info(JSON.stringify(workers, null, 2))
            ...
            output.table(
              ['ID', 'Provider', 'Name', 'Queue', 'Processes', 'Timeout', 'Restart', 'Reconciliation'],
              workers.map((item) => [
                item.id,
                item.provider,
                item.name,
  ...
  app
    .command('workers:restart <worker>', 'Gracefully restart an allowlisted systemd worker')
    .option('--env <environment>', 'Target environment')
    .option('--confirm <name>', 'Exact worker name confirmation')
    .action(async (workerId: string, options: { env?: string; confirm?: string }) =>
      run(async () =>
        withContext(options.env, async (value) => {
          const workers = value.store.listWorkers(value.controlPlane.project.id, value.environmentRecord.id)
          const worker = value.store.getWorker(workerId) ?? workers.find((item) => item.name === workerId)
          if (!worker || worker.environmentId !== value.environmentRecord.id)
            throw new Error(`Worker ${workerId} was not found in ${value.env}`)
          if (options.confirm !== worker.name) throw new Error(`Restart requires --confirm "${worker.name}"`)
          if (worker.provider !== 'systemd')
            throw new Error(
              `${worker.provider} workers are provider/config managed and do not expose a safe restart adapter`,
            )
          const operation = resolveDashboardOperation(String(worker.target.operationId ?? ''), value.config, {
            workers: workers.map((item) => ({ name: item.name })),
          })
          if (!operation || operation.danger || operation.group !== 'worker')
            throw new Error('The allowlisted worker restart operation is unavailable')
          const result = await runDashboardOperation(value.config, value.env, operation)
          value.store.reconcileWorker(worker.id, result.ok ? 'in_sync' : 'unavailable', {
