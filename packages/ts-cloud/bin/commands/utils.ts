...
export function registerUtilsCommands(app: CLI, version: string): void {
  app.command('upgrade', 'Upgrade CLI to latest version').action(async () => {
    unsupportedCommand('upgrade', {
      message: `Self-update is disabled for this ${version} build because no signed update-manifest source is configured.`,
      nextAction: 'Install a verified release with your package manager or configure platform maintenance updates.',
    })
  })

  app.command('doctor', 'Check system requirements and AWS credentials').action(async () => {
  ...
  app.command('regions', 'List available AWS regions').action(async () => {
