...
describe('runtime model', () => {
  ...
  it('redacts nested secret-shaped fields', () => {
    expect(
      redactRuntimeConfig({ env: { DB_PASSWORD: 'secret', APP_ENV: 'prod' }, token: 'abc', items: [{ apiKey: 'x' }] }),
    ).toEqual({
      env: { DB_PASSWORD: '[REDACTED]', APP_ENV: 'prod' },
      token: '[REDACTED]',
...
describe('systemd adapter fixtures', () => {
  it('parses and normalizes service records', () => {
    const output =
      'TSCLOUD_SYSTEMD\tacme-web@r1.service\tloaded\tactive\trunning\tAcme web\tenabled\t431\t1048576\t2\t2026-07-21T11:00:00Z\t/etc/systemd/system/acme.service\t/var/www/web/releases/r1\tweb\nnoise'
...
describe('container and cloud adapter fixtures', () => {
  it('maps Docker config, networks, mounts, and secret names only', () => {
    const records = parseDockerInspect(
      JSON.stringify([
        {
          Id: 'abc123',
          Name: '/api',
          Config: {
            ...
            Labels: { 'ts-cloud.service': 'api', 'ts-cloud.release': 'r1' },
            Env: ['APP_ENV=prod', 'API_TOKEN=hidden'],
          },
          State: { Status: 'running', StartedAt: '2026-07-21T11:30:00Z' },
          RestartCount: 3,
          HostConfig: { NetworkMode: 'bridge', Memory: 1024 },
          Mounts: [{ Source: '/srv/api', Destination: '/app', Type: 'bind', RW: false }],
          NetworkSettings: {
            Networks: { bridge: { NetworkID: 'net1', IPAddress: '172.17.0.2' } },
            Ports: { '3000/tcp': [{ HostIp: '127.0.0.1', HostPort: '8080' }] },
          },
        },
      ]),
    )
    ...
    expect(workload).toMatchObject({
      ...
      status: 'running',
      restartCount: 3,
      links: { service: 'api', release: 'r1' },
    })
    expect(workload.networks[0].ports?.[0]).toEqual({
      container: 3000,
      host: 8080,
      protocol: 'tcp',
      address: '127.0.0.1',
    ...
    expect(workload.config).toEqual({
      image: 'acme/api@sha256:1',
      labels: { 'ts-cloud.service': 'api', 'ts-cloud.release': 'r1' },
      environmentKeys: ['APP_ENV', 'API_TOKEN'],
      networkMode: 'bridge',
    })
  })

  it('maps ECS services and tasks', () => {
    const [workload] = ecsWorkloads(
      [
        {
          serviceArn: 'arn/service/acme/api',
          serviceName: 'api',
          clusterArn: 'arn/cluster/acme',
          status: 'ACTIVE',
          desiredCount: 2,
          runningCount: 1,
          pendingCount: 1,
          taskDefinition: 'arn/task-definition/api:42',
          launchType: 'FARGATE',
        },
      ],
      [
      ...
      [
        {
          ...
          containerDefinitions: [
            {
              name: 'api',
              image: 'registry/acme/api@sha256:42',
              portMappings: [{ containerPort: 3000, protocol: 'tcp' }],
              mountPoints: [{ sourceVolume: 'data', containerPath: '/app/data', readOnly: false }],
              logConfiguration: { options: { 'awslogs-group': '/ecs/acme-api' } },
            },
          ],
        },
      ],
  ...
  it('maps Lambda without exposing environment values', () => {
    const [workload] = lambdaWorkloads(
      [
        {
          FunctionName: 'acme-production-http',
          FunctionArn: 'arn:lambda:http',
          Runtime: 'provided.al2023',
          State: 'Active',
          MemorySize: 1024,
          Environment: { Variables: { APP_ENV: 'prod', DB_PASSWORD: 'hidden' } },
        },
      ],
      context,
    )
      ...
      [
        {
          ...
          provider: 'systemd',
          discover: async () =>
            systemdWorkloads([{ unit: 'api.service', active: 'active', sub: 'running' }], context, 'good'),
        },
        {
          id: 'bad',
          provider: 'docker',
