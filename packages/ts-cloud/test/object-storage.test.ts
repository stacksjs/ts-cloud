...
describe('resolveS3Endpoint', () => {
  it('preserves AWS virtual-hosted behavior when no endpoint is given', () => {
    expect(resolveS3Endpoint({ region: 'us-east-1', path: '/file.txt', bucket: 'my-bucket' }))
      .toEqual({ host: 'my-bucket.s3.us-east-1.amazonaws.com', path: '/file.txt' })
  })

  it('uses the bare regional host when no bucket is scoped (e.g. ListBuckets)', () => {
    expect(resolveS3Endpoint({ region: 'eu-west-1', path: '/' }))
      .toEqual({ host: 's3.eu-west-1.amazonaws.com', path: '/' })
  })

  it('leaves a bucket-in-path request untouched (path-style already encoded)', () => {
    expect(resolveS3Endpoint({ region: 'us-east-1', path: '/my-bucket?policy=' }))
      .toEqual({ host: 's3.us-east-1.amazonaws.com', path: '/my-bucket?policy=' })
  })

  it('builds Backblaze B2 virtual-hosted hosts', () => {
    expect(resolveS3Endpoint({
      region: 'us-west-004',
      path: '/pkg.tgz',
      bucket: 'pantry',
...
describe('resolveObjectStorage', () => {
  ...
  const TOUCHED = [
    'OBJECT_STORAGE_PROVIDER', 'STORAGE_PROVIDER',
    ...
    'B2_REGION', 'B2_APPLICATION_KEY_ID', 'B2_KEY_ID', 'B2_APPLICATION_KEY', 'B2_SECRET_KEY',
    'HETZNER_S3_REGION', 'HETZNER_REGION', 'HETZNER_S3_ACCESS_KEY', 'HETZNER_S3_SECRET_KEY',
  ]
  ...
  it('falls back to generic S3_* credentials for non-AWS providers', () => {
    ...
    const r = resolveObjectStorage({ provider: 'backblaze', region: 'us-west-004' })
    expect(r.credentials).toEqual({ accessKeyId: 'genericid', secretAccessKey: 'genericsecret', sessionToken: undefined })
  })
