...
export function createDataPipelinePreset(options: {
  ...
  const { name, slug, retentionDays = 7 } = options
          ...
          crawlers: [
            {
              ...
              s3Targets: [`s3://${slug}-raw/`, `s3://${slug}-processed/`],
      ...
      monitoring: {
        dashboard: {
          ...
          widgets: [
            {
              type: 'metric',
              metrics: ['KinesisIncomingRecords', 'LambdaInvocations', 'S3ObjectsCreated', 'GlueJobsSucceeded'],
            },
          ],
        },
        alarms: [
