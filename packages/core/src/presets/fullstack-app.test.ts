...
describe('existing static frontend full-stack preset', () => {
  it('builds only the backend stack with managed stateful services', () => {
    const config = createExistingStaticFullStackPreset({ name: 'Example', slug: 'example', domain: 'origin-api.example.com', imageUri: '123.dkr.ecr.us-east-1.amazonaws.com/example@sha256:abc', certificateArn: 'arn:aws:acm:us-east-1:123:certificate/one' })
  ...
  it('injects managed endpoints and secret references without literal credentials', () => {
    const config = createExistingStaticFullStackPreset({ name: 'Example', slug: 'example', domain: 'origin-api.example.com', imageUri: 'image@sha256:abc' })
    const result = new CloudFormationBuilder(config as any).build()
    const container = result.Resources.AppTaskDefinition?.Properties?.ContainerDefinitions[0]
    expect(container.Environment.find((item: any) => item.Name === 'DB_HOST').Value).toEqual({ 'Fn::GetAtt': ['PostgresDb', 'Endpoint.Address'] })
