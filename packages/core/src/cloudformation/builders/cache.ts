 ...
 */
export function addCacheResources(builder: CloudFormationBuilder, config: CacheConfig): void {
  if (config.redis) {
 ...
 * Add Redis cluster
 */
function addRedisCluster(builder: CloudFormationBuilder, config: CacheConfig['redis']): void {
  if (!config) return
  ...
  builder.addResource(
    ...
    'AWS::ElastiCache::SubnetGroup',
    {
      ...
      Tags: [{ Key: 'Name', Value: Fn.sub('${AWS::StackName}-cache-subnet-group') }],
    },
    {
  ...
  builder.addResource(
    ...
    'AWS::EC2::SecurityGroup',
    {
      ...
      Tags: [{ Key: 'Name', Value: Fn.sub('${AWS::StackName}-cache-sg') }],
    },
    {
  ...
  if (config.numCacheNodes > 1 || config.automaticFailoverEnabled) {
    builder.addResource(
      ...
      'AWS::ElastiCache::ReplicationGroup',
      {
        ...
        CacheParameterGroupName: config.parameterGroup ? Fn.ref('CacheParameterGroup') : undefined,
        ...
        Tags: [{ Key: 'Name', Value: Fn.sub('${AWS::StackName}-redis') }],
      },
      {
 ...
 */
function addMemcachedCluster(builder: CloudFormationBuilder, config: CacheConfig['memcached']): void {
  if (!config) return
  ...
  builder.addResource(
    ...
    'AWS::ElastiCache::CacheCluster',
    {
      ...
      VpcSecurityGroupIds: [Fn.ref('CacheSecurityGroup')],
      PreferredMaintenanceWindow: 'sun:05:00-sun:06:00',
      Tags: [{ Key: 'Name', Value: Fn.sub('${AWS::StackName}-memcached') }],
    },
    {
 ...
 */
function addElastiCacheCluster(builder: CloudFormationBuilder, config: CacheConfig['elasticache']): void {
  if (!config) return
  ...
  builder.addResource(
    ...
    'AWS::ElastiCache::CacheCluster',
    {
      ...
      CacheSubnetGroupName: Fn.ref('CacheSubnetGroup'),
      VpcSecurityGroupIds: [Fn.ref('CacheSecurityGroup')],
      Tags: [{ Key: 'Name', Value: Fn.sub(`\${AWS::StackName}-${config.engine}`) }],
    },
    {
      dependsOn: ['CacheSubnetGroup', 'CacheSecurityGroup'],
    },
